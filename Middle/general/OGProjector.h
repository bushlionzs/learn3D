...
namespace Orphigine {
    ...
    protected:
		...
		*/
		...
		bool mForceUpdate;//如果是true,则不管位置是否改变都要构建新的vertexbuf
		bool mDirty;

		bool m_bUsingProjScene;  // 是否使用ProjScene显示影子，ProjScene是一次渲染全部影子
    ...
    public:
        ...
        */
		...
		void _updateVertexBuffer(bool bUpdateAnyway);  // 更新接受投影的mesh, bUpdateAnyway为true时将会强行更新
		...
		bool isUsingProjScene() const { return m_bUsingProjScene; }  // 是否通过ProjScene统一渲染
		void enableUsingProjScene(bool bEnable);  // 激活/禁用通过ProjScene统一渲染

		bool uploadToProjScene();  // 把自己的投影信息上传到ProjScene，只有这样影子才能显示出来（位置更新后可重复上传，没关系，这是正确的）
		void erasureFromProjScene();  // 把自己的投影信息从ProjScene中擦除，擦除之后除非再次上传（uploadToProjScene）否则不会显示
    };
