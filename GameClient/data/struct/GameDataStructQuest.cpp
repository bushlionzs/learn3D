...
=======================================================================================================================
*/
_QUEST_INFO*ScriptQuestInterface::g_misInfoPtr = NULL;
...
{
	const OWN_QUEST		*OwnQuest = GAME_DATA_PLAYER_PTR->GetMySelfDetailAttrib()->GetQuest(nIndex);
	int32					scriptId = OwnQuest->m_idScript;
	_QUEST_EXT_INFO	*pMei = GetQuestExtInfo(OwnQuest->m_idQuest);
	...
	for(i = 0; i < pMei->m_QuestRewardItem.size(); i++)
	{
		if(pMei->m_QuestRewardItem[i].pItemImpl)
			ITEM_MANAGER_PTR->DestroyItem((KItem *) pMei->m_QuestRewardItem[i].pItemImpl);
	}

	pMei->m_QuestRewardItem.clear();

	MisExtInfo::iterator	it = g_misExtInfo.find(OwnQuest->m_idQuest);
...
void ScriptQuestInterface::RemoveQuestInfo(int32 nIndex)
{
	...
	if(NULL == OwnQuest) return;

	int32		scriptId = OwnQuest->m_idScript;
	uint32	i = 0;

	g_misInfoPtr = &(GetQuestInfo(OwnQuest->m_idQuest));
	...
	for(i = 0; i < g_misInfoPtr->m_QuestDemandItem.size(); i++)
	{
		...
		if(g_misInfoPtr->m_QuestDemandItem[i].pItemImpl)
			ITEM_MANAGER_PTR->DestroyItem((KItem *) (g_misInfoPtr->m_QuestDemandItem[i].pItemImpl));
	}
	...
	for(i = 0; i < g_misInfoPtr->m_vecQuestDemandNPC.size(); ++i)
	{
		...
		for(int32 j = 0; j < FindNPC.vecItemInfo.size(); ++j)
		{
			...
			if(FindNPC.vecItemInfo[j].pItemImpl)
				ITEM_MANAGER_PTR->DestroyItem((KItem *) (FindNPC.vecItemInfo[j].pItemImpl));
		}
	}
	...
	MisInfo::iterator	it = g_misInfo.find(OwnQuest->m_idQuest);
...
void ScriptQuestInterface::ClearQuest_ScriptId(int32 nScriptId)
{
	...
	for(i = 0; i < pQuestInfo->m_QuestDemandItem.size(); i++)
	{
		...
		{
			ITEM_MANAGER_PTR->DestroyItem((KItem *) (pQuestInfo->m_QuestDemandItem[i].pItemImpl));
	...
	for(i = 0; i < pQuestInfo->m_QuestRewardItem.size(); i++)
	{
		...
		{
			ITEM_MANAGER_PTR->DestroyItem((KItem *) (pQuestInfo->m_QuestRewardItem[i].pItemImpl));
...
{
	if(NULL == OBJECT_MANAGER_PTR->GetMySelf()) return;
	if(NULL == OBJECT_MANAGER_PTR->GetMySelf()->GetCharacterData()) return;
	...
	if(pQuestList->nQuestType == 0 || pQuestList->nQuestType == 2)
	{
		...
		{
			uExpReward += pQuestList->nExpRewardP2 * OBJECT_MANAGER_PTR->GetMySelf()->GetCharacterData()->Get_Level();
		}

		if(pQuestList->nExpRewardP3 > 0)
		{
			uExpReward += pQuestList->nExpRewardP3 *
				OBJECT_MANAGER_PTR->GetMySelf()->GetCharacterData()->Get_Level() *
				OBJECT_MANAGER_PTR->GetMySelf()->GetCharacterData()->Get_Level();
		}
		...
		{
			uMoneyReward += pQuestList->nMoneyRewardP2 * OBJECT_MANAGER_PTR->GetMySelf()->GetCharacterData()->Get_Level();
		}
		...
		if(pQuestList->nShengWangType >= 0)
		{
			...
			{
				nShengWang += pQuestList->nShengWangRewartP2 * OBJECT_MANAGER_PTR->GetMySelf()->GetCharacterData()->Get_Level();
			};
		...
		if(!g_misInfoPtr->m_bFill)
		{
			...
			{
				_QUEST_INFO::QuestRewardItem	newRewardItem;
				KItem							*pItem = ITEM_MANAGER_PTR->CreateNewItem(pQuestList->nFixItemID1);
			...
			{
				_QUEST_INFO::QuestRewardItem	newRewardItem;
				KItem							*pItem = ITEM_MANAGER_PTR->CreateNewItem(pQuestList->nFixItemID2);
			...
			{
				_QUEST_INFO::QuestRewardItem	newRewardItem;
				KItem							*pItem = ITEM_MANAGER_PTR->CreateNewItem(pQuestList->nFixItemID3);
			...
			{
				_QUEST_INFO::QuestRewardItem	newRewardItem;
				KItem							*pItem = ITEM_MANAGER_PTR->CreateNewItem(pQuestList->nFixItemID4);
			...
			TABFile::_ConvertStringToVector(pQuestList->szSelItemID1, vRet, "|", TRUE, TRUE);
			if (!vRet.empty())
			{
				if (vRet.size() == 1 && pQuestList->nSelItemNum1 > 0)
				{
					...
					_QUEST_INFO::QuestRewardItem	newRewardItem;
					KItem							*pItem = ITEM_MANAGER_PTR->CreateNewItem(id);
				...
				{
					KCharatcterBaseData		*pData = OBJECT_MANAGER_PTR->GetMySelf()->GetCharacterData();
			...
			{
				_QUEST_INFO::QuestRewardItem	newRewardItem;
				KItem							*pItem = ITEM_MANAGER_PTR->CreateNewItem(pQuestList->nSelItemID2);
			...
			{
				_QUEST_INFO::QuestRewardItem	newRewardItem;
				KItem							*pItem = ITEM_MANAGER_PTR->CreateNewItem(pQuestList->nSelItemID3);
			...
			{
				_QUEST_INFO::QuestRewardItem	newRewardItem;
				KItem							*pItem = ITEM_MANAGER_PTR->CreateNewItem(pQuestList->nSelItemID4);
			...
			{
				_QUEST_INFO::QuestRewardItem	newRewardItem;
				KItem							*pItem = ITEM_MANAGER_PTR->CreateNewItem(pQuestList->nSelItemID5);
			...
			{
				_QUEST_INFO::QuestRewardItem	newRewardItem;
				KItem							*pItem = ITEM_MANAGER_PTR->CreateNewItem(pQuestList->nSelItemID6);
			...
			for(int32 iMisIdx = 0; iMisIdx < 5; iMisIdx++)
			{
				...
				else if(nQuestDest >= 20000 && nQuestDest <= 29999) /* 送信任务 */
				{
					...
					if(pQuestDelivery->nDeliveryItemID1 != -1 && pQuestDelivery->nDeliveryItemNum1 > 0)
					{
						...
						KItem					*pItem = ITEM_MANAGER_PTR->CreateNewItem(pQuestDelivery->nDeliveryItemID1);
					...
					if(pQuestDelivery->nDeliveryItemID2 != -1 && pQuestDelivery->nDeliveryItemNum2 > 0)
					{
						...
						KItem					*pItem = ITEM_MANAGER_PTR->CreateNewItem(pQuestDelivery->nDeliveryItemID2);
						_QUEST_INFO::ItemInfo item;
					...
					if(pQuestDelivery->nDeliveryItemID3 != -1 && pQuestDelivery->nDeliveryItemNum3 > 0)
					{
						...
						KItem					*pItem = ITEM_MANAGER_PTR->CreateNewItem(pQuestDelivery->nDeliveryItemID3);
						_QUEST_INFO::ItemInfo item;
				...
				else if(nQuestDest >= 50000 && nQuestDest <= 59999) /* 收集任务 */
				{
					...
					{
						KItem							*pItem = ITEM_MANAGER_PTR->CreateNewItem(pQuestLookItem->nItemID1);
		...
		if(g_misInfoPtr->m_misName.empty())
		{
			...
			GAME_DATA_QUEST_PTR->FormatQuestString(g_misInfoPtr->m_misName);
		}

		if(g_misInfoPtr->m_misDesc.empty())
		{
			g_misInfoPtr->m_misDesc = pQuestList->szQuestDesc;
			GAME_DATA_QUEST_PTR->FormatQuestString(g_misInfoPtr->m_misDesc);
		}

		if(g_misInfoPtr->m_misManual.empty())
		{
			g_misInfoPtr->m_misManual = pQuestList->szQuestManual;
			GAME_DATA_QUEST_PTR->FormatQuestString(g_misInfoPtr->m_misManual);
		}
	}
	...
	else if(pQuestList->nQuestType == 1)
	{
		...
		if(!g_misInfoPtr->m_bFill)
		{
			...
			const STRING	&strFileName = SCRIPT_SYS_PTR->GetQuestFileName(pQuestList->nScriptID);

			if(strFileName.empty())
			{
				return;
				;//KLThrow("Invalid script id:%d", nScriptID);
			}

			char szFileName[MAX_PATH] = { 0 };
			_snprintf(szFileName, MAX_PATH, "%s", strFileName.c_str() + 1);

			//if (g_bDebug && FALSE == g_bDebugPackRes)
			//	_snprintf(szFileName, MAX_PATH, "%s", strFileName.c_str() + 1);	 // lua文件
			//else
			//	_snprintf(szFileName, MAX_PATH, "%sc", strFileName.c_str() + 1); // luac文件				

			char	*lpAddress = NULL;
			DWORD dwSize = RESOURCE_SYS_PTR->Load(szFileName, lpAddress, RESOURCE_GROUP);

			if(dwSize > 0)
			{
				/* 判断起始和结束标记 */
				char	*pTagBegin = strstr(lpAddress, szScriptQuestBeginStr);
				char	*pTagEnd = strstr(lpAddress, szScriptQuestEndStr);
				...
				RESOURCE_SYS_PTR->UnLoad(lpAddress, dwSize);
...
void ScriptQuestInterface::parseQuestContinue_CP(char *pDest, char *pSrc, int32 j)
{
	...
	GAME_DATA_QUEST_PTR->FormatQuestString(g_misInfoPtr->m_misContinue);
}
...
void ScriptQuestInterface::parseQuestCompleted_CP(char *pDest, char *pSrc, int32 j)
{
	...
	GAME_DATA_QUEST_PTR->FormatQuestString(g_misInfoPtr->m_misCompleted);
}
...
void ScriptQuestInterface::parseQuestHelp_CP(char *pDest, char *pSrc, int32 j)
{
	...
	GAME_DATA_QUEST_PTR->FormatQuestString(g_misInfoPtr->m_misHelp);
}
...
void ScriptQuestInterface::parseTarget_CP(char *pDest, char *pSrc, int32 j)
{
	...
	while(pCur++)
	{
		...
		else if(digit == 3 && bParseInfo)
		{
			...
			{
				KItem	*pItem = ITEM_MANAGER_PTR->CreateNewItem(misTget);
...
void ScriptQuestInterface::parseBonusItem_CP(char *pDest, char *pSrc, int32 j)
{
	...
	while(pCur++)
	{
		...
		else if(digit == 3 && isdigit(*pCur))
		{
			...
			KItem	*pItem = ITEM_MANAGER_PTR->CreateNewItem(newRewardItem.ItemData.m_ItemBonus.m_uItemID);
...
void ScriptQuestInterface::parseBonusChoiceItem_CP(char *pDest, char *pSrc, int32 j)
{
	...
	while(pCur++)
	{
		...
		else if(digit == 3 && isdigit(*pCur))
		{
			...
			KItem	*pItem = ITEM_MANAGER_PTR->CreateNewItem(itemId);
