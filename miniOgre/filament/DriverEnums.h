...
enum class TimerQueryResult : int8_t {
    TimerQueryResult_ERROR = -1,     // an error occurred, result won't be available
 ...
 */
struct DepthRange {
    float znear = 0.0f;    //!< mapping of the near plane to window coordinates.
    float zfar = 1.0f;     //!< mapping of the far plane to window coordinates.
};
...
enum class FenceStatus : int8_t {
    FenceStatus_ERROR = -1,                 //!< An error occurred. The Fence condition is not satisfied.
...
enum class TextureFormat : uint16_t {
    // 8-bits per element
    R8, R8_SNORM, R8UI, R8I, STENCIL8, BYTE_LA,
...
{
    using SetLayout = std::array<Handle<filament::backend::HwDescriptorSetLayout>, 4>;
...
using StereoscopicType = filament::backend::Platform::StereoscopicType;
