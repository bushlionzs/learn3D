...
#include "filament/CommandBufferQueue.h"
...
//#ifndef NDEBUG
//    auto circular_size = circularBuffer.size();
//    size_t totalUsed = circular_size - mFreeSpace;
...
void CommandBufferQueue::releaseBuffer(CommandBufferQueue::Slice const& buffer) {
    std::lock_guard<utils::Mutex> const lock(mLock);
    auto remove = uintptr_t(buffer.end) - uintptr_t(buffer.begin);
    mFreeSpace += remove;
    mCondition.notify_one();
}
