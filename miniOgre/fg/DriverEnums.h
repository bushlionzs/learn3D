...
static constexpr uint64_t MAX_SUPPORTED_RENDER_TARGET_COUNT = 8;
 ...
 */
struct DepthRange {
    float near_range = 0.0f;    //!< mapping of the near plane to window coordinates.
    float far_range = 1.0f;     //!< mapping of the far plane to window coordinates.
};
...
enum class FenceStatus : int8_t {
    FENCE_ERROR = -1,                 //!< An error occurred. The Fence condition is not satisfied.
...
enum class SyncStatus : int8_t {
    SYNC_ERROR = -1,          //!< An error occurred. The Sync is not signaled.
