...
void RenderPassNode::execute(FrameGraphResources const& resources, RenderSystem& driver) noexcept {
...
void RenderPassNode::resolve() noexcept {
    ...
    for (auto& rt : mRenderTargetData) {
        ...
        constexpr size_t DEPTH_INDEX = backend::MAX_SUPPORTED_RENDER_TARGET_COUNT + 0;
        constexpr size_t STENCIL_INDEX = backend::MAX_SUPPORTED_RENDER_TARGET_COUNT + 1;

        for (size_t i = 0; i < backend::MAX_SUPPORTED_RENDER_TARGET_COUNT + 2; i++) {
...
void RenderPassNode::RenderPassData::devirtualize(FrameGraph& fg,
    ...
    if (UTILS_LIKELY(!imported)) {
        ...
        MRT colorInfo{};
        for (size_t i = 0; i < backend::MAX_SUPPORTED_RENDER_TARGET_COUNT; i++) {
            if (attachmentInfo[i]) {
        ...
        for (size_t i = 0; i < 2; i++) {
            if (attachmentInfo[backend::MAX_SUPPORTED_RENDER_TARGET_COUNT + i]) {
                auto const* pResource = static_cast<Resource<FrameGraphTexture> const*>(
                        fg.getResource(attachmentInfo[backend::MAX_SUPPORTED_RENDER_TARGET_COUNT + i]));
...
std::string RenderPassNode::graphvizify() const noexcept {
    ...
    for (auto const& rt :mRenderTargetData) {
        s.append("\\nS:");
        s.append(std::to_string((uint32_t)rt.backend.params.flags.discardStart).c_str());
        s.append(", E:");
        s.append(std::to_string((uint32_t)rt.backend.params.flags.discardEnd).c_str());
        s.append(", C:");
        s.append(std::to_string((uint32_t)rt.backend.params.flags.clear).c_str());
    }
...
void PresentPassNode::execute(FrameGraphResources const&, RenderSystem&) noexcept {
}
