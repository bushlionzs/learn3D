...
template<>
struct utils::EnableBitMaskOperators<filament::backend::TextureUsage> : public std::true_type { };

template<>
struct utils::EnableIntegerOperators<filament::backend::TextureUsage> : public std::true_type { };

template<>
struct utils::EnableBitMaskOperators<filament::backend::TargetBufferFlags> : public std::true_type { };

template<>
struct utils::EnableIntegerOperators<filament::backend::TargetBufferFlags> : public std::true_type { };
...
protected:
    ...
    std::string usageString() const noexcept override {
        return std::to_string((uint32_t)usage);
    }
};
...
private:
    ...
    void assertConnect(FrameGraphTexture::Usage u) {
        /*ASSERT_PRECONDITION((u & this->usage) == u,
                ...
                std::to_string((uint32_t)u).c_str(), this->name, std::to_string((uint32_t)this->usage).c_str());*/
    }
};
