...
namespace Ogre {
	...
	void MovementAffector::_initParticle(Particle* pParticle)
	{
		if (mUseStartVelocity)
		{
			...
			pParticle->mDirection.x = Math::RangeRandom( mStartVelocityMin.x, mStartVelocityMax.x );
			pParticle->mDirection.y = Math::RangeRandom( mStartVelocityMin.y, mStartVelocityMax.y );
			pParticle->mDirection.z = Math::RangeRandom( mStartVelocityMin.z, mStartVelocityMax.z );
		}
	}
	//-----------------------------------------------------------------------
	void MovementAffector::_affectParticles(ParticleSystem* pSystem, Real timeElapsed)
	{
...

		{
			...
			p->mDirection += mAcceleration * timeElapsed;;

			// apply the randomness direction
			p->mDirection += Vector3(Math::RangeRandom(mRandomnessMin.x, mRandomnessMax.x) * timeElapsed,
	...
	//-----------------------------------------------------------------------
	///*
	char * MovementAffector::readFromBuffer(char * pCurrent)
	{
		int length = sizeof(int) + sizeof(Ogre::Vector3) * 7;

		memcpy(&mUseStartVelocity,pCurrent,length);
		pCurrent += length;

		return pCurrent;
	}
	//-----------------------------------------------------------------------
	void MovementAffector::saveToFile(std::ofstream & of)
	{
		/*of.write((const char *)(&mUseStartVelocity),sizeof(int) + sizeof(Ogre::Vector3) * 7);*/
	}
	//*/
	bool MovementAffector::loadFromStream(Serializer* pSerializer,DataStreamPtr& stream)
	{
		if(pSerializer && stream->eof() == false)
		{
			unsigned short chunkId = pSerializer->readChunk(stream);
			if(chunkId != Ogre::ParticleSystem::PSFF_MOVEMENT_AFFECTOR)
			{
				pSerializer->skipChunk(stream);
				return false;
			}			
			//ÊôÐÔ
			pSerializer->readData(stream,&mUseStartVelocity,1,sizeof(mUseStartVelocity));
			pSerializer->readData(stream,&mStartVelocityMin,1,sizeof(mStartVelocityMin));
			pSerializer->readData(stream,&mStartVelocityMax,1,sizeof(mStartVelocityMax));

			pSerializer->readData(stream,&mAcceleration,1,sizeof(mAcceleration));
			pSerializer->readData(stream,&mVelocityLossMin,1,sizeof(mVelocityLossMin));
			pSerializer->readData(stream,&mVelocityLossMax,1,sizeof(mVelocityLossMax));

			pSerializer->readData(stream,&mRandomnessMin,1,sizeof(mRandomnessMin));
			pSerializer->readData(stream,&mRandomnessMax,1,sizeof(mRandomnessMax));
		return true;
		}
		return false;
	}
	bool MovementAffector::saveToBinaryFile(Serializer* pSerializer)
	{
		
		return false;
	}
	std::size_t	MovementAffector::calcSerializedSize()
	{
		return 0;
	}
	//-----------------------------------------------------------------------
