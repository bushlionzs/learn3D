...
public:
	 ...
	 */
	int32				GetIndex(const vector2 *pfvPoint) const;

	/* 得到相邻的凸点 */
	int32				GetNeighborBulgeIndex(int32 nIndex, BOOL bCountSelf, BOOL bClockwise) const;
	int32				GetNeighborBulgeIndex(const vector2 *pfvPoint, BOOL bCountSelf, BOOL bClockwise) const;
	const vector2	*GetNeighborBulge(int32 nIndex, BOOL bCountSelf, BOOL bClockwise) const;
	const vector2	*GetNeighborBulge(const vector2 *pfvPoint, BOOL bCountSelf, BOOL bClockwise) const;

	/* 计算从nStart点到nEnd点绕边经过的距离 */
	FLOAT			GetDistanceSq(int32 nStart, int32 nEnd, BOOL bClockwise, BOOL bOnlyBugle) const;

	const vector2	*GetPoint(int32 nIndex, BOOL bLoop) const;
	const vector2	*GetPoint(int32 nIndex) const;
