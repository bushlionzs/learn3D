...
namespace Ogre {
		...
		for (auto it = particles.begin(); it != particles.end(); it++)
        {
			...
			else
			{
				...
				else if (mUseInterpolatedScale)
				{
					...
					Real			particle_time	= 
                        needFmod ? fmod( ( 1.0f - (p->mTimeToLive / life_time) ) * mRepeatTimes, 1.0f ) : ( 1.0f - (p->mTimeToLive / life_time) ) ;
    ...
    //-----------------------------------------------------------------------
	///*
	char * ScaleInterpolatorAffector::readFromBuffer(char * pCurrent)
	{
		int length = sizeof(int) * 3 + sizeof(ScaleValue) * (MAX_STAGES + 1)
			+ sizeof(Real) * (MAX_STAGES + 5);

		memcpy(&mUseUniformSize,pCurrent,length);
		pCurrent += length;

		return pCurrent;
	}
    //-----------------------------------------------------------------------
	void ScaleInterpolatorAffector::saveToFile(std::ofstream & of)
	{
		/*of.write((const char *)(&mUseUniformSize),sizeof(int) * 3 + sizeof(ScaleValue) * (MAX_STAGES + 1)
			+ sizeof(Real) * (MAX_STAGES + 5) );*/
	}
	//*/
	bool ScaleInterpolatorAffector::loadFromStream(Serializer* pSerializer,DataStreamPtr& stream)
	{
		if(pSerializer && stream->eof() == false)
		{
			unsigned short chunkId = pSerializer->readChunk(stream);
			if(chunkId != Ogre::ParticleSystem::PSFF_SCALE_INTERPOLATOR_AFFECTOR)
			{
				pSerializer->skipChunk(stream);
				return false;
			}			
			//ÊôÐÔ
			pSerializer->readData(stream,&mUseUniformSize,1,sizeof(mUseUniformSize));
			pSerializer->readData(stream,&mWidthRangeStart,1,sizeof(mWidthRangeStart));
			pSerializer->readData(stream,&mWidthRangeEnd,1,sizeof(mWidthRangeEnd));
			pSerializer->readData(stream,&mHeightRangeStart,1,sizeof(mHeightRangeStart));
			pSerializer->readData(stream,&mHeightRangeEnd,1,sizeof(mHeightRangeEnd));

			pSerializer->readData(stream,&mUseConstantScale,1,sizeof(mUseConstantScale));
			pSerializer->readData(stream,&mConstantScaleValue,1,sizeof(mConstantScaleValue));
			pSerializer->readData(stream,&mUseInterpolatedScale,1,sizeof(mUseInterpolatedScale));
			pSerializer->readData(stream,mScaleAdj,1,MAX_STAGES * sizeof(ScaleValue));
	...
	bool ScaleInterpolatorAffector::saveToBinaryFile(Serializer* pSerializer)
	{
		return false;
	}
	std::size_t	ScaleInterpolatorAffector::calcSerializedSize()
	{
