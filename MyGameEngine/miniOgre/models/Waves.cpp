...
Waves::Waves(int m, int n, float dx, float dt, float speed, float damping)
{
    ...
    for(int i = 0; i < m; ++i)
    {
        ...
        for(int j = 0; j < n; ++j)
        {
            ...
            mPrevSolution[i*n + j] = Ogre::Vector3(x, 0.0f, z);
            mCurrSolution[i*n + j] = Ogre::Vector3(x, 0.0f, z);
            mNormals[i*n + j] = Ogre::Vector3(0.0f, 1.0f, 0.0f);
            mTangentX[i*n + j] = Ogre::Vector3(1.0f, 0.0f, 0.0f);
        }
    }
}
	...
	if( t >= mTimeStep )
	{
		...
		//for(int i = 1; i < mNumRows - 1; ++i)
		{
			for(int j = 1; j < mNumCols-1; ++j)
			{
				...
				mTangentX[i*mNumCols+j] = Ogre::Vector3(2.0f*mSpatialStep, r-l, 0.0f);
