		...
		if (node)
		{
			for (int i = 0; i < node->getChildCount(); ++i)
			{
				...
				if (IsUnique(list, displName, eventName))
				{
					...
					// in or out?
					ReadParamNodes<SUIParameterDesc>(eventNode, pNewEvent->InputParams.Params, "name", true, pParent, "in_param");
					ReadParamNodes<SUIParameterDesc>(eventNode, pNewEvent->OutputParams.Params, "name", true, pParent, "out_param");

					// keep support for the older system
					ReadParamNodes<SUIParameterDesc>(eventNode, pNewEvent->InputParams.Params, "name", true, pParent, "param");
				}
				else
...
bool CFlashUIElement::Init(bool bLoadAsset)
{
	...
	m_pFlashplayer->SetExternalInterfaceHandler(this);
...
//------------------------------------------------------------------------------------
void CFlashUIElement::HandleExternalInterfaceCall(const char* pMethodName, const SFlashVarValue* pArgs, int numArgs, void* pUserData /*= 0*/, SFlashVarValue* pResult /*= 0*/)
{
	CRY_PROFILE_FUNCTION(PROFILE_ACTION);
	...
	{
		switch (pArgs[i].GetType())
		{
		case SFlashVarValue::eBool:
			args.AddArgument(pArgs[i].GetBool());
			break;
		case SFlashVarValue::eInt:
			args.AddArgument(pArgs[i].GetInt());
			break;
		case SFlashVarValue::eUInt:
			args.AddArgument(pArgs[i].GetUInt());
			break;
		case SFlashVarValue::eFloat:
			args.AddArgument(pArgs[i].GetFloat());
			break;
		case SFlashVarValue::eDouble:
			args.AddArgument(static_cast<float>(pArgs[i].GetDouble()));
			break;
		case SFlashVarValue::eConstStrPtr:
			args.AddArgument(pArgs[i].GetConstStrPtr());
			break;
		case SFlashVarValue::eConstWstrPtr:
			args.AddArgument(pArgs[i].GetConstWstrPtr());
			break;
		case SFlashVarValue::eNull:
			args.AddArgument("NULL");
			break;
		case SFlashVarValue::eObject:
			args.AddArgument("OBJECT");
			break;
		case SFlashVarValue::eUndefined:
			args.AddArgument("UNDEFINED");
		default:
			CryLogAlways("CFlashUIElement::HandleExternalInterfaceCall could not descern type of argument%i", i);
			break;
		}
	}

	if (HandleInternalCommand(pMethodName, args))
	{
		UIACTION_LOG("%s (%i): Internal command received %s (args: %i)", GetName(), m_iInstanceID, pMethodName, args.GetAsString());
	}
	else
	{
		SUIEventDesc* pEventDesc = NULL;
		for (TUIEventsLookup::iterator iter = m_events.begin(); iter != m_events.end(); ++iter)
		{
			if (!strcmp((*iter)->sName, pMethodName))
			{
				pEventDesc = *iter;
				break;
			}
		...
		{
			UIACTION_LOG("%s (%i): ExternalInterface %s sended (args: %s)", GetName(), m_iInstanceID, pMethodName, args.GetAsString());

			for (TUIEventListener::Notifier notifier(m_eventListener); notifier.IsValid(); notifier.Next())
				notifier->OnUIEventEx(this, pMethodName, args, pUserData);
		}
	}
}
