...
namespace ProfilerRegistry
{
	...
	struct SRegistrar
	{
		...
		{
			std::string strProfilerName = szProfilerName;
			auto pos = strProfilerName.find(' ');

			CRY_ASSERT_MESSAGE(pos == string::npos, "Profiler name mustn't contain spaces");

			pos = strProfilerName.find(' ');
