...
private:
	...
	bool InitScriptSystem(const SSystemInitParams& startupParams) { return true; }
	...
	bool InitMovieSystem(const SSystemInitParams& startupParams) { return true; }
	bool InitSchematyc(const SSystemInitParams& startupParams) { return true; }
	bool InitEntitySystem(const SSystemInitParams& startupParams) { return true; }
	bool InitDynamicResponseSystem(const SSystemInitParams& startupParams);
	bool InitLiveCreate(const SSystemInitParams& startupParams) { return true; }
...
public:
	...
	virtual bool CompressDataBlock(const void* input, size_t inputSize, void* output, size_t& outputSize, int level = 3)
	{
		return false;
	}
	virtual bool DecompressDataBlock(const void* input, size_t inputSize, void* output, size_t& outputSize)
	{
		return false;
	}
	virtual void AutoDetectSpec(const bool detectResolution) {}
	virtual IAVI_Reader* CreateAVIReader() { return nullptr; }
	...
	virtual CryTest::ITestSystem* GetITestSystem() { return nullptr; }
	virtual ICryFactory* LoadModuleWithFactory(const char* szDllName, const CryInterfaceID& moduleInterfaceId);
	virtual bool UnloadEngineModule(const char* szDllName);
private:
