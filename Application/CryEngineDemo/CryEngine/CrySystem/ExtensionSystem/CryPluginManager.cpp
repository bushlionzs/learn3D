...
bool CCryPluginManager::OnPluginLoaded(bool notifyUserOnFailure)
{
	...
	NotifyEventListeners(containedPlugin.m_pPlugin, IEventListener::EEvent::Initialized);
	return true;
}
...
bool CCryPluginManager::UnloadAllPlugins()
{
	...
	for (auto it = m_pluginContainer.rbegin(); it != m_pluginContainer.rend(); ++it)
	{
		...
		// Notification to listeners, that the plugin is about to be un-initialized
		NotifyEventListeners(it->m_pPlugin, IEventListener::EEvent::Unloaded);
...
void CCryPluginManager::NotifyEventListeners(std::shared_ptr<Cry::IEnginePlugin> pPlugin, IEventListener::EEvent event)
{
	for (const TPluginListenerPair& pluginPair : m_pluginListenerMap)
	{
		for (const CryClassID& classId : pluginPair.second)
		{
			if (pPlugin->GetFactory()->ClassSupports(classId))
			{
				pluginPair.first->OnPluginEvent(pPlugin->GetFactory()->GetClassID(), event);
			}
		}
	}
}
