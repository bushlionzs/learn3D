	...
	auto customForwardState = [](CDeviceGraphicsPSODesc& psoDesc, const SGraphicsPipelineStateDescription& desc)
	{
		...
		case OS_ALPHA_BLEND:
			if (bDepthFixup && CRendererCVars::CV_r_HDRTexFormat)
				psoDesc.m_RenderState |= GS_BLEND(BS_SRC1_ALPHA, BS_INV_SRC1_ALPHA, BS_OP_ADD, BS_ONE, BS_ONE, BS_OP_MIN);
			else
				psoDesc.m_RenderState |= GS_BLEND(BS_SRC_ALPHA, BS_INV_SRC_ALPHA, BS_OP_ADD, BS_ONE, BS_ONE, BS_OP_MIN);
			break;
		case OS_ADD_BLEND:
			psoDesc.m_RenderState |= GS_BLEND(BS_ONE, BS_ONE, BS_OP_ADD, BS_ONE, BS_ONE, BS_OP_MIN);
			break;
		case OS_MULTIPLY_BLEND:
			psoDesc.m_RenderState |= GS_BLEND(BS_DST_COLOR, BS_SRC_COLOR, BS_OP_ADD, BS_ONE, BS_ONE, BS_OP_MIN);
			break;
		case 0:
