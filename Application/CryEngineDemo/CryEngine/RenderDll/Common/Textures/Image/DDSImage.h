...
struct FileWrapper
{
	explicit FileWrapper(CCryFile& file) : m_bInMem(false), m_pFile(&file) {}
	FileWrapper(const void* p, size_t nLen) : m_bInMem(true), m_pRaw((const char*)p), m_nRawLen(nLen), m_nSeek(0) {}
