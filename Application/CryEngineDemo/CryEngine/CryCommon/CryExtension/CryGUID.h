...
struct CryGUID
{
	...
	struct StringUtils
	{
		...
		struct SGuidStringSerializer : public Serialization::IString
		{
			...
			void        set(const char* value) { strcpy(buffer,value); }
	...
	inline void ToString(char *output,size_t const output_size_in_bytes) const
	{
		...
		snprintf(output,output_size_in_bytes,"%.8x-%.4x-%.4x-%.2x%.2x-%.2x%.2x%.2x%.2x%.2x%.2x", Data1, Data2, Data3, Data4[0], Data4[1], Data4[2], Data4[3], Data4[4], Data4[5], Data4[6], Data4[7]);
	}
