...
template<class TFuncType, typename Name = std::nullptr_t>
...
public:
	...
	template<typename ... T, 
		typename TName = Name, // Dependent non-type parameter is necessary for mutual exclusive SFINAE to work
		typename = typename std::enable_if<
			!std::is_same<TName, std::nullptr_t>::value &&
	...
	CGenericJob(T... t) // by value because we are going to store them anyway
	{
		...
		SetJobProgramData(gEnv->GetJobManager()->GetJobHandle(Name::value, Invoke));
	}
	...
	template<typename ... T,
		typename TName = Name, // Dependent non-type parameter is necessary for mutual exclusive SFINAE to work
		typename = typename std::enable_if<std::is_same<TName, std::nullptr_t>::value>::type
>
