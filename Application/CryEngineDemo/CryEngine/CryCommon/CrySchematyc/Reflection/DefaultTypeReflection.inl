...
inline void Int64ToString(IString& output, const int64& input)
{
	output.Format("%" PRIi64, input);
}

inline void ReflectType(CTypeDesc<int64>& desc)
{
	desc.SetGUID("{91C63F73-121E-46EE-BF21-0FDBCCCC469A}"_cry_guid);
	desc.SetLabel("Int64");
	desc.SetDescription("Signed 64bit integer");
	desc.SetFlags(ETypeFlags::Switchable);
	desc.SetDefaultValue(0);
	desc.SetToStringOperator<&Int64ToString>();
}

// Reflect 'uint64' type.

inline void UInt64ToString(IString& output, const uint64& input)
{
	output.Format("%" PRIu64, input);
}

inline void ReflectType(CTypeDesc<uint64>& desc)
{
	desc.SetGUID("{0172E517-97BB-4ED6-A030-87B9E65B2997}"_cry_guid);
	desc.SetLabel("UInt64");
	desc.SetDescription("Unsigned 64bit integer");
	desc.SetFlags(ETypeFlags::Switchable);
	desc.SetDefaultValue(0);
	desc.SetToStringOperator<&UInt64ToString>();
}
...
inline void DoubleToString(IString& output, const double& input)
{
	output.Format("%.8f", input);
}

inline void ReflectType(CTypeDesc<double>& desc)
{
	desc.SetGUID("{618768AF-2715-453E-BD5C-66FAA900C7BB}"_cry_guid);
	desc.SetLabel("Double");
	desc.SetDescription("64bit floating point number");
	desc.SetDefaultValue(0.0);
	desc.SetToStringOperator<&DoubleToString>();
}
...
{
	static_assert(sizeof(ExplicitEntityId) == sizeof(uint32), "Unexpected EntityId data type.");

	output.Format("%" PRIu32, static_cast<EntityId>(input));
}
