...
constexpr int XboxMaxSizeGamerTagID = 15;  //< Documentation: Gamertags must be only 15 characters long. 
constexpr int XboxMaxSizeUserID = 20;  //< XUID is from 64 bit unsigned integer.
...
struct IPlatformOS
{
	...
	struct SPlatformEvent
	{
		enum EEventType
		{
			...
			eET_SystemDialog,                   //!< Showing / Terminating a system dialog.
	...
	//! Interface for platform dialogs
	enum class EDialogStatusType
	{
		Waiting,
		Active,
		Finished, // When system call complete but not fully processed
		Terminated, // Normal termination
		TerminatedError, // Abnormal termination
	};

	struct SDialogErrorInfo
	{
		int32 errorCode = 0;
		uint32 userIndex = 0;
	};

	struct SDialogMsgInfo
	{
		...
		int32 msgCode = 0;
		uint32 userIndex = 0;
		const char* userMsg = nullptr;
		EButtonType userMsgButtonType = EButtonType::Ok;
	};

	struct SDialogWebBrowserInfo
	{
		DynArray<const char*> m_displayableDomains; // Only used when skipParentalCheck is true
		const char* url = nullptr;
		bool skipParentalCheck = false;
		uint32 userIndex = 0;
	};

	struct SDialogSignInInfo
	{
		uint32 userIndex = 0;
		int32 orbisUserId = 0;
	};

	struct SCommerceDialogInfo
	{
		uint32 userIndex = 0;
		DynArray<CryStringLocal> categoryLabels;
		uint64 features = 0;
		uint32 serviceLabel = 0;
	};

	struct SDialogMultiplayerSubscriptionInfo : SCommerceDialogInfo
	{};

	struct SDialogBrowseProductInfo : SCommerceDialogInfo
	{};

	struct SDialogUserProfileInfo
	{
		uint32 userIndex = 0;
		uint64 targetUser = 0;
	};

	struct SDialogAddFriendInfo
	{
		uint32 userIndex = 0;
		uint64 targetUser = 0;
	};

	using TDialogInfoVariant = CryVariant<
		SDialogErrorInfo,
		SDialogMsgInfo,
		SDialogSignInInfo,
		SDialogWebBrowserInfo,
		SDialogMultiplayerSubscriptionInfo,
		SDialogBrowseProductInfo,
		SDialogUserProfileInfo,
		SDialogAddFriendInfo>;

	enum class EDialogType
	{
		Error = cry_variant::get_index<SDialogErrorInfo, TDialogInfoVariant>::value,
		Message = cry_variant::get_index<SDialogMsgInfo, TDialogInfoVariant>::value,
		WebBrowser = cry_variant::get_index<SDialogWebBrowserInfo, TDialogInfoVariant>::value,
		SignIn = cry_variant::get_index<SDialogSignInInfo, TDialogInfoVariant>::value,
		MultiplayerSubscription = cry_variant::get_index<SDialogMultiplayerSubscriptionInfo, TDialogInfoVariant>::value,
		BrowseProduct = cry_variant::get_index<SDialogBrowseProductInfo, TDialogInfoVariant>::value,
		UserProfile = cry_variant::get_index<SDialogUserProfileInfo, TDialogInfoVariant>::value,
		AddFriend = cry_variant::get_index<SDialogAddFriendInfo, TDialogInfoVariant>::value,
	};

	struct SPlatformDialogEvent : public SPlatformEvent
	{
		SPlatformDialogEvent(const TDialogInfoVariant& dialogInfo, unsigned int user)
			: SPlatformEvent(user, SPlatformEvent::eET_SystemDialog)
			, m_dialogInfo(dialogInfo)
			, m_dialogStatus(EDialogStatusType::TerminatedError)
			, m_dialogErrorCode(-1)
		{
		}

		struct SMultiplayerSubscription
		{
			int32 m_userId;
			bool m_bHasSubscription;
		} m_multiplayerSubscription;

		const TDialogInfoVariant& m_dialogInfo;
		EDialogStatusType m_dialogStatus;
		int32 m_dialogErrorCode; // Valid only if m_dialogStatus == TerminatedError
	};
	...
	//! Get the user index (if any) associated with a given AccountId of a user.
	virtual unsigned int UserGetIndexFromXUID(const SUserXUID& accountId) const = 0;
#elif CRY_PLATFORM_ORBIS
	//! Get the UserId (Platform, unique id for user in this console) of a user.
	virtual int32 UserGetId(unsigned int userIndex) const = 0;
	//! Get the user index (if any) associated with a given UserId (Platform, unique id for user in this console) of a user.
	virtual unsigned int UserGetIndexFromId(int32 userId) const = 0;
#endif
	...
	virtual const EKeyId GetSystemEnterButton() const { return eKI_Unknown; };
	virtual const EKeyId GetSystemCancelButton() const { return eKI_Unknown; };
...
#if CRY_PLATFORM_ORBIS
	virtual bool ShowDialog(const TDialogInfoVariant& dialogInfo, const int retryShowAttempts = 0, const bool waitForCompletion = false) = 0; // Returns true if handled
