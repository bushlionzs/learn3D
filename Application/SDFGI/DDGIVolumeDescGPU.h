...
{
    uint32_t     rayDataUAVIndex;                    // Index of the ray data UAV on the descriptor heap or in a RWTexture2D resource array
    uint32_t     rayDataSRVIndex;                    // Index of the ray data SRV on the descriptor heap or in a Texture2D resource array
    uint32_t     probeIrradianceUAVIndex;            // Index of the probe irradiance UAV on the descriptor heap or in a RWTexture2DArray resource array
    uint32_t     probeIrradianceSRVIndex;            // Index of the probe irradiance SRV on the descriptor heap or in a Texture2DArray resource array
    //------------------------------------------------- 16B
    uint32_t     probeDistanceUAVIndex;              // Index of the probe distance UAV on the descriptor heap or in a RWTexture2DArray resource array
    uint32_t     probeDistanceSRVIndex;              // Index of the probe distance SRV on the descriptor heap or in a Texture2DArray resource array
    uint32_t     probeDataUAVIndex;                  // Index of the probe data UAV on the descriptor heap or in a RWTexture2DArray resource array
    uint32_t     probeDataSRVIndex;                  // Index of the probe data SRV on the descriptor heap or in a Texture2DArray resource array
    //------------------------------------------------- 32B
    uint32_t     probeVariabilityUAVIndex;           // Index of the probe variability UAV on the descriptor heap or in a RWTexture2DArray resource Array
    uint32_t     probeVariabilitySRVIndex;           // Index of the probe variability SRV on the descriptor heap or in a Texture2DArray resource array
...
struct DDGIVolumeDescGPUPacked
{
    ...
    //------------------------------------------------- 80B
    Ogre::Vector3   probeSpacing;
    uint32_t     packed0;       // probeCounts.x (10), probeCounts.y (10), probeCounts.z (10), unused (2)
    //------------------------------------------------- 96B
    uint32_t     packed1;       // probeRandomRayBackfaceThreshold (16), probeFixedRayBackfaceThreshold (16)
    uint32_t     packed2;       // probeNumRays (16), probeNumIrradianceInteriorTexels (8), probeNumDistanceInteriorTexels (8)
    uint32_t     packed3;       // probeScrollOffsets.x (15) sign bit (1), probeScrollOffsets.y (15) sign bit (1)
...
struct DDGIVolumeDescGPU
{
    ...
    Ogre::Vector3   probeSpacing;                       // world-space distance between probes
    Ogre::Vector3i     probeCounts;                        // number of probes on each axis of the volume
    ...
    Ogre::Vector3i     probeScrollOffsets;                 // grid-space offsets used for scrolling movement
    ...
    // Feature Options
    uint32_t     probeRayDataFormat;                 // texture format of the ray data texture (EDDGIVolumeTextureFormat)
    uint32_t     probeIrradianceFormat;              // texture format of the irradiance texture (EDDGIVolumeTextureFormat)
...
static inline DDGIVolumeDescGPUPacked PackDDGIVolumeDescGPU(const DDGIVolumeDescGPU input)
{
...
static inline DDGIVolumeDescGPU UnpackDDGIVolumeDescGPU(const DDGIVolumeDescGPUPacked input)
{
