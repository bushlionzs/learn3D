...
namespace Configs
{
    ...
    void StoreWorldCounts(std::string source, Ogre::Vector3i& destination)
    {
    ...
    void Store(std::string source, Ogre::Vector3i& destination)
    {
        std::vector<std::string> values = Split(source, ' ');
        destination = { static_cast<int32_t>(stol(values[0])), static_cast<int32_t>(stol(values[1])), static_cast<int32_t>(stol(values[2])) };
    }

    void StoreWorldVector(std::string source, Ogre::Vector3& destination)
    {
    ...
    void StoreEulerAngles(std::string source, Ogre::Vector3& destination)
    {
        assert(false);
    }

    void StoreWorldDistance(std::string source, Ogre::Vector3& destination)
    {
    ...
    void Store(std::string source, Ogre::Vector3& destination)
    {
        std::vector<std::string> values = Split(source, ' ');
        destination = { stof(values[0]), stof(values[1]), stof(values[2]) };
    }
    ...
    {
        destination = (EDDGIVolumeProbeVisType)stoi(source);
    }
    ...
    bool ParseConfigDDGIEntry(const std::vector<std::string>& tokens, const std::string& rhs, Config& config, uint32_t lineNumber, std::ofstream& log)
    {
        ...
        if (tokens[1].compare("volume") == 0)
        {
            ...
            if (tokens[3].compare("name") == 0) 
            { 
                config.ddgi.volumes[volumeIndex].name = data; 
                return true; 
            }
            if (tokens[3].compare("origin") == 0)
            { 
                StoreWorldVector(data, config.ddgi.volumes[volumeIndex].origin); 
                return true; 
            }
            if (tokens[3].compare("rotation") == 0) 
            { 
                StoreEulerAngles(data, config.ddgi.volumes[volumeIndex].eulerAngles); 
                return true; 
            }
            if (tokens[3].compare("probeCounts") == 0) 
            { 
                StoreWorldCounts(data, config.ddgi.volumes[volumeIndex].probeCounts); 
                return true; 
            }
    ...
    */
    ...
    bool ParseConfig(const char* buffer, Config& config, std::ofstream& log)
    {
        ...
        while (!stream.eof())
        {
            ...
            if (tokens[0].compare("app") == 0) 
            { 
                CHECK(ParseConfigAppEntry(tokens, expression[1], config, lineNumber, log), "parse config application entry!", log);
                continue;
            };
            if (tokens[0].compare("shaders") == 0) 
            { 
                CHECK(ParseConfigShadersEntry(tokens, expression[1], config, lineNumber, log), "parse config shaders entry!", log); 
                continue; 
            };
            if (tokens[0].compare("scene") == 0) 
            { 
                CHECK(ParseConfigSceneEntry(tokens, expression[1], config, lineNumber, log), "parse config scene entry!", log); 
                continue; 
            };
            if (tokens[0].compare("input") == 0) 
            { 
                CHECK(ParseConfigInputEntry(tokens, expression[1], config, lineNumber, log), "parse config input entry!", log); 
                continue; 
            };
            if (tokens[0].compare("pt") == 0) 
            { 
                CHECK(ParseConfigPathTraceEntry(tokens, expression[1], config, lineNumber, log), "parse config path trace entry!", log); 
                continue; 
            };
            if (tokens[0].compare("ddgi") == 0) 
            { 
                CHECK(ParseConfigDDGIEntry(tokens, expression[1], config, lineNumber, log), "parse config ddgi entry!", log); 
                continue; 
            };
            if (tokens[0].compare("rtao") == 0) 
            { 
                CHECK(ParseConfigRTAOEntry(tokens, expression[1], config, lineNumber, log), "parse config rtao entry!", log); 
                continue; 
            };
            if (tokens[0].compare("pp") == 0) 
            { 
                CHECK(ParseConfigPostProcessEntry(tokens, expression[1], config, lineNumber, log), "parse config post process entry!", log); 
                continue; 
            };
        }
