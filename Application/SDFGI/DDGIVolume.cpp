...
bool ValidateShaderBytecode(const std::string& bytecode)
{
    if (bytecode.empty()) 
        return false;
    return true;
...
void GetDDGIVolumeTextureDimensions(const DDGIVolumeDesc& desc, EDDGIVolumeTextureType type, uint32_t& width, uint32_t& height, uint32_t& arraySize)
{
    ...
    else
    {
        ...
        else if (type == EDDGIVolumeTextureType::VariabilityAverage)
        {
            ...
            const Ogre::Vector3i NumThreadsInGroup = { 4, 8, 4 };
            // Also divide by sample footprint per-thread, should match ThreadSampleFootprint in ReductionCS.hlsl
            const Ogre::Vector3i DimensionScale = { NumThreadsInGroup.x * 4, NumThreadsInGroup.y * 2, NumThreadsInGroup.z };
...
DDGIVolumeDescGPU DDGIVolumeBase::GetDescGPU() const
{
    ...
    descGPU.probeScrollOffsets.x = std::min(32767, abs(m_probeScrollOffsets.x)) * Ogre::Math::Sign(m_probeScrollOffsets.x);
    descGPU.probeScrollOffsets.y = std::min(32767, abs(m_probeScrollOffsets.y)) * Ogre::Math::Sign(m_probeScrollOffsets.y);
    descGPU.probeScrollOffsets.z = std::min(32767, abs(m_probeScrollOffsets.z)) * Ogre::Math::Sign(m_probeScrollOffsets.z);
...
Ogre::Vector3 DDGIVolumeBase::GetOrigin() const
{
...
Ogre::Vector3 DDGIVolumeBase::GetProbeWorldPosition(int probeIndex) const
{
    // NOTE: If the probe relocation is enabled, the probe offset textures need to be read and added to this value.
    Ogre::Vector3i probeCoords = GetProbeGridCoords(probeIndex);
    Ogre::Vector3 probeGridWorldPosition = m_desc.probeSpacing * probeCoords;
    Ogre::Vector3 probeGridShift = (m_desc.probeSpacing * (m_desc.probeCounts - 1)) / 2.f;

    return (m_desc.origin + probeGridWorldPosition - probeGridShift);
}

Ogre::AxisAlignedBox DDGIVolumeBase::GetAxisAlignedBoundingBox() const
{
    Ogre::Vector3 origin = m_desc.origin;
    Ogre::Vector3 extent = Ogre::Vector3(m_desc.probeSpacing * (m_desc.probeCounts - 1)) / 2.f;
    Ogre::AxisAlignedBox a = { (origin - extent), (origin + extent) };

    // Early out: no rotation
    if (m_desc.eulerAngles == Ogre::Vector3{ 0.f, 0.f, 0.f })
    {
        return a;
    }
    ...
    for (size_t i = 0; i < 3; ++i)
    {
        ...
        {
            float e = m_rotationMatrix[i][j] * a.getMinimum()[j];
            float f = m_rotationMatrix[i][j] * a.getMaximum()[j];
            if (e < f)
            {
                Vector3 vMin = b.getMinimum() + e;
                Vector3 vMax = b.getMaximum() + f;
                b.setMinimum(vMin);
                b.setMaximum(vMax);
            }
            else
            {
                Vector3 vMin = b.getMinimum() + f;
                Vector3 vMax = b.getMaximum() + e;
                b.setMinimum(vMin);
                b.setMaximum(vMax);
            }
        }
...
Ogre::OrientedBoundingBox DDGIVolumeBase::GetOrientedBoundingBox() const
{
    Ogre::OrientedBoundingBox obb = {};
    obb.origin = m_desc.origin;
    obb.rotation = m_rotationQuaternion;
    obb.e = Ogre::Vector3(m_desc.probeSpacing * (m_desc.probeCounts - 1)) / 2.f;
...
void DDGIVolumeBase::SetEulerAngles(const Ogre::Vector3& eulerAngles)
{
...
void DDGIVolumeBase::ComputeScrolling()
{
    ...
    Ogre::Vector3 translation = m_probeScrollAnchor - GetOrigin();
    m_probeScrollDirections = 
        { 
            Ogre::Math::Sign(translation.x), 
            Ogre::Math::Sign(translation.y), 
            Ogre::Math::Sign(translation.z) 
        };

    // Get the number of grid cells between the (effective) origin and the scroll anchor
    Ogre::Vector3i scroll =
    {
        Ogre::Math::AbsFloor(translation.x / m_desc.probeSpacing.x),
        Ogre::Math::AbsFloor(translation.y / m_desc.probeSpacing.y),
        Ogre::Math::AbsFloor(translation.z / m_desc.probeSpacing.z),
    };
...
void DDGIVolumeBase::ComputeRandomRotation()
{
    ...
    float u1 = Ogre::Math::TWO_PI * GetRandomFloat();
    float cos1 = cosf(u1);
    float sin1 = sinf(u1);

    float u2 = Ogre::Math::TWO_PI * GetRandomFloat();
    float cos2 = cosf(u2);
    float sin2 = sinf(u2);
    ...
    Ogre::Matrix3 transform(_11, _12, _13, _21, _22, _23, _31, _32, _33);
