...
struct DDGIVolumeDesc
{
    ...
    Ogre::Vector3          origin = {};                            // World-space origin of the volume
    Ogre::Vector3          eulerAngles = {};                       // Euler rotation angles XYZ (in radians)
    Ogre::Vector3          probeSpacing = {};                      // World-space distance between probes on each axis of the grid

    Ogre::Vector3i            probeCounts = { -1, -1, -1 };           // Number of probes on each axis
...
public:
    ...
    void SetOrigin(const Ogre::Vector3& value) { m_desc.origin = value; }

    void SetScrollAnchor(const Ogre::Vector3& value) { m_probeScrollAnchor = value; }

    void SetProbeSpacing(const Ogre::Vector3& value) { m_desc.probeSpacing = value; }

    void SetEulerAngles(const Ogre::Vector3& eulerAngles);
    ...
    Ogre::Vector3 GetScrollAnchor() const { return m_probeScrollAnchor; }

    Ogre::Vector3i GetScrollOffsets() const { return m_probeScrollOffsets; }

    Ogre::Vector3 GetProbeSpacing() const { return m_desc.probeSpacing; }

    Vector3i GetProbeCounts() const { return m_desc.probeCounts; }
    ...
    Ogre::Vector3 GetEulerAngles() const { 
        return m_desc.eulerAngles; }

    Ogre::Vector3 GetProbeWorldPosition(int probeIndex) const;

    Ogre::AxisAlignedBox GetAxisAlignedBoundingBox() const;
    
    Ogre::OrientedBoundingBox GetOrientedBoundingBox() const;
    ...
    float GetVolumeAverageVariability() const { 
        return m_averageVariability; };

protected:

    void ComputeRandomRotation();
    void ComputeScrolling();
    Ogre::Vector3i GetProbeGridCoords(int probeIndex) const;
...
protected:
    ...
    Ogre::Vector3         m_probeScrollAnchor = { 0.f, 0.f, 0.f }; 
    // Grid-space space offsets for scrolling movement
    Ogre::Vector3i           m_probeScrollOffsets = { 0, 0, 0 }; 
    // Direction of scrolling movement
    Ogre::Vector3i           m_probeScrollDirections = { 0, 0, 0 }; 
    // If probes of a plane need to be cleared due to scrolling movement
    bool           m_probeScrollClear[3] = { 0, 0, 0 };                    
    // Average variability for last update's probe irradiance values
    float          m_averageVariability = 0;                              
    // Toggles whether the volume will insert performance markers in the graphics command list.
    bool           m_insertPerfMarkers = false;                           
private:
    void ScrollReset();
};
