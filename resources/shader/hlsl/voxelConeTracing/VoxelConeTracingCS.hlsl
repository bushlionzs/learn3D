...
{
    float3 offset = float3(VCTMainCB.VoxelSampleOffset, VCTMainCB.VoxelSampleOffset, VCTMainCB.VoxelSampleOffset);
    float3 voxelTextureUV = worldPosition / VoxelizationCB.WorldVoxelScale * 2.0f;
...
float4 TraceCone(float3 pos, float3 normal, float3 direction, float aperture, out float occlusion, bool calculateAO, uint voxelResolution)
{
    ...
    occlusion = 0.0f;
    float voxelWorldSize = VoxelizationCB.WorldVoxelScale / voxelResolution;
    float dist = voxelWorldSize;
    float3 startPos = pos + normal * dist;
    
    float3 weight = direction * direction;

    while (dist < VCTMainCB.MaxConeTraceDistance && color.a < 1.0f)
    {
        ...
        if (occlusion < 1.0f && calculateAO)
            occlusion += ((1.0f - occlusion) * voxelColor.a) / (1.0f + VCTMainCB.AOFalloff * diameter);
        
        dist += diameter * VCTMainCB.SamplingFactor;
    }
...
{
    float4 result;
    float3 viewDirection = normalize(VCTMainCB.CameraPos.rgb - worldPos);
    ...
    return VCTMainCB.IndirectSpecularStrength * result * float4(specular.rgb, 1.0f) * specular.a;
}
...
float4 CalculateIndirectDiffuse(float3 worldPos, float3 normal, out float ao, uint voxelResolution)
{
    ...
    return VCTMainCB.IndirectDiffuseStrength * result;
}
...
void CSMain(uint3 Gid : SV_GroupID, uint3 GTid : SV_GroupThreadID, uint3 DTid : SV_DispatchThreadID)
{
    ...
    float3 normal = normalize(normalBuffer[inPos * VCTMainCB.UpsampleRatio].rgb);
    float4 worldPos = worldPosBuffer[inPos * VCTMainCB.UpsampleRatio];
    float4 albedo = albedoBuffer[inPos * VCTMainCB.UpsampleRatio];
