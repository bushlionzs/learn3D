...
--事件交互入口
--**********************************
function x418097_ProcEventEntry( sceneId, selfId, bagIndex )
-- 不需要这个接口，但要保留空函数
end
...
--**********************************
function x418097_IsSkillLikeScript( sceneId, selfId)
	return 1; --这个脚本需要动作支持
end
...
--**********************************
function x418097_CancelImpacts( sceneId, selfId )
...
--**********************************
function x418097_OnConditionCheck( sceneId, selfId )
...
--**********************************
function x418097_OnDeplete( sceneId, selfId )
	return 1;
end
...
--注：这里是技能生效一次的入口
--**********************************
function x418097_OnActivateOnce( sceneId, selfId, impactId )
	if(DepletingUsedItem(sceneId, selfId)) == 1 then
		SendSpecificImpactToUnit(sceneId, selfId, selfId, selfId,8342, 0);

		--排名相关处理
		x418097_OnTopListYanghua(sceneId,selfId)
...
--注：这里是技能生效一次的入口
--**********************************
function x418097_OnActivateEachTick( sceneId, selfId)
...
--烟发触发
---------------------------------------------------------------------------------------------------
function x418097_OnTopListYanghua(mapId,playerId)
