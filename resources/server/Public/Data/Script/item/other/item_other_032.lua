...
--	end
	...
	AddBindItem( sceneId, 12030202, 1 )	--江湖请柬（12030202）*1 
	AddBindItem( sceneId, 12041101, 15 )	--修行草（12041101）*15
	AddBindItem( sceneId, 12030013, 50 )	--芝华士（12030013）*50
	AddItem( sceneId, 11960005, 1000 )	--波斯绿毡毯（11960005）*1000
	AddItem( sceneId, 11990110, 1 )	--募集券（11990110）*1
	AddItem( sceneId, 12030027, 1 )	--五耀之证（12030027）*1
	AddItem( sceneId, 12041103, 5 )	--活力丹（12041103）*5
	...
	if ret > 0 then
		if(DepletingUsedItem(sceneId, selfId)) == 1 then
			...
			BeginQuestEvent(sceneId)
			AddQuestText(sceneId,"打开礼包，获得了套餐道具！同时获得返还金卡20两！")
			EndQuestEvent(sceneId)
			DispatchQuestTips(sceneId,selfId)
		else
			BeginQuestEvent(sceneId)
		...
		BeginQuestEvent(sceneId)
		AddQuestText(sceneId,"背包已满，无法得到物品！您需要至少有10个背包空间！")
		EndQuestEvent(sceneId)
		DispatchQuestTips(sceneId,selfId)
	end
