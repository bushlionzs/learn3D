...
x300226_g_ScriptId = 300226
x300226_g_TransPosloulan  ="bianjing2gudaodunhuang"
x300226_g_TransPoslaiyin  ="bianjing2gudaodunhuang"
x300226_g_TransPoskunlun  ="bianjing2gudaodunhuang"
x300226_g_TransPosdunhuang="bianjing2gudaodunhuang"


--玩家进入一个 area 时触发
function x300226_ProcAreaEntered( sceneId, selfId, zoneId, MissionId )
	--print("x300226_ProcAreaEntered sceneId = " .. sceneId .. " selfId = " .. selfId)
	...
	if selfType == 9 then	--Bus进入区域 把bus和bus里的人跨场景
		...
		if busType == 1 then --BUS_TYPE_NORMAL
			...
			for	i = 0, memberCount - 1 do
				...
				x300226_TransToPosition(sceneId, ObjID)
			end
			--删除BUS,但乘员仍在车上
...
--		end
	...
	x300226_TransToPosition(sceneId, selfId)
end

function x300226_TransToPosition(sceneId, selfId)

	local guo = GetCurCountry(sceneId,selfId)
	
	if guo == 0 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300226_g_TransPosloulan)
		
		elseif guo == 1 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300226_g_TransPoslaiyin)
		
		elseif guo == 2 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300226_g_TransPoskunlun)
		
		elseif guo == 3 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300226_g_TransPosdunhuang)
	
	end
end

--玩家在一个 area 呆了一段时间没走则定时触发
function x300226_ProcTiming( sceneId, monsterobjid )

end

--玩家离开一个 area 时触发
function x300226_ProcAreaLeaved( sceneId, monsterobjid, ScriptId, MissionId )

end


function x300226_ProcEventEntry( sceneId, monsterobjid )
