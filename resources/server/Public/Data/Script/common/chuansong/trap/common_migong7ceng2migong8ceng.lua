...
x300397_g_ScriptId = 300397
x300397_g_TransPos0	=	"migong7ceng2migong8cengloulan"  
x300397_g_TransPos1	=	"migong7ceng2migong8cengtianshan"  
x300397_g_TransPos2	=	"migong7ceng2migong8cengkunlun"  
x300397_g_TransPos3	=	"migong7ceng2migong8cengdunhuang"


--玩家进入一个 area 时触发
function x300397_ProcAreaEntered( sceneId, selfId, zoneId, MissionId )
	------------------------------------------------------
	...
	if selfType == 9 then	--Bus进入区域 把bus和bus里的人跨场景
		...
		if busType == 1 then --BUS_TYPE_NORMAL
			...
			for	i = 0, memberCount - 1 do
				...
				x300397_TransToPosition(sceneId, ObjID)
			end
			--删除BUS,但乘员仍在车上
...
--		end
	...
	x300397_TransToPosition(sceneId, selfId)

end

function x300397_TransToPosition(sceneId, selfId)

	if sceneId == 110 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300397_g_TransPos0)
	elseif sceneId == 210 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300397_g_TransPos1)
	elseif sceneId == 310 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300397_g_TransPos2)
	elseif sceneId == 410 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300397_g_TransPos3)
	end
end

--玩家在一个 area 呆了一段时间没走则定时触发
function x300397_ProcTiming( sceneId, monsterobjid )

end

--玩家离开一个 area 时触发
function x300397_ProcAreaLeaved( sceneId, monsterobjid, ScriptId, MissionId )

end


function x300397_ProcEventEntry( sceneId, monsterobjid )
