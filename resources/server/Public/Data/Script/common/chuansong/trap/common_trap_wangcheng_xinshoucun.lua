...
x550503_g_ScriptId = 550503
x550503_g_TransPos0	=	"loulanwangcheng2xinshoucun"
x550503_g_TransPos1	=	"laiyinwangcheng2xinshoucun"
x550503_g_TransPos2	=	"kunlunwangcheng2xinshoucun"
x550503_g_TransPos3	=	"dunhuangwangcheng2xinshoucun"


--玩家进入一个 area 时触发
function x550503_ProcAreaEntered( sceneId, selfId, zoneId, MissionId )
	------------------------------------------------------
	...
	if selfType == 9 then	--Bus进入区域 把bus和bus里的人跨场景
		...
		if busType == 1 then --BUS_TYPE_NORMAL
			...
			for	i = 0, memberCount - 1 do
				...
				x550503_TransToPosition(sceneId, ObjID)
			end
			--删除BUS,但乘员仍在车上
...
--		end
	...
	x550503_TransToPosition(sceneId, selfId)

end

function x550503_TransToPosition(sceneId, selfId)

	local nCountry = GetCurCountry(sceneId,selfId)

	if sceneId == 50 and nCountry == 0 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x550503_g_TransPos0)
	elseif sceneId == 150 and nCountry == 1 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x550503_g_TransPos1)
	elseif sceneId == 250 and nCountry == 2 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x550503_g_TransPos2)
	elseif sceneId == 350 and nCountry == 3 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x550503_g_TransPos3)
	else
		BeginQuestEvent(sceneId)
...
function x550503_ProcTiming( sceneId, monsterobjid )

end

--玩家离开一个 area 时触发
function x550503_ProcAreaLeaved( sceneId, monsterobjid, ScriptId, MissionId )

end


function x550503_ProcEventEntry( sceneId, monsterobjid )
