...
x300462_g_ScriptId = 300462
x300462_g_TransPos0	=	"loulanfenglingdu2wangcheng"  
x300462_g_TransPos1	=	"laiyinfenglingdu2wangcheng"  
x300462_g_TransPos2	=	"kunlunfenglingdu2wangcheng"  
x300462_g_TransPos3	=	"dunhuangfenglingdu2wangcheng"


--玩家进入一个 area 时触发
function x300462_ProcAreaEntered( sceneId, selfId, zoneId, MissionId )
	------------------------------------------------------
	...
	if selfType == 9 then	--Bus进入区域 把bus和bus里的人跨场景
		...
		if busType == 1 then --BUS_TYPE_NORMAL
			...
			for	i = 0, memberCount - 1 do
				...
				x300462_TransToPosition(sceneId, ObjID)
			end
			--删除BUS,但乘员仍在车上
...
--		end
	...
	x300462_TransToPosition(sceneId, selfId)

end

function x300462_TransToPosition(sceneId, selfId)

	if sceneId == 38 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300462_g_TransPos0)
	elseif sceneId == 38 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300462_g_TransPos1)
	elseif sceneId == 38 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300462_g_TransPos2)
	elseif sceneId == 38 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300462_g_TransPos3)
	end
end

--玩家在一个 area 呆了一段时间没走则定时触发
function x300462_ProcTiming( sceneId, monsterobjid )

end

--玩家离开一个 area 时触发
function x300462_ProcAreaLeaved( sceneId, monsterobjid, ScriptId, MissionId )

end


function x300462_ProcEventEntry( sceneId, monsterobjid )
