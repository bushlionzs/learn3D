...
x300459_g_TransPos="loulanbianjing2helin"
x300459_g_Notice = "您的等级还不足#R40级#Y，无法前往皇城和林！"

--玩家进入一个 area 时触发
function x300459_ProcAreaEntered( sceneId, selfId, zoneId, MissionId )
	...
	if selfType == 9 then	--Bus进入区域 把bus和bus里的人跨场景
		...
		if busType == 1 then --BUS_TYPE_NORMAL
			...
			for	i = 0, memberCount - 1 do
				...
				CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,ObjID,x300459_g_TransPos)
			end
			--删除BUS,但乘员仍在车上
...
--		end
	...
	if nLevel < 40 then 
		Msg2Player(sceneId,selfId,"#Y"..x300459_g_Notice.."#W",4,3)
	else	
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300459_g_TransPos)
	end
	
end

--玩家在一个 area 呆了一段时间没走则定时触发
function x300459_ProcTiming( sceneId, monsterobjid )

end

--玩家离开一个 area 时触发
function x300459_ProcAreaLeaved( sceneId, monsterobjid, ScriptId, MissionId )

end


function x300459_ProcEventEntry( sceneId, monsterobjid )
