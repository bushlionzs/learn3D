...
x300278_g_ask ={"回到撒马尔罕","确定"}
x300278_g_answer ={"\t你要离开这里，返回撒马尔罕吗？"}

function x300278_ProcEventEntry(sceneId, selfId, NPCId, MissionId,which)	--点击该任务后执行此脚本
		...
		elseif which == 2 then
            ...
            if level>= 40 then
            	if IsQuestHaveDone(sceneId, selfId, 1512) > 0 then
            		...
            		BeginQuestEvent(sceneId)
            		AddQuestText(sceneId,"\t很抱歉，你没有开通#G撒马尔罕的传送#W，我不能将你送到那里。")
            		EndQuestEvent()
            		DispatchQuestEventList(sceneId, selfId, NPCId)
            	end
            end
