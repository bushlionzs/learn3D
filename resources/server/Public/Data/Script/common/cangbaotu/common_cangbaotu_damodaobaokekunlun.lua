...
x300334_g_ScriptId 					= 	300334                  --脚本ID
x300334_g_MissionId 				= 	8097                    --任务ID

x300334_g_MissionKind 				= 	1                       --任务类型
x300334_g_LevelLess					= 	0                      --场景开放等级  <=0 表示不限制

x300334_g_MissionName				= 	"【个人】盗宝客"
x300334_g_MissionTarget				= 	"  击败@npc_%d(%d/1)"          --任务目标
x300334_g_MissionInfo				= 	""            --任务信息
x300334_g_ContinueInfo				= 	"\t怎么？你对付不了那些可恶的盗宝客么？看来我们要另外物色人选了。"  --任务继续信息
x300334_g_MissionCompleted			= 	"\t真没想到啊！你这么快就解决了那里的麻烦。拿好，这就是我们事先约定好的报酬。\n\t等你收集齐了#Y四张藏宝图碎片#W之后就可以在我这里将这些神秘的碎片重新拼合成为一张完整的藏宝图。"  --任务完成信息
x300334_g_MissionHelp				=	"#G据说这些盗宝客身上也带有类似的藏宝图，在清缴他们的过程中，你也很有可能会获得藏宝图的一部分。#W"          --任务提示信息
...
--奖励
x300334_g_ExpBonus					= 	0                    	--奖励：经验 （数值填0，字符串为空，表示没有该项奖励）
x300334_g_BonusItem					=	{}	--奖励：物品

x300334_g_BonusMoney1               = 	0   --任务奖励绑定银币
x300334_g_BonusMoney2               =   0   --任务奖励非绑定银币
x300334_g_BonusMoney3               =   0   --任务奖励绑定金币
x300334_g_BonusMoney4               =   0   --任务奖励朝廷声望
x300334_g_BonusMoney5               =   0   --任务奖励江湖声望
x300334_g_BonusMoney6               =   0   --任务奖励荣誉
...
--任务目标顺序
x300334_MP_TARGET1					= 1         -- 要杀怪1
x300334_MP_TARGET4					= 4         -- 
x300334_MP_FINISH                   = 7         -- 玩家接任务时的等级
x300334_MP_ISCOMPLETE				= 0	        -- 代表完成 0未完成， 1完成

-- 接受任务提示
x300334_g_strAcceptMission          = "您接受了任务：【个人】盗宝客"
-- 放弃任务提示
x300334_g_strAbandonMission         = "您放弃了任务：【个人】盗宝客"
-- 完成任务提示
x300334_g_strFinishMission          = "您完成了任务：【个人】盗宝客"
-- 放弃任务后禁止再领任务的时限(分钟)
...
x300334_SubmitNPCGUID				= 130041	--交任务NPC GUID
...
-- 任务小提示
x300334_g_MissionTips               = "#G据说这些盗宝客身上也带有类似的藏宝图，在清缴他们的过程中，你也很有可能会获得藏宝图的一部分。#W"
-- 任务描述
x300334_g_MissionDescription        = "\t草场附近出现了大量的盗宝客。\n\t如果你能帮我击败他们，我会给你#Y藏宝图碎片#W作为回报，这藏宝图中所藏的宝物虽然比不得国宝，但也算是价值连城了，绝对让你不虚此行。\n\t#G注：本任务每天只能接取10次。#W"
-- 任务攻略
x300334_g_MissionBook               = "\t找到并消灭那些盗宝客，完成任务后，你将得到相应的回报。"
...
x300334_g_strOverMaxTimes           = format( "每天只能接#R%d#cffcf00次这个任务", x300334_g_MaxTimes)
...
--任务入口函数
--**********************************
function x300334_ProcEventEntry(sceneId, selfId, NPCId, MissionId, nExtIdx)	--点击该任务后执行此脚本
	local bHaveMission	= IsHaveQuestNM(sceneId, selfId, x300334_g_MissionId)
	if( bHaveMission > 0) then
		local bDone = x300334_CheckSubmit(sceneId, selfId, NPCId)
		if( bDone > 0) then
			if nExtIdx ~= -1 then
				x300334_DispatchCompletedInfo( sceneId, selfId, NPCId ) --完成任务显示
			end
		else
			if nExtIdx ~= -1 then
				x300334_DispatchContinueInfo( sceneId, selfId, NPCId ) --未完成任务显示
			end
		end
	else
		x300334_DispatchMissionInfo( sceneId, selfId, NPCId )	--任务信息显示
	end
end
...
function x300334_ProcQuestAccept( sceneId, selfId, targetId, missionId )
	if IsHaveQuestNM(sceneId, selfId, x300334_g_MissionId) > 0 then
        ...
        -- if x300334_IsPunish( sceneId, selfId) > 0 then
            -- Msg2Player( sceneId, selfId, "距你上次放弃任务不足"..x300334_g_PunishTime.."分钟，不能领取任务", 8, 3)
            -- return
        -- end
...
function x300334_GetBonus( sceneId, selfId,NpcID, SelectId )
	...
	if x300334_g_BonusMoney1 > 0 then
	    ...
	    Msg2Player(sceneId,selfId,format("获得#R银卡#{_MONEY%d}#W的奖励。", x300334_g_BonusMoney1),4,2)
	end

	--奖励金钱2
	if x300334_g_BonusMoney2 > 0 then
	    AddMoney( sceneId, selfId, 0, x300334_g_BonusMoney2 )
	    Msg2Player(sceneId,selfId,format("获得银币#{_MONEY%d}的奖励。", x300334_g_BonusMoney2),4,2)
	end

	--奖励金钱1
	if x300334_g_BonusMoney3 > 0 then
	    AddMoney( sceneId, selfId, 3, x300334_g_BonusMoney3 )
	    Msg2Player(sceneId,selfId,format("获得金卡#{_MONEY%d}的奖励。", x300334_g_BonusMoney3),4,2)
	end
	...
	if x300334_g_BonusMoney4 > 0 then
		...
		Msg2Player(sceneId,selfId,format("获得江湖声望%d的奖励。", x300334_g_BonusMoney4),4,2)
	end
	...
	if x300334_g_BonusMoney5 > 0 then
		...
		Msg2Player(sceneId,selfId,format("获得朝廷声望%d的奖励。", x300334_g_BonusMoney5),4,2)
	end

	--朝廷声望
	if x300334_g_BonusMoney6 > 0 then
		AddHonor(sceneId,selfId,x300334_g_BonusMoney6)
		Msg2Player(sceneId,selfId,format("获得荣誉值%d的奖励。", x300334_g_BonusMoney6),4,2)
	end
    ...
    if teamSize <= 1 then -- 非组队，单独做任务
        ...
        for i = 0, teamSize - 1 do
            ...
            if memberId >= 0 and IsPlayerStateNormal( sceneId, memberId) == 1 then
                if IsHaveQuest( sceneId, memberId, x300334_g_MissionId) > 0 then -- 如果该成员接受了狩猎任务
