...
x300950_g_ScriptId 					= 300950                  --脚本ID
x300950_g_MissionId 				= 7760                    --任务ID

x300950_g_MissionKind 				= 1                       --任务类型
x300950_g_LevelLess					= 40                      --场景开放等级  <=0 表示不限制

x300950_g_MissionName				= "【国家】蔡州城进贡"
x300950_g_MissionTarget				= "  找到#{_ITEM%d}(%d/%d)"      --任务目标
x300950_g_MissionInfo				= "\t强大的国家不会满足于这一点点荣誉。不断的升级领地图腾会使得领地工资和完成出国任务的经验，荣誉有更高加成。去带来些升级所缺部件，记住，我们是为了荣誉而战！"      --任务信息
x300950_g_ContinueInfo				= "\t找到#{_ITEM%d}"	  --任务继续信息
x300950_g_MissionCompleted			= "\t感谢你为领地图腾升级所做的贡献！"	  --任务完成信息
x300950_g_MissionHelp				= "\t每天所需部件为随机，部件可以与其他人交换以更快的完成任务。"      --任务提示信息
x300950_g_Missionruse				= "\t使用生活技能，制造1个#{_ITEM%d}"      --任务攻略
...
--奖励
x300950_g_ExpBonus					= 0     --奖励：经验 （数值填0，字符串为空，表示没有该项奖励）
x300950_g_BonusItem					= {}	--奖励：物品

x300950_g_BonusMoney1               = 0  --任务奖励绑定银币
x300950_g_BonusMoney2               = 0  --任务奖励非绑定银币
x300950_g_BonusMoney3               = 0  --任务奖励绑定金币
x300950_g_BonusMoney4               = 0  --任务奖励朝廷声望
x300950_g_BonusMoney5               = 0  --任务奖励江湖声望
x300950_g_BonusMoney6               = 0  --任务奖励荣誉
...
function x300950_ProcEventEntry(sceneId, selfId, targetId, MissionId, nExtIdx)
	...
	if nExtIdx > 0 then
		if IsHaveQuest( sceneId, selfId, x300950_g_MissionId) > 0 then
			...
			x300950_DispatchContinueInfo(sceneId, selfId, targetId) --未完成任务显示
        else
			x300950_DispatchMissionInfo(sceneId, selfId, targetId)	--任务信息显示
		end
	else
		...
		if GetQuestParam( sceneId, selfId, misIndex, 7) == 0 then
            ...
            x300950_DispatchCompletedInfo(sceneId, selfId, targetId) --完成任务显示
        end
	end
end
...
function x300950_ProcQuestAccept(sceneId, selfId, targetId, missionId)
    ...
    if IsHaveQuest(sceneId, selfId, x300950_g_MissionId) > 0 then
        ...
        Msg2Player(sceneId, selfId, "您完成了任务:"..x300950_g_MissionName, 8, 3)
        Msg2Player(sceneId, selfId, "您完成了任务:"..x300950_g_MissionName, 8, 2)
		...
		if GetLevel( sceneId, selfId) < x300950_g_LevelLess then
			Msg2Player( sceneId, selfId, format("很抱歉，您的等级不足%d级，无法接受【国家】图腾进贡任务。", x300950_g_LevelLess), 8, 3)
			return
		end
		...
		if x300950_GetDayCount(sceneId, selfId) == 0 then
			Msg2Player( sceneId, selfId, "同一块领地一天只能完成"..x300950_g_MaxCount.."次任务", 8, 3)
            return
		end
		
		local ret = AddQuest( sceneId, selfId, x300950_g_MissionId, x300950_g_ScriptId, 0, 0, 1, 0) -- 给玩家添加任务
		if ret == 0 then
		...
		-- 显示任务接受信息
		Msg2Player( sceneId, selfId, "您获得了任务："..x300950_g_MissionName, 8, 3)
		Msg2Player( sceneId, selfId, "您获得了任务："..x300950_g_MissionName, 8, 2)
...
function x300950_ProcQuestAbandon( sceneId, selfId, MissionId)
	if IsHaveQuest( sceneId, selfId, x300950_g_MissionId) > 0 then
		DelQuest( sceneId, selfId, x300950_g_MissionId)
        x300950_SetDayCount(sceneId, selfId)
		Msg2Player( sceneId, selfId, "您放弃了任务："..x300950_g_MissionName, 8, 3)
		Msg2Player( sceneId, selfId, "您放弃了任务："..x300950_g_MissionName, 8, 2)
	end
end
...
function x300950_ProcQuestAttach( sceneId, selfId, targetId, npcGuid, misIndex, MissionId)
	if IsHaveQuestNM( sceneId, selfId, x300950_g_MissionId) == 0 then	-- 如果没有这个任务
		return
	end
end
...
function x300950_ProcQuestLogRefresh( sceneId, selfId, MissionId)
    ...
    AddQuestLogCustomText( sceneId,
							"",									-- 标题
                            x300950_g_MissionName,              -- 任务名字
                            format(x300950_g_MissionTarget, x300950_g_MissionItemTable[rnd].itemId, 
								count, x300950_g_MissionItemTable[rnd].count),            --任务目标
                            "@npc_"..x300950_g_TargetNPC,       --任务NPC
                             format(x300950_g_Missionruse,x300950_g_MissionItemTable[rnd].itemId), 				--任务攻略
                            x300950_g_MissionInfo,              --任务描述
                            x300950_g_MissionHelp			--任务小提示
                            )
