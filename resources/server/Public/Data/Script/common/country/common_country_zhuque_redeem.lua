x300659_g_ScriptId = 300659
x300659_g_MissionName = "【国家】领取朱雀守护津贴"
x300659_g_GameId =1038
x300659_g_GongValud = 32

function x300659_ProcEnumEvent( sceneId, selfId, targetId, index )
	if GetGameOpenById(x300659_g_GameId) > 0 then
		-- if IsInCountrySpecialGuild(sceneId, selfId,1) > 0  then
		AddQuestNumText(sceneId,x300659_g_ScriptId, x300659_g_MissionName, 3)
		-- end
	end
	
end

function x300659_ProcEventEntry( sceneId , selfId , targetId , state , index )
	GetCountryQuestData(sceneId,selfId,CD_INDEX_CHUGUO_COUNT, x300659_g_ScriptId,-1,"OnReturn1")
end

function x300659_OnReturn1(sceneId, selfId,MissionData,MissionId)
	...
	local zhuqueguild = IsInCountrySpecialGuild(sceneId, selfId,1)
	local toggle = 0
		if IsQinglongZhuque(sceneId, selfId, 1) > 0 then--是朱雀帮主
			awardRongyu =floor(count*3/10)
			awardExp =floor(level*70*count)
			awardJinKa =floor(100*count*0.5/10)
			awardShengWang =floor(count*3)
		elseif IsInCountrySpecialGuild(sceneId, selfId,1) > 0 then
		...
		if IsQinglongZhuque(sceneId, selfId, 1) > 0 then
			awardJinKa =floor(100*count*0.5/10)
		...
		DispatchQuestInfo(sceneId,selfId,-1,x300659_g_ScriptId,-1)
		
		return 	
end


function x300659_ProcAcceptCheck( sceneId, selfId, NPCId )
		return 1
end

function x300659_ProcAccept( sceneId, selfId)
	local hour,minute,second = GetHourMinSec()
		...
		if	GetLevel(sceneId, selfId) < 60 then
			BeginQuestEvent(sceneId)
			AddQuestText(sceneId, "很抱歉，达到60级才可以领取朱雀守护津贴。")
			DispatchQuestTips(sceneId, selfId)
			EndQuestEvent(sceneId)
			Msg2Player(sceneId,selfId,"很抱歉，达到60级才可以领取朱雀守护津贴。",8,2)
			return
		end		
		if IsInCountrySpecialGuild(sceneId, selfId,1)~=1 then
				BeginQuestEvent(sceneId)
				AddQuestText(sceneId, "很抱歉，朱雀守护帮会成员才可以领取朱雀守护津贴。")
				DispatchQuestTips(sceneId, selfId)
				EndQuestEvent(sceneId)
				Msg2Player(sceneId,selfId,"很抱歉，朱雀守护帮会成员才可以领取朱雀守护津贴。",8,2)		
			return
		end
		
		if GetExploit(sceneId, selfId) < x300659_g_GongValud then			
			local msg = format("很抱歉，您的功勋值不足%d点，不能领取。",x300659_g_GongValud)
			BeginQuestEvent(sceneId)
		...
		if lastDay ~= 0 and GetDayOfYear() == lastDay then
			BeginQuestEvent(sceneId)
			AddQuestText(sceneId, "很抱歉，已经领过朱雀守护津贴。")
			DispatchQuestTips(sceneId, selfId)
			EndQuestEvent(sceneId)
			Msg2Player(sceneId,selfId,"很抱歉，已经领过朱雀守护津贴。",8,2)
			return
		end
		...
		GetJoinGuildTime(sceneId,selfId,x300659_g_ScriptId)
end

function x300659_OnMemberJoinTimeReturn( sceneId, selfId, param )
	
	if param == -1 or GetCurrentTime()-param<24*3600 then
		BeginQuestEvent(sceneId)
		AddQuestText(sceneId, "很抱歉，您加入朱雀守护帮的时间低于24小时，不能领取津贴。")
		DispatchQuestTips(sceneId, selfId)
		EndQuestEvent(sceneId)
		Msg2Player(sceneId,selfId,"很抱歉，您加入朱雀守护帮的时间低于24小时，不能领取津贴。",8,2)
		return		
	end
	
	GetCountryQuestData(sceneId,selfId,CD_INDEX_CREATE_ZHUQUE_DATE, x300659_g_ScriptId,-1,"Newqinglong")
end

function x300659_Newqinglong(sceneId, selfId,MissionData,MissionId)
	local curDay = GetDayTime()
		if curDay == MissionData then
			BeginQuestEvent(sceneId);
			AddQuestText(sceneId, "很抱歉，当天上台的朱雀守护成员不能领取工资");
			EndQuestEvent();
			DispatchQuestTips(sceneId, selfId);
			Msg2Player(sceneId,selfId,"很抱歉，当天上台的朱雀守护成员不能领取工资",4,2)
			return
		end
	GetCountryQuestData(sceneId,selfId,CD_INDEX_CHUGUO_COUNT, x300659_g_ScriptId,-1,"OnReturn2")
end


function x300659_OnReturn2(sceneId, selfId,MissionData,MissionId)
	local selfGuildId = GetGuildID(sceneId, selfId)
	...
	if IsQinglongZhuque(sceneId, selfId, 1) > 0 then
		...
		--添加玩法日志
			GamePlayScriptLog( sceneId, selfId, 1442)
			
	elseif IsInCountrySpecialGuild(sceneId, selfId,1) > 0 then --帮众
		local level = GetLevel(sceneId, selfId)
