...
x300881_g_ScriptId 					= 	300881                  --脚本ID


x300881_g_MissionKind 				= 	1                       --任务类型
x300881_g_LevelLess					= 	40                      --场景开放等级  <=0 表示不限制





x300881_g_MissionName				= 	"敌国情报"
x300881_g_MissionTarget				= 	""
x300881_g_MissionInfo				= 	""
x300881_g_ContinueInfo				= 	""
x300881_g_MissionCompleted			= 	""
x300881_g_MissionHelp				=	""          --任务提示信息

--目标
x300881_g_ExtTarget					=	{ {type=20,n=10,target="获得情报"}}

--奖励

x300881_g_ExpBonus					= 	0;--x300881_AddExpAward               	--奖励：经验
x300881_g_BonusItem					=	{}	--奖励：物品

x300881_g_BonusMoney1               = 	0
x300881_g_BonusMoney2               =   0
x300881_g_BonusMoney3               =   0

x300881_g_BonusChoiceItem           =   {}


--MisDescEnd

x300881_g_LevelNeeded = 65

x300881_g_DayCountLimited	=  1

x300881_g_MissionId1 = 7661  
x300881_g_MissionId2 = 7652
x300881_g_MissionId3 = 7658
x300881_g_MissionId4 = 7655
x300881_g_CountryDunhuang =  351--四个国家的场景id
x300881_g_CountryLouLan =  51
x300881_g_CountryKunlun = 251
x300881_g_CountryLaiYin = 151

x300881_g_NeedItemID = 13011006
x300881_g_NeedItemCount = 1

x300881_g_MissionBonusInfo  = 	"#Y获得经验#R%d#Y的奖励。"
              
function x300881_ProcEnumEvent( sceneId, selfId, targetId, MissionId )
	
end


function x300881_CheckSubmit( sceneId, selfId, MissionId )
	if IsHaveQuestNM( sceneId, selfId, MissionId) == 0 then
		return 0;
	end

	if MissionId ~= x300881_g_MissionId1 and MissionId ~= x300881_g_MissionId2 and MissionId ~= x300881_g_MissionId3 and MissionId ~= x300881_g_MissionId4 then
		return 0;
	end

	local ItemCnt = GetItemCount( sceneId, selfId, x300881_g_NeedItemID )

	if ItemCnt<x300881_g_NeedItemCount then
		return 0;
	end
...
--选中任务
function x300881_ProcEventEntry( sceneId, selfId, targetId, MissionId, index )
	...
	if IsHaveQuestNM( sceneId, selfId, MissionId ) == 0 then	-- 如果没有这个任务
		...
		if	IsHaveQuestNM( sceneId, selfId, x300881_g_MissionId1 ) > 0 or IsHaveQuestNM( sceneId, selfId, x300881_g_MissionId2 ) > 0 or IsHaveQuestNM( sceneId, selfId, x300881_g_MissionId3 ) > 0 or IsHaveQuestNM( sceneId, selfId, x300881_g_MissionId4 ) > 0 then
		...
		--不要给出npc身上没有任务的这个提示！
			...
			if ret > 0 then	-- 如果可以接这个任务
				...
				x300881_DispatchMissionInfo( sceneId, selfId, targetId, MissionId )
		...
		if x300881_CheckSubmit( sceneId, selfId, MissionId )>0 then
			--QuestComplateInfoNM( sceneId, selfId, targetId, MissionId ) -- 显示完成信息
			x300881_DispatchCompletedInfo( sceneId, selfId, targetId,MissionId )
		else
			--QuestNotComplateInfoNM( sceneId, selfId, targetId, MissionId ) -- 显示任务未完成信息
			x300881_DispatchContinueInfo( sceneId, selfId, targetId,MissionId )
		end
	end
...
--接受任务
function x300881_ProcQuestAccept( sceneId, selfId, targetId, MissionId )
	...
	if MissionId == x300881_g_MissionId1 then
		nDayCount = x300881_GetDayCount(sceneId, selfId, DQ_MIBAO_DUNHUANG_DATE,DQ_MIBAO_DUNHUANG_DAYCOUNT)
	elseif MissionId == x300881_g_MissionId2 then
		nDayCount = x300881_GetDayCount(sceneId, selfId, DQ_MIBAO_LOULAN_DATE,DQ_MIBAO_LOULAN_DAYCOUNT)
	elseif MissionId == x300881_g_MissionId3 then
		nDayCount = x300881_GetDayCount(sceneId, selfId, DQ_MIBAO_KUNLUN_DATE,DQ_MIBAO_KUNLUN_DAYCOUNT)
	elseif MissionId == x300881_g_MissionId4 then
		nDayCount = x300881_GetDayCount(sceneId, selfId, DQ_MIBAO_LAIYIN_DATE,DQ_MIBAO_LAIYIN_DAYCOUNT)
	else
		return -1;
	end

	if nDayCount>=x300881_g_DayCountLimited then
		BeginQuestEvent(sceneId);
...
--完成任务
--返回1代表成功，0代表交任务失败
function x300881_ProcQuestSubmit( sceneId, selfId, targetId, selectId, MissionId )
	...
	local misIndex = GetQuestIndexByID( sceneId, selfId, MissionId )
	local ret = x300881_CheckSubmit( sceneId, selfId, MissionId ) --QuestCheckSubmitNM( sceneId, selfId, targetId, MissionId, misIndex ) -- 获取完成任务检查信息
	if ret > 0 then -- 如果可以完成
		return x300881_MissionComplate( sceneId, selfId, targetId, selectId, MissionId )
	end
	return 0
end

--任务完成
function x300881_MissionComplate( sceneId, selfId, targetId, selectId, MissionId )
	...
	if ret == 1 then -- 如果成功完成
		...
		if ItemCnt > 0 then --奖励
			DelItem(sceneId, selfId,x300881_g_NeedItemID,ItemCnt)	
		else	
		end	
		--提示玩家任务完成

		if MissionId == x300881_g_MissionId1 then
			x300881_SetDayCount(sceneId, selfId, DQ_MIBAO_DUNHUANG_DATE,DQ_MIBAO_DUNHUANG_DAYCOUNT)
		elseif MissionId == x300881_g_MissionId2 then
			x300881_SetDayCount(sceneId, selfId, DQ_MIBAO_LOULAN_DATE,DQ_MIBAO_LOULAN_DAYCOUNT)
		elseif MissionId == x300881_g_MissionId3 then
			x300881_SetDayCount(sceneId, selfId, DQ_MIBAO_KUNLUN_DATE,DQ_MIBAO_KUNLUN_DAYCOUNT)
		elseif MissionId == x300881_g_MissionId4 then
			x300881_SetDayCount(sceneId, selfId, DQ_MIBAO_LAIYIN_DATE,DQ_MIBAO_LAIYIN_DAYCOUNT)
		else
			return -1;
		end
		...
		x300881_GiveReward(sceneId,selfId)
		...
		if CountryIsFortuneTime(sceneId, selfId, 1) > 0 then
			...
			local strMsg = format("CGL:<Info>x300881_MissionComplate sceneId=%d,country=%d", sceneId, country )
    		WriteLog( 1, strMsg )
		end
...
function x300881_GiveReward(sceneId,selfId)
    ...
    if nExpBonus > 0 then
		...
		DispatchQuestTips(sceneId, selfId);
  		...
  		CallScriptFunction( 256271, "Finishqingbao", sceneId, selfId)
...
function x300881_ProcQuestAttach( sceneId, selfId, npcId, npcGuid,misIndex, MissionId )
...
--放弃
--********************************************************************
function x300881_ProcQuestAbandon( sceneId, selfId,MissionId )
	local ret = 0
...
--杀死怪物
function x300881_ProcQuestObjectKilled( sceneId, selfId, objdataId, objId, MissionId )

end


--物品改变
function x300881_ProcQuestItemChanged( sceneId, selfId, itemdataId, MissionId )--寻找材料的任务完成后再添加合成任务
	...
	if NeedNum > 0 then
		...
		if Num < NeedNum then --还没有完成任务
			...
			x300881_ProcQuestLogRefresh( sceneId, selfId, MissionId)
		elseif Num >= NeedNum then
			...
			if MisCareNPC == 0 then --任务自动完成
				...
				if ret > 0 then -- 如果可以完成
					x300881_ProcQuestLogRefresh( sceneId, selfId, MissionId)
					return x300881_MissionComplate( sceneId, selfId, selfId, -1, MissionId )
				end
			end
...
function x300881_CountryCheck( sceneId, selfId, MissionId )
	if MissionId == x300881_g_MissionId1 then
		if sceneId == x300881_g_CountryDunhuang then
			return 1
		end
	elseif MissionId == x300881_g_MissionId2 then
		if sceneId == x300881_g_CountryLouLan then
			return 1
		end
	elseif MissionId == x300881_g_MissionId3 then
		if sceneId == x300881_g_CountryKunlun then
			return 1
		end
	elseif MissionId == x300881_g_MissionId4 then
		if sceneId == x300881_g_CountryLaiYin then
			return 1
		end
	end
...
function x300881_OnLogInOrDie( sceneId, selfId )
	
   x300881_OnMissionFailed( sceneId, selfId )
end



function x300881_OnMissionFailed( sceneId, selfId )
	
	x300881_DiGuoQiangLiangOnDie(sceneId, selfId)

end

function x300881_DiGuoQiangLiangOnDie( sceneId, selfId )
	if 0==0 then
		return
	end

	local MissionId = x300881_g_MissionId1
	local itemId = x300881_g_NeedItemID;
	...
	MissionId = x300881_g_MissionId2
	if IsHaveQuestNM( sceneId, selfId,MissionId  ) > 0 then	-- 如果有这个任务
	...
	MissionId = x300881_g_MissionId3
	if IsHaveQuestNM( sceneId, selfId,MissionId  ) > 0 then	-- 如果有这个任务
	...
	MissionId = x300881_g_MissionId4
	if IsHaveQuestNM( sceneId, selfId,MissionId  ) > 0 then	-- 如果有这个任务
...
--取得当前次数
---------------------------------------------------------------------------------------------------
function x300881_GetDayCount(sceneId, selfId, nDayIndex, nDayCountIndex)
...
--设置当天次数
---------------------------------------------------------------------------------------------------
function x300881_SetDayCount(sceneId, selfId, nDayIndex, nDayCountIndex)
...
--检查任务接受结果
---------------------------------------------------------------------------------------------------
function x300881_ProcAcceptCheck(sceneId, selfId, NPCId)
	return 1
end
...
--向客户端发送任务信息
---------------------------------------------------------------------------------------------------
function x300881_DispatchMissionInfo( sceneId, selfId, NPCId, MissionId)
	...
	elseif GetLevel(sceneId, selfId) >=70 then
		...
		if MissionId == x300881_g_MissionId1 then
			...
			AddQuestText( sceneId,"  在本国边塞找到@npc_134511，传送到敦煌边塞，到敦煌暗道获取密报。")
			AddQuestText( sceneId," ")
		...
		elseif MissionId == x300881_g_MissionId2 then
			...
			--任务目标
			AddQuestText( sceneId,"#Y任务目标：")
			AddQuestText( sceneId,"  在本国边塞找到@npc_134511，传送到楼兰边塞，到楼兰暗道获取密报。")
			AddQuestText( sceneId," ")
		...
		elseif MissionId == x300881_g_MissionId3 then
			...
			--任务目标
			AddQuestText( sceneId,"#Y任务目标：")
			AddQuestText( sceneId,"  在本国边塞找到@npc_134511，传送到昆仑边塞，到昆仑暗道获取密报。")
			AddQuestText( sceneId," ")
		...
		elseif MissionId == x300881_g_MissionId4 then
			...
			--任务目标
			AddQuestText( sceneId,"#Y任务目标：")
			AddQuestText( sceneId,"  在本国边塞找到@npc_134511，传送到天山边塞，到天山暗道获取密报。")
			AddQuestText( sceneId," ")
	...
	EndQuestEvent()
	DispatchQuestInfoNM(sceneId, selfId, NPCId, x300881_g_ScriptId, MissionId,0);
end

---------------------------------------------------------------------------------------------------
--向客户端发送任务未完成
---------------------------------------------------------------------------------------------------
function x300881_DispatchContinueInfo( sceneId, selfId, NPCId,MissionId )
	...
	EndQuestEvent()
	DispatchQuestDemandInfo(sceneId, selfId, NPCId, x300881_g_ScriptId, MissionId,0);
...
--向客户端发送任务完成信息
---------------------------------------------------------------------------------------------------
function x300881_DispatchCompletedInfo( sceneId, selfId, NPCId,MissionId )
	...
	EndQuestEvent()
	DispatchQuestContinueInfoNM(sceneId, selfId, NPCId, x300881_g_ScriptId, MissionId);
...
--任务日志刷新
---------------------------------------------------------------------------------------------------
function x300881_QuestLogRefresh( sceneId, selfId, MissionId)

	
	local ret = x300881_CheckSubmit( sceneId, selfId, MissionId )
	...
	if MissionId == x300881_g_MissionId1 then
		...
		if ret>0 then
			str = "  从@npc_30015到敦煌暗道获取@npc_30027（1/1）"
		else
			str = "  从@npc_30015到敦煌暗道获取@npc_30027（0/1）"
		end
	elseif MissionId == x300881_g_MissionId2 then
		if ret>0 then
			str = "  从@npc_30012到楼兰暗道获取@npc_30024（1/1）"
		else
			str = "  从@npc_30012到楼兰暗道获取@npc_30024（0/1）"
		end
	elseif MissionId == x300881_g_MissionId3 then
		if ret>0 then
			str = "  从@npc_30014到昆仑暗道获取@npc_30026（1/1）"
		else
			str = "  从@npc_30014到昆仑暗道获取@npc_30026（0/1）"
		end
	elseif MissionId == x300881_g_MissionId4 then
		if ret>0 then
			str = "  从@npc_30013到天山暗道获取@npc_30025（1/1）"
		else
			str = "  从@npc_30013到天山暗道获取@npc_30025（0/1）"
		end
	else
		return -1;
	end
...
function x300881_ProcQuestLogRefresh( sceneId, selfId, MissionId)
	
	x300881_QuestLogRefresh( sceneId, selfId, MissionId );
end


function x300881_OnGetTargetItem( sceneId, selfId, MissionId)
