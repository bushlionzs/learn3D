x310159_g_scriptId                      = 310159

x310159_g_Time					= { min = 2000,max = 2059 }	--时间限制
----------------------------------------------------------------------------------------------
--枚举
----------------------------------------------------------------------------------------------
function x310159_ProcEnumEvent( sceneId, selfId, targetId, MissionId )
    --合服开关
	...
	if now < x310159_g_Time.min or now > x310159_g_Time.max then
		return
	end
	...
	if camp == 1 then
    	...
    	local nTargetSceneId = Country*100 + 104
    ...
    --开启国王召集按钮
    AddQuestNumText(sceneId,x310159_g_scriptId,"【屠城战】攻方国王召集令",3);
    ...
    local strMsg = format("CBL:<Info>x310159_ProcEnumEvent [kingissue] sceneId = %d, name=%s, nCountryId=%d", sceneId, name, CountryID  )
    WriteLog( 1, strMsg )
end

function x310159_OnReturn(sceneId, selfId,MissionData,MissionId)
    ...
    if MissionData == nil or MissionData > 3 or MissionData <= 0 then
		...
		local strMsg = format("CBL:<Error>x310159_OnReturn [kingissue] sceneId = %d, name=%s, count=%d, nCountryId=%d", sceneId, name, MissionData, CountryID  )
        WriteLog( 1, strMsg )
	...
	local strMsg = format("CBL:<Info>x310159_OnReturn [kingissue] sceneId = %d, name=%s, leftcount=%d, nCountryId=%d", sceneId, name, 2, CountryID  )
    WriteLog( 1, strMsg )	
...
--脚本默认事件
----------------------------------------------------------------------------------------------
function x310159_ProcEventEntry( sceneId ,selfId, npcId,idScript,idExt )
    ...
    --回调国王拉人事件
    GetCountryQuestData(sceneId,selfId,CD_INDEX_COUNTRYBATTLELITE_KINGISUSE, x310159_g_scriptId,-1,"OnReturn")
    
end


function x310159_ProcAcceptCheck( sceneId, selfId, NPCId )
    return 1
end
...
--接受
----------------------------------------------------------------------------------------------
function x310159_ProcAccept( sceneId, selfId )
end

----------------------------------------------------------------------------------------------
--放弃
----------------------------------------------------------------------------------------------
function x310159_ProcQuestAbandon( sceneId, selfId, MissionId )
end

----------------------------------------------------------------------------------------------
--继续
----------------------------------------------------------------------------------------------
function x310159_OnContinue( sceneId, selfId, targetId )
end

----------------------------------------------------------------------------------------------
--检测是否可以提交
----------------------------------------------------------------------------------------------
function x310159_CheckSubmit( sceneId, selfId )
end

----------------------------------------------------------------------------------------------
--提交
----------------------------------------------------------------------------------------------
function x310159_ProcQuestSubmit( sceneId, selfId, targetId,selectRadioId, MissionId )
end

----------------------------------------------------------------------------------------------
--杀死怪物或玩家
----------------------------------------------------------------------------------------------
function x310159_ProcQuestObjectKilled( sceneId, selfId, objdataId, objId, MissionId )
end

----------------------------------------------------------------------------------------------
--进入区域事件
----------------------------------------------------------------------------------------------
function x310159_ProcAreaEntered( sceneId, selfId, zoneId, MissionId )
end

----------------------------------------------------------------------------------------------
--道具改变
----------------------------------------------------------------------------------------------
function x310159_ProcQuestItemChanged( sceneId, selfId, itemdataId, MissionId )
end