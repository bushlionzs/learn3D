...
x300883_g_ScriptId 					= 	300883                  --脚本ID


x300883_g_MissionKind 				= 	1                       --任务类型
x300883_g_LevelLess					= 	40                      --场景开放等级  <=0 表示不限制





x300883_g_MissionName				= 	"敌国刺杀"
x300883_g_MissionTarget				= 	""
x300883_g_MissionInfo				= 	""
x300883_g_ContinueInfo				= 	""
x300883_g_MissionCompleted			= 	""
x300883_g_MissionHelp				=	""          --任务提示信息

--目标
x300883_g_ExtTarget					=	{ {type=20,n=10,target="获得刺杀"}}

--奖励

x300883_g_ExpBonus					= 	0;--x300883_AddExpAward               	--奖励：经验
x300883_g_BonusItem					=	{}	--奖励：物品

x300883_g_BonusMoney1               = 	0
x300883_g_BonusMoney2               =   0
x300883_g_BonusMoney3               =   0

x300883_g_BonusChoiceItem           =   {}


--MisDescEnd


x300883_g_KillMissionId =  7007

x300883_g_DayCountLimited   = 1

x300883_g_Mis_Count = 1
x300883_g_ItemID = 13010021         --合成后的物品ID
x300883_g_NeedItem1 = 13011007 --打怪要掉的物品
x300883_g_NeedItem2 = 13011008
x300883_g_NeedItemID = 13011009
--x300883_g_Fail = MD_COUNTRY_KILLMONSTER_ABANDON
x300883_g_LevelNeeded = 100

x300883_g_MissionId1 = 7671
x300883_g_MissionId2 = 7662
x300883_g_MissionId3 = 7668
x300883_g_MissionId4 = 7665
x300883_g_CountryDunhuang =  351--四个国家的场景id
x300883_g_CountryLouLan =  51
x300883_g_CountryKunlun = 251
x300883_g_CountryLaiYin = 151


x300883_g_NeedItemCount = 1


x300883_g_MissionBonusInfo			= 	"#Y获得经验#R%d#Y的奖励。"
         
function x300883_ProcEnumEvent( sceneId, selfId, targetId, MissionId )

end

--选中任务
function x300883_ProcEventEntry( sceneId, selfId, targetId, MissionId, index )

	----PrintStr("x300883_ProcEventEntry")
	...
	if IsHaveQuestNM( sceneId, selfId, MissionId ) == 0 then	-- 如果没有这个任务
		...
		if ret > 0 then	-- 如果可以接这个任务
			...
			x300883_DispatchMissionInfo( sceneId, selfId, targetId, MissionId )
		...
		if x300883_CheckSubmit( sceneId, selfId, MissionId ) > 0 then -- 如果任务可以完成
			--QuestComplateInfoNM( sceneId, selfId, targetId, MissionId ) -- 显示完成信息
			x300883_DispatchCompletedInfo( sceneId, selfId, targetId,MissionId )
		else
			--QuestNotComplateInfoNM( sceneId, selfId, targetId, MissionId ) -- 显示任务未完成信息
			x300883_DispatchContinueInfo( sceneId, selfId, targetId,MissionId )
		end
	end
				
end

function x300883_ProcQuestAttach( sceneId, selfId, npcId, npcGuid,misIndex, MissionId )
...
--完成任务
--返回1代表成功，0代表交任务失败
function x300883_ProcQuestSubmit( sceneId, selfId, targetId, selectId, MissionId )
	...
	if x300883_CheckSubmit( sceneId, selfId, MissionId )>0 then
		x300883_MissionComplate(sceneId, selfId, targetId, selectId, MissionId)
	end

	return 0
end

function x300883_CheckSubmit( sceneId, selfId, MissionId )
	if IsHaveQuestNM( sceneId, selfId, MissionId) == 0 then
		return 0;
	end

	if MissionId ~= x300883_g_MissionId1 and MissionId ~= x300883_g_MissionId2 and MissionId ~= x300883_g_MissionId3 and MissionId ~= x300883_g_MissionId4 then
		return 0;
	end

	local ItemCnt = GetItemCount( sceneId, selfId, x300883_g_NeedItemID )

	if ItemCnt<x300883_g_NeedItemCount then
		return 0;
	end
...
--放弃
--********************************************************************
function x300883_ProcQuestAbandon( sceneId, selfId,MissionId )
	
	if IsHaveQuestNM( sceneId, selfId, MissionId ) <= 0 then	-- 如果有这个任务直接退出
		return 0
	end

	local n = GetItemCount( sceneId, selfId, x300883_g_NeedItemID );

	if MissionId == x300883_g_MissionId1 then
		x300883_SetDayCount(sceneId, selfId, DQ_SHAGUAI_DUNHUANG_DATE,DQ_SHAGUAI_DUNHUANG_DAYCOUNT)
	elseif MissionId == x300883_g_MissionId2 then
		x300883_SetDayCount(sceneId, selfId, DQ_SHAGUAI_LOULAN_DATE,DQ_SHAGUAI_LOULAN_DAYCOUNT)
	elseif MissionId == x300883_g_MissionId3 then
		x300883_SetDayCount(sceneId, selfId, DQ_SHAGUAI_KUNLUN_DATE,DQ_SHAGUAI_KUNLUN_DAYCOUNT)
	elseif MissionId == x300883_g_MissionId4 then
		x300883_SetDayCount(sceneId, selfId, DQ_SHAGUAI_LAIYIN_DATE,DQ_SHAGUAI_LAIYIN_DAYCOUNT)
	else
		return -1;
	end
	
	if n > 0 then
		DelItem(sceneId, selfId,x300883_g_NeedItemID,n)
...
--接受
--********************************************************************
function x300883_ProcQuestAccept( sceneId, selfId, targetId, MissionId )
	...
	if	IsHaveQuestNM( sceneId, selfId, x300883_g_MissionId1 ) > 0 or IsHaveQuestNM( sceneId, selfId, x300883_g_MissionId2 ) > 0 or IsHaveQuestNM( sceneId, selfId, x300883_g_MissionId3 ) > 0 or IsHaveQuestNM( sceneId, selfId, x300883_g_MissionId4 ) > 0 then
		BeginQuestEvent(sceneId)
	...
	if MissionId == x300883_g_MissionId1 then
		nDayCount = x300883_GetDayCount(sceneId, selfId, DQ_SHAGUAI_DUNHUANG_DATE,DQ_SHAGUAI_DUNHUANG_DAYCOUNT)
	elseif MissionId == x300883_g_MissionId2 then
		nDayCount = x300883_GetDayCount(sceneId, selfId, DQ_SHAGUAI_LOULAN_DATE,DQ_SHAGUAI_LOULAN_DAYCOUNT)
	elseif MissionId == x300883_g_MissionId3 then
		nDayCount = x300883_GetDayCount(sceneId, selfId, DQ_SHAGUAI_KUNLUN_DATE,DQ_SHAGUAI_KUNLUN_DAYCOUNT)
	elseif MissionId == x300883_g_MissionId4 then
		nDayCount = x300883_GetDayCount(sceneId, selfId, DQ_SHAGUAI_LAIYIN_DATE,DQ_SHAGUAI_LAIYIN_DAYCOUNT)
	else
		return -1;
	end

	if nDayCount>=x300883_g_DayCountLimited then
		BeginQuestEvent(sceneId);
...
--杀死怪物
function x300883_ProcQuestObjectKilled( sceneId, selfId, objdataId, objId, MissionId )
	
	----PrintStr(objdataId)
	if x300883_CountryCheck( sceneId, selfId, MissionId ) < 0 then
		return -1;
	end
						...
						if GetHp(sceneId, humanObjId) > 0 then
							if IsHaveQuestNM( sceneId, humanObjId, MissionId ) > 0 then	--如果这个人拥有任务
								...
								if InstIndex >= 0 then
									...
									if HaveItem( sceneId, humanObjId, ItemID ) <  x300883_g_NeedItemCount then --如果没有这个物品 
...
--物品改变
function x300883_ProcQuestItemChanged( sceneId, selfId, itemdataId, MissionId )--寻找材料的任务完成后再添加合成任务

	----PrintStr("x300883_ProcQuestItemChanged")
	----PrintNum(itemdataId)
	----PrintNum(MissionId)
	--if x300883_CountryCheck( sceneId, selfId, MissionId ) < 0 then
	--	return 
	...
	if NeedNum > 0 then
		...
		if Num < x300883_g_NeedItemCount then --还没有完成任务
			--BeginQuestEvent(sceneId)
			...
			x300883_QuestLogRefresh( sceneId, selfId, MissionId)
		elseif Num >= x300883_g_NeedItemCount then
			--已经完成任务
			BeginQuestEvent(sceneId)
			...
			DispatchQuestTips( sceneId, selfId )
			--PrintNum(ObjIndex)
			SetQuestByIndex( sceneId, selfId, misIndex, ObjIndex, x300883_g_NeedItemCount )         --by zheng
...
function x300883_MissionComplate(sceneId, selfId, targetId, selectId, MissionId)
	...
	if ret == 1 then -- 如果成功完成
		...
		if ItemCnt > 0 then --双倍奖励
			DelItem(sceneId, selfId,x300883_g_NeedItemID,ItemCnt)	
		else	
		end	
		--提示玩家任务完成
		...
		x300883_GiveReward(sceneId,selfId)
		DelQuestNM( sceneId, selfId, MissionId )
		...
		if CountryIsFortuneTime(sceneId, selfId, 1) > 0 then
			...
			--记日志
			local strMsg = format("CGL:<Info>x300883_MissionComplate sceneId=%d,country=%d", sceneId, country )
    		WriteLog( 1, strMsg )
		end
...
function x300883_GiveReward(sceneId,selfId)
    ...
    if nExpBonus > 0 then
		...
		DispatchQuestTips(sceneId, selfId);
  		...
  		CallScriptFunction( 256270, "Finishcisha", sceneId, selfId)
...
function x300883_OnLogInOrDie( sceneId, selfId )
	
   x300883_OnMissionFailed( sceneId, selfId )
end



function x300883_OnMissionFailed( sceneId, selfId )
	
	x300883_CountryKillOnDie(sceneId, selfId)

end

function x300883_CountryKillOnDie( sceneId, selfId )
	...
	local MissionId = -1
	if IsHaveQuestNM( sceneId, selfId, x300883_g_MissionId1 ) > 0 then
		MissionId = x300883_g_MissionId1
	elseif IsHaveQuestNM( sceneId, selfId, x300883_g_MissionId2 ) > 0 then
		MissionId = x300883_g_MissionId2
	elseif IsHaveQuestNM( sceneId, selfId, x300883_g_MissionId3 ) > 0 then
		MissionId = x300883_g_MissionId3
	elseif IsHaveQuestNM( sceneId, selfId, x300883_g_MissionId4 ) > 0 then
		MissionId = x300883_g_MissionId4
	end
	if MissionId  ~= -1 then
		if GetItemCount( sceneId, selfId,x300883_g_NeedItemID ) > 0 then  --//如果任务完成
			return 0;
		end
		local misIndex = GetQuestIndexByID( sceneId, selfId, MissionId )
...
function x300883_CountryCheck( sceneId, selfId, MissionId )
	if MissionId == x300883_g_MissionId1 then
		if sceneId == x300883_g_CountryDunhuang then
			return 1
		end
	elseif MissionId == x300883_g_MissionId2 then
		if sceneId == x300883_g_CountryLouLan then
			return 1
		end
	elseif MissionId == x300883_g_MissionId3 then
		if sceneId == x300883_g_CountryKunlun then
			return 1
		end
	elseif MissionId == x300883_g_MissionId4 then
		if sceneId == x300883_g_CountryLaiYin then
			return 1
		end
	end
	return -1
...
function x300883_ProcQuestLogRefresh( sceneId,selfId,missionId)
...
--取得当前次数
---------------------------------------------------------------------------------------------------
function x300883_GetDayCount(sceneId, selfId, nDayIndex, nDayCountIndex)
...
--设置当天次数
---------------------------------------------------------------------------------------------------
function x300883_SetDayCount(sceneId, selfId, nDayIndex, nDayCountIndex)
...
--检查任务接受结果
---------------------------------------------------------------------------------------------------
function x300883_ProcAcceptCheck(sceneId, selfId, NPCId)
	return 1
end
...
--向客户端发送任务信息
---------------------------------------------------------------------------------------------------
function x300883_DispatchMissionInfo( sceneId, selfId, NPCId, MissionId)
	...
	elseif GetLevel(sceneId, selfId) >=70 then
		...
		if MissionId == x300883_g_MissionId1 then
			...
			AddQuestText( sceneId,"  在本国边塞找到@npc_131511，传送到敦煌边塞，杀死敦煌边塞（36，53）处的边塞主将，获得将军令。")
			AddQuestText( sceneId," ")
		...
		elseif MissionId == x300883_g_MissionId2 then
			...
			--任务目标
			AddQuestText( sceneId,"#Y任务目标：")
			AddQuestText( sceneId,"  在本国边塞找到@npc_131511，传送到楼兰边塞，杀死楼兰边塞（211，173）处的边塞主将，获得将军令。")
			AddQuestText( sceneId," ")
		...
		elseif MissionId == x300883_g_MissionId3 then
			...
			--任务目标
			AddQuestText( sceneId,"#Y任务目标：")
			AddQuestText( sceneId,"  在本国边塞找到@npc_131511，传送到昆仑边塞，杀死昆仑边塞（212，58）处的边塞主将，获得将军令。")
			AddQuestText( sceneId," ")
		...
		elseif MissionId == x300883_g_MissionId4 then
			...
			--任务目标
			AddQuestText( sceneId,"#Y任务目标：")
			AddQuestText( sceneId,"  在本国边塞找到@npc_131511，传送到天山边塞，杀死天山边塞（36，201）处的边塞主将，获得将军令。")
			AddQuestText( sceneId," ")
	...
	EndQuestEvent()
	DispatchQuestInfoNM(sceneId, selfId, NPCId, x300883_g_ScriptId, MissionId,0);
end

---------------------------------------------------------------------------------------------------
--向客户端发送任务未完成
---------------------------------------------------------------------------------------------------
function x300883_DispatchContinueInfo( sceneId, selfId, NPCId,MissionId )
	...
	EndQuestEvent()
	DispatchQuestDemandInfo(sceneId, selfId, NPCId, x300883_g_ScriptId, MissionId,0);
...
--向客户端发送任务完成信息
---------------------------------------------------------------------------------------------------
function x300883_DispatchCompletedInfo( sceneId, selfId, NPCId,MissionId )
	...
	EndQuestEvent()
	DispatchQuestContinueInfoNM(sceneId, selfId, NPCId, x300883_g_ScriptId, MissionId);
...
--任务日志刷新
---------------------------------------------------------------------------------------------------
function x300883_QuestLogRefresh( sceneId, selfId, MissionId)

	
	local ret = x300883_CheckSubmit( sceneId, selfId, MissionId )
	...
	if MissionId == x300883_g_MissionId1 then
		if ret>0 then
			str = "  从@npc_30011到敦煌边塞杀死@npc_30023（1/1）"
		else
			str = "  从@npc_30011到敦煌边塞杀死@npc_30023（0/1）"
		end
	elseif MissionId == x300883_g_MissionId2 then
		if ret>0 then
			str = "  从@npc_30008到楼兰边塞杀死@npc_30020（1/1）"
		else
			str = "  从@npc_30008到楼兰边塞杀死@npc_30020（0/1）"
		end
	elseif MissionId == x300883_g_MissionId3 then
		if ret>0 then
			str = "  从@npc_30010到昆仑边塞杀死@npc_30022（1/1）"
		else
			str = "  从@npc_30010到昆仑边塞杀死@npc_30022（0/1）"
		end
	elseif MissionId == x300883_g_MissionId4 then
		if ret>0 then
			str = "  从@npc_30009到天山边塞杀死@npc_30021（1/1）"
		else
			str = "  从@npc_30009到天山边塞杀死@npc_30021（0/1）"
		end
	else
		return -1;
	end
