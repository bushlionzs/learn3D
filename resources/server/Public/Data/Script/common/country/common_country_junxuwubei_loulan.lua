...
x300797_g_MissionId 				= 	9415                    --任务ID
x300797_g_ScriptId 					= 	300797                  --脚本ID
x300797_g_MissionKind 				= 	1                       --任务类型
x300797_g_LevelLess					= 	75                      --场景开放等级  <=0 表示不限制


x300797_g_MissionName				= 	"【个人】军需武备"
x300797_g_MissionTarget				= 	""          --任务目标
x300797_g_MissionInfo				= 	"\t时下多事之秋，王国中有很多棘手的事情需要得力的人去做，我希望你能助上一臂之力。\n\t你是否要接受任务？"         
x300797_g_ContinueInfo				= 	"\t怎么？你连这些简单的任务都无法完成？看来我们要另外物色人选了。"  --任务继续信息
x300797_g_MissionCompleted			= 	"\t你的表现让我很欣慰，希望你以后能够继续为我们的王国多做贡献！"  --任务完成信息
x300797_g_MissionHelp				=	"\t军需武备任务每轮为#G30环#W子任务，环数越高，#G经验奖励#W也越高！\n\t军需武备中收集的物品，很多可以通过生活技能制作，或和其他玩家交易得来。"          --任务提示信息

--目标
x300797_g_ExtTarget					=	{ {type=20,n=1,target="【个人】军需武备"}}

--奖励

x300797_g_ExpBonus					= 	0;--x300797_AddExpAward               	--奖励：经验
x300797_g_BonusItem					=	{}	--奖励：物品

x300797_g_BonusMoney1               = 	0
x300797_g_BonusMoney2               =   0
x300797_g_BonusMoney3               =   0

x300797_g_BonusChoiceItem           =   {}


--MisDescEnd

x300797_g_GameId					=	1030
x300797_g_DayCountLimited			=	1
--x300797_g_nDaySpace					=	5    --多少天能做一次

x300797_g_EnterTime					=	{                       --接受任务的时间
											{min=0*60+0,  max=23*60+59},
										}

x300797_g_MaxStep					=	30
x300797_g_ReplyNpcId				=	123827
x300797_g_ReplyNpcObjId				=	3
----------------------------- common start ------------------------

x300797_g_BaseBonusRate = {50000,52000,54000,56000,58000,60000,62000,64000,66000,68000}  --基础奖励基数
x300797_g_ExtraBonusRate = {1,1.6,2.9}  --10环乘数

----------------------------- common end---------------------------

----------------------------- 收集 start ------------------------
x300797_g_MissionInfo2				= 	"\t现在世道纷乱，百姓处在水深火热之中，王国中的有识之士准备组织起来保境安民，但目前国中物资匮乏，急需补充。"
x300797_g_ContinueInfo2				= 	"\t这件物品着这么难以找到吗？"
x300797_g_MissionCompleted2			= 	"\t谢谢你的慷慨解囊。"
x300797_g_MissionHelp2					=	"\t军需武备任务每轮为#G30环#W子任务，环数越高，#G经验奖励#W也越高！\n\t军需武备中收集的物品，很多可以通过生活技能制作，或和其他玩家交易得来。"            --任务提示信息

x300797_g_MissionTable2 =	{ --needvalue:帮会功德
...
function x300797_GetMissionId( sceneId, selfId )
	return x300797_g_MissionId
end

function x300797_GetMissionStep(sceneId, selfId)
...
---调步骤
	...
	if nLastFlag>=x300797_GetMaxStep(sceneId,selfId) then
		nLastFlag = 0
	end
	return nLastFlag+1
...
function x300797_SaveMissionStep(sceneId, selfId)

	local nLastFlag = GetQuestData(sceneId, selfId, MD_GUILD_CIRCLE_MISSION_CURRENT_ID[1], MD_GUILD_CIRCLE_MISSION_CURRENT_ID[2], MD_GUILD_CIRCLE_MISSION_CURRENT_ID[3])



	if nLastFlag<(x300797_GetMaxStep(sceneId,selfId)-1) then
		...
		x300797_DispatchMissionInfo( sceneId, selfId, x300797_g_ReplyNpcObjId, myMissionId, 0 )
	else
		...
		x300797_SetDayCount(sceneId, selfId)
...
function x300797_ResetMissionStep(sceneId, selfId)
	SetQuestData(sceneId, selfId, MD_GUILD_CIRCLE_MISSION_CURRENT_ID[1], MD_GUILD_CIRCLE_MISSION_CURRENT_ID[2], MD_GUILD_CIRCLE_MISSION_CURRENT_ID[3],0)
end

function x300797_GetMissionTypeById(nStep)
	
	local nIndex =0
	for i,item in x300797_g_MissionTable2 do
		nIndex = nIndex+1;
...
function x300797_GetMaxStep(sceneId,selfId)
	return x300797_g_MaxStep
end

function x300797_GetData(sceneId, selfId)
	local nStep = x300797_GetMissionStep(sceneId, selfId)
	local nIndex = x300797_GetMissionTypeById(nStep)
	...
	local nBaseBonusRate = x300797_g_BaseBonusRate[n1]  --基础奖励基数
	local nExtraBonusRate = x300797_g_ExtraBonusRate[n2] --还数奖励基数
...
function x300797_ProcEnumEvent( sceneId, selfId, targetId, MissionId )
	if GetGameOpenById(x300797_g_GameId)<=0 then
		return
	end
    ...
    if level < x300797_g_LevelLess then
       return
    end
	...
	if IsHaveQuestNM( sceneId, selfId, myMissionId ) == 0 then
		...
		AddQuestNumText(sceneId, myMissionId, x300797_g_MissionName,8,1);
		
	else
		local state = GetQuestStateNM(sceneId, selfId, targetId, myMissionId);

		AddQuestNumText(sceneId, myMissionId, x300797_g_MissionName,state,state);
...
--向客户端发送任务未完成
---------------------------------------------------------------------------------------------------
function x300797_ProcEventEntry( sceneId, selfId, targetId, MissionId, nExt )
	
	
	
	if GetGameOpenById(x300797_g_GameId)<=0 then
		x300797_ShowTips(sceneId, selfId, "此活动未开启")
		return
	end
	...
	if IsHaveQuestNM( sceneId, selfId, myMissionId ) == 0 then
		...
		x300797_DispatchMissionInfo( sceneId, selfId, targetId, myMissionId, 0 )
		else
			x300797_ShowTips(sceneId, selfId, "你没有任务"..x300797_g_MissionName)
			return
		end
		...
		if nExt==7 then -- 任务完成
			x300797_DispatchCompletedInfo( sceneId, selfId, targetId,myMissionId )
			
		elseif nExt==30 then
			x300797_DispatchSongxinInfo( sceneId, selfId, targetId,myMissionId )
		else
			
			x300797_DispatchContinueInfo( sceneId, selfId, targetId,myMissionId )
		end
	end
...
function x300797_DispatchSongxinInfo( sceneId, selfId, NPCId,MissionId )
...
--向客户端发送任务信息
---------------------------------------------------------------------------------------------------
function x300797_DispatchMissionInfo( sceneId, selfId, NPCId, MissionId, nExt )
	

	local nStep,nIndex,nExpBonus = x300797_GetData(sceneId, selfId)

	BeginQuestEvent(sceneId)
		AddQuestText(sceneId,"#Y"..x300797_g_MissionName)
		
			
		AddQuestText(sceneId,x300797_g_MissionInfo)
		AddQuestText(sceneId," ")
			
		--AddQuestText( sceneId,"#Y任务目标:")
					
		AddQuestText( sceneId,x300797_g_MissionTarget)
		--AddQuestText(sceneId," ")
	...
	DispatchQuestInfoNM(sceneId, selfId, NPCId, x300797_g_ScriptId, MissionId,0);
end
...
--向客户端发送任务未完成
---------------------------------------------------------------------------------------------------
function x300797_DispatchContinueInfo( sceneId, selfId, NPCId,MissionId )
	...
	EndQuestEvent()
	DispatchQuestDemandInfo(sceneId, selfId, NPCId, x300797_g_ScriptId, MissionId,0);
...
--向客户端发送任务完成信息
---------------------------------------------------------------------------------------------------
function x300797_DispatchCompletedInfo( sceneId, selfId, NPCId,MissionId )
	local myMissionId = x300797_GetMissionId( sceneId, selfId )
	if IsHaveQuestNM( sceneId, selfId, myMissionId ) <= 0 then	-- 如果有这个任务直接退出
		return 0
	end
	...
	DispatchQuestContinueInfoNM(sceneId, selfId, NPCId, x300797_g_ScriptId, MissionId);

end


function x300797_ProcAcceptCheck(sceneId, selfId, targetId, MissionId )
...
function x300797_CheckRequest(sceneId, selfId)
	if GetGameOpenById(x300797_g_GameId)<=0 then
		x300797_ShowTips(sceneId, selfId, "此活动未开启")
		return 0
	end
	...
	if GetLevel(sceneId, selfId) <x300797_g_LevelLess then
		x300797_ShowTips(sceneId, selfId, "您的等级不足，无法接取此任务")
		return 0
	end

	--if GetGuildID( sceneId, selfId ) == -1 then
	--  x300797_ShowTips(sceneId, selfId, "您还没有加入帮会，无法接取此任务")
	--  return 0
	--end

	
	if IsHaveQuestNM( sceneId, selfId, myMissionId ) > 0 then
		x300797_ShowTips(sceneId, selfId, "您已经接取了该任务，请先完成当前任务")
		return 0;
	end


	if x300797_g_DayCountLimited >0 then
		--local nDayCount, nLastDay = x300797_GetDayCount(sceneId, selfId)

		
		--if nDayCount>= x300797_g_DayCountLimited then
		--	local today = GetDayOfYear() 

		--	local nNeedDay = x300797_g_nDaySpace-(today-nLastDay)

		--	x300797_ShowTips(sceneId, selfId, format("很抱歉，【个人】军需武备任务，请%d天后来接任务！",nNeedDay))
		--	return 0;
		--end
		if x300797_GetDayCount(sceneId, selfId) > 0 then
			x300797_ShowTips(sceneId, selfId, "很抱歉，请下周再来接取本任务！")
			x300797_ResetMissionStep(sceneId, selfId)
			return 0
		end
	end
...
function x300797_ProcQuestAccept(sceneId, selfId, targetId, MissionId)
	
	if x300797_CheckRequest(sceneId, selfId)<=0 then
	...
	if bFullMission>0 then
		x300797_ShowTips(sceneId, selfId, "任务已满，无法接受任务")
		return 0
	end
	---------------------------- add end -------------------------------
	local nStep,nIndex,nExpBonus = x300797_GetData(sceneId, selfId)

	local ret = AddQuest( sceneId, selfId, myMissionId, x300797_g_ScriptId, 1, 0, 1,1)-- 给玩家添加任务		
		
	if ret > 0 then
		local misIndex = GetQuestIndexByID( sceneId, selfId, myMissionId )

---调步骤
		local nRandom = random(1,x300797_g_MissionTable2[nIndex].ncount)
		if nRandom < 1 then
			nRandom = 1
		elseif nRandom > x300797_g_MissionTable2[nIndex].ncount then
			nRandom = x300797_g_MissionTable2[nIndex].ncount
		end
		
		local str = "您接受了任务："..x300797_g_MissionName;
		x300797_ShowTips(sceneId, selfId, str)
		Msg2Player(sceneId,selfId,str,8,2)
		
		SetQuestByIndex( sceneId, selfId, misIndex, 6, nRandom )
		

		x300797_ShowSubMission( sceneId, selfId, targetId,MissionId,nRandom )

		x300797_CheckShoujiHaveAll(sceneId, selfId, nRandom)

		GamePlayScriptLog(sceneId, selfId, 1271)
		return 1
	else
		
		x300797_ShowTips(sceneId, selfId, "接受任务错误")
...
function x300797_ShowTips(sceneId, selfId, str)
	BeginQuestEvent(sceneId);
...
function x300797_ProcQuestAbandon( sceneId, selfId, MissionId )
	...
	if ret>0 then
		...
		local str = "您放弃了任务:"..x300797_g_MissionName;
	    x300797_ShowTips(sceneId, selfId, str)
		Msg2Player(sceneId,selfId,str,8,2)
		...
		x300797_ShowTips(sceneId, selfId, "放弃任务失败")
	end
end


function x300797_ProcQuestSubmit( sceneId, selfId, targetId, selectRadioId, MissionId )
	...
	if x300797_CheckSubmit(sceneId, selfId )>0 then
		if x300797_SubmitShouji(sceneId, selfId,targetId)<=0 then
				return 0
			end
		...
		if ret>0 then
			...
			local str = "您完成了任务:"..x300797_g_MissionName;
			x300797_ShowTips(sceneId, selfId, str)
			Msg2Player(sceneId,selfId,str,8,2)

			
			
			x300797_GiveReward(sceneId,selfId)

			x300797_SaveMissionStep(sceneId, selfId)

			CallScriptFunction( 300797, "ProcEventEntry", sceneId, selfId, targetId, x300797_g_MissionId )	
			GamePlayScriptLog(sceneId, selfId, 1272)
			
		else
			x300797_ShowTips(sceneId, selfId, "交任务失败")
		end
...
function x300797_SubmitShouji(sceneId, selfId,targetId)
	...
	local itemid_t = x300797_g_MissionTable2[nIndex].needitem[nRandom].itemid
	local count_t = x300797_g_MissionTable2[nIndex].needitem[nRandom].count
	...
	if GetItemClass(itemid_t)~=10 then
		...
		if nRet<=0 then
			x300797_ShowTips(sceneId, selfId, "删除物品失败")
		...
		BeginUICommand( sceneId)
			UICommand_AddInt( sceneId, x300797_g_ScriptId)
...
function x300797_OnComitSubmitItem(sceneId, selfId,bagIndex)
...
function x300797_GiveReward(sceneId,selfId)
...
function x300797_CheckSubmit(sceneId, selfId )
...
--向客户端发送题目
---------------------------------------------------------------------------------------------------
function x300797_ShowSubMission( sceneId, selfId, NPCId,MissionId, nRandom )
	...
	AddQuestText(sceneId,"#Y"..x300797_g_MissionName)
		...
		local str1 = "  收集"..x300797_g_MissionTable2[nIndex].needitem[nRandom].count.."个#G@itemid_"..x300797_g_MissionTable2[nIndex].needitem[nRandom].itemid.."\n"
		
		AddQuestText(sceneId,x300797_g_MissionInfo2)
		AddQuestText( sceneId,"\n#Y任务目标:")
...
--任务日志刷新
---------------------------------------------------------------------------------------------------
function x300797_QuestLogRefresh( sceneId, selfId, MissionId)
	...
	AddQuestLogCustomText( sceneId,
							"",						-- 标题
							x300797_GetMissionName(sceneId,selfId),        -- 任务名字
							x300797_GetMissionTask(sceneId,selfId),		--任务目标
							"@npc_"..x300797_GetReplyNpcId(sceneId,selfId),			--任务NPC
							x300797_GetMissionMethod(sceneId,selfId),               --任务攻略
							x300797_GetMissionText(sceneId,selfId),	--任务描述
							x300797_GetMissionHelp(sceneId,selfId)					--任务小提示
							)
...
function x300797_GetMissionName(sceneId,selfId)
	
	local nStep,nIndex,nExpBonus = x300797_GetData(sceneId, selfId)

	return x300797_g_MissionName..format("(%d/%d)",nStep,x300797_GetMaxStep(sceneId,selfId));
end

function x300797_GetMissionTask(sceneId,selfId)
	local myMissionId = x300797_GetMissionId( sceneId, selfId )
	local misIndex = GetQuestIndexByID( sceneId, selfId, myMissionId )
	...
	local str1=""
		...
		local id = x300797_g_MissionTable2[nIndex].needitem[nRandom].itemid
		local count_t = x300797_g_MissionTable2[nIndex].needitem[nRandom].count
...
function x300797_GetReplyNpcId(sceneId,selfId)
	return x300797_g_ReplyNpcId;
end

function x300797_GetMissionMethod(sceneId,selfId)
...
function x300797_GetMissionText(sceneId,selfId)
	
	

	return x300797_g_MissionInfo
end

function x300797_GetMissionHelp(sceneId,selfId)
	local nStep,nIndex,nExpBonus = x300797_GetData(sceneId, selfId)
	
		return x300797_g_MissionHelp2
	
end



function x300797_ProcQuestLogRefresh( sceneId, selfId, MissionId)
	
	x300797_QuestLogRefresh( sceneId, selfId, MissionId );
end

--// system function start
function x300797_ProcQuestAttach( sceneId, selfId, npcId, npcGuid,misIndex, MissionId )
...
--取得当前次数
---------------------------------------------------------------------------------------------------
function x300797_GetDayCount(sceneId, selfId)
...
--设置当天次数
---------------------------------------------------------------------------------------------------
function x300797_SetDayCount(sceneId, selfId)
...
--function x300797_CheckSceneOnKill(sceneId, selfId)
	--return 1
--end

--定点使用物品任务
function x300797_PositionUseItem( sceneId, selfId, BagIndex, impactId )
	return 1
end
...
-------------------------------------------------------------------
--物品改变
function x300797_ProcQuestItemChanged( sceneId, selfId, itemdataId, MissionId, nChangeType, nCount)
	...
	if itemid_t==itemdataId then
		...
		if Num<count_t then
			...
			if bCompleted<=0 then
				...
				--x300797_ShowTips(sceneId, selfId, "已完成任务:"..x300797_g_MissionName)
	...
	x300797_QuestLogRefresh( sceneId, selfId, myMissionId );

    
	
end

function x300797_CheckShoujiHaveAll(sceneId, selfId, nRandom)
	local nStep,nIndex,nExpBonus = x300797_GetData(sceneId, selfId)


	local itemid_t = x300797_g_MissionTable2[nIndex].needitem[nRandom].itemid
	x300797_ProcQuestItemChanged( sceneId, selfId,itemid_t, -1, -1, -1)
	
end

function x300797_OnSelectSubmitItem( sceneId, selfId , bagIndex, nGuid_Serial, nGuid_Server, nGuid_World)
	...
	--x300797_ShowTips(sceneId, selfId, format("bagIndex=%d,itemGuid1=%d, itemGuild2=%d, itemGuid3=%d",bagIndex , nGuid_Serial, nGuid_Server, nGuid_World))
	...
	if (CheckItemMatchInBag( sceneId, selfId , bagIndex, itemid_t,nGuid_Serial, nGuid_Server, nGuid_World))>0 then  --如果物品检查正确
		x300797_OnComitSubmitItem(sceneId, selfId,bagIndex)
	end
end


-- 离开帮会
function x300797_OnLeaveGuild(sceneId, selfId, MissionId)

	--DelQuestNM( sceneId, selfId, MissionId)
	--local message = format("您已经不在帮会中了，删除了任务：%s", x300797_g_MissionName);
	--BeginQuestEvent(sceneId);
...
function x300797_ProcQuestObjectKilled(sceneId, selfId, objdataId, objId, MissionId)
end
