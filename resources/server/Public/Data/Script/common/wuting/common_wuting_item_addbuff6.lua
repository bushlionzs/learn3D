...
x300177_g_Impact1   = -1--效果ID，先无视好了
x300177_g_Impact2   = -1 --不用
...
function x300177_OnConditionCheck( sceneId, selfId )
    -- if sceneId ~= 14 then
        ...
        -- AddQuestText( sceneId, "只能在大都美丽会内使用黑雪茄" );
    ...
    -- else
        ...
        if GetImpactContinuanceByDataIndex( sceneId, selfId, x300177_g_BuffId) - GetImpactContinuanceElapsed( sceneId, selfId, x300177_g_BuffId) > 35400000 then
            Msg2Player( sceneId, selfId, "已经达到累加上限，不能继续使用黑雪茄", 8, 3)
            return 0
        end
        ...
        -- if IsHaveSpecificImpact( sceneId, selfId, 7646) == 0 then
            -- Msg2Player( sceneId, selfId, "您身上需要有基础奖励才能使用黑雪茄！", 8, 3)
			-- Msg2Player( sceneId, selfId, "您身上需要有基础奖励才能使用黑雪茄，您可以在酒保或兔侍女处花费少量银卡（现银）领取基础奖励！", 8, 2)
            -- return 0
        -- end
