...
x570013_g_ExtraType					=	2 --1: 荣誉， 2： 声望
x570013_g_ExtraCount				=	2000 -- 所需荣誉/声望值
x570013_g_BonusItemId				= 11000217



x570013_g_MissionName				= "【兑换】国传炫彩钻石"
x570013_g_EnterInfo					= "\t2个#G国传钻石碎片#W和2000#G声望#W\n\t兑换1个#G国传炫彩钻石#W。"
-------------------------
--***********************
--ProcEnumEvent
--***********************
...
function x570013_CheckRequest(sceneId, selfId)
	...
	if ItemCnt<x570013_g_NeedItemCount then
		local str =  format("碎片不足%d个！",x570013_g_NeedItemCount)
	...
	if x570013_g_ExtraType==1 then		--荣誉
		nExtraCount = GetHonor(sceneId, selfId)
	elseif x570013_g_ExtraType==2 then	--声望
	...
	if nExtraCount<x570013_g_ExtraCount then
		...
		if x570013_g_ExtraType==1 then		--荣誉
			str = format("您荣誉不足%d！",x570013_g_ExtraCount)
		elseif x570013_g_ExtraType==2 then	--声望
			str = format("您声望不足%d！",x570013_g_ExtraCount)
		else
			return 0;
		end
