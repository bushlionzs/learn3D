...
--任务文本描述
x570007_g_MissionName = "【龙眼石】兑换神传帽子"
x570007_g_EXCName = "兑换神传帽子"		--兑换名称 将在任务提示和任务面板使用
x570007_g_EXCItemName = "高级龙眼石"		--兑换所需要的道具名称
x570007_g_ItemId = 11050003           --所需道具ID
x570007_g_ItemNum = 350                 --所需数量  
...
x570007_g_MissionInfo = format("\t您确定要使用#G%s#W%s吗？\n\t将需要#R%d颗#W#G%s#W。",x570007_g_EXCItemName,x570007_g_EXCName,x570007_g_ItemNum,x570007_g_EXCItemName)	--任务说明文字
x570007_g_MissionError = format("\t您没有足够数量的%s，无法%s。",x570007_g_EXCItemName,x570007_g_EXCName)	--报错信息

--MisDescEnd
--**********************************
--任务入口函数
--**********************************
function x570007_ProcEventEntry( sceneId, selfId, targetId ,scriptId,seleteId)	--点击该任务后执行此脚本

	local zhiye = GetZhiye(sceneId, selfId)
	local longyannum = GetItemCount(sceneId, selfId, x570007_g_ItemId)
	local strText = format("\t您现在有#R%d颗#W#G%s#W！",longyannum,x570007_g_EXCItemName)
