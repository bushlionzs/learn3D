...
x570021_g_MissionName				= "第八轮兑换[钻石原石]"
x570021_g_EnterInfo					= "\t1个#G次生钻石矿#W兑换1个#G钻石原石#W，\n\t#R每轮仅能兑换一次，请慎重选择兑换物品！"
-------------------------
--***********************
--ProcEnumEvent
--***********************
...
function x570021_CheckRequest(sceneId, selfId)
	...
	if( x570021_g_DayCountUntil > 0 ) then
		...
		if x570021_GetDayCount(sceneId, selfId) >= x570021_g_DayCountUntil then
			...
			local str = format("本轮仅能兑换%d次",x570021_g_DayCountUntil)
...
--		local str = format("当前波数不是第%d波",x570021_G_STEP)
--
--		BeginQuestEvent(sceneId);
--			AddQuestText(sceneId, str);
--		EndQuestEvent();
...
--	end
	...
	if ItemCnt<x570021_g_NeedItemCount then
		local str = format("所需任务道具不足%d个",x570021_g_NeedItemCount)
