...
x570016_g_ExtraType					=	2     --1: 荣誉， 2： 声望， 3：现银， 4：帮贡
x570016_g_ExtraCount				=	15000 --所需声望值
x570016_g_BonusItemId				= 11000220



x570016_g_MissionName				= "【兑换】晕光钻石"
...
function x570016_CheckRequest(sceneId, selfId)
	...
	if ItemCnt<x570016_g_NeedItemCount then
		local str =  format("原石不足%d个！",x570016_g_NeedItemCount)
	...
	if x570016_g_ExtraType==1 then		--荣誉
		nExtraCount = GetHonor(sceneId, selfId)
	elseif x570016_g_ExtraType==2 then	--声望
		nExtraCount = GetShengWang( sceneId, selfId )
    elseif x570016_g_ExtraType==3 then  --现银
        nExtraCount = GetMoney( sceneId, selfId, 0 )
    elseif x570016_g_ExtraType==4 then  --帮贡
	...
	if nExtraCount<x570016_g_ExtraCount then
		...
		if x570016_g_ExtraType==1 then		--荣誉
			str = format("您荣誉不足%d！",x570016_g_ExtraCount)
		elseif x570016_g_ExtraType==2 then	--声望
			str = format("您声望不足%d！",x570016_g_ExtraCount)
		elseif x570016_g_ExtraType==3 then  --现银
			str = format("您现银不足%d两！",x570016_g_ExtraCount/1000)
        elseif x570016_g_ExtraType==4 then  --帮贡
   			str = format("您帮贡不足%d！",x570016_g_ExtraCount)
        else
			return 0;
		end
