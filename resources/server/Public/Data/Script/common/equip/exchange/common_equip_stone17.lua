...
x570027_g_MissionName				= "第九轮兑换[国传钻石碎片]"
x570027_g_EnterInfo					= "\t1个#G国传钻石碎片劵#W兑换1个#G国传钻石碎片#W，\n\t#R每轮仅能兑换一次，请慎重选择兑换物品！"
-------------------------
--***********************
--ProcEnumEvent
--***********************
...
function x570027_CheckRequest(sceneId, selfId)
	...
	if( x570027_g_DayCountUntil > 0 ) then
		...
		if x570027_GetDayCount(sceneId, selfId) >= x570027_g_DayCountUntil then
			...
			local str = format("本轮仅能兑换%d次",x570027_g_DayCountUntil)
	...
	if nCurrentGameStep~=x570027_G_STEP then
		local str = format("当前波数不是第%d波",x570027_G_STEP)
	...
	if ItemCnt<x570027_g_NeedItemCount then
		local str = format("所需任务道具不足%d个",x570027_g_NeedItemCount)
