...
--任务文本描述
x570006_g_MissionName="【龙眼石】兑换国传腰带"
x570006_g_EXCName = "兑换国传腰带"		--兑换名称 将在任务提示和任务面板使用
x570006_g_EXCItemName = "中级龙眼石"		--兑换所需要的道具名称
x570006_g_ItemId = 11050002         --所需道具ID
x570006_g_ItemNum = 80  							--所需数量
...
x570006_g_MissionInfo = format("\t您确定要使用#G%s#W%s吗？\n\t将需要#R%d颗#W#G%s#W。",x570006_g_EXCItemName,x570006_g_EXCName,x570006_g_ItemNum,x570006_g_EXCItemName)	--任务说明文字
x570006_g_MissionError = format("\t您没有足够数量的%s，无法%s。",x570006_g_EXCItemName,x570006_g_EXCName)	--报错信息

--MisDescEnd
--**********************************
--任务入口函数
--**********************************
function x570006_ProcEventEntry( sceneId, selfId, targetId ,scriptId,seleteId)	--点击该任务后执行此脚本

	local zhiye = GetZhiye(sceneId, selfId)
	local longyannum = GetItemCount(sceneId, selfId, x570006_g_ItemId)
	local strText = format("\t您现在有#R%d颗#W#G%s#W！",longyannum,x570006_g_EXCItemName)
