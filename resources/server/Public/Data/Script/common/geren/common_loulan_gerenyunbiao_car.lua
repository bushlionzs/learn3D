...
x300623_g_ScriptId = 300623
x300623_g_MissionName = "创建镖车"

--无敌impact在表中的索引,玩家进入BUS后获得一个无敌impact,
x300623_g_GodIndex = 1210
...
x300623_g_MissionId = {
						{ ID = 7001,iCostMoney = 1000, GameId = 51 },
						{ ID = 7009,iCostMoney = 2000, GameId = 61 },
						{ ID = 7010,iCostMoney = 3000,GameId = 71 },
						{ ID = 7543,iCostMoney = 5000,GameId = 81 },
						{ ID = 7544,iCostMoney = 6000,GameId = 741 },
						{ ID = 7545,iCostMoney = 7500,GameId = 751 },
					  }
...
function x300623_ProcEventEntry(sceneId, selfId, targetId)	--点击该任务后执行此脚本
...
--**********************************
function x300623_ProcEnumEvent(sceneId, selfId, targetId)
	--AddQuestNumText(sceneId, x300623_g_ScriptId, x300623_g_MissionName);
end


function x300623_OnCreateBus( sceneId, selfId,MissionId ,BusID)

    local PosX = 65
    local PosZ = 179.5
	local ObjID = CreateBus(sceneId, x300623_g_Carinfo[BusID].CarType, PosX, PosZ, x300623_g_Carinfo[BusID].BaseAI, x300623_g_Carinfo[BusID].AIScript, x300623_g_Carinfo[BusID].Script, selfId, x300623_g_GodIndex,MissionId,-1)
end

function x300623_OnCreateBusOK(sceneId,selfId,busId,targetId,MissionId)	
...
function x300623_DeleteBus(sceneId, ObjId)
...
--selfId：玩家ID
--targetId：BUSID
function x300623_OnIntoBusEvent(sceneId, selfId,targetId)
...
function x300623_OnIntoBusAfter(sceneId, selfId,BusID)
	...
	for i,itm in x300623_g_MissionId do
...
--玩家离开BUS
--selfId：玩家ID
function x300623_OnLeaveBusEvent(sceneId, selfId,BusId)
		BeginQuestEvent(sceneId)
...
function x300623_OnDie(sceneId,BusId,idKiller)
	...
	if idKiller == -1 then
		...
		if OwnerID ~= -1 then
			x300623_Abandon( sceneId, OwnerID  )
...
function x300623_OnTime(sceneId,BusId)

end

function x300623_OnLifeTimeOut(sceneId,BusId)
    local selfId = GetBusMemberObjId(sceneId,BusId,0)
    if(IsPlayerStateNormal(sceneId, selfId) ~= 0) then
		x300623_Abandon( sceneId, selfId  )
	end
	DeleteBus(sceneId, BusId,1)
end

function x300623_OnWaitTimeOut(sceneId,BusId)
	local selfId = GetBusMemberObjId(sceneId,BusId,0)
	if(IsPlayerStateNormal(sceneId, selfId) ~= 0) then
		x300623_Abandon( sceneId, selfId  )
...
function x300623_OnBusStopWhenOwnerFarAway(sceneId, ownerId)
end
...
function x300623_CleaupMission( sceneId, selfId )
...
--更新当天接任务次数
---------------------------------------------------------------------------------------------------
function x300623_SetDayCount(sceneId, selfId)
