...
x300981_g_ScriptId = 300981

x300981_g_MissionId_List = {
							7800,	--楼兰的对应任务号
							7801,	--天山的对应任务号
							7802,	--昆仑的对应任务号
							7803,	--敦煌的对应任务号
							}
x300981_g_MissionId = 7801


x300981_g_Title = "防御之路"

x300981_g_Profession_Enum = {	--12种职业
								0,	--武士  
								1,	--剑侠  
								2, 	--骑射  
								3, 	--火枪  
...
x300981_g_Profession_Desc = {	--12种职业
								"武士",
								"剑侠",  
								"骑射",  
								"火枪",  
...
--x300981_g_Profession  = 0   --全局变量，每次点击任务npc的时候会随机一个
--x300981_g_Profession_Desc = "" --全局变量，每次点击任务npc的时候随机一个
--x300981_g_Kill_Count  = 0   --全局变量，记录击败某种职业的玩家的个数  
--x300981_g_Completed   = 0   --全局变量，标记是否完成任务
x300981_g_Quest_Max_Count = 1	--全局变量，本任务只限接1次,使用专精之书·防御可以多接1次


x300981_g_MissionName = "【个人】防御之路"
x300981_g_MissionInfo				= 	"\t防御与攻击的关系很微妙。想要追求至高的防御，没有一定的攻击也是没有意义的。所以我要给你一个考验——击败某一个职业的6名敌国高手。通过了这个考验之后我会帮你接近那至高的防御。#G（击败的玩家不低于自己20级才有效。）" 
x300981_g_Mission_Kill_Cnt = 6				   

x300981_g_CommitNPC     = "@npc_141103"

--print(".........................................")					   
--需要的等级
function x300981_ProcEnumEvent(sceneId, selfId, NPCId, MissionId)
	
	if GetCurCountry(sceneId,selfId) ~=1 or GetLevel(sceneId, selfId) < 80 then
		return -1
	end	

	--PrintStr("x300981_ProcEnumEvent")
	local state = GetQuestStateNM(sceneId,selfId,NPCId,x300981_g_MissionId)
	AddQuestTextNM( sceneId, selfId, NPCId, x300981_g_MissionId, state, -1 )
...
--如果没有任务；接任务
--如果有任务，显示是否继续或者任务完成
function x300981_ProcEventEntry(sceneId, selfId, NPCId,nScriptId, nFlag)
	...
	--PrintStr("x300981_ProcEventEntry")
	...
	local nCnt = x300981_GetDayCount(sceneId, selfId) + 1
	print("-----------------nCntItem_tuozhanzhishu",nCntItem_tuozhanzhishu,"nCnt",nCnt)
    if IsHaveQuestNM( sceneId, selfId, x300981_g_MissionId ) == 0 then	-- 如果没有这个任务 
				
				if nCnt == 1 then --第一次接任务
							x300981_DistachMission_Info(sceneId,selfId,NPCId,1)
				elseif  nCnt == 2 then
							if nCntItem_tuozhanzhishu > 0  then 
															...
															x300981_DistachMission_Info(sceneId,selfId,NPCId,2)
															--Msg2Player( sceneId, selfId, "接受防御任务消耗专精之书·防御道具一个", 8, 3)
							else
															BeginQuestEvent(sceneId)
															AddQuestText( sceneId, "#Y"..x300981_g_MissionName)
				...
				local misIndex = GetQuestIndexByID( sceneId, selfId, x300981_g_MissionId)
				...
				if isFinish > 0 then -- 如果任务可以完成
							    --x300340_MissionComplateInfo( sceneId, selfId, targetId, MissionId) -- 显示完成信息
							  	BeginQuestEvent( sceneId)
								AddQuestText( sceneId, "#Y"..x300981_g_MissionName.."#W\n\t不错，不错。按照咱们的约定，我会帮助你的防御更上一层楼。" )
								AddQuestText( sceneId, "\n#Y任务奖励： ")
								AddQuestText( sceneId, "专防训练点数：13点 ")	
								EndQuestEvent( sceneId)
								DispatchQuestInfoNM( sceneId, selfId, NPCId, x300981_g_ScriptId, x300981_g_MissionId)
								--SetQuestByIndex( sceneId, selfId, misIndex, 7,0) --完成任务标志清0
								
				else
								--PrintNum(isFinish)			        
								--x300340_MissionContinue( sceneId, selfId, targetId, x300981_g_MissionId) -- 显示任务未完成信息
								...
								if lastday ~= today then --如果是第一次杀人
									nRemain = x300981_g_Mission_Kill_Cnt
								else										
									local nKillCnt = GetQuestParam( sceneId, selfId, misIndex , 2 )
									nRemain = x300981_g_Mission_Kill_Cnt - nKillCnt
								end
								...
								print("------------------------------->",nIdx)
								if nIdx <=0 or nIdx > getn(x300981_g_Profession_Desc) then
									return -1
								end
								local strPro   = x300981_g_Profession_Desc[nIdx]								
								BeginQuestEvent( sceneId)
								AddQuestText( sceneId, "#Y"..x300981_g_MissionName)
...
function x300981_DistachMission_Info(sceneId,selfId,NPCId,nFlag)
							--local misIndex = GetQuestIndexByID( sceneId, selfId, x300981_g_MissionId ) 
							...
							if nFlag == 2 then		
												BeginQuestEvent( sceneId)
												AddQuestText( sceneId, "#Y"..x300981_g_MissionName)
												AddQuestText( sceneId, x300981_g_MissionInfo)
												--任务目标
...
function x300981_ProcQuestAccept(sceneId, selfId, targetId, MissionId)
	--PrintStr("x300981_ProcQuestAccept")
    ...
    if IsHaveQuestNM( sceneId, selfId, x300981_g_MissionId) == 0 then   -- 没有这个任务
				        -- 检查当天是否已经做过这个任务,如果接过一次而且有专精之书·防御道具的话，可以再接一次任务

				        ---------------------------------if x300981_GetDayCount(sceneId, selfId) ~= 0 then
				        ...
				        ---------------------------------end
						--如果不是本国的成员，则无法接任务
						if GetCurCountry(sceneId, selfId) ~= 1 then
							--print("00000000000000000000000000")
							return -1
						end
						
						local nCnt = x300981_GetDayCount(sceneId, selfId)
						...
						local ret	= AddQuest( sceneId, selfId, x300981_g_MissionId, x300981_g_ScriptId, 1, 0, 0) --注意这里的参数
						if ret == 0 then
											...
											-- 更新接任务时间
											x300981_SetDayCount(sceneId, selfId)   								
											BeginQuestEvent( sceneId)
						...
						DelQuest(sceneId, selfId , x300981_g_MissionId) --完成任务后删除任务  
						--x300981_SetDayCount(sceneId, selfId)
...
--如果是则计数加1
function x300981_UpdateKillCountProfession(sceneId,selfId,dieId)
	...
	if IsHaveQuestNM( sceneId, selfId, x300981_g_MissionId ) == 0 then	-- 如果没有这个任务
						    ...
						    if teamSize <= 1 then -- 非组队，单独做任务
											...
											local misIndex 		=  GetQuestIndexByID( sceneId, selfId, x300981_g_MissionId )
											local nIndex 		=  GetQuestParam(sceneId, selfId, misIndex,3)
											local nProfession   =  x300981_g_Profession_Enum[nIndex]
											
											--判断当前杀人的数目,这里仅仅借用了计算接任务次数的MD的天数
											local today = GetDayOfYear()
											local lastday = GetQuestData(sceneId, selfId, MD_FANGYUZHILU_DATE[1], MD_FANGYUZHILU_DATE[2], MD_FANGYUZHILU_DATE[3])
											if lastday ~= today then --如果是第一次杀人
														if nProfession      == GetZhiye(sceneId, dieId) and GetCurCountry(sceneId,dieId) ~=1  then 
																local nKillCnt = 1
																...
																x300981_QuestLogRefresh( sceneId, selfId, x300981_g_MissionId)
														end
											else	
														if nProfession      == GetZhiye(sceneId, dieId) and GetCurCountry(sceneId,dieId) ~=1 then          --如果被击败的玩家的职业和接任务的随机的玩家职业符合
																local nKillCnt = GetQuestParam( sceneId, selfId, misIndex , 2 )
																nKillCnt       = nKillCnt + 1
																SetQuestByIndex(sceneId, selfId, misIndex,2,nKillCnt) --使用第3位保存本次任务击败的某种职业的玩家的数量 
																if nKillCnt >= x300981_g_Mission_Kill_Cnt then
								        ...
								        for i = 0, teamSize - 1 do
								            ...
								            if memberId >= 0 and IsPlayerStateNormal( sceneId, memberId) == 1 then
													if IsHaveQuestNM( sceneId, memberId, x300981_g_MissionId) ~= 0 then -- 如果该成员接受了防御之路			
																				local misIndex = GetQuestIndexByID( sceneId, memberId, x300981_g_MissionId)
															                    if GetQuestParam( sceneId, memberId, misIndex, 0) == 0 or GetQuestParam( sceneId, memberId, misIndex, 7) == 0 then
																								...
																								if lastday ~= today then --如果是第一次杀人
																											if nProfession      == GetZhiye(sceneId, dieId) and GetCurCountry(sceneId,dieId) ~=1  then 
																													...
																													x300981_QuestLogRefresh( sceneId, memberId, x300981_g_MissionId)
																											end
																								else	
																											if nProfession      == GetZhiye(sceneId, dieId) and GetCurCountry(sceneId,dieId) ~=1 then          --如果被击败的玩家的职业和接任务的随机的玩家职业符合
...
function x300981_ProcQuestSubmit(sceneId, selfId, NPCId, selectRadioId, MissionId)
	--判断是否有任务
	local bHaveMission = IsHaveQuestNM(sceneId, selfId, x300981_g_MissionId);
...
--此函数仅供专精之书·防御道具脚本调用
--function x300981_Update_Quest_Max_Count(sceneID,selfId)
--	--x300981_g_Quest_Max_Count = 2
--	local misIndex = GetQuestIndexByID( sceneId, selfId, x300981_g_MissionId )
...
--放弃
--********************************************************************
function x300981_ProcQuestAbandon( sceneId, selfId, MissionId )
	--放弃任务同样计数
	--PrintStr("x300981_ProcQuestAbandon")
	DelQuest( sceneId, selfId, x300981_g_MissionId )
	--x300981_SetDayCount(sceneId, selfId)   
...
---------------------------------------------------------------------------------------------------
function x300981_GetDayCount(sceneId, selfId)
	--PrintStr("x300981_GetDayCount")
	if x300981_g_Quest_Max_Count > 0 then
		local today = GetDayOfYear()
		local lastday = GetQuestData(sceneId, selfId, MD_FANGYUZHILU_DATE[1], MD_FANGYUZHILU_DATE[2], MD_FANGYUZHILU_DATE[3])
...
--from common_daguanyuan_tianfu.lua
---------------------------------------------------------------------------------------------------
function x300981_SetDayCount(sceneId, selfId)
	--PrintStr("x300981_SetDayCount")
    local today = GetDayOfYear()
	local lastday = GetQuestData(sceneId, selfId, MD_FANGYUZHILU_DATE[1], MD_FANGYUZHILU_DATE[2], MD_FANGYUZHILU_DATE[3])
	if lastday ~= today then
...
function x300981_ProcQuestObjectKilled(sceneId, selfId, objdataId, objId, MissionId) 
	--print("----------------->","x300981_ProcQuestObjectKilled")
	--需要在这里判断是否击败的练功人偶的怪物！
	--to be fixed!!!
    ...
    if teamSize <= 1 then -- 非组队，单独做任务
				...
				x300981_QuestLogRefresh( sceneId, selfId, MissionId)
		            ...
		            if memberId >= 0 and IsPlayerStateNormal( sceneId, memberId) == 1 then
							if IsHaveQuestNM( sceneId, memberId, x300981_g_MissionId) ~= 0 then -- 如果该成员接受了防御之路			
									                    ...
									                    if GetQuestParam( sceneId, memberId, misIndex, 0) == 0 or GetQuestParam( sceneId, memberId, misIndex, 7) == 0 then
																		...
																		x300981_QuestLogRefresh( sceneId, memberId, MissionId)
														end
							end
				  end
			end
...
function x300981_ProcQuestLogRefresh( sceneId, selfId, MissionId)
	x300981_QuestLogRefresh( sceneId, selfId, MissionId );
end

function x300981_QuestLogRefresh( sceneId, selfId, MissionId)
	--print("----------------------x300981_QuestLogRefresh")
    if IsHaveQuestNM( sceneId, selfId, x300981_g_MissionId ) == 0 then	-- 如果没有这个任务 
    		return
    end
	...
	if nKillCnt < x300981_g_Mission_Kill_Cnt then
				AddQuestLogCustomText( sceneId,
										x300981_g_MissionName,		-- 标题
										x300981_g_MissionName,      -- 任务名字
										mission.."("..strTarget.."/6)",					--任务目标
										x300981_g_CommitNPC,		--任务NPC
										mission,               		--任务攻略
										x300981_g_MissionInfo.."\n \n#Y任务奖励：#W\n专防训练点数：13点 ",				--任务描述
	...
	EndQuestEvent()
	DispatchQuestLogUpdate(sceneId, selfId, x300981_g_MissionId);
end
