...
x300763_g_ScriptId = 300763
x300763_g_MissionName = "创建镖车"
x300763_g_Mis_Count   = 3
--无敌impact在表中的索引,玩家进入BUS后获得一个无敌impact,
x300763_g_GodIndex = 1210
...
function x300763_ProcEventEntry(sceneId, selfId, targetId)	--点击该任务后执行此脚本
...
--**********************************
function x300763_ProcEnumEvent(sceneId, selfId, targetId)
	--AddQuestNumText(sceneId, x300763_g_ScriptId, x300763_g_MissionName);
end


function x300763_OnCreateBus( sceneId, selfId,MissionId ,BusID)
  --local PosX,PosZ = GetWorldPos(sceneId, selfId)
  local PosX = 66	
	local PosZ = 172
	local ObjID = CreateBus(sceneId, x300763_g_Carinfo[BusID].CarType, PosX, PosZ, x300763_g_Carinfo[BusID].BaseAI, x300763_g_Carinfo[BusID].AIScript, x300763_g_Carinfo[BusID].Script, selfId, x300763_g_GodIndex,MissionId,-1)
end

function x300763_OnCreateBusOK(sceneId,selfId,busId,targetId,MissionId)
...
function x300763_DeleteBus(sceneId, ObjId)
...
--selfId：玩家ID
--targetId：BUSID
function x300763_OnIntoBusEvent(sceneId, selfId,targetId)
...
function x300763_OnIntoBusAfter(sceneId, selfId,BusID)
	...
	local iCostMoney = x300763_CalcCostMoney(sceneId, selfId)
...
--玩家离开BUS
--selfId：玩家ID
function x300763_OnLeaveBusEvent(sceneId, selfId,BusId)
		BeginQuestEvent(sceneId)
...
function x300763_OnDie(sceneId,BusId,idKiller)
	...
	if idKiller == -1 then
		...
		if OwnerID ~= -1 then
			x300763_Abandon( sceneId, OwnerID  )
	...
	if ObjType == 1 then     --被玩家杀死
		...
		local iCostMoney = x300763_CalcAddMoney(sceneId, BusId)
...
function x300763_OnTime(sceneId,BusId)

end

function x300763_OnLifeTimeOut(sceneId,BusId)
    local selfId = GetBusMemberObjId(sceneId,BusId,0)
    if(IsPlayerStateNormal(sceneId, selfId) ~= 0) then
		x300763_Abandon( sceneId, selfId  )
	end
	DeleteBus(sceneId, BusId,1)
end

function x300763_OnWaitTimeOut(sceneId,BusId)
	local selfId = GetBusMemberObjId(sceneId,BusId,0)
	if(IsPlayerStateNormal(sceneId, selfId) ~= 0) then
		x300763_Abandon( sceneId, selfId  )
...
function x300763_OnBusStopWhenOwnerFarAway(sceneId, ownerId)
end
...
--放弃
--********************************************************************
function x300763_Abandon( sceneId, selfId )

	if IsHaveQuestNM( sceneId, selfId, x300763_g_MissionId) ~= 0 then	-- 如果有这个任务
		DelQuestNM( sceneId, selfId, x300763_g_MissionId)
		x300763_SetDayCount(sceneId, selfId)
		return
	end

end

function x300763_CleaupMission( sceneId, selfId )
...
--取得此任务当天当前已完成次数
---------------------------------------------------------------------------------------------------
function x300763_GetDayCount(sceneId, selfId)

	if x300763_g_Mis_Count > 0 then
...
--更新当天接任务次数
---------------------------------------------------------------------------------------------------
function x300763_SetDayCount(sceneId, selfId)
	local today = GetDayOfYear()
...
--计算花多少钱
function x300763_CalcCostMoney(sceneId, selfId)
	local count = x300763_GetDayCount(sceneId, selfId)
	if count == 0 then
		return 75000
	elseif count == 1 then
		return 150000
...
--计算奖励多少钱
function x300763_CalcAddMoney(sceneId, BusId)
	local nBusIndex = GetBusIndex(sceneId,BusId)
	print ("x300763_CalcAddMoney,nBusIndex:",nBusIndex)
	if nBusIndex == x300763_g_Carinfo[1].CarType then
			return 52500
	elseif nBusIndex == x300763_g_Carinfo[2].CarType then
			return 105000
	elseif nBusIndex == x300763_g_Carinfo[3].CarType then
			return 157500
	end
	return 0
end