...
function x300903_ProcEnumEvent( sceneId, selfId, targetId, MissionId )
	...
	for i,itm in x300903_g_SubMissionId do
...
function x300903_ProcEventEntry( sceneId, selfId, targetId, MissionId )
	local missionDesc = x300903_GetMissionDesc(sceneId, selfId)
	if missionDesc ~= nil then
		if IsHaveQuestNM( sceneId, selfId, missionDesc.MissionId ) > 0 then
			...
			if(GetQuestParam(sceneId, selfId, misIndex, 7) == 1) then
				x300903_DispatchCompletedInfo( sceneId, selfId, targetId) --完成任务显示
			else
				x300903_DispatchContinueInfo( sceneId, selfId, targetId	) --未完成任务显示
			end
		end
		return
	end
	for i,itm in x300903_g_SubMissionId do
		if 	GetLevel(sceneId,selfId) <= itm.level then
			x300903_DispatchMissionInfo( sceneId, selfId, targetId, itm.MissionId )
			return
		end
...
function x300903_CheckSubmit(sceneId, selfId,index)
	for i,itm in x300903_g_SubMissionId do
...
function x300903_DispatchCompletedInfo( sceneId, selfId, targetId,index )
	local missionDesc = x300903_GetMissionDesc(sceneId, selfId)
	...
	--1、经验
		...
		--2 帮供
		if x300903_g_BonusMoney8>0 then
			AddQuestMoneyBonus8(sceneId,x300903_g_BonusMoney8)
		end
		--3 帮会经验
		-- if x300903_g_GuildExpBonus>0 then
			-- AddQuestGuildExpBonus(sceneId,x300903_g_GuildExpBonus)
		-- end
		EndQuestEvent(sceneId)
		DispatchQuestContinueInfoNM(sceneId, selfId, targetId, x300903_g_ScriptId, missionDesc.MissionId);
end

function x300903_DispatchContinueInfo( sceneId, selfId, targetId,index )
	...
	--任务提示信息
		EndQuestEvent()
		DispatchQuestDemandInfo(sceneId, selfId, targetId, x300903_g_ScriptId, missionDesc.MissionId,0);
...
--向客户端发送任务信息
---------------------------------------------------------------------------------------------------
function x300903_DispatchMissionInfo( sceneId, selfId, NPCId,index )
		local missionDesc
		for i,itm in x300903_g_SubMissionId do
			if 	GetLevel(sceneId,selfId) <= itm.level then
				missionDesc = itm
		...
		--任务信息
		local BangExp =x300903_g_BangExp
		AddQuestText(sceneId,"#Y"..missionDesc.MissionName)
		AddQuestText(sceneId,format("%s", missionDesc.MissionInfo))
		AddQuestText(sceneId," ")
		...
		-- end
		EndQuestEvent(sceneId)
		DispatchQuestInfoNM(sceneId, selfId, NPCId, x300903_g_ScriptId, missionDesc.MissionId);
...
--获得奖励
---------------------------------------------------------------------------------------------------
function x300903_OnReturn1(sceneId, selfId,MissionData,MissionId,SubMissionId)
	...
	x300903_GetBonus( sceneId,selfId, isMultiple,SubMissionId )
end

function x300903_GetBonus( sceneId, selfId, isMultiple,SubMissionId )
...
--奖励帮贡
    if x300903_g_BonusMoney8 > 0 then
		AddGuildUserPoint(sceneId,selfId,x300903_g_BonusMoney8)	--增加帮贡
		local BonusMoney8 = x300903_g_BonusMoney8
		BeginQuestEvent(sceneId)
		AddQuestText(sceneId, "获得#R帮贡"..BonusMoney8.."点#o的奖励");
		EndQuestEvent(sceneId)
		DispatchQuestTips(sceneId,selfId)	
		Msg2Player(sceneId,selfId,format("获得#R帮贡"..BonusMoney8.."点#o的奖励", x300903_g_BonusMoney8),8,2)
			if isMultiple == 1 then
			local pos = GetGuildOfficial(sceneId, selfId)
				if pos == 5 then
					local guildid =GetGuildID(sceneId, selfId)
				...
				elseif pos == 4 then
					...
					EndQuestEvent(sceneId)
					DispatchQuestTips(sceneId,selfId)	
					Msg2Player(sceneId,selfId,format("帮会多倍任务额外获得#R帮贡"..BonusMoney8.."点#o的奖励", x300903_g_BonusMoney8),8,2)
				end
			end
	end
end
...
--接受
--**********************************
function x300903_ProcQuestAccept( sceneId, selfId, targetId, MissionId )
	...
	if x300903_GetDayCount(sceneId, selfId) >= x300903_g_Mis_Count then
		BeginQuestEvent(sceneId)
...
--完成任务
--返回1代表成功，0代表交任务失败
function x300903_ProcQuestSubmit( sceneId, selfId, targetId, selectId, MissionId )
	...
	CallScriptFunction( 256267, "Finishdanmu", sceneId, selfId)
	GamePlayScriptLog(sceneId,selfId,102)

	x300903_SetDayCount(sceneId, selfId)
	GetGuildQuestData(sceneId, selfId, GD_GUILD_INDEX_MULTI_MISSION_HEIMU, x300903_g_ScriptId,MissionId,"OnReturn1",missionDesc.MissionId)
	return 0
end

function x300903_ProcQuestAttach( sceneId, selfId, npcId, npcGuid,misIndex, MissionId )
...
--放弃
--********************************************************************
function x300903_ProcQuestAbandon( sceneId, selfId,MissionId )
...
function x300903_CheckWhichItem(sceneId, selfId)
	for i,itm in x300903_g_SubMissionId do
...
function x300903_GetMissionDesc(sceneId, selfId)
	for i,itm in x300903_g_SubMissionId do
		if IsHaveQuestNM(sceneId, selfId,itm.MissionId ) > 0 then
			return itm
		end
...
function x300903_ProcQuestItemChanged( sceneId, selfId, itemdataId, MissionId, nChangeType )
	local nCheckItemId = x300903_CheckWhichItem(sceneId, selfId)
	...
	if nChangeType ==1 then
		...
		if itemNum < NeedNum then
			...
			SetQuestByIndex( sceneId, selfId, misIndex, 7, 0 )
			x300903_QuestLogRefresh( sceneId, selfId, MissionId );
		end
		return
	end
...
function x300903_QuestLogRefresh( sceneId, selfId, MissionId)
	...
	if x300903_g_BonusMoney8 > 0 then
	AddQuestMoneyBonus8(sceneId, x300903_g_BonusMoney8 )	--增加帮贡
	end
	-- if x300903_g_GuildExpBonus>0 then
	-- AddQuestGuildExpBonus(sceneId,x300903_g_GuildExpBonus)
	-- end
...
function x300903_ProcQuestLogRefresh( sceneId, selfId, MissionId)
...
--取得此任务当天当前已完成次数
---------------------------------------------------------------------------------------------------
function x300903_GetDayCount(sceneId, selfId)

	if x300903_g_Mis_Count > 0 then
...
--更新当天接任务次数
---------------------------------------------------------------------------------------------------
function x300903_SetDayCount(sceneId, selfId)
	local today = GetDayOfYear()
