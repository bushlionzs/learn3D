...
--MisDescBegin
x300869_g_ScriptId 					= 	300869                  --脚本ID

x300869_g_MissionKind 				= 	1                       --任务类型
x300869_g_LevelLess					= 	20                      --任务最小等级  <=0 表示不限制
x300869_g_MissionName				= 	"【帮会】帮会神兽"           --任务名称(512)

x300869_g_MissionTarget				= 	"  护送神兽经过@npc_130531中转后到达@npc_131516处"          --任务目标
x300869_g_MissionInfo				= 	"\t帮主带领帮会成员，每天可在王城参加一次【帮会】帮会神兽任务。\n\t帮主需要在帮会任务发布人处缴纳#G10两现银#W才能接受此任务。帮会成员只需此时在帮主周围，便可自动接到任务，进入护送神兽状态。\n\t在护送神兽过程中，神兽只跟随帮主移动，如帮主离神兽过远，神兽会#G停止前进#W。如果神兽受到袭击并死亡，帮主的#G押金#W将被对方抢走，全体成员#G任务失败#W。\n\t帮主离开神兽一段时间或掉线，神兽消失，所有人#G任务失败#W。\n\t由于路途遥远，神兽中途需要休息，帮主在@npc_130531处点击#R神兽驿站#W按扭，当前在驿站周围的帮众都会获得经验奖励。并且在最后完成护兽任务时都会获得经验加成的奖励。\n\t顺利护送神兽到达指定地点，帮主可得到返还的押金和#G巨额经验#W，参与护送的帮会成员得到#G巨额经验#W。\n\t帮主到达指定地点提交任务，此时只有帮主周围并且已经参与护送的帮会成员才能完成任务并获得#G巨额奖励#W，同时帮会增加100点帮会经验#W。当帮主发布#G护兽令#W时2小时内回复任务可获得翻倍奖励（经验，帮贡）和2点使命点！"            --任务信息
x300869_g_ContinueInfo				= 	"神兽乃祥瑞之兽，我在等待它的降临"  --任务继续信息
x300869_g_MissionCompleted			= 	"恭喜您顺利完成任务！这是您应得的奖励，请收下！"  --任务完成信息
x300869_g_MissionHelp				=	"\t接受任务后，会在帮主屏幕正下方出现辅助图标：#R上\下车#W和护送#G成员列表。帮主通过列表踢除表内成员，解除其护送神兽状态。当帮主提交任务时，周围参与护送的帮众越多，您所获得的奖励也会更多。随着您所在帮会等级的提高，你获得的奖励也会相应增加。"          --任务提示信息
x300869_g_ExtTarget		=	{ {type=20,n=1,target="护送神兽"}} --目标
 
--MisDescEnd


--奖励
x300869_g_BonusMoney8               =   150  --任务奖励帮贡
x300869_g_GuildExpBonus				=	1
--x300869_g_BangExp 					=	2
x300869_SubmitNPCGUID				= 131516 --交任务NPC GUID
x300869_g_BusScriptId = 300870
x300869_g_Leader_Index = 5		--帮主在帮会职务中的索引
x300869_g_MissionId  = 7022
x300869_g_CostMoney = 10000
x300869_g_Exp = 100*3*25*1*1*0.4            
x300869_g_RingMissionIndex = 1	--环任务在主任务中的位置

function x300869_ProcEnumEvent( sceneId, selfId, targetId, MissionId )

	--初始化全局变量
	--RD_GUILDCONTEND_KILL_FAMILY = 1
	SetPlayerRuntimeData(sceneId,selfId,RD_GUILDCONTEND_KILL_FAMILY,1)
	...
	if IsHaveQuestNM( sceneId, selfId, x300869_g_MissionId ) == 0 then	-- 如果没有这个任务
		local state = GetQuestStateNM(sceneId,selfId,targetId,x300869_g_MissionId)
		AddQuestNumText(sceneId,x300869_g_MissionId,x300869_g_MissionName,8);
		return 1
	end
	return 0
end
...
--选中任务
function x300869_ProcEventEntry( sceneId, selfId, NPCId, MissionId,nExtIdx )

	local bHaveMission	= IsHaveQuestNM(sceneId, selfId, MissionId);
	if(bHaveMission > 0) then

		local npcGuid = GetMonsterGUID(sceneId,NPCId)
		if x300869_SubmitNPCGUID == npcGuid then
			x300869_DispatchCompletedInfo( sceneId, selfId, NPCId) --完成任务显示			
		end

	else
		if nExtIdx ~= x300869_g_ScriptId then
			x300869_DispatchMissionInfo( sceneId, selfId, NPCId )	--任务信息显示		
			--RD_GUILDCONTEND_KILL_FAMILY = 2
			SetPlayerRuntimeData(sceneId,selfId,RD_GUILDCONTEND_KILL_FAMILY,2)
		else
...
--接受
--**********************************
function x300869_ProcQuestAccept( sceneId, selfId, targetId, MissionId )
	...
	local pos = GetGuildOfficial(sceneId, selfId)
  	if pos ~= x300869_g_Leader_Index then			--不是帮主
  		BeginQuestEvent(sceneId)
	...
	if level < x300869_g_LevelLess then
		BeginQuestEvent(sceneId)
	...
	--if RD_GUILDCONTEND_KILL_FAMILY == 3 then
		if GetPlayerRuntimeData(sceneId,selfId,RD_GUILDCONTEND_KILL_FAMILY) == 3 then
	 		GetGuildQuestData(sceneId, selfId, GD_GUILD_INDEX_YUNBIAO_ACCEPT_TIME, x300869_g_ScriptId,MissionId,"OnReturn")
	 	end

	--if RD_GUILDCONTEND_KILL_FAMILY == 2 then	
		if GetPlayerRuntimeData(sceneId,selfId,RD_GUILDCONTEND_KILL_FAMILY) == 2 then 
				BeginQuestEvent(sceneId)
						--RD_GUILDCONTEND_KILL_FAMILY = 3
						SetPlayerRuntimeData(sceneId,selfId,RD_GUILDCONTEND_KILL_FAMILY,3)
						AddQuestText(sceneId,"#Y【帮会】帮会神兽#W\n  \n\t当您领取神兽时，帮会成员需在您的身边才可接收到接受任务的提示；在您完成任务时，只有在您身边的帮会成员才能完成任务获得奖励。\n \n#G您确定要领取帮会神兽吗?")
				EndQuestEvent()
				DispatchQuestInfoNM(sceneId, selfId, targetId, x300869_g_ScriptId, x300869_g_MissionId)
		end
...
function x300869_OnReturn1(sceneId, selfId,MissionData,MissionId,targetId)
 ...
 local curTime = GetCurrentTime()
	...
	x300869_GetBonus( sceneId,selfId, isMultiple, membercount,raingMisFinishing )
end

---------------------------------------------------------------------------------------------------
function x300869_GetBonus( sceneId, selfId, isMultiple, membercount, raingMisFinishing)	
--奖励经验
	...
	local ExpBonus = x300869_g_Exp * level* exExp
    ...
    if ExpBonus > 0 then
		...
		if isMultiple == 1 then
			local ExpBonus1 = ceil (x300869_g_Exp * level *3)* exExp
			local Exp3 = ceil (x300869_g_Exp* level*3 )* exExp
			local ExpBonus2 = ceil (x300869_g_Exp * level *2)* exExp
			local pos = GetGuildOfficial(sceneId, selfId)
		...
		if 	GuildLevel == 2 then
			local level = GetLevel(sceneId, selfId)
			local exp = ceil (x300869_g_Exp* level *0.25 )
			if membercount >= 1  then
		...
		if 	GuildLevel == 3 then
			local level = GetLevel(sceneId, selfId)
			local exp = ceil (x300869_g_Exp * level *0.5 )
			if membercount >= 1  then
		...
		if 	GuildLevel == 4 then
			local level = GetLevel(sceneId, selfId)
			local exp = ceil (x300869_g_Exp * level *0.75 )
			if membercount >= 1  then
		...
		if 	GuildLevel == 5 then
			local level = GetLevel(sceneId, selfId)
			local exp = ceil (x300869_g_Exp * level *1 )
			if membercount >= 1  then
		...
		if 	GuildLevel == 6 then
			local level = GetLevel(sceneId, selfId)
			local exp = ceil (x300869_g_Exp * level *1.25 )
			if membercount >= 1  then
					if membercount>=70 then
						membercount=70
					end
		...
		if 	GuildLevel == 7 then
			local level = GetLevel(sceneId, selfId)
			local exp = ceil (x300869_g_Exp * level *1.5 )
			if membercount >= 1  then
					if membercount>=70 then
						membercount=70
					end
	...
	--奖励帮贡
    if x300869_g_BonusMoney8 > 0 then
		AddGuildUserPoint(sceneId,selfId,x300869_g_BonusMoney8)	--增加帮贡
		local BonusMoney8 = x300869_g_BonusMoney8
		BeginQuestEvent(sceneId)
		AddQuestText(sceneId, "获得#R帮贡"..BonusMoney8.."点#o的奖励" );
		EndQuestEvent(sceneId)
		DispatchQuestTips(sceneId,selfId)
		Msg2Player(sceneId,selfId,format("获得#R帮贡"..BonusMoney8.."点#o的奖励", x300869_g_BonusMoney8),8,2)
		if isMultiple == 1 then
			local pos = GetGuildOfficial(sceneId, selfId)
				if pos == 5 then
	...
	-- 奖励帮会经验
	local bangExp = x300869_g_GuildExpBonus
	if bangExp > 0 then
		local pos = GetGuildOfficial(sceneId, selfId)
		if pos == x300869_g_Leader_Index then
...
--返回押金
	local pos = GetGuildOfficial(sceneId, selfId)
  	if pos == x300869_g_Leader_Index then			--是帮主
		if x300869_g_CostMoney > 0 then
			AddMoney( sceneId , selfId ,0, x300869_g_CostMoney )
...
--完成任务
--返回1代表成功，0代表交任务失败
function x300869_ProcQuestSubmit( sceneId, selfId, targetId, selectId, MissionId )
...
function x300869_ProcQuestAttach( sceneId, selfId, npcId, npcGuid, misIndex, MissionId )

	if IsHaveQuestNM( sceneId, selfId, x300869_g_MissionId ) == 0 then	-- 如果没有这个任务
		return
	end

	if npcGuid == x300869_SubmitNPCGUID then	--点击的NPC的名字和任务中NPC的名字相同，场景也判断过了
		local state = GetQuestStateNM(sceneId,selfId,npcId,x300869_g_MissionId)
		AddQuestNumText( sceneId, x300869_g_MissionId,x300869_g_MissionName, 7,x300869_g_ScriptId ) -- 显示任务信息
	end
end
...
--清除帮会护送任务数据
--********************************************************************
function x300869_OnGuildConvoyClearMemberQuest( sceneId, selfId,state )
	...
	local ret = DelQuest( sceneId, selfId, x300869_g_MissionId  )
	if ret == 1 then
...
--提交玩家帮会护送任务：给奖励
--********************************************************************
function x300869_OnGuildConvoySubmitQuest( sceneId, selfId )

	local misIndex = GetQuestIndexByID(sceneId, selfId, x300869_g_MissionId); 
	
	--得到环任务状态
	local raingMisFinishing = GetQuestParam( sceneId, selfId,misIndex,x300869_g_RingMissionIndex )

	--清除任务数据  
	--GetQuestNameNM( sceneId, selfId, MissionId )
	if DelQuest( sceneId, selfId, x300869_g_MissionId  ) < 1 then
		return 
	end
	...
	GetGuildQuestData(sceneId, selfId, GD_GUILD_INDEX_MULTI_MISSION_HUSHOU, x300869_g_ScriptId,membercount,"OnReturn1",raingMisFinishing)
end
...
--放弃
--********************************************************************
function x300869_ProcQuestAbandon( sceneId, selfId, MissionId )
...
function x300869_ClearBusMission( sceneId, selfId )
	AbandonGuildConvoyQuest(sceneId, selfId, 18)
end

function x300869_OnReturn(sceneId, selfId,MissionData,MissionId,targetId)
	...
	if MissionData == CurDaytime then					--上次发布的时间是今天
		...
		if CurrMoney < x300869_g_CostMoney then
		    BeginQuestEvent(sceneId)
	    ...
	    local BusID = CallScriptFunction( x300869_g_BusScriptId, "OnCreateBus", sceneId, selfId,MissionId )
		--if BusID ~= -1 then
...
--接受
--********************************************************************
function x300869_ProcAccept( sceneId, selfId )
	...
	local pos = GetGuildOfficial(sceneId, selfId)
  	if pos == x300869_g_Leader_Index then			--是帮主
		...
		local num = CostMoney( sceneId , selfId ,0, x300869_g_CostMoney,301 )--成功返回1 失败返回-1
		if num == -1 then
			BeginQuestEvent(sceneId)
	...
	local CurDaytime = GetDayTime()						--当前时间(天)
	local ret = AddQuest( sceneId, selfId,x300869_g_MissionId , x300869_g_ScriptId,0,0,0,1 ) -- 给玩家添加任务
	...
	if ret == 1 then
		...
		x300869_QuestLogRefresh( sceneId, selfId, x300869_g_MissionId)
		...
		--环任务初始化
		SetQuestByIndex(sceneId, selfId, misIndex, x300869_g_RingMissionIndex, 0);		

  		--AddDispatchQuestInfoNM( sceneId, selfId, -1, x300869_g_MissionId )
  		local pos = GetGuildOfficial(sceneId, selfId)
  		if pos == x300869_g_Leader_Index then			--是帮主
  			local GuildID = GetGuildID(sceneId,selfId)
  			SetGuildQuestData(sceneId, GuildID, GD_GUILD_INDEX_YUNBIAO_ACCEPT_TIME , CurDaytime)		
...
--向客户端发送任务信息
---------------------------------------------------------------------------------------------------
function x300869_DispatchMissionInfo( sceneId, selfId, NPCId,index )

	BeginQuestEvent(sceneId)

		--任务信息
		AddQuestText(sceneId,"#Y"..x300869_g_MissionName)
		AddQuestText(sceneId,format("%s", x300869_g_MissionInfo))
		AddQuestText(sceneId," ")

		--任务目标
		AddQuestText( sceneId,"#Y任务目标：")
		AddQuestText( sceneId,format("%s", x300869_g_MissionTarget))
		AddQuestText( sceneId," ")

		--提示信息
		if x300869_g_MissionHelp ~= "" then

			AddQuestText(sceneId,"#Y任务提示：")
			AddQuestText(sceneId,format("%s", x300869_g_MissionHelp))
			AddQuestText(sceneId," ")
		end
		-- if level >= 80 then
		...
		local level = GetLevel(sceneId, selfId)
		--local gongdezhi =GetMeritInfo_Multiple(sceneId, selfId)
		local ExpBonus = x300869_g_Exp * level
		ExpBonus =floor(ExpBonus+0.5)
	    if ExpBonus > 0 then
		...
		--2、帮贡
		if x300869_g_BonusMoney8 > 0 then
			--AddQuestMoneyBonus8(sceneId, x300869_g_BonusMoney8 )
			AddQuestText(sceneId,"帮贡：150点")	
			
		end	
		--帮会经验
		if x300869_g_GuildExpBonus>0 then
			--AddQuestGuildExpBonus(sceneId,x300869_g_GuildExpBonus)
	...
	EndQuestEvent(sceneId)
	DispatchQuestInfoNM(sceneId, selfId, NPCId, x300869_g_ScriptId, x300869_g_MissionId);
end

---------------------------------------------------------------------------------------------------
--向客户端发送任务未完成
---------------------------------------------------------------------------------------------------
function x300869_DispatchContinueInfo( sceneId, selfId, NPCId )
	...
	EndQuestEvent()
	--DispatchQuestEventList(sceneId,selfId,NPCId);
	DispatchQuestDemandInfo(sceneId, selfId, NPCId, x300869_g_ScriptId, x300869_g_MissionId,0);
...
--向客户端发送任务完成信息
---------------------------------------------------------------------------------------------------
function x300869_DispatchCompletedInfo( sceneId, selfId, NPCId )
	
	BeginQuestEvent(sceneId)

		--任务完成信息
		AddQuestText(sceneId,"#Y"..x300869_g_MissionName)
		AddQuestText(sceneId,x300869_g_MissionCompleted)
		AddQuestText(sceneId," ")
	...
	EndQuestEvent(sceneId)
	DispatchQuestContinueInfoNM(sceneId, selfId, NPCId, x300869_g_ScriptId, x300869_g_MissionId);
...
--任务日志刷新
---------------------------------------------------------------------------------------------------
function x300869_QuestLogRefresh( sceneId, selfId, MissionId)
 BeginQuestEvent(sceneId)
	...
	if bHaveMission > 0 then
		...
		AddQuestLogCustomText( sceneId,
								"",						-- 标题
								x300869_g_MissionName,        -- 任务名字
								x300869_g_MissionTarget,		--任务目标
								"",					--任务NPC
								"将神兽安全护送到边境交给@npc_131516",                 --任务攻略
								x300869_g_MissionInfo,	--任务描述
								""					--任务小提示
								)
	end
...
function x300869_ProcQuestLogRefresh( sceneId, selfId, MissionId)
	x300869_QuestLogRefresh( sceneId, selfId, x300869_g_MissionId)
end


function x300869_ProcQuestObjectKilled( sceneId, selfId, objdataId, objId, MissionId )
end


--成员超出护送范围的处理
function x300869_OnMeberOffline(sceneId,selfId)
