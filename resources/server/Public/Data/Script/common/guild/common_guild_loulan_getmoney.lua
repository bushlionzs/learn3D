...
x300671_g_ScriptId = 300671
x300671_g_MissionName = "【帮会】帮会跑商"
x300671_g_MissionId = 7026

x300671_g_Min_Level = 20

x300671_g_SubmitParamIndex = 0
x300671_g_SubmitMoney = 10000
...
x300671_g_MoneyBuff = 8353
x300671_gameopenid = 1029


function x300671_ProcEnumEvent( sceneId, selfId, targetId, MissionId )
	...
	if GetGuildID(sceneId, selfId) ~= -1 then
		...
		AddQuestNumText(sceneId, MissionId, x300671_g_MissionName, state, -1)
	end
end

function x300671_DispatchMissionInfo( sceneId, selfId, NPCId )
...
--选中任务
function x300671_ProcEventEntry( sceneId, selfId, targetId, MissionId )
	...
	if IsHaveQuestNM( sceneId, selfId, MissionId ) == 1 then	-- 如果有这个任务
	 	if GetGuildID(sceneId, selfId) == -1 then
			...
			EndQuestEvent()
			DispatchQuestDemandInfo(sceneId, selfId, targetId, x300671_g_ScriptId, MissionId,0);
			return
		end	
		...
		if GetQuestParam( sceneId, selfId, misIndex, 6) == 0 then	--任务失败，重新领取
			...
			if x300671_IsMissionCompleted( sceneId, selfId, MissionId ) == 1 then -- 如果任务可以完成
				BeginQuestEvent(sceneId)
				...
				--x300671_DispatchMissionInfo( sceneId, selfId, targetId )
				--AddQuestText(sceneId,"#W帮会金钱：20点")
				EndQuestEvent()
				DispatchQuestContinueInfoNM(sceneId, selfId, targetId, x300671_g_ScriptId, MissionId);
			else
				BeginQuestEvent(sceneId)
				AddQuestText(sceneId,"#Y【帮会】帮会跑商")
	...
	GetGuildQuestData(sceneId, selfId, GD_GUILD_INDEX_GET_MONEY_COUNT, x300671_g_ScriptId, MissionId, "ProcAcceptMission", targetId );
	
end

function x300671_ProcAcceptMission(sceneId, selfId,MissionData,MissionId,targetId)
	...
	--AddQuestText(sceneId,"#W帮会金钱：20点")
	EndQuestEvent()
	DispatchQuestInfoNM(sceneId, selfId, targetId, x300671_g_ScriptId, MissionId);
	
end

function x300671_ProcQuestAccept( sceneId, selfId, targetId, MissionId )
	if GetGameOpenById(x300671_gameopenid)<=0 then
		return
	end
	...
	if GetLevel(sceneId, selfId) < x300671_g_Min_Level then
		BeginQuestEvent(sceneId)
	...
	--背包中加物品
	x300671_DelMissionItems(sceneId, selfId)
	if x300671_AddMissionItems(sceneId, selfId) == 0 then
		BeginQuestEvent(sceneId)
	...
	AddQuestNM(sceneId, selfId, x300671_g_MissionId);
	--SendSpecificImpactToUnit( sceneId, selfId, selfId, selfId, x300671_g_MoneyBuff, 0)
	
	local misIndex = GetQuestIndexByID( sceneId, selfId, MissionId)
	SetQuestParam(sceneId, selfId, misIndex, 0, 0)
	
	local str = format("您领取了任务：%s", x300671_g_MissionName)
	...
	x300671_ProcQuestLogRefresh(sceneId, selfId, MissionId)
...
--完成任务
--返回1代表成功，0代表交任务失败
function x300671_ProcQuestSubmit( sceneId, selfId, targetId, selectId, MissionId )
	...
	if HaveItem(sceneId, selfId,x300671_Item_Money) <= 0 then
		BeginQuestEvent(sceneId)
	...
	if x300671_IsMissionCompleted(sceneId, selfId, MissionId) ~= 1 then 
		return 0
	end
	
	GetGuildQuestData(sceneId, selfId, GD_GUILD_INDEX_GET_MONEY_COUNT, x300671_g_ScriptId, MissionId, "OnMissionComplete" );
	
end

-- 任务完成
function x300671_OnMissionComplete(sceneId, selfId,MissionData,MissionId,targetId)
	...
	if MissionData >= x300671_Max_Count-1 then
	...
	--CancelSpecificImpact( sceneId, selfId,  x300671_g_MoneyBuff)
	...
	local message = format("您完成了任务：%s", x300671_g_MissionName);
	BeginQuestEvent(sceneId);
	AddQuestText(sceneId, message);
	EndQuestEvent();
	DispatchQuestTips(sceneId, selfId);
	...
	x300671_GiveBonus(sceneId,selfId,99 - MissionData)
...
--给奖励
function x300671_GiveBonus(sceneId,selfId,count)
...
function x300671_ProcQuestAttach( sceneId, selfId, npcId, npcGuid,misIndex, MissionId )
	if IsHaveQuestNM( sceneId, selfId, MissionId) == 0 then	-- 如果没有这个任务
		return
	end
end

function x300671_ProcQuestAbandon( sceneId, selfId, MissionId )
	
	DelQuestNM( sceneId, selfId, MissionId)
	x300671_DelMissionItems(sceneId, selfId)
	
	local message = format("您放弃了任务：%s", x300671_g_MissionName);
	BeginQuestEvent(sceneId);
	AddQuestText(sceneId, message);
	EndQuestEvent();
	DispatchQuestTips(sceneId, selfId);
...
function x300671_ProcQuestLogRefresh( sceneId, selfId, MissionId)

	local misIndex = GetQuestIndexByID( sceneId, selfId, MissionId)
	local nMoney = GetQuestParam( sceneId, selfId, misIndex, x300671_g_SubmitParamIndex)
	local strTarget = "  在@npc_123834，@npc_123835，@npc_123833处，出售帮会货物赚取足够金钱("..nMoney.."/"..x300671_g_SubmitMoney..")";
...
-- 判断钱够不够了
function x300671_IsMissionCompleted(sceneId, selfId, MissionId)

	local misIndex = GetQuestIndexByID( sceneId, selfId, MissionId)
	local nMoney = GetQuestParam( sceneId, selfId, misIndex, x300671_g_SubmitParamIndex)
	if nMoney > x300671_g_SubmitMoney then
		return 1
	else
		return 0
	end
...
-- 添加任务物品
function x300671_AddMissionItems(sceneId, selfId)
	...
	AddItem(sceneId, x300671_Item_Money, 1)
...
-- 删除任务物品
function x300671_DelMissionItems(sceneId, selfId)
	local i = 0
	for i, item in x300671_Item_Goods do
	...
	if HaveItem(sceneId,selfId,x300671_Item_Money) > 0 then
		DelItem(sceneId,selfId,x300671_Item_Money,1)
	end
end

-- 离开帮会
function x300671_OnLeaveGuild(sceneId, selfId, MissionId)

	DelQuestNM( sceneId, selfId, MissionId)
	x300671_DelMissionItems(sceneId, selfId)
	
	local message = format("您已经不在帮会中了，删除了任务：%s", x300671_g_MissionName);
	BeginQuestEvent(sceneId);
	AddQuestText(sceneId, message);
	EndQuestEvent();
	DispatchQuestTips(sceneId, selfId);
