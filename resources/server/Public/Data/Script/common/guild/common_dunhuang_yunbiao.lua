...
--MisDescBegin
x300867_g_ScriptId 					= 	300867                  --脚本ID

x300867_g_MissionKind 				= 	1                       --任务类型
x300867_g_LevelLess					= 	20                      --任务最小等级  <=0 表示不限制
x300867_g_MissionName				= 	"【帮会】帮会神兽"           --任务名称(512)

x300867_g_MissionTarget				= 	"  护送神兽经过@npc_133501中转后到达@npc_134516处"          --任务目标
x300867_g_MissionInfo				= 	"\t帮主带领帮会成员，每天可在王城参加一次【帮会】帮会神兽任务。\n\t帮主需要在帮会任务发布人处缴纳#G10两现银#W才能接受此任务。帮会成员只需此时在帮主周围，便可自动接到任务，进入护送神兽状态。\n\t在护送神兽过程中，神兽只跟随帮主移动，如帮主离神兽过远，神兽会#G停止前进#W。如果神兽受到袭击并死亡，帮主的#G押金#W将被对方抢走，全体成员#G任务失败#W。\n\t帮主离开神兽一段时间或掉线，神兽消失，所有人#G任务失败#W。\n\t由于路途遥远，神兽中途需要休息，帮主在@npc_133501处点击#R神兽驿站#W按扭，当前在驿站周围的帮众都会获得经验奖励。并且在最后完成护兽任务时都会获得经验加成的奖励。\n\t顺利护送神兽到达指定地点，帮主可得到返还的押金和#G巨额经验#W，参与护送的帮会成员得到#G巨额经验#W。\n\t帮主到达指定地点提交任务，此时只有帮主周围并且已经参与护送的帮会成员才能完成任务并获得#G巨额奖励#W，同时帮会增加100点帮会经验#W。当帮主发布#G护兽令#W时2小时内回复任务可获得翻倍奖励（经验，帮贡）和2点使命点！"            --任务信息
x300867_g_MissionCompleted			= 	"恭喜您顺利完成任务！这是您应得的奖励，请收下！"  --任务完成信息
x300867_g_MissionHelp				=	"\t接受任务后，会在帮主屏幕正下方出现辅助图标：#R上\下车#W和护送#G成员列表。帮主通过列表踢除表内成员，解除其护送神兽状态。当帮主提交任务时，周围参与护送的帮众越多，您所获得的奖励也会更多。随着您所在帮会等级的提高，你获得的奖励也会相应增加。"          --任务提示信息
x300867_g_ExtTarget		=	{ {type=20,n=1,target="护送神兽"}} --目标
 
--MisDescEnd


--奖励
x300867_g_BonusMoney8               =   150  --任务奖励帮贡
x300867_g_GuildExpBonus				=	1
--x300867_g_BangExp 					=	2
x300867_SubmitNPCGUID				= 134516 --交任务NPC GUID
x300867_g_BusScriptId = 300868
x300867_g_Leader_Index = 5		--帮主在帮会职务中的索引
x300867_g_MissionId  = 7021
x300867_g_CostMoney = 10000
x300867_g_Exp = 100*3*25*1*1*0.4            
x300867_g_RingMissionIndex = 1	--环任务在主任务中的位置
...
function x300867_ProcEnumEvent( sceneId, selfId, targetId, MissionId )
	...
	if IsHaveQuestNM( sceneId, selfId, x300867_g_MissionId ) == 0 then	-- 如果没有这个任务
...
function x300867_ProcEventEntry( sceneId, selfId, NPCId, MissionId,nExtIdx )
	...
	if(bHaveMission > 0) then
		...
		if x300867_SubmitNPCGUID == npcGuid then
			x300867_DispatchCompletedInfo( sceneId, selfId, NPCId) --完成任务显示			
		end

	else
		if nExtIdx ~= x300867_g_ScriptId then
			x300867_DispatchMissionInfo( sceneId, selfId, NPCId )	--任务信息显示	
...
function x300867_ProcQuestAccept( sceneId, selfId, targetId, MissionId )
	...
	local pos = GetGuildOfficial(sceneId, selfId)
  	if pos ~= x300867_g_Leader_Index then			--不是帮主
  		BeginQuestEvent(sceneId)
...
--奖励经验
    ...
    if x300867_g_BonusMoney8 > 0 then
		AddGuildUserPoint(sceneId,selfId,x300867_g_BonusMoney8)	--增加帮贡
		local BonusMoney8 = x300867_g_BonusMoney8
		BeginQuestEvent(sceneId)
		AddQuestText(sceneId, "获得#R帮贡"..BonusMoney8.."点#o的奖励" );
		EndQuestEvent(sceneId)
		DispatchQuestTips(sceneId,selfId)
		Msg2Player(sceneId,selfId,format("获得#R帮贡"..BonusMoney8.."点#o的奖励", x300867_g_BonusMoney8),8,2)
		if isMultiple == 1 then
			local pos = GetGuildOfficial(sceneId, selfId)
				if pos == 5 then
...
--返回押金
	local pos = GetGuildOfficial(sceneId, selfId)
  	if pos == x300867_g_Leader_Index then			--是帮主
...
function x300867_ProcQuestAttach( sceneId, selfId, npcId, npcGuid, misIndex, MissionId )
	...
	if npcGuid == x300867_SubmitNPCGUID then	--点击的NPC的名字和任务中NPC的名字相同，场景也判断过了
		local state = GetQuestStateNM(sceneId,selfId,npcId,x300867_g_MissionId)
		AddQuestNumText( sceneId, x300867_g_MissionId,x300867_g_MissionName, 7,x300867_g_ScriptId ) -- 显示任务信息
	end
end
...
function x300867_ProcAccept( sceneId, selfId )
	...
	local pos = GetGuildOfficial(sceneId, selfId)
  	if pos == x300867_g_Leader_Index then			--是帮主
		...
		local num = CostMoney( sceneId , selfId ,0, x300867_g_CostMoney,301 )--成功返回1 失败返回-1
		if num == -1 then
			BeginQuestEvent(sceneId)
	...
	local CurDaytime = GetDayTime()						--当前时间(天)
	local ret = AddQuest( sceneId, selfId,x300867_g_MissionId , x300867_g_ScriptId,0,0,0,1 ) -- 给玩家添加任务
 ...
 BeginQuestEvent(sceneId)
	...
	if bHaveMission > 0 then
		...
		AddQuestLogCustomText( sceneId,
								"",						-- 标题
								x300867_g_MissionName,        -- 任务名字
								x300867_g_MissionTarget,		--任务目标
								"",					--任务NPC
								"将神兽安全护送到边境交给@npc_134516",                 --任务攻略
								x300867_g_MissionInfo,	--任务描述
								""					--任务小提示
								)
	end
