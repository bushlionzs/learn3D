...
x300663_g_GrowpointId = 107 		--对应生长点ID
x300663_g_ItemIndex = 12010001
x300663_g_BufIndex = 1210

--生成函数开始************************************************************************
function 	x300663_OnCreate(sceneId,growPointType,x,y)
	local ItemBoxId = ItemBoxEnterScene(x, y, x300663_g_GrowpointId, sceneId, 0, x300663_g_ItemIndex)
end
--生成函数结束**********************************************************************


--打开前函数开始&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
function	 x300663_OnOpen(sceneId,selfId,targetId)
    return x300663_OnOpenItemBox(sceneId, selfId, targetId, x300663_g_GrowpointId, x300663_g_ItemIndex)
end
--打开前函数结束&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&


--回收函数开始########################################################################
function	 x300663_OnRecycle(sceneId,selfId,targetId)
	return   x300663_OnGuildRecycle( sceneId, selfId, targetId, x300663_g_GrowpointId, x300663_g_ItemIndex )
end
--回收函数结束########################################################################
...
--打开后函数开始@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
function	x300663_OnProcOver(sceneId,selfId,targetId)
...
function	x300663_OpenCheck(sceneId,selfId,AbilityId,AblityLevel)
...
--采集任务
--打开ItemBox
function x300663_OnOpenItemBox( sceneId, selfId, targetId, gpType, needItemID )
	...
	--得到粮仓当前帮会
	local guild = GetGuildBattleSceneData_Param(sceneId,LD_GUILD_BATTLE_BARN2_GUILD)
	...
	if guild == GetGuildID(sceneId, selfId) then	--是本方粮仓
		...
		if GetGuildBattleSceneData_Param(sceneId,LD_GUILD_BATTLE_BARN1_GUILD) ~= guild and GetGuildBattleSceneData_Param(sceneId,LD_GUILD_BATTLE_BARN4_GUILD) ~= guild then
  			--占领相邻粮仓才能交
 			BeginQuestEvent(sceneId)
...
--回收
function x300663_OnGuildRecycle( sceneId, selfId, targetId, gpType, needItemID )
	...
	SetGuildBattleSceneData_Param( sceneId,LD_GUILD_BATTLE_BARN2_GUILD,GetGuildID(sceneId, selfId) )

	--重设被占领时间
	SetGuildBattleSceneData_Param( sceneId,LD_GUILD_BATTLE_BARN2_TIME,-1 )

    CallScriptFunction( GUILDBATTLE_SCRIPTID,"UpdateSceneDataFlag",sceneId,2 )
