...
x300664_g_GrowpointId = 108 		--对应生长点ID
x300664_g_ItemIndex = 12010001
x300664_g_BufIndex = 1210

--生成函数开始************************************************************************
function 	x300664_OnCreate(sceneId,growPointType,x,y)
	local ItemBoxId = ItemBoxEnterScene(x, y, x300664_g_GrowpointId, sceneId, 0, x300664_g_ItemIndex)
end
--生成函数结束**********************************************************************


--打开前函数开始&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
function	 x300664_OnOpen(sceneId,selfId,targetId)
    return x300664_OnOpenItemBox(sceneId, selfId, targetId, x300664_g_GrowpointId, x300664_g_ItemIndex)
end
--打开前函数结束&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&


--回收函数开始########################################################################
function	 x300664_OnRecycle(sceneId,selfId,targetId)
	return   x300664_OnGuildRecycle( sceneId, selfId, targetId, x300664_g_GrowpointId, x300664_g_ItemIndex )
end
--回收函数结束########################################################################
...
--打开后函数开始@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
function	x300664_OnProcOver(sceneId,selfId,targetId)
...
function	x300664_OpenCheck(sceneId,selfId,AbilityId,AblityLevel)
...
--采集任务
--打开ItemBox
function x300664_OnOpenItemBox( sceneId, selfId, targetId, gpType, needItemID )
	...
	--得到粮仓当前帮会
	local guild = GetGuildBattleSceneData_Param(sceneId,LD_GUILD_BATTLE_BARN3_GUILD)
...
--回收
function x300664_OnGuildRecycle( sceneId, selfId, targetId, gpType, needItemID )
	...
	SetGuildBattleSceneData_Param( sceneId,LD_GUILD_BATTLE_BARN3_GUILD,GetGuildID(sceneId, selfId) )

	--重设被占领时间
	SetGuildBattleSceneData_Param( sceneId,LD_GUILD_BATTLE_BARN3_TIME,-1 )

    CallScriptFunction( GUILDBATTLE_SCRIPTID,"UpdateSceneDataFlag",sceneId,3 )
