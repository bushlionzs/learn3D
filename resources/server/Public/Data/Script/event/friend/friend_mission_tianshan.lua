...
x300722_g_ScriptId = 300722                 --脚本ID
x300722_g_LevelLess					= 	30

--MisDescEnd

--意气相投

x300722_g_MissionId = 1201

x300722_g_SubMission = {
[0] = 6547,
[1] = 6522,
[2] = 6523,
[3] = 6524,
...
x300722_g_addFriendPoint = 150
x300722_g_MissionName ="【好友】意气相投"
x300722_g_ParamIndexBegin = 2
x300722_g_ParamIndexEnd = 6
function x300722_CalcMissionEXP(sceneId, selfId)
	--return GetLevel(sceneId, selfId) * 100 * 3 * 20
	local haveSubMissionId = x300722_HaveWhichSubMission(sceneId, selfId)
	...
	for i=x300722_g_ParamIndexBegin, x300722_g_ParamIndexEnd do
...
function x300722_SetCompleteTimes(sceneId, selfId, times)
...
function x300722_SetCompleteDay(sceneId, selfId)
		...
		x300722_SetCompleteTimes(sceneId, selfId, 1)
end

function x300722_GetCompleteDay(sceneId, selfId)
...
function x300722_SetKillMonsterNum(sceneId, selfId, num)
		...
		x300722_KillMonsterRefresh(sceneId, selfId)
end

function x300722_GetKillMonsterNum(sceneId, selfId)
...
function x300722_AddKillMonsterNum(sceneId, selfId, num)
	num = num + x300722_GetKillMonsterNum(sceneId, selfId)
	x300722_SetKillMonsterNum(sceneId, selfId, num)
end
	
function x300722_GetSubMissionId(humanLevel)
	...
	return x300722_g_SubMission[missionLevel]
end

function x300722_DeleteAllSubMission( sceneId, selfId )
	for i, mission in x300722_g_SubMission do
		DelQuestNM( sceneId, selfId, mission )
	end
end

function x300722_IsSubMission( sceneId, selfId, MissionId )
	for i, mission in x300722_g_SubMission do
...
function x300722_HaveWhichSubMission(sceneId, selfId)
	for i, mission in x300722_g_SubMission do
...
function x300722_IsHaveSubMission(sceneId, selfId)
	for i, mission in x300722_g_SubMission do
		if IsHaveQuestNM( sceneId, selfId, mission ) == 1 then
			return 1
		end
...
function x300722_HaveDoneWhichSubMission(sceneId, selfId, targetId)
...
function x300722_ProcEventEntry( sceneId, selfId, targetId, MissionId, index )
	local haveSubMissionId = x300722_HaveWhichSubMission(sceneId, selfId)
	local haveDoneMissionId = -1
	if haveSubMissionId > 0 then
		haveDoneMissionId = x300722_HaveDoneWhichSubMission(sceneId, selfId, targetId)
	end
	
	local exp = x300722_CalcMissionEXP(sceneId, selfId)
	...
	if haveDoneMissionId > 0 then
		...
		AddQuestText(sceneId, "\t好友度:"..x300722_g_addFriendPoint.."点")
		AddQuestText(sceneId, "\t经验值:"..exp.."点")
		EndQuestEvent()
	  DispatchQuestContinueInfoNM(sceneId, selfId, targetId, x300722_g_ScriptId, MissionId);
	  return
	end
	...
	if haveSubMissionId > 0 then
		...
		AddQuestText(sceneId, "\t经验值:"..exp.."点")
		EndQuestEvent()
		DispatchQuestDemandInfo(sceneId, selfId, targetId, x300722_g_ScriptId, MissionId,0);
		return
	end
	...
	DispatchQuestInfo(sceneId, selfId, targetId, x300722_g_MissionId, -1)
...
--6	非循环/循环任务	未完成	2
--7	非循环任务	已完成	3
function x300722_ProcEnumEvent(sceneId, selfId, targetId, MissionId)
	...
	AddQuestNumText(sceneId, x300722_g_MissionId, x300722_g_MissionName, state, -1)
...
function x300722_ProcAcceptCheck(sceneId, selfId, NPCId)
...
function x300722_CheckPushList(sceneId, selfId, NPCId)
	return 1
end

function x300722_DoAccept( sceneId, selfId )
	...
	local completeDay = x300722_GetCompleteDay(sceneId, selfId)
	...
	for i = 0, teamSize - 1 do
		local targetId = GetNearTeamMember(sceneId, selfId, i)
		local completeDay = x300722_GetCompleteDay(sceneId, targetId)
		if( completeDay == today) then
	...
	local missionId = x300722_GetSubMissionId(averageLevel)
	
	local bHaveMission = x300722_IsHaveSubMission(sceneId, selfId)
	...
	for i = 0, teamSize - 1 do
		local targetId = GetNearTeamMember(sceneId, selfId, i)
		local bHaveMission = x300722_IsHaveSubMission(sceneId, targetId)
		if bHaveMission == 1 then
	...
	for i = 0, teamSize - 1 do
		...
		local str = format("您领取了任务:%s", x300722_g_MissionName)
		...
		for j = i + 1, teamSize - 1 do
			...
			if IsFriend(sceneId, aId, bId) ~= 1 then
				...
				--SetFriendPoint(sceneId, aId, bId, point + x300722_g_addFriendPoint)
			end
			if IsFriend(sceneId, bId, aId) ~= 1 then
				AddFriend(sceneId, bId, aName, 0)
			else
				local point = GetFriendPoint(sceneId, bId, aId)
				--SetFriendPoint(sceneId, bId, aId, point + x300722_g_addFriendPoint)
			end
			--str = format("由于领取任务：%s，你与%s增加了%d好友度", x300722_g_MissionName, bName, x300722_g_addFriendPoint)
			--BeginQuestEvent(sceneId);
			--AddQuestText(sceneId, str);
			--EndQuestEvent();
			--DispatchQuestTips(sceneId, aId);
			--str = format("由于领取任务：%s，你与%s增加了%d好友度", x300722_g_MissionName, aName, x300722_g_addFriendPoint)
			--BeginQuestEvent(sceneId);
			--AddQuestText(sceneId, str);
			--EndQuestEvent();
		...
		QuestCom(sceneId,aId,x300722_g_MissionId)
		x300722_SetCompleteDay(sceneId, aId, today)
		x300722_SetKillMonsterNum(sceneId, aId, 0)
	...
	for i = 0, teamSize - 1 do
		...
		local paramIndex = x300722_g_ParamIndexBegin
...
--**********************************
function x300722_ProcQuestAccept( sceneId, selfId, targetId, MissionId )
	local str = x300722_DoAccept( sceneId, selfId )
	if str ~= nil then
...
function x300722_ProcQuestAbandon(sceneId, selfId, MissionId)

	x300722_DeleteAllSubMission( sceneId, selfId )
	QuestUnCom(sceneId,selfId,x300722_g_MissionId)
	local today = GetDayOfYear()
	x300722_SetCompleteDay(sceneId, selfId, today)
...
function x300722_CheckSubmit( sceneId, selfId, MissionId )
	local haveSubMissionId = x300722_HaveWhichSubMission(sceneId, selfId)
	local haveDoneMissionId = -1
	if haveSubMissionId > 0 then
		haveDoneMissionId = x300722_HaveDoneWhichSubMission(sceneId, selfId, 1)
	end
...
--**********************************
function x300722_ProcQuestSubmit( sceneId, selfId, targetId, selectId, MissionId )
  ...
  if IsHaveQuestNM( sceneId,selfId,haveDoneMissionId )<= 0 then
	...
	for i=x300722_g_ParamIndexBegin, x300722_g_ParamIndexEnd do
			...
			guid = tonumber(format("%u",guid))
			if guid > 0 and GetFriendName(sceneId, selfId, guid) ~= "" then
				local str = format("你与%s增加了%d点好友度", GetFriendName(sceneId, selfId, guid), x300722_g_addFriendPoint)
				strTable[i] = str
				guidTable[i] = guid
	...
	if ret > 0 then -- 如果成功完成
		...
		for i=x300722_g_ParamIndexBegin, x300722_g_ParamIndexEnd do
			if nil ~= guidTable[i] then
				...
				local point = GetFriendPointByGuid(sceneId, selfId, guid)
				SetFriendPointByGuid(sceneId, selfId, guid, point + x300722_g_addFriendPoint)
			end
		end
...
function x300722_ProcQuestObjectKilled(sceneId, selfId, objdataId, objId, MissionId)

	if x300722_IsSubMission( sceneId, selfId, MissionId ) < 0 then
		return -1;
	end
	...
	if NeedKilledNum > 0 then --单纯的杀怪任务
		...
		for i = 0, HumenNum-1 do
        	...
        	if humanObjId ~= -1 and IsPlayerStateNormal(sceneId, humanObjId) > 0 and GetHp(sceneId, humanObjId) > 0 and IsHaveQuestNM( sceneId, humanObjId, MissionId ) > 0 then	--如果这个人拥有任务
          		...
          		local KilledNum = x300722_GetKillMonsterNum(sceneId, humanObjId)

                if KilledNum < NeedKilledNum then

                	x300722_AddKillMonsterNum(sceneId, humanObjId, 1)
                	--NeedKilledNum, InstIndex, ObjName = GetQuestNeedKillObjInfoNM( sceneId, humanObjId, MissionId, objdataId, objId )
                  	...
                  	if KilledNum == NeedKilledNum - 1 then
						...
						x300722_ProcQuestLogRefresh(sceneId, humanObjId, -1)
...
function x300722_ProcAreaEntered(sceneId, selfId, zoneId, MissionId)
	

end

function x300722_ProcTiming(sceneId, selfId )
	 
		
end

function x300722_ProcAreaLeaved(sceneId, selfId, ScriptId, MissionId )
...
function x300722_ProcQuestItemChanged(sceneId, selfId, itemdataId, MissionId)
	
end
function x300722_ProcQuestAttach( sceneId, selfId, npcId, npcGuid, misIndex, MissionId )
...
function x300722_PositionUseItem( sceneId, selfId, BagIndex, impactId )
	return  CallScriptFunction( MISSION_SCRIPT, "PositionUseItem",sceneId, selfId, BagIndex, impactId)
end

function x300722_OnOpenItemBox( sceneId, selfId, targetId, gpType, needItemID )
	return CallScriptFunction( MISSION_SCRIPT, "OnOpenItemBox",sceneId, selfId, targetId, gpType, needItemID)
end

function x300722_OnRecycle( sceneId, selfId, targetId, gpType, needItemID )
	return CallScriptFunction( MISSION_SCRIPT, "OnRecycle",sceneId, selfId, targetId, gpType, needItemID)
end

function x300722_OnProcOver( sceneId, selfId, targetId )
	CallScriptFunction( MISSION_SCRIPT, "OnProcOver",sceneId, selfId, targetId)
end

function x300722_OpenCheck( sceneId, selfId, targetId )
	CallScriptFunction( MISSION_SCRIPT, "OpenCheck", sceneId, selfId, targetId )
end

function x300722_KillMonsterRefresh(sceneId, selfId)
	local subMission = x300722_HaveWhichSubMission(sceneId, selfId)
  local szMissionTarget = x300722_GetMissionTarget(sceneId, selfId)
	BeginQuestEvent(sceneId)

	AddQuestLogCustomText( sceneId,
							"",						-- 标题
							x300722_g_MissionName,        -- 任务名字
...
function x300722_GetMissionHelp(sceneId, selfId)
	local subMission = x300722_HaveWhichSubMission(sceneId, selfId)
	local szMissionHelp = ""
	if tonumber(6547) == tonumber(subMission) then
		szMissionHelp = "到王国迷宫一层消灭200只@npc_90151后向@npc_126055复命。"
	elseif tonumber(6522) == tonumber(subMission) then
		szMissionHelp = "到王国迷宫二层消灭200只@npc_90153后向@npc_126055复命。"
	elseif tonumber(6523) == tonumber(subMission) then
		szMissionHelp = "到王国迷宫三层消灭200只@npc_90155后向@npc_126055复命。"
	elseif tonumber(6524) == tonumber(subMission) then
		szMissionHelp = "到王国迷宫四层消灭200只@npc_90157后向@npc_126055复命。"
	elseif tonumber(6525) == tonumber(subMission) then
		szMissionHelp = "到王国迷宫五层消灭200只@npc_90159后向@npc_126055复命。"
	elseif tonumber(6526) == tonumber(subMission) then
		szMissionHelp = "到王国迷宫六层消灭200只@npc_90161后向@npc_126055复命。"
	elseif tonumber(6527) == tonumber(subMission) then
		szMissionHelp = "到王国迷宫七层消灭200只@npc_90163后向@npc_126055复命。"
	elseif tonumber(6528) == tonumber(subMission) then
		szMissionHelp = "到王国迷宫八层消灭200只@npc_14004后向@npc_126055复命。"
	elseif tonumber(6529) == tonumber(subMission) then
		szMissionHelp = "到王国迷宫九层消灭200只@npc_14004后向@npc_126055复命。"
	end
	return szMissionHelp
end

function x300722_GetMissionTarget(sceneId, selfId)
	local subMission = x300722_HaveWhichSubMission(sceneId, selfId)
	local curKillMonsterNum = x300722_GetKillMonsterNum(sceneId, selfId)
	local szMonsterNum = "(" .. curKillMonsterNum .. "/200)"
	local szMissionTarget = ""
	if tonumber(6547) == tonumber(subMission) then
		szMissionTarget = "  除掉200只@npc_90151" .. szMonsterNum
	elseif tonumber(6522) == tonumber(subMission) then
		szMissionTarget = "  除掉200只@npc_90153" .. szMonsterNum
	elseif tonumber(6523) == tonumber(subMission) then
		szMissionTarget = "  除掉200只@npc_90155" .. szMonsterNum
	elseif tonumber(6524) == tonumber(subMission) then
		szMissionTarget = "  除掉200只@npc_90157" .. szMonsterNum
	elseif tonumber(6525) == tonumber(subMission) then
		szMissionTarget = "  除掉200只@npc_90159" .. szMonsterNum
	elseif tonumber(6526) == tonumber(subMission) then
		szMissionTarget = "  除掉200只@npc_90161" .. szMonsterNum
	elseif tonumber(6527) == tonumber(subMission) then
		szMissionTarget = "  除掉200只@npc_90163" .. szMonsterNum
	elseif tonumber(6528) == tonumber(subMission) then
...
function x300722_QuestLogRefresh( sceneId, selfId, MissionId)
	local subMission = x300722_HaveWhichSubMission(sceneId, selfId)
	local szMissionDesc = "\t为了让大家更多的结识朋友，为了让大家更快的增加好友度，"
	.."您可以组队来找我领取意气相投。哪怕队中有陌生人，只要接取意气相投也会把你们自动加为好友。"
	.."成功完成本任务，会增加组队间好友的好友度。"
	
	local szMissionHelp = x300722_GetMissionHelp(sceneId, selfId)
	
	local szMissionTarget = x300722_GetMissionTarget(sceneId, selfId)
	local szMissionNPC = "@npc_126055"
	local exp = x300722_CalcMissionEXP(sceneId, selfId)
	BeginQuestEvent(sceneId)
		local szMissionBonus = "\n#Y\n奖励内容：\n#W好友度:"..x300722_g_addFriendPoint.."点";
		local szMissionExp = "\n#W经验值:"..exp.."点";
	AddQuestLogCustomText( sceneId,
							"标题",						-- 标题
							x300722_g_MissionName,        -- 任务名字
...
function x300722_ProcQuestLogRefresh( sceneId, selfId, MissionId)
	x300722_QuestLogRefresh( sceneId, selfId, MissionId );
end