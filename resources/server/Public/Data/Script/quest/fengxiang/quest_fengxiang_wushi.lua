...
--任务入口函数
--**********************************
function x910012_ProcEventEntry( sceneId, selfId, targetId )	--点击该任务后执行此脚本
	BeginQuestEvent(sceneId)
			...
			if ret > 0 then
				...
				if ret > 0 then
					AddItemListToPlayer(sceneId,selfId)
					BeginQuestEvent(sceneId)
						local strText = "你获得了一套50级荣誉装"
						AddQuestText(sceneId,strText);
					EndQuestEvent(sceneId)
					DispatchQuestTips(sceneId,selfId)	
					AddQuestText(sceneId,"这是50级荣誉装，你收好了！")
				else
				--职业物品没有加成功
					BeginQuestEvent(sceneId)
					...
					EndQuestEvent(sceneId)
					DispatchQuestTips(sceneId,selfId)
					AddQuestText(sceneId,"这是一部分50级荣誉装！")
				end	
			else
				BeginQuestEvent(sceneId)
