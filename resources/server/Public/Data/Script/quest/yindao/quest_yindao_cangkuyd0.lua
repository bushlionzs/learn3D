...
x203774_g_Name	={}
x203774_g_ExtTarget={{type=20,n=1,target="找到@npc_123764"}}
x203774_g_MissionName="【个人】有存有取"
x203774_g_MissionInfo="\t看你的行囊已经被撑得变了形，把东西全带在身上，是不是很不方便？去找@npc_123764吧，他能够帮你解决这一系列的问题！"  --任务描述
x203774_g_MissionTarget=""		
x203774_g_MissionCompleted="\t我这里提供寄存服务，你可以储存一些不必时刻随身携带的东西，比如宝石或者平常不用的装备和材料等。当然，如果你想拥有足够大的空间的话，我还可以为你开启更大的仓库，不过这要你有足够的现金才行。 "					--完成任务npc说话的话
...
x203774_g_NpcGUID ={{ guid = 123764, name = "巴图存"} }
--MisDescEnd
--**********************************
...
function x203774_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x203774_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x203774_g_MissionId) > 0 then
...
function x203774_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x203774_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x203774_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203774_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203774_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x203774_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x203774_g_MissionName.."！", 0, 3)
				if x203774_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x203774_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x203774_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x203774_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x203774_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x203774_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x203774_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x203774_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x203774_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x203774_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x203774_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x203774_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x203774_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x203774_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
