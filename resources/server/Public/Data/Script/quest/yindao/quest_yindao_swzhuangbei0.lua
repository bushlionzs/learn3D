...
x203758_g_Name	={}
x203758_g_ExtTarget={{type=20,n=1,target="在@npc_123727处兑处换一件声望装"}}
x203758_g_MissionName="【个人】第一件声望装"
x203758_g_MissionInfo="\t亲爱的朋友，在我的手中有很多声望套装，换上这些套装你的能力会有明显的提升。\n\t点击声望商店后，左键点击#G换取世传君主套装#c000000的选项，会打开世传君主装备商店。其中有#G五件套装#c000000，选取你喜欢的一件#G右键点击该套装#c000000，在声望足够的情况下便兑换成功。\n\t这里我推荐你首先兑换世传君主套装中的兵器，如果你不小心将套装卖出，可点击商店中的#G回购#c000000再将套装买回。"  --任务描述
x203758_g_MissionTarget=""		
x203758_g_MissionCompleted="\t这件装备必然对你日后成长和能力提高起到不可低估的作用。\n\t一定要记住，日后在获得足够的声望还可以来我这里兑换装备。"					--完成任务npc说话的话
x203758_g_ContinueInfo="\t你不想兑换一件声望套装吗？"
...
x203758_g_NpcGUID ={{ guid = 123727, name = "声望商店"} }
--MisDescEnd
--**********************************
...
function x203758_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x203758_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x203758_g_MissionId) > 0 then
...
function x203758_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务："..x203758_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务："..x203758_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务："..x203758_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务："..x203758_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x203758_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x203758_g_MissionName.."！", 0, 3)
				if x203758_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x203758_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x203758_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x203758_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x203758_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x203758_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x203758_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x203758_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x203758_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x203758_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x203758_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x203758_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x203758_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x203758_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
