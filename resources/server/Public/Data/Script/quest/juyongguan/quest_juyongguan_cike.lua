...
x203001_g_Name	={"耶律楚材"}
x203001_g_ExtTarget={{type=20,n=1,target="击败@npc_19024"}}
x203001_g_MissionName="【剧场】逼反楚材<三>"
x203001_g_MissionInfo="\t（耶律楚材展开奏疏，看后良久没有说话）赵王竟然在奏疏中说我心怀不轨，阴谋作乱？！。。。\n\t（这时你突然感觉到一股杀气，一个黑影窜到你们面前！）"  --任务描述
x203001_g_MissionTarget="击败@npc_19024"		
x203001_g_MissionCompleted="\t（刺客见事情不济，诡异的笑了一下，随后气绝身亡，原来吞服了藏于口中的毒药）可恶！嗯，这是什么？"
x203001_g_ContinueInfo="\t留下活口！"
x203001_g_MissionHelp =	"#G如果您过久没有击败怪物，怪物会自行消失，您可以放弃并重新接取本任务。"
...
--**********************************
function x203001_ProcEventEntry(sceneId, selfId, NPCId, MissionId,which)	--点击该任务后执行此脚本
...
function x203001_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x203001_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x203001_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203001_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203001_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x203001_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x203001_g_MissionName.."！", 0, 3)
				if x203001_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x203001_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x203001_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x203001_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x203001_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x203001_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x203001_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x203001_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x203001_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x203001_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x203001_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x203001_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x203001_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x203001_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
			...
			for i = 0, Num-1 do
				...
				if humanObjId >= 0 then
		            ...
		            SetQuestByIndex( sceneId, humanObjId, misIndex, 0, 1)
				        ...
				        Msg2Player( sceneId, humanObjId, format("击败黑衣刺客   %d/1", GetQuestParam(sceneId,humanObjId,misIndex,0)), 8, 3)
				        Msg2Player( sceneId, humanObjId, format("击败黑衣刺客   %d/1", GetQuestParam(sceneId,humanObjId,misIndex,0)), 8, 2)
				 		end
			    end
				end
			end
