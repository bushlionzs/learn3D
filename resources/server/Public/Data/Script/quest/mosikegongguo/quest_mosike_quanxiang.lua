...
x203331_g_Name	={"郭宝玉","亚米尼亚大公" }
x203331_g_ExtTarget={{type=20,n=1,target="劝降@npc_117009"}}
x203331_g_MissionName="【剧场】策反大公<二>"
x203331_g_MissionTalk={"你并非真心与我们为敌！","他们给过你男人的尊严么？！","拔都大汗让你继续统治公国！"}
x203331_g_MissionInfo="\t我看亚米尼亚大公并非真心愿意帮助莫斯科大公，他妻子明目张胆的背叛和莫斯科大公等人的欺压早已在他心中燃起了仇恨的火焰，现在正是劝他归顺的最好时机！"  --任务描述
x203331_g_MissionTarget=""		
x203331_g_MissionCompleted="\t高级的技能能让你更轻松的解决掉你的敌人，所以记得当你的等级提高以后经常回来学习新的技能。\n\t人不能一口吃成胖子，但胖子却是一口一口吃出来的。持之以恒的练习，才能成为一代宗师。"
...
x203331_g_NpcGUID = {{ guid = 117009, name = "亚米尼亚大公"} }
--MisDescEnd
--**********************************
...
function x203331_ProcEventEntry(sceneId, selfId, NPCId, MissionId,which)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x203331_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x203331_g_MissionId) > 0 then
...
function x203331_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x203331_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x203331_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203331_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203331_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x203331_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x203331_g_MissionName.."！", 0, 3)
				if x203331_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x203331_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x203331_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x203331_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x203331_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x203331_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x203331_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x203331_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x203331_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x203331_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x203331_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x203331_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x203331_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x203331_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
