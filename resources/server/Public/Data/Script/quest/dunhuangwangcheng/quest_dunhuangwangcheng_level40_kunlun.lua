...
x256258_g_Name	={}
x256258_g_ExtTarget={{type=20,n=1,target="按任务攻略的指引升级到40级"}}
x256258_g_MissionName="【个人】四十而明"
x256258_g_MissionInfo="\t又见面了，亲爱的朋友，是否在升级的途中感到有些疲惫？那么，请暂时的停下来，听我说说：\n\t和你的朋友一起每天@npcsp_前往轮回台_129009一次，是升级的最佳选择，@npc_129066会发布国家押运任务，@npc_131510会发布国家守边任务。\n\t也别忘记了那些老朋友@npc_129042，@npc_129179，@npc_129060，或者继续和你的朋友们@npcsp_前往三环副本_129041，当然，在@npc_129029领取双倍经验也是必不可少的。"  --任务描述
x256258_g_MissionTarget=""		
x256258_g_MissionCompleted="\t四十级的你，已经具备在《成吉思汗2》中立足的资本。\n\t我这里为你准备了一些小礼物，希望能对你今后的历程有所帮助。"					--完成任务npc说话的话
x256258_g_ContinueInfo="别太心急，我为你的四十级准备了不少礼物，但是......你应该先到四十级再来。"
...
x256258_g_NpcGUID ={{ guid = 129056, name = "万事通"} }
--MisDescEnd
--**********************************
-- 最大等级
...
function x256258_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x256258_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x256258_g_MissionId) > 0 then
...
function x256258_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x256258_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x256258_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x256258_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x256258_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x256258_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x256258_g_MissionName.."！", 0, 3)
				if x256258_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x256258_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x256258_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x256258_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x256258_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x256258_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x256258_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x256258_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x256258_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x256258_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x256258_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x256258_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x256258_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x256258_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
