...
x256204_g_Name	={}
x256204_g_ExtTarget={{type=20,n=1,target="找到@npc_138566学习并使用4级#R散兵队形#W"}}
x256204_g_MissionName="【个人】新的技能"
x256204_g_MissionInfo="\t你已经出人头地，但还需要继续修炼，找到@npc_138566，学习并使用4级#R散兵队形#W。"  --任务描述
x256204_g_MissionTarget=""		
x256204_g_MissionCompleted="\t很好，希望你继续保持修炼！"					--完成任务npc说话的话
x256204_g_ContinueInfo="\t我对没有能力的人不感兴趣！"
...
function x256204_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x256204_g_MissionId) > 0 then
		return 
	end
...
function x256204_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x256204_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x256204_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x256204_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x256204_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x256204_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x256204_g_MissionName.."！", 0, 3)
				if x256204_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x256204_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x256204_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x256204_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x256204_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x256204_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x256204_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x256204_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x256204_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x256204_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x256204_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x256204_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x256204_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x256204_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
