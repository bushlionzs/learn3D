...
x256248_g_Name	={}
x256248_g_ExtTarget={{type=20,n=1,target="找到@npc_138511的传送人"}}
x256248_g_MissionName="【个人】囚龙岛的梦魇"
x256248_g_MissionInfo="\t关于囚笼岛的那个传说你可曾知晓？\n\t原来你也不清楚，哎，我也是知之不详，你若有兴趣可以@npc_138511，也许会有意外的收获。"  --任务描述
x256248_g_MissionTarget=""		
x256248_g_MissionCompleted="\t那是一个百余年前的传说，只有进入过囚笼岛的人才有机会探明真相，你若有兴趣不妨进去#G试炼#W一下，一探究竟。 "					--完成任务npc说话的话
...
x256248_g_NpcGUID ={{ guid = 138511, name = "特木隆"} }
--MisDescEnd
--**********************************
...
function x256248_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x256248_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x256248_g_MissionId) > 0 then
...
function x256248_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x256248_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x256248_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x256248_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x256248_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x256248_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x256248_g_MissionName.."！", 0, 3)
				if x256248_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x256248_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x256248_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x256248_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x256248_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x256248_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x256248_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x256248_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x256248_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x256248_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x256248_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x256248_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x256248_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x256248_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
