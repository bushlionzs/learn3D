...
x256250_g_Name	={}
x256250_g_ExtTarget={{type=20,n=1,target="找到@npc_138506"}}
x256250_g_MissionName="【个人】骑乘恢复"
x256250_g_MissionInfo="\t你的骑乘似乎有些疲倦，王城里的@npc_138506可以为你的骑乘回复活力而且不需要草料。\n\t记住，对待它就要像对待亲人一样，它也需要你的关注和照顾。"  --任务描述
x256250_g_MissionTarget=""		
x256250_g_MissionCompleted="\t要给骑乘恢复体力？你这可是问对地方了，就连王城的阿塔儿大人都常来请教我，我可以瞬间将所有的骑乘恢复到最佳状态，你只需要付出一点点金钱，仅仅是一点点而已。 "					--完成任务npc说话的话
...
x256250_g_NpcGUID ={{ guid = 138506, name = "奥尔格"} }
--MisDescEnd
--**********************************
...
function x256250_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x256250_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x256250_g_MissionId) > 0 then
...
function x256250_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x256250_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x256250_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x256250_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x256250_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x256250_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x256250_g_MissionName.."！", 0, 3)
				if x256250_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x256250_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x256250_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x256250_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x256250_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x256250_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x256250_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x256250_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x256250_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x256250_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x256250_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x256250_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x256250_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x256250_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
