...
x256102_g_Name	={}
x256102_g_ExtTarget={{type=20,n=1,target="将复仇神箭交给@npc_103040，回到王城。"}}
x256102_g_MissionName="【剧场】复仇神箭"
x256102_g_MissionInfo="\t有大智慧的铁木真，十年积蓄，十年忍耐，终于，让他等来了灭亡塔塔尔部的机会。"  --任务描述
x256102_g_MissionTarget=""		
x256102_g_MissionCompleted="\t历史从不曾化为尘埃，它会一直烙印在人们的心中。"					--完成任务npc说话的话
x256102_g_ContinueInfo="\t传说铁木真的祖先，为了重返祖先故土，杀了七十头牤牛，制成七十个风匣，又祭了苍天，方才点燃烟火，终于将阻碍归途的大山化出一个山口来，这就是蒙古“化铁出山”的传说。"
...
x256102_g_NpcGUID ={{ guid = 103040, name = "钥匙老人"}}
--MisDescEnd
--**********************************
...
function x256102_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x256102_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x256102_g_MissionId) > 0 then
...
function x256102_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x256102_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x256102_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x256102_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x256102_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x256102_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x256102_g_MissionName.."！", 0, 3)
				if x256102_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x256102_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x256102_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x256102_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x256102_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x256102_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x256102_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x256102_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x256102_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x256102_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x256102_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x256102_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x256102_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x256102_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
...
function x256102_ProcQuestAttach( sceneId, selfId, npcId, npcGuid, misIndex, MissionId )
    ...
    if bFind == 0 then
		...
		if IsHaveQuest(sceneId,selfId, x256102_g_MissionId) > 0 then
				local state = GetQuestStateNM(sceneId,selfId,npcId,x256102_g_MissionId)        ---取得任务状态 ，根据状态添加按钮 AddQuestTextNM
				AddQuestTextNM( sceneId, selfId, npcId, x256102_g_MissionId, state, -1 )
		end
end
