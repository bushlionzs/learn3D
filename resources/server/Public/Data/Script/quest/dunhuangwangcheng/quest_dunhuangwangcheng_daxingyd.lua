...
x256218_g_Name	={}
x256218_g_ExtTarget={{type=20,n=1,target="用#G水晶#W将一件#G声望装#W@npc_82068"}}
x256218_g_MissionName="【个人】第一次升星"
x256218_g_MissionInfo="\t亲爱的朋友，在你的身上有足够的#G水晶#W可以使你将一件#G世传君主装备#W升级到3星。 \n\t鼠标左键点击装备升星大师，再用鼠标左键点击#G装备升星#W的选项，会打开打星的功能栏，同时背包也会自动打开。\n\t 右键点击所要#G打造的装备#W，该装备会自动放入打星功能栏中，此时鼠标左键在下方选择#G宝石#W种类和#G打星#W等级，然后点击#G自动打星或打星#W就可以进行打星的操作了。\n\t打星完成后点击关闭功能栏，打星就此结束，已打星的装备也会自动回到您的背包中。\n\t #G注：装备的前三星是不会失败的，之后就可能会出现打星失败的情况。#W\n\t#G四-五星打星失败将会回到三星，五星以上失败将会回到零星！#W"  --任务描述
x256218_g_MissionTarget=""		
x256218_g_MissionCompleted="\t我想你已经仔细看过那件被打上3颗星的声望装备，感觉如何？升星后的装备确实比升星前要强化了不少吧。"					--完成任务npc说话的话
x256218_g_ContinueInfo="\t你不愿意为你的那件声望装备打上三星吗？"
...
x256218_g_NpcGUID ={{ guid = 138553, name = "托哈格"} }
--MisDescEnd
--**********************************
...
function x256218_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x256218_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x256218_g_MissionId) > 0 then
...
function x256218_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务："..x256218_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务："..x256218_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务："..x256218_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务："..x256218_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x256218_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x256218_g_MissionName.."！", 0, 3)
				if x256218_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x256218_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x256218_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x256218_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x256218_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x256218_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x256218_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x256218_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x256218_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x256218_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x256218_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x256218_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x256218_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x256218_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
