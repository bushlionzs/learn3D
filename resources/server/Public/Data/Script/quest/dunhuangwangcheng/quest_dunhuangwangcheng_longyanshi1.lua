...
x256252_g_Name	={}
x256252_g_ExtTarget={{type=20,n=1,target="询问@npc_126042"}}
x256252_g_MissionName="【个人】珍宝龙眼石"
x256252_g_MissionInfo="\t前几天我击败了盗墓贼之后缴获了几颗龙眼石，我还不知有何作用，请你帮我去请教一下@npc_126042，她对这些东西很有研究，问问这些龙眼石有什么作用。"  --任务描述
x256252_g_MissionTarget=""		
x256252_g_MissionCompleted="\t龙眼石本是炼金的极品材料，如今可是取之不易啊！你如果对此物感兴趣我倒是可以赠与你几颗 。"					--完成任务npc说话的话
...
x256252_g_NpcGUID ={{ guid = 126042, name = "龙眼石兑换"} }
--MisDescEnd
--**********************************
...
function x256252_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x256252_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x256252_g_MissionId) > 0 then
...
function x256252_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务："..x256252_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务："..x256252_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务："..x256252_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务："..x256252_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x256252_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x256252_g_MissionName.."！", 0, 3)
				if x256252_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x256252_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x256252_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x256252_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x256252_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x256252_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x256252_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x256252_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x256252_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x256252_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x256252_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x256252_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x256252_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x256252_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
