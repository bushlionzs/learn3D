...
x256291_g_Name	={}
x256291_g_ExtTarget={{type=20,n=1,target="完成一次@npc_28810"},{type=20,n=1,target="到边塞完成@npc_28811"},}
x256291_g_MissionName="【个人】国家押运和国家守边"
x256291_g_MissionInfo="\t时隔数日，亲爱的朋友我们又见面了，现在你的威名在王城可谓家喻户晓。\n\t这次@npc_132063和@npc_134552邀请你去帮助他们，我看你就不要推辞了，不过要小心，这些任务是有危险的。"  --任务描述
x256291_g_MissionTarget=""		
x256291_g_MissionCompleted="\t呵呵，这次你为我们的王国立了大功，做功臣的感觉如何啊？\n\t记得每天都去他们那里看看，一定会对你有不少帮助。"					--完成任务npc说话的话
x256291_g_ContinueInfo="\t年轻人做事有些急躁是难免的，做事要戒骄戒躁不要半途而废。"
...
x256291_g_NpcGUID ={{ guid = 132002, name = "万事通"} }
--MisDescEnd
--**********************************
-- 押运完成标记
...
function x256291_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x256291_g_MissionId) > 0 then
		return
	end
	if IsHaveQuest(sceneId,selfId, x256291_g_MissionId) > 0 then
...
function x256291_Msg2toplayer( sceneId, selfId,type)
    if type == 0 then
        Msg2Player(sceneId, selfId, "您接受了任务："..x256291_g_MissionName.."！", 0, 2)
        Msg2Player(sceneId, selfId, "您接受了任务："..x256291_g_MissionName.."！", 0, 3)
    elseif type == 1 then
        Msg2Player(sceneId, selfId, "您放弃了任务："..x256291_g_MissionName.."！", 0, 2)
        Msg2Player(sceneId, selfId, "您放弃了任务："..x256291_g_MissionName.."！", 0, 3)
    elseif type == 2 then
        Msg2Player(sceneId, selfId, "您完成了任务"..x256291_g_MissionName.."！", 0, 2)
        Msg2Player(sceneId, selfId, "您完成了任务"..x256291_g_MissionName.."！", 0, 3)
        if x256291_g_ExpBonus > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R经验"..x256291_g_ExpBonus.."#Y的奖励", 0, 2)
        end
        if x256291_g_BonusMoney1 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x256291_g_BonusMoney1.."文#Y的奖励", 0, 2)
        end
        if x256291_g_BonusMoney2 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R现银"..x256291_g_BonusMoney2.."文#Y的奖励", 0, 2)
        end
        if x256291_g_BonusMoney3 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x256291_g_BonusMoney3.."文#Y的奖励", 0, 2)
        end
        if x256291_g_BonusMoney4 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x256291_g_BonusMoney4.."#Y的奖励", 0, 2)
        end
        if x256291_g_BonusMoney5 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R声望"..x256291_g_BonusMoney5.."#Y的奖励", 0, 2)
        end
        if x256291_g_BonusMoney6 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x256291_g_BonusMoney6.."#Y的奖励", 0, 2)
        end
    end
end
...
function x256291_ProcQuestLogRefresh( sceneId, selfId, MissionId)
    ...
    local strTarget = format( "任务已经完成，请回报@npc_%d", x256291_g_NpcGUID[1].guid)
