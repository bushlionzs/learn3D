...
x256116_g_Name	={}
x256116_g_ExtTarget={{type=20,n=1,target="将西征神箭交给@npc_117000"}}
x256116_g_MissionName="【剧场】西征神箭"
x256116_g_MissionInfo="\t莫斯科一战，拔都凭借自己过人的智慧和勇气将莫斯科大公布置的阴谋一个个击破，最后除掉了这个罪魁祸首，一举夺得斯拉夫中心城市——莫斯科。"  --任务描述
x256116_g_MissionTarget=""		
x256116_g_MissionCompleted="\t莫斯科大公阴谋败露，困兽犹斗而死，基辅大公只身逃往老巢。成吉思汗的孙子，未来的金帐大汗拔都，取得了西征路上的第一场大胜。"					--完成任务npc说话的话
x256116_g_ContinueInfo="\t人类征战史上又一个篇章拉开了序幕！"
...
x256116_g_NpcGUID ={{ guid = 117000, name = "钥匙老人"}}
--MisDescEnd
--**********************************
...
function x256116_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x256116_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x256116_g_MissionId) > 0 then
...
function x256116_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x256116_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x256116_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x256116_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x256116_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x256116_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x256116_g_MissionName.."！", 0, 3)
				if x256116_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x256116_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x256116_g_BonusMoney1 > 0 then
				...
				if x256116_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x256116_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x256116_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x256116_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x256116_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x256116_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x256116_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x256116_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x256116_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x256116_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
...
local bFind  = 0
    ...
    if bFind == 0 then
		...
		if IsHaveQuest(sceneId,selfId, x256116_g_MissionId) > 0 then
				local state = GetQuestStateNM(sceneId,selfId,npcId,x256116_g_MissionId)        ---取得任务状态 ，根据状态添加按钮 AddQuestTextNM
				AddQuestTextNM( sceneId, selfId, npcId, x256116_g_MissionId, state, -1 )
		end
end
