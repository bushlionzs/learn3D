...
x256232_g_Name	={}
x256232_g_ExtTarget={{type=20,n=1,target="在@npc_138535的帮助下#G加入一个帮会#W"},}
x256232_g_MissionName="【个人】第一次入帮"
x256232_g_MissionInfo="\t亲爱的朋友，一个人只身行走江湖的艰难恐怕你早有感触，加入一个帮会吧，它会给你家一样的温暖。\n\t鼠标点击@npc_138535，点击#G加入帮会#W，在新界面上点击#G确定#W，在#G加入帮会#W功能栏中，用鼠标点击你所选的帮会，最后点击#G申请加入#W等待帮主批准入帮。"  --任务描述
x256232_g_MissionTarget=""		
x256232_g_MissionCompleted="\t看来你已经找到和你志同道合的兄弟，我在这里恭喜你了。\n\t在帮会中互助是十分重要的，努力增强帮会的实力，也会使你个人收获丰厚。"					--完成任务npc说话的话
x256232_g_ContinueInfo="别太心急，加入帮会可不是轻易完成的，要等待机会啊！\n\t#G注：加入帮会完成帮会任务你会获得丰厚的奖励。#W"
x256232_g_MissionHelp =	"注：加入帮会完成帮会任务你会获得丰厚的奖励。"
...
x256232_g_NpcGUID ={{ guid = 138535, name = "杰拜"} }
--MisDescEnd
--*********************************
...
function x256232_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x256232_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x256232_g_MissionId) > 0 then
...
function x256232_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务："..x256232_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务："..x256232_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务："..x256232_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务："..x256232_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x256232_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x256232_g_MissionName.."！", 0, 3)
				if x256232_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x256232_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x256232_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x256232_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x256232_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x256232_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x256232_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x256232_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x256232_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x256232_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x256232_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x256232_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x256232_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x256232_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
