...
x256057_g_Name	={}
x256057_g_ExtTarget={{type=20,n=1,target="去@npc_82003烤鱼"}}
x256057_g_MissionName="【主线】头领的要求<二>"
x256057_g_MissionInfo="\t（胡和鲁瞪着眼睛对你大吼）\n\t你还愣着干什么，还不快把鱼给老子烤熟了！惹急了，我拿你下酒！"  --任务描述
x256057_g_MissionTarget=""		
x256057_g_MissionCompleted="\t真没看出来，你烤的这鱼还真不错，明天不用原来那个阿六送了，以后就你来吧。"					--完成任务npc说话的话
x256057_g_ContinueInfo="\t烤鱼呢！"
x256057_g_MissionHelp =	"#G胡和鲁前方就有一个火堆，看起来很适合烤鱼。"
...
x256057_g_NpcGUID ={}
x256057_g_GroupPoint	=	{ {type=10,id =178,target = "火堆"} }--专门用于采集点特效用，与任务相关未必就是完成任务的必要因素，因此，只要相关就可以设置这个属性
--MisDescEnd
--**********************************
...
function x256057_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x256057_g_MissionId) > 0 then
		return 
	end
		...
		if x256057_ProcAcceptCheck(sceneId, selfId, NPCId) > 0 then
					...
					if retmiss ==0 then
						...
						return 0
									...
									EndQuestEvent()
									AddQuestText(sceneId,"您获得了物品：鲜鱼")
			  					--DispatchQuestTips(sceneId, selfId)
...
function x256057_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x256057_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x256057_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x256057_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x256057_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x256057_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x256057_g_MissionName.."！", 0, 3)
				if x256057_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x256057_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x256057_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x256057_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x256057_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x256057_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x256057_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x256057_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x256057_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x256057_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x256057_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x256057_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x256057_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x256057_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
