...
x256265_g_Name	={}
x256265_g_ExtTarget={{type=20,n=1,target="帮会神兽"},{type=20,n=1,target="帮会暗战"},{type=20,n=1,target="帮会图腾"},{type=20,n=1,target="帮会除魔"},}
x256265_g_MissionName="【个人】兴帮之功"
x256265_g_MissionInfo="\t你好，我亲爱的朋友，你如愿的加入了你所向往的帮会。\n\t为了让我们的帮会一天天的强大起来，帮中兄弟要齐心完成帮中的帮会神兽，帮会暗战，帮会图腾，帮会除魔任务，在完成后你会得到很丰厚的回报。"  --任务描述
x256265_g_MissionTarget=""			
x256265_g_MissionCompleted="\t帮中的兄弟对你的表现十分满意，希望你以后再接再厉。"					--完成任务npc说话的话
x256265_g_ContinueInfo="\t帮会任务，是需要和帮中兄弟合作完成的，你不要太心急。"
...
x256265_g_NpcGUID ={{ guid = 126020, name = "帮会任务发布人"} }
--MisDescEnd
--**********************************
-- 帮会内政
--x256265_g_MP_nz   = 1
...
function x256265_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x256265_g_MissionId) > 0 then
		return
	end
	if IsHaveQuest(sceneId,selfId, x256265_g_MissionId) > 0 then
    ...
    elseif x256265_ProcAcceptCheck(sceneId, selfId, NPCId) > 0 then
        ...
        AddQuestText( sceneId,"找到@npc_126020完成:")
...
function x256265_GetBonus( sceneId, selfId,NpcID)
    ...
    if x256265_g_BonusMoney8 > 0 then
		AddGuildUserPoint(sceneId,selfId,x256265_g_BonusMoney8)	--增加帮贡
...
function x256265_Msg2toplayer( sceneId, selfId,type)
    if type == 0 then
        Msg2Player(sceneId, selfId, "您接受了任务："..x256265_g_MissionName.."！", 0, 2)
        Msg2Player(sceneId, selfId, "您接受了任务："..x256265_g_MissionName.."！", 0, 3)
    elseif type == 1 then
        Msg2Player(sceneId, selfId, "您放弃了任务："..x256265_g_MissionName.."！", 0, 2)
        Msg2Player(sceneId, selfId, "您放弃了任务："..x256265_g_MissionName.."！", 0, 3)
    elseif type == 2 then
        Msg2Player(sceneId, selfId, "您完成了任务"..x256265_g_MissionName.."！", 0, 2)
        Msg2Player(sceneId, selfId, "您完成了任务"..x256265_g_MissionName.."！", 0, 3)      
        if x256265_g_ExpBonus > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R经验"..x256265_g_ExpBonus.."#Y的奖励", 0, 2)
        end
        if x256265_g_BonusMoney1 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x256265_g_BonusMoney1.."文#Y的奖励", 0, 2)
        end
        if x256265_g_BonusMoney2 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R现银"..x256265_g_BonusMoney2.."文#Y的奖励", 0, 2)
        end
        if x256265_g_BonusMoney3 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x256265_g_BonusMoney3.."文#Y的奖励", 0, 2)
        end
        if x256265_g_BonusMoney4 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x256265_g_BonusMoney4.."#Y的奖励", 0, 2)
        end
        if x256265_g_BonusMoney5 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R声望"..x256265_g_BonusMoney5.."#Y的奖励", 0, 2)
        end
        if x256265_g_BonusMoney6 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x256265_g_BonusMoney6.."#Y的奖励", 0, 2)
        end
    end
end
...
function x256265_ProcQuestLogRefresh( sceneId, selfId, MissionId)
    ...
    local strTarget = format( "任务已经完成，请回报@npc_%d", x256265_g_NpcGUID[1].guid)
