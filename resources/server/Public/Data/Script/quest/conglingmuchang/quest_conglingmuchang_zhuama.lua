...
x203539_g_Name	={}
x203539_g_ExtTarget={{type=20,n=1,target="抓到一匹@npc_125027"}}
x203539_g_MissionName="【新手】白鹿长老的考验"
x203539_g_MissionInfo="\t在成吉思汗的世界中，想要成为真正的英雄，一匹好马是你必不可少的伙伴。\n\t看见我身后的这些马了吧，你只要将鼠标移动到小马驹身上，点击左键就可以捕捉了。\n\t现在，按我教给你的方法，去选择一匹来做为你第一个伙伴吧。"  --任务描述
x203539_g_MissionTarget="抓到一匹@npc_125027"		
x203539_g_MissionCompleted="\t牢记驯服它们的方法，或许今后你将会得到更多更好的坐骑。"					--完成任务npc说话的话
x203539_g_ContinueInfo="\t你驯服它了么？"
x203539_g_MissionHelp =	"#G将鼠标移动至#R马匹#G上，点击#R左键#G一下，耐心等待抓马读条完毕，系统会提示您捕捉#R成功#G，如果失败了，很可能是由于您和马的#R距离过远#G导致的，您只需要#R靠近马匹#G，重新抓一次即可。#W"
...
function x203539_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x203539_g_MissionId) > 0 then
		return 
	end
...
function x203539_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x203539_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x203539_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203539_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203539_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x203539_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x203539_g_MissionName.."！", 0, 3)
				if x203539_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G经验"..x203539_g_ExpBonus.."#cffcf00的奖励", 0, 2)
  			end
				if x203539_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#cffcf00获得#G银卡"..x203539_g_BonusMoney1.."文#cffcf00的奖励", 0, 2)
	  		end
				if x203539_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#cffcf00获得#G现银"..x203539_g_BonusMoney2.."文#cffcf00的奖励", 0, 2)
	  		end
				if x203539_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#cffcf00获得#G金卡"..x203539_g_BonusMoney3.."文#cffcf00的奖励", 0, 2)
				end
				if x203539_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G朝廷声望值"..x203539_g_BonusMoney4.."#cffcf00的奖励", 0, 2)
				end
				if x203539_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G声望"..x203539_g_BonusMoney5.."#cffcf00的奖励", 0, 2)
				end
				if x203539_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G荣誉"..x203539_g_BonusMoney6.."#cffcf00的奖励", 0, 2)
				end
		end
end
