...
x203541_g_Name	={}
x203541_g_ExtTarget={{type=20,n=1,target="找到@npc_123828"}}
x203541_g_MissionName="【新手】踏入王城"
x203541_g_MissionInfo="\t我们的王城，就在前方！\n\t那里有更高级的武器，技能，马匹，更有着无数和你一样的年轻士兵等候着你的到来。\n\t去找王城御卫长报道吧，幸运的小子！"  --任务描述
x203541_g_MissionTarget="找到@npc_123828"		
x203541_g_MissionCompleted="\t欢迎来到王城，新兵！ "					--完成任务npc说话的话
...
x203541_g_NpcGUID ={{ guid = 123828, name = "王城御卫长"} }
--MisDescEnd     
...
function x203541_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x203541_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x203541_g_MissionId) > 0 then
		...
		if x203541_ProcAcceptCheck(sceneId, selfId, NPCId) > 0 then
					...
					if retmiss ==0 then                                          ------添加失败 ==0 ，1 成功                                                                  
		  			...
		  			x203541_Msg2toplayer( sceneId, selfId,0)
						--if x203541_CheckSubmit(sceneId, selfId, NPCId) > 0 then
								local misIndex = GetQuestIndexByID(sceneId,selfId,x203541_g_MissionId)       ---- 取得任务状态                                               
...
function x203541_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x203541_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x203541_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203541_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203541_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x203541_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x203541_g_MissionName.."！", 0, 3)
				if x203541_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G经验"..x203541_g_ExpBonus.."#cffcf00的奖励", 0, 2)
  			end
				if x203541_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#cffcf00获得#G银卡"..x203541_g_BonusMoney1.."文#cffcf00的奖励", 0, 2)
	  		end
				if x203541_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#cffcf00获得#G现银"..x203541_g_BonusMoney2.."文#cffcf00的奖励", 0, 2)
	  		end
				if x203541_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#cffcf00获得#G金卡"..x203541_g_BonusMoney3.."文#cffcf00的奖励", 0, 2)
				end
				if x203541_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G朝廷声望值"..x203541_g_BonusMoney4.."#cffcf00的奖励", 0, 2)
				end
				if x203541_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G声望"..x203541_g_BonusMoney5.."#cffcf00的奖励", 0, 2)
				end
				if x203541_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G荣誉"..x203541_g_BonusMoney6.."#cffcf00的奖励", 0, 2)
				end
		end
end
...
function x203541_ProcQuestAttach( sceneId, selfId, npcId, npcGuid, misIndex, MissionId )
    ...
    if bFind == 0 then
		...
		if IsHaveQuest(sceneId,selfId, x203541_g_MissionId) > 0 then
				local state = GetQuestStateNM(sceneId,selfId,npcId,x203541_g_MissionId)        ---取得任务状态 ，根据状态添加按钮 AddQuestTextNM
				AddQuestTextNM( sceneId, selfId, npcId, x203541_g_MissionId, state, -1 )
		end
end
