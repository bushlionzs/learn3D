...
--x203531_g_Name	={"穆林","斡鲁斯" }
x203531_g_ExtTarget={{type=20,n=5,target="消灭5只@npc_125021"}}
x203531_g_MissionName="【新手】新兵器的威力"
--x203531_g_MissionTalk={"武士的特点","技能的特色","如何能成为宗师"}
x203531_g_MissionInfo="\t获得新兵器之后，记得按下“B”键打开背包，右键点击你刚刚获得的武器，就可以装备上新武器了。村西有一些草原狼，建议你在装备武器后去拿它们检验一下你新武器的威力。\n\t在这之后，你可以去找雄鹰长老请教一番。"  --任务描述
x203531_g_MissionTarget=""		
x203531_g_MissionCompleted="\t（雄鹰长老满意的对你点点头说道）\n\t有了新兵器之后，是不是感觉自己的实力有明显的提升？\n\t不过，年轻的勇士，新的兵器需要搭配新的技能才能让你的实力更进一步。这2个技能就送给你了，希望你能迅速它们的精髓。"
x203531_g_ContinueInfo={"\t你还是再去熟悉熟悉手里的兵器，我再传授给你技能。","\t你还是再去熟悉熟悉手里的兵器，我再传授给你技能。","\t你还是再去熟悉熟悉手里的兵器，我再传授给你技能。"}
x203531_g_MissionHelp =	"按下“B”键打开背包，右键点击就可以装备你刚刚获得的武器。"
...
x203531_g_NpcGUID = {{ guid = 125015, name = "雄鹰长老"} }
...
function x203531_ProcEventEntry(sceneId, selfId, NPCId, MissionId,which)	--点击该任务后执行此脚本
   CallScriptFunction( UTILITY_SCRIPT, "ShowNewUITips", sceneId, selfId, 1,0 )
	if IsQuestHaveDone(sceneId, selfId, x203531_g_MissionId) > 0 then
		return 
	end
...
function x203531_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x203531_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x203531_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203531_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203531_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x203531_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x203531_g_MissionName.."！", 0, 3)
				if x203531_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G经验"..x203531_g_ExpBonus.."#cffcf00的奖励", 0, 2)
  			end
				if x203531_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#cffcf00获得#G银卡"..x203531_g_BonusMoney1.."文#cffcf00的奖励", 0, 2)
	  		end
				if x203531_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#cffcf00获得#G现银"..x203531_g_BonusMoney2.."文#cffcf00的奖励", 0, 2)
	  		end
				if x203531_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#cffcf00获得#G金卡"..x203531_g_BonusMoney3.."文#cffcf00的奖励", 0, 2)
				end
				if x203531_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G朝廷声望值"..x203531_g_BonusMoney4.."#cffcf00的奖励", 0, 2)
				end
				if x203531_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G声望"..x203531_g_BonusMoney5.."#cffcf00的奖励", 0, 2)
				end
				if x203531_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G荣誉"..x203531_g_BonusMoney6.."#cffcf00的奖励", 0, 2)
				end
		end
end
