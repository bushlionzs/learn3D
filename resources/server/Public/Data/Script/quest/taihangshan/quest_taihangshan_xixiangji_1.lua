...
x270011_g_Name	={}
x270011_g_ExtTarget={{type=20,n=1,target="到@npc_141219处等待张生把孙飞虎引开后杀了孙飞虎"}}
x270011_g_MissionName="【主线】美人计<二>"
x270011_g_MissionInfo="\t好，我这就依计行事，换上这套衣服前去汉军营附近，诱那飞虎校尉出来，剩下的事就靠侠士你多帮忙了。"  --任务描述
x270011_g_MissionTarget=""		
x270011_g_MissionCompleted="\t想不到张生一个文弱书生还有这个能耐和胆量。"					--完成任务npc说话的话
x270011_g_ContinueInfo="\t这计划能行吗？还是让莺莺快跑吧！"
...
x270011_g_huwei1 = -1
x270011_g_huwei2 = -1
x270011_g_NpcGUID ={{ guid = 141201, name = "崔母"}}
...
function x270011_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x270011_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x270011_g_MissionId) > 0 then
...
function x270011_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x270011_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x270011_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x270011_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x270011_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x270011_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x270011_g_MissionName.."！", 0, 3)
				if x270011_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x270011_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x270011_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x270011_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x270011_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x270011_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x270011_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x270011_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x270011_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x270011_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x270011_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x270011_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x270011_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x270011_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
                 ...
                 if IsHaveQuest(sceneId,humanObjId, x270011_g_MissionId) > 0 then
                    local misIndex = GetQuestIndexByID(sceneId,humanObjId,x270011_g_MissionId)
										if GetQuestParam(sceneId,humanObjId,misIndex,0) < 1 then
												...
												Msg2Player(sceneId, humanObjId, format("杀死孙飞虎   %d/1", GetQuestParam(sceneId,humanObjId,misIndex,0)), 0, 3)
		...
		if IsHaveQuest(sceneId,selfId, x270011_g_MissionId) > 0 then
			if x270011_g_doing == 0 then
				if x270011_CheckSubmit(sceneId, selfId) <= 0 then
					--if GetName(sceneId,x270011_g_sunfeihu)~="孙飞虎" then
							...
							if x270011_g_PROTECTINFO.ScneneTimerIndex >= 0 then
								--if 	x270011_g_sunfeihu == -1 and	x270011_g_huwei1 == -1 then
											x270011_g_doing = 1
											x270011_g_sunfeihu=CreateMonster(sceneId, 1617,63,38,5,1,-1,-1,20,80000,180,"孙飞虎")
											x270011_g_huwei1=CreateMonster(sceneId, 1618,62,41,3,1,-1,-1,20,80000,180,"护卫")
											x270011_g_huwei2=CreateMonster(sceneId, 1618,62,38,3,1,-1,-1,20,80000,180,"护卫")
											if IsObjValid (sceneId,x270011_g_sunfeihu) == 1 then
													SetPatrolId(sceneId, x270011_g_sunfeihu, 0)
											end
											if IsObjValid (sceneId,x270011_g_huwei1) == 1 then
													SetPatrolId(sceneId, x270011_g_huwei1, 1)
											end
											if IsObjValid (sceneId,x270011_g_huwei2) == 1 then
													SetPatrolId(sceneId, x270011_g_huwei2, 2)
											end
								--end	
							end
					--end
...
function x270011_OnScneneTimer(sceneId)
	if x270011_g_doing==1   then
		if GetCurrentTime() - x270011_g_PROTECTINFO.StartTime >= 2   then
						x270011_g_doing = 2
						if IsObjValid (sceneId,x270011_g_sunfeihu) == 1 then
								NpcTalk(sceneId, x270011_g_sunfeihu, "你说刚才那姑娘来了没？", -1)
						end
		end
	elseif x270011_g_doing==2   then
		if GetCurrentTime() - x270011_g_PROTECTINFO.StartTime >= 4   then
						x270011_g_doing = 3
						if IsObjValid (sceneId,x270011_g_huwei1) == 1 then
								NpcTalk(sceneId, x270011_g_huwei1, "看她刚才是吓傻了，没准跑了吧！", -1)
						end
		end
	elseif x270011_g_doing==3   then
		if GetCurrentTime() - x270011_g_PROTECTINFO.StartTime >= 6   then
						x270011_g_doing = 4
						if IsObjValid (sceneId,x270011_g_sunfeihu) == 1 then
								NpcTalk(sceneId, x270011_g_sunfeihu, "往哪里跑？跑到天边我也能抓回来！", -1)
						end

		end
	elseif x270011_g_doing==4   then
		if GetCurrentTime() - x270011_g_PROTECTINFO.StartTime >= 8   then
						x270011_g_doing = 5
						if IsObjValid (sceneId,x270011_g_huwei2) == 1 then
								NpcTalk(sceneId, x270011_g_huwei2, "没准看上孙将军英明神武自己颠颠过来了！", -1)
						end
		end
	elseif x270011_g_doing==5   then
		if GetCurrentTime() - x270011_g_PROTECTINFO.StartTime >= 10   then
						x270011_g_doing = 6
						if IsObjValid (sceneId,x270011_g_sunfeihu) == 1 then
								NpcTalk(sceneId, x270011_g_sunfeihu, "哈哈哈哈", -1)
						end
						if IsObjValid (sceneId,x270011_g_huwei1) == 1 then
								NpcTalk(sceneId, x270011_g_huwei1, "哈哈哈哈", -1)
						end
						x270011_g_zhangsheng=CreateMonster(sceneId, 1621,95,45,3,1,-1,-1,20,30000,180,"崔莺莺","张生扮")
		end
	elseif x270011_g_doing==6   then
		if GetCurrentTime() - x270011_g_PROTECTINFO.StartTime >= 6  then
						x270011_g_doing = 7
						if IsObjValid (sceneId,x270011_g_huwei1) == 1 then
								NpcTalk(sceneId, x270011_g_huwei1, "诶，这不是来了？", -1)
						end
						if IsObjValid (sceneId,x270011_g_huwei1) == 1 then
								SetMonsterDir(sceneId,x270011_g_huwei1,1)
						end
		end
	elseif x270011_g_doing==7   then
		if GetCurrentTime() - x270011_g_PROTECTINFO.StartTime >= 8  then
						x270011_g_doing = 8
						if IsObjValid (sceneId,x270011_g_sunfeihu) == 1 then
								NpcTalk(sceneId, x270011_g_sunfeihu, "哈哈，来了！省了我不少麻烦！", -1)
						end
						if IsObjValid (sceneId,x270011_g_sunfeihu) == 1 then
								SetMonsterDir(sceneId,x270011_g_sunfeihu,0)
						end
						if IsObjValid (sceneId,x270011_g_huwei2) == 1 then
								SetMonsterDir(sceneId,x270011_g_huwei2,2)
						end
		end
	elseif x270011_g_doing==8   then
		if GetCurrentTime() - x270011_g_PROTECTINFO.StartTime >= 10 then
						x270011_g_doing = 9
						if IsObjValid (sceneId,x270011_g_huwei2) == 1 then
								NpcTalk(sceneId, x270011_g_huwei2, "呀！她怎么掉头走了！", -1)
						end
						if IsObjValid (sceneId,x270011_g_zhangsheng) == 1 then
								SetPatrolId(sceneId, x270011_g_zhangsheng, 3)
						end
		end
	elseif x270011_g_doing==9   then
		if GetCurrentTime() - x270011_g_PROTECTINFO.StartTime >= 12 then
						x270011_g_doing = 10
						if IsObjValid (sceneId,x270011_g_sunfeihu) == 1 then
								NpcTalk(sceneId, x270011_g_sunfeihu, "害羞了，这种我喜欢！", -1)
								SetPatrolId(sceneId, x270011_g_sunfeihu, 0)
						end
		end
	elseif x270011_g_doing==10   then
		if GetCurrentTime() - x270011_g_PROTECTINFO.StartTime >= 20   then
						x270011_g_doing = 11
						if IsObjValid (sceneId,x270011_g_huwei2) == 1 then
								NpcTalk(sceneId, x270011_g_huwei2, "我们俩在这等着吧！", -1)
						end
		end
	elseif x270011_g_doing==11   then
		if GetCurrentTime() - x270011_g_PROTECTINFO.StartTime >= 14   then
						x270011_g_doing = 12
						if IsObjValid (sceneId,x270011_g_huwei1) == 1 then
								NpcTalk(sceneId, x270011_g_huwei1, "恩，我打赌一炷香时间他就能回来！", -1)
								SetMonsterDir(sceneId,x270011_g_huwei1,1)
						end
						if IsObjValid (sceneId,x270011_g_huwei2) == 1 then
								SetMonsterDir(sceneId,x270011_g_huwei2,2)
	...
	elseif x270011_g_doing==12   then
		if GetCurrentTime() - x270011_g_PROTECTINFO.StartTime >= 16  then
						x270011_g_doing = 13
						if IsObjValid (sceneId,x270011_g_sunfeihu) == 1 then
								NpcTalk(sceneId, x270011_g_sunfeihu, "我看你要跑到哪里！", -1)
						end
		end
	elseif x270011_g_doing==13   then
		if GetCurrentTime() - x270011_g_PROTECTINFO.StartTime >= 18  then
						x270011_g_doing = 14
						if IsObjValid (sceneId,x270011_g_zhangsheng) == 1 then
								SetMonsterDir(sceneId,x270011_g_zhangsheng, 3)
						end
		end
	elseif x270011_g_doing==14   then
		if GetCurrentTime() - x270011_g_PROTECTINFO.StartTime >= 20 then
						x270011_g_doing = 15
						if IsObjValid (sceneId,x270011_g_sunfeihu) == 1 then
								NpcTalk(sceneId, x270011_g_sunfeihu, "什么你居然是男的！你到底是谁？", -1)
								SetMonsterCamp(sceneId,x270011_g_sunfeihu,21)
                SetMonsterDir(sceneId,x270011_g_zhangsheng,45)
						end
		end
	elseif x270011_g_doing==15   then
		if GetCurrentTime() - x270011_g_PROTECTINFO.StartTime >=22 then
						x270011_g_doing = 16
						if IsObjValid (sceneId,x270011_g_zhangsheng) == 1 then
								NpcTalk(sceneId, x270011_g_zhangsheng, "壮士，剩下的拜托你了！", -1)
						end
		end
