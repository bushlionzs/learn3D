...
x270010_g_ExtTarget={{type=20,n=1,target="到@npc_141218放下酒坛询问红娘的爷爷"}}
...
function x270010_useitem(sceneId,selfId)
		...
		local distanceRet = 100 - (75 - x) * (75 - x) - (215 - z) * (215 - z)
		local misIndex = GetQuestIndexByID(sceneId,selfId,x270010_g_MissionId)
		if sceneId == 21 then
		 			...
		 			if x270010_g_doing==0 then
							...
							SetQuestByIndex(sceneId,selfId,misIndex,7,1)
				 			...
				 			if x270010_g_PROTECTINFO.ScneneTimerIndex >= 0 then
									x270010_g_yuelao=CreateMonster(sceneId, 444,75,210,3,1,-1,-1,20,15000,0,"月老")
        					x270010_g_doing =1
