...
x202000_g_Name	={}
x202000_g_ExtTarget={{type=20,n=5,target="解救5名@npc_105322"}}
x202000_g_MissionName="【剧场】营救"
x202000_g_MissionInfo="\t我知道你的能力，你会有办法救出我们乞颜的勇士他们都被装在麻袋里面，要赶紧救出他们，不能让他们葬身熊口！"  --任务描述
x202000_g_MissionTarget=""		
x202000_g_MissionCompleted="\t看来还是有些兄弟葬身熊口了！他们会得到长生天的祝福的！这些活下来的人都是你救的，我们欠你一条命！"					--完成任务npc说话的话
x202000_g_ContinueInfo="\t这些都是跟我出生入死的弟兄，一定要救他们出来！"
...
x202000_g_GroupPoint	={ {type=10,id =311,target = "装人麻袋"} }
--MisDescEnd
--**********************************
...
function x202000_ProcEventEntry(sceneId, selfId, NPCId, MissionId, which)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x202000_g_MissionId) > 0 then
		return 
	end
...
function x202000_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x202000_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x202000_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x202000_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x202000_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x202000_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x202000_g_MissionName.."！", 0, 3)
				if x202000_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x202000_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x202000_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x202000_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x202000_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x202000_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x202000_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x202000_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x202000_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x202000_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x202000_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x202000_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x202000_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x202000_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
				...
				if IsHaveQuest(sceneId,selfId, x202000_g_MissionId) > 0 then
                    local misIndex = GetQuestIndexByID(sceneId,selfId,x202000_g_MissionId)
										if GetQuestParam(sceneId,selfId,misIndex,0) < 5 then
												...
												Msg2Player(sceneId, selfId, format("解救被俘的勇士   %d/5", GetQuestParam(sceneId,selfId,misIndex,0)), 0, 3)
												x202000_CheckSubmit( sceneId, selfId)
										end
										local x,z =GetWorldPos(sceneId,selfId)
										local creatid =CreateMonster(sceneId, 152,x,z,3,1,-1,-1,20,4000,180,"被俘的勇士")
