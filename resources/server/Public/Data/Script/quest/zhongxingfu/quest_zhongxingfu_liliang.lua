...
x203306_g_Name	={"李凉"}
x203306_g_ExtTarget={{type=20,n=1,target="击败@npc_110041"}}
x203306_g_MissionName="【剧场】此情不渝"
x203306_g_MissionInfo="\t公主向你讲述了她与一品堂堂主自小青梅竹马，却因为蒙古攻西夏不得不分开，希望你能成全一对痴心恋人。\n\t但是旁边的李谅却心有不甘，跃跃欲试。"  --任务描述
x203306_g_MissionTarget="击败@npc_110041"		
x203306_g_MissionCompleted="\t(回鹘公主出现打破了僵局，她愿意替代西夏公主嫁给大汗)\n\t如此，如此甚好。。。国已不存，我现在只想和心爱的人一起远走高飞！你们，保重……"
...
--**********************************
function x203306_ProcEventEntry(sceneId, selfId, NPCId, MissionId,which)	--点击该任务后执行此脚本
...
function x203306_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x203306_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x203306_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203306_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203306_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x203306_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x203306_g_MissionName.."！", 0, 3)
				if x203306_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x203306_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x203306_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x203306_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x203306_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x203306_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x203306_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x203306_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x203306_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x203306_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x203306_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x203306_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x203306_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x203306_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
			...
			for i = 0, Num-1 do
				...
				if humanObjId >= 0 then
		            ...
		            SetQuestByIndex( sceneId, humanObjId, misIndex, 0, 1)
				        ...
				        Msg2Player( sceneId, humanObjId, format("击败李谅   %d/1", GetQuestParam(sceneId,humanObjId,misIndex,0)), 8, 3)
				        Msg2Player( sceneId, humanObjId, format("击败李谅   %d/1", GetQuestParam(sceneId,humanObjId,misIndex,0)), 8, 2)
				 		end
			    end
				end
			end
