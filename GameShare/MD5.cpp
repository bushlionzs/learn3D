...
namespace ku
{
	...
	static void *lmemcpy(void *dest, const void *source, unsigned int count)
	{
	...
	char* PrintMD5(unsigned char md5Digest[16])
	{
		...
		for (i = 0; i < 64; i++) ((unsigned int *)chBuffer)[i] = 0UL;
		//memset(chEach, 0, 10);
	...
	//	try
		{
			...
			for (register int i = 0; i < 256; i++) ((unsigned int *)chBuffer)[i] = 0UL;
	...
	// context.
	void md5::Update(unsigned char* chInput, unsigned int nInputLen)
	{
	...
	void md5::Finalize()
	{
		...
		for (register int i = 0; i < 16; i++) ((unsigned int *)m_Buffer)[i] = 0UL;
	}
	...
	// md5::Encode
	// Encodes input (unsigned int) into output (unsigned char). Assumes nLength is
	// a multiple of 4.
	void md5::Encode(unsigned char* dest, unsigned int* src, unsigned int nLength)
	{
		unsigned int i, j;
	...
	// md5::Decode
	// Decodes input (unsigned char) into output (unsigned int). Assumes nLength is
	// a multiple of 4.
	void md5::Decode(unsigned int* dest, unsigned char* src, unsigned int nLength)
	{
		unsigned int i, j;
