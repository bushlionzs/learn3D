...
unsigned	vlong::getArr(int id) 
{
	//return value->a[id]; 
	return value->get(id);
}

void vlong::setShare(unsigned int val) 
{ 
	value->share = val;
}

void	vlong::setN(unsigned val) 
{
	value->n = val; 
}

void	vlong::setZ(unsigned val)
{ 
	value->a = new unsigned[val];
	value->z = val;
}

void	vlong::setArr(int id,unsigned val) 
{
	value->a[id] = val; 
}

 #if defined _70_WORLD
char* vlong::tostring(char* s,int len)
{
	unsigned lz,t,j,l;
	char st[9];
	char *ps;
...
void  vlong::Binary2Str(char* input,char* output,int offset,int inPutLen)
{
	int len = 0;
