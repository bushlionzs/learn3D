x300820_g_ScriptId	= 300820
x300820_g_curCountryID = 0
x300820_g_CountryTransPos = {
...
function x300820_ProcEnumEvent( sceneId, selfId, targetId, MissionId )
end

--点击（按钮）点击该任务后执行此脚本
function x300820_ProcEventEntry( sceneId, selfId, targetId)		
	...
	DispatchQuestInfo( sceneId, selfId, targetId, 300820, -1 );
...
--点击（确定）
function x300820_ProcAcceptCheck( sceneId, selfId, NPCId )

	-- 只能到弱国
	-- if GetCountryRank(x300820_g_curCountryID) ~= 3 then
		-- BeginQuestEvent(sceneId)
	...
	local diffDay = x300820_GetDiffDay(lastDay,curDay)
	if diffDay < 7 then
		BeginQuestEvent(sceneId)
...
--接受
--**********************************
function x300820_ProcAccept( sceneId, selfId )

	-- 开始更改国籍流程
	-- 1.扣除金钱
	local nCost = x300820_GetBetrayCost()
	if x300820_CostMoney(sceneId, selfId, nCost ) == 0 then
		BeginQuestEvent(sceneId)
	...
	SendCountryBetrayMsg( sceneId, selfId, x300820_g_curCountryID, x300820_g_ScriptId )
...
--**********************************
function x300820_OnCountryBetrayResult( sceneId, selfId, result )
	...
	--1.通知客户端
	BeginQuestEvent(sceneId)
	local strText = "恭喜，您已成功改变国籍为"..GetCountryName(x300820_g_curCountryID)
	AddQuestText( sceneId, strText )
	EndQuestEvent( sceneId )
	DispatchQuestTips( sceneId, selfId )	
	...
	local newSceneId 	= x300820_g_CountryTransPos[x300820_g_curCountryID].sceneid
	local newPosX		= x300820_g_CountryTransPos[x300820_g_curCountryID].posX
	local newPosZ		= x300820_g_CountryTransPos[x300820_g_curCountryID].posZ
	SetPlayerNextSceneInfo(sceneId, selfId, newSceneId, newPosX, newPosZ)
	NewWorld(sceneId, selfId, newSceneId, newPosX, newPosZ, x300820_g_ScriptId)
...
--放弃
--**********************************
function x300820_ProcQuestAbandon( sceneId, selfId, MissionId )
end
--**********************************
--继续
--**********************************
function x300820_OnContinue( sceneId, selfId, targetId )
end
--**********************************
--检测是否可以提交
--**********************************
function x300820_CheckSubmit( sceneId, selfId )
end
--**********************************
--提交
--**********************************
function x300820_ProcQuestSubmit( sceneId, selfId, targetId,selectRadioId, MissionId )
end
--**********************************
--杀死怪物或玩家
--**********************************
function x300820_ProcQuestObjectKilled( sceneId, selfId, objdataId, objId, MissionId )
end
--**********************************
--进入区域事件
--**********************************
function x300820_ProcAreaEntered( sceneId, selfId, zoneId, MissionId )
end
--**********************************
--道具改变
--**********************************
function x300820_ProcQuestItemChanged( sceneId, selfId, itemdataId, MissionId )
end


-- 取得叛国需要的金卡钱数，单位文
function x300820_GetBetrayCost( )

	local nRank = GetCountryRank(x300820_g_curCountryID)
	local nCost = 0
...
--扣除玩家金卡或现金
function x300820_CostMoney( sceneId, selfId, value )
...
--计算时间差
function x300820_GetDiffDay( lastDay, curDay )
	local diffDay = 0;
