...
x300859_g_MissionId 				= 	7575                     --任务ID
x300859_g_ScriptId 					= 	300859                  --脚本ID
x300859_g_MissionKind 				= 	1                       --任务类型
x300859_g_LevelLess					= 	30                      --场景开放等级  <=0 表示不限制


x300859_g_MissionName				= 	"【国家】国家守边"
x300859_g_MissionTarget				= 	"我国正面临敌国严重的骚扰和威胁，需要大家共同出力，守卫边关。你愿意接受此任务，守卫边关一段时间吗？"
x300859_g_MissionInfo				= 	"您接受任务后，经过一段时间即可向我汇报军情，并完成任务得到奖励。当您的#G计时器#W变成#R红色#W时就可准备找我#G回复#W了，如#G错过#W时间，任务就会#G失败#W，请你注意时间。"
x300859_g_ContinueInfo				= 	"你还在等什么？快去守边"
x300859_g_MissionCompleted			= 	"守边完成"
x300859_g_MissionHelp				=	"  #W任务期间死亡，离线或超时，任务自动置成失败状态，找明安重新领取。"          --任务提示信息

--目标
x300859_g_ExtTarget					=	{ {type=20,n=1,target="守边"}}

--奖励

x300859_g_ExpBonus					= 	0;--x300859_AddExpAward               	--奖励：经验
x300859_g_BonusItem					=	{}	--奖励：物品

x300859_g_BonusMoney1               = 	0
x300859_g_BonusMoney2               =   0
x300859_g_BonusMoney3               =   0

x300859_g_BonusChoiceItem           =   {}


--MisDescEnd

x300859_g_Mission_Id				=	7575

x300859_g_DayCountLimited			=	3
x300859_g_BuffId					=	7764		


function x300859_GetMissionId( sceneId, selfId )
	return x300859_g_Mission_Id
end

function x300859_ProcEnumEvent( sceneId, selfId, targetId, MissionId )
	
	local level = GetLevel( sceneId,selfId )
    if level < x300859_g_LevelLess then
       return
    end

	local myMissionId = x300859_GetMissionId( sceneId, selfId )
	
	
	
	if IsHaveQuestNM( sceneId, selfId, myMissionId ) == 0 then
		
		AddQuestNumText(sceneId, myMissionId, x300859_g_MissionName,8,1);
	else
		local state = GetQuestStateNM(sceneId, selfId, targetId, myMissionId);

		
		AddQuestNumText(sceneId, myMissionId, x300859_g_MissionName,state,state);
...
--向客户端发送任务未完成
---------------------------------------------------------------------------------------------------
function x300859_ProcEventEntry( sceneId, selfId, targetId, MissionId, nExt )
	local myMissionId = x300859_GetMissionId( sceneId, selfId )	
	if IsHaveQuestNM( sceneId, selfId, myMissionId ) == 0 then
		x300859_DispatchMissionInfo( sceneId, selfId, targetId, myMissionId, 0 )
	else
		if nExt==7 then -- 任务完成
			x300859_DispatchCompletedInfo( sceneId, selfId, targetId,myMissionId )
		
		else
			
			x300859_DispatchContinueInfo( sceneId, selfId, targetId,myMissionId )
		end
	end
end


function x300859_ProcAcceptCheck(sceneId, selfId, targetId, MissionId )

	return 1;

end

function x300859_CheckRequest(sceneId, selfId)
	local myMissionId = x300859_GetMissionId( sceneId, selfId )

	if GetLevel(sceneId, selfId) <x300859_g_LevelLess then
		x300859_ShowTips(sceneId, selfId, "你等级不足")
		return 0
	end

	if IsHaveQuestNM( sceneId, selfId, myMissionId ) > 0 then
		x300859_ShowTips(sceneId, selfId, "你已经有这个任务，接受任务失败")
		return 0;
	end

	if x300859_g_DayCountLimited >0 then
		local nDayCount = x300859_GetDayCount(sceneId, selfId)
		if nDayCount>= x300859_g_DayCountLimited then
			x300859_ShowTips(sceneId, selfId, "很抱歉，您已经接取过了【国家】国家守边任务3次，请明日再来接！")
			return 0;
		end
	end
...
function x300859_ProcQuestAccept(sceneId, selfId, targetId, MissionId)
	
	
	if x300859_CheckRequest(sceneId, selfId)<=0 then
	...
	if bFullMission>0 then
		x300859_ShowTips(sceneId, selfId, "任务已满，无法接受任务")
		return 0
	end
	---------------------------- add end -------------------------------
	...
	if ret > 0 then
		...
		x300859_ShowTips(sceneId, selfId, str)
		Msg2Player(sceneId,selfId,str,8,2)

		local misIndex = GetQuestIndexByID( sceneId, selfId, myMissionId )
		SetQuestByIndex( sceneId, selfId, misIndex, 5, 0 ) --Tick=0

		--CancelSpecificImpact( sceneId, selfId, x300859_g_BuffId)
		SendSpecificImpactToUnit(sceneId, selfId,selfId,selfId, x300859_g_BuffId,0 )

		
		return 1
	else
		
		x300859_ShowTips(sceneId, selfId, "接受任务错误")
...
function x300859_ShowTips(sceneId, selfId, str)
...
function x300859_ProcQuestAbandon( sceneId, selfId, MissionId )
	...
	if ret>0 then
		...
		CancelSpecificImpact(sceneId, selfId, x300859_g_BuffId);
		x300859_SetDayCount(sceneId, selfId)
	else
		x300859_ShowTips(sceneId, selfId, "放弃任务失败")
	end
...
function x300859_ProcQuestSubmit( sceneId, selfId, targetId, selectId, MissionId )

	local myMissionId = x300859_GetMissionId( sceneId, selfId )
	
	if x300859_CheckSubmit(sceneId, selfId )>0 then
		...
		if ret>0 then
			...
			x300859_GiveReward(sceneId,selfId)

			
			x300859_SetDayCount(sceneId, selfId)
			x300859_SetSuccessCount(sceneId, selfId)

			CancelSpecificImpact( sceneId, selfId, x300859_g_BuffId)
			
		else
			x300859_ShowTips(sceneId, selfId, "交任务失败")
		end
		
	end
end

function x300859_GiveReward(sceneId,selfId)
	--奖励经验
	local level = GetLevel(sceneId, selfId)
	...
	elseif nDayCount == 2 then
		...
		--write log
		local msg = "guojiashoubian: name="..GetName(sceneId, selfId)..",guid="..GetPlayerGUID( sceneId,selfId )..",nSuccssCount="..x300859_GetSuccessCount(sceneId, selfId)

		WriteLog(1,msg)
		--write end


		if x300859_GetSuccessCount(sceneId, selfId)>=2 then
...
function x300859_CheckSubmit(sceneId, selfId )
	local myMissionId = x300859_GetMissionId( sceneId, selfId )
	if IsHaveQuestNM( sceneId, selfId, myMissionId ) <= 0 then	-- 如果有这个任务直接退出
		return 0
	end
...
--取得当前次数
---------------------------------------------------------------------------------------------------
function x300859_GetDayCount(sceneId, selfId)
...
--设置当天次数
---------------------------------------------------------------------------------------------------
function x300859_SetDayCount(sceneId, selfId)
...
--取得当前成功次数
---------------------------------------------------------------------------------------------------
function x300859_GetSuccessCount(sceneId, selfId)
...
--设置当天成功次数
---------------------------------------------------------------------------------------------------
function x300859_SetSuccessCount(sceneId, selfId)
...
--向客户端发送任务信息
---------------------------------------------------------------------------------------------------
function x300859_DispatchMissionInfo( sceneId, selfId, NPCId, MissionId, nExt )
	...
	AddQuestText( sceneId,"  您接受任务后，守卫边塞，在@npc_131511附近守卫，每隔一段时间，会自动获得经验奖励。经过一段时间即可向我汇报军情，并完成任务得到奖励，当达到80级时可获得天赋值奖励。")
	AddQuestText( sceneId," ")
	...
	DispatchQuestInfoNM(sceneId, selfId, NPCId, x300859_g_ScriptId, MissionId,0);
end
...
--向客户端发送任务未完成
---------------------------------------------------------------------------------------------------
function x300859_DispatchContinueInfo( sceneId, selfId, NPCId,MissionId )
	...
	BeginQuestEvent(sceneId)
		...
		AddQuestText(sceneId,"  在@npc_131511附近守卫，每隔一段时间，会自动获得经验奖励。#r")
		if nDayCount == 0 then
			local localexp1 = (GetLevel( sceneId, selfId ))* 15000 *0.15*0.3
			local localexp2 = (GetLevel( sceneId, selfId ))* 15000 *0.15*0.3
			local localexp = 0
	...
	EndQuestEvent()
	DispatchQuestDemandInfo(sceneId, selfId, NPCId, x300859_g_ScriptId, MissionId,0);
...
--向客户端发送任务完成信息
---------------------------------------------------------------------------------------------------
function x300859_DispatchCompletedInfo( sceneId, selfId, NPCId,MissionId )
	...
	EndQuestEvent()
	DispatchQuestContinueInfoNM(sceneId, selfId, NPCId, x300859_g_ScriptId, MissionId);
	CallScriptFunction( 256290, "FinishShoubian", sceneId, selfId)
end
...
--任务日志刷新
---------------------------------------------------------------------------------------------------
function x300859_QuestLogRefresh( sceneId, selfId, MissionId)
	local ret = x300859_CheckSubmit(sceneId, selfId )
	local str=""
	if ret==1 then
		str = "守边(1/1)"
	else
	...
	AddQuestLogCustomText( sceneId,
							...
							"@npc_131510",			--任务NPC
							"",               --任务攻略
							"  您接受任务后，守卫边塞，在@npc_131511附近守卫，每隔一段时间，会自动获得经验奖励。经过一段时间即可向@npc_131510汇报军情，并完成任务得到奖励，当达到80级时可获得天赋值奖励。",	--任务描述
...
function x300859_ProcQuestLogRefresh( sceneId, selfId, MissionId)
	
	x300859_QuestLogRefresh( sceneId, selfId, MissionId );
end

--// system function start
function x300859_ProcQuestAttach( sceneId, selfId, npcId, npcGuid,misIndex, MissionId )
	
	
end

function x300859_ProcQuestObjectKilled( sceneId, selfId, objdataId, objId, MissionId )
end

function x300859_ProcessQuestFail(sceneId, selfId, MissionId)
end

function x300859_OnPlayerRelive(sceneId, selfId, nMode)
	...
	if bCompleted<1  then   --如果任务没有完成
		...
		if IsHaveSpecificImpact(sceneId, selfId, x300859_g_BuffId)<=0 then --如果没有buff,设置任务已经完成
		
			SetQuestByIndex( sceneId, selfId, misIndex, 0, 1 )  
			SetQuestByIndex( sceneId, selfId, misIndex, 7, 1 )  
			x300859_QuestLogRefresh( sceneId, selfId, myMissionId)
		...
		--if IsHaveSpecificImpact(sceneId, selfId, x300859_g_BuffId)>0 then --如果还有这个buff, 说明有异常
		--
...
function x300859_OnPlayerLogin(sceneId, selfId)
	x300859_OnPlayerRelive(sceneId, selfId, 1)
end
