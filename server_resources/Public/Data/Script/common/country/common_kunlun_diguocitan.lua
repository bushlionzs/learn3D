...
x300884_g_ScriptId 					= 	300884                  --脚本ID


x300884_g_MissionKind 				= 	1                       --任务类型
x300884_g_LevelLess					= 	40                      --场景开放等级  <=0 表示不限制





x300884_g_MissionName				= 	"敌国除奸"
x300884_g_MissionTarget				= 	""
x300884_g_MissionInfo				= 	""
x300884_g_ContinueInfo				= 	""
x300884_g_MissionCompleted			= 	""
x300884_g_MissionHelp				=	""          --任务提示信息

--目标
x300884_g_ExtTarget					=	{ {type=20,n=1,target="杀死张五常"}}

--奖励

x300884_g_ExpBonus					= 	0;--x300884_AddExpAward               	--奖励：经验
x300884_g_BonusItem					=	{}	--奖励：物品

x300884_g_BonusMoney1               = 	0
x300884_g_BonusMoney2               =   0
x300884_g_BonusMoney3               =   0

x300884_g_BonusChoiceItem           =   {}


--MisDescEnd

x300884_g_CountryDunhuang = 351 --四个国家的场景id
x300884_g_CountryLoulan = 51
x300884_g_CountryKunlun = 251
x300884_g_CountryLaiyin = 151

x300884_g_MissionId1 = 7672		-- 敦煌
x300884_g_MissionId2 = 7663		-- 楼兰
x300884_g_MissionId3 = 7669		-- 昆仑
x300884_g_MissionId4 = 7666		-- 天山

x300884_g_DayCountLimited			= 1


x300884_g_MissionBonusInfo			= 	"#Y获得经验#R%d#Y的奖励。"

function x300884_ProcEnumEvent( sceneId, selfId, targetId, MissionId )
	
end

--选中任务
function x300884_ProcEventEntry( sceneId, selfId, targetId, MissionId, index )
	...
	if IsHaveQuestNM( sceneId, selfId, MissionId ) == 0 then	-- 如果没有这个任务
		...
		if ret > 0 then	-- 如果可以接这个任务
			...
			x300884_DispatchMissionInfo( sceneId, selfId, targetId, MissionId )
		...
		if x300884_IsSuccessed( sceneId, selfId, MissionId )>0 then
			--QuestComplateInfoNM( sceneId, selfId, targetId, MissionId ) -- 显示完成信息
			x300884_DispatchCompletedInfo( sceneId, selfId, targetId,MissionId )
		else
			--QuestNotComplateInfoNM( sceneId, selfId, targetId, MissionId ) -- 显示任务未完成信息
			x300884_DispatchContinueInfo( sceneId, selfId, targetId,MissionId )
		end
	end
end
...
--接受任务
function x300884_ProcQuestAccept( sceneId, selfId, targetId, MissionId )
	if IsHaveQuestNM( sceneId, selfId, MissionId ) > 0 then	-- 如果有这个任务直接退出
		return 0
	end

	if	IsHaveQuestNM( sceneId, selfId, x300884_g_MissionId1 ) > 0 or IsHaveQuestNM( sceneId, selfId, x300884_g_MissionId2 ) > 0 or IsHaveQuestNM( sceneId, selfId, x300884_g_MissionId3 ) > 0 or IsHaveQuestNM( sceneId, selfId, x300884_g_MissionId4 ) > 0 then
		BeginQuestEvent(sceneId)
	...
	if MissionId == x300884_g_MissionId1 then
		nDayCount = x300884_GetDayCount(sceneId, selfId, DQ_CITAN_DUNHUANG_DATE,DQ_CITAN_DUNHUANG_DAYCOUNT)
	elseif MissionId == x300884_g_MissionId2 then
		nDayCount = x300884_GetDayCount(sceneId, selfId, DQ_CITAN_LOULAN_DATE,DQ_CITAN_LOULAN_DAYCOUNT)
	elseif MissionId == x300884_g_MissionId3 then
		nDayCount = x300884_GetDayCount(sceneId, selfId, DQ_CITAN_KUNLUN_DATE,DQ_CITAN_KUNLUN_DAYCOUNT)
	elseif MissionId == x300884_g_MissionId4 then
		nDayCount = x300884_GetDayCount(sceneId, selfId, DQ_CITAN_LAIYIN_DATE,DQ_CITAN_LAIYIN_DAYCOUNT)
	else
		return -1;
	end

	if nDayCount>=x300884_g_DayCountLimited then
		BeginQuestEvent(sceneId);
...
--完成任务
--返回1代表成功，0代表交任务失败
function x300884_ProcQuestSubmit( sceneId, selfId, targetId, selectId, MissionId )
	...
	local misIndex = GetQuestIndexByID( sceneId, selfId, MissionId )
	local ret = x300884_IsSuccessed( sceneId, selfId, MissionId ); --QuestCheckSubmitNM( sceneId, selfId, targetId, MissionId, misIndex ) -- 获取完成任务检查信息
	if ret > 0 then -- 如果可以完成
		
		return x300884_MissionComplate( sceneId, selfId, targetId, selectId, MissionId )
	end
	return 0
end

--任务完成
function x300884_MissionComplate( sceneId, selfId, targetId, selectId, MissionId )
	...
	if ret == 1 then -- 如果成功完成
		--提示玩家任务完成
		if MissionId == x300884_g_MissionId1 then
			x300884_SetDayCount(sceneId, selfId, DQ_CITAN_DUNHUANG_DATE,DQ_CITAN_DUNHUANG_DAYCOUNT)
		elseif MissionId == x300884_g_MissionId2 then
			x300884_SetDayCount(sceneId, selfId, DQ_CITAN_LOULAN_DATE,DQ_CITAN_LOULAN_DAYCOUNT)
		elseif MissionId == x300884_g_MissionId3 then
			x300884_SetDayCount(sceneId, selfId, DQ_CITAN_KUNLUN_DATE,DQ_CITAN_KUNLUN_DAYCOUNT)
		elseif MissionId == x300884_g_MissionId4 then
			x300884_SetDayCount(sceneId, selfId, DQ_CITAN_LAIYIN_DATE,DQ_CITAN_LAIYIN_DAYCOUNT)
		else
			return -1;
		end
		...
		x300884_GiveReward(sceneId,selfId)
		...
		if CountryIsFortuneTime(sceneId, selfId, 1) > 0 then
			...
			--记日志
			local strMsg = format("CGL:<Info>x300884_MissionComplate sceneId=%d,country=%d", sceneId, country )
    		WriteLog( 1, strMsg )
		end
...
function x300884_GiveReward(sceneId,selfId)
    ...
    if nExpBonus > 0 then
		...
		DispatchQuestTips(sceneId, selfId);
  		...
  		CallScriptFunction( 256270, "Finishchujian", sceneId, selfId)
...
function x300884_ProcQuestAttach( sceneId, selfId, npcId, npcGuid,misIndex, MissionId )
	if IsHaveQuestNM( sceneId, selfId, MissionId) == 0 then	-- 如果没有这个任务
		return
	end
...
--放弃
--********************************************************************
function x300884_ProcQuestAbandon( sceneId, selfId,MissionId )
	if IsHaveQuestNM( sceneId, selfId, MissionId ) <= 0 then
		return -1;
	end
...
--杀死怪物
function x300884_ProcQuestObjectKilled( sceneId, selfId, objdataId, objId, MissionId )
	
	
	if x300884_CountryCheck( sceneId, selfId, MissionId ) < 0 then
		return -1;
	end
			...
			if IsPlayerStateNormal(sceneId, humanObjId) > 0 then
				if GetHp(sceneId, humanObjId) > 0 then
					if IsHaveQuestNM( sceneId, humanObjId, MissionId ) > 0 then	--如果这个人拥有任务
						x300884_OnKillTheNpc( sceneId, humanObjId, MissionId )
					end
				end
			end
		end
...
function x300884_IsSuccessed( sceneId, selfId, MissionId )
	if IsHaveQuestNM( sceneId, selfId, MissionId ) > 0 then
...
function x300884_OnKillTheNpc(sceneId, selfId, MissionId)
	if IsHaveQuestNM( sceneId, selfId, MissionId ) > 0 then
	--已经完成任务
		if x300884_IsSuccessed( sceneId, selfId, MissionId ) >0 then
			return 0;
		...
		x300884_QuestLogRefresh( sceneId, selfId, MissionId)
	end	
...
--物品改变
function x300884_ProcQuestItemChanged( sceneId, selfId, itemdataId, MissionId )
	return 0
end

function x300884_CountryCheck( sceneId, selfId, MissionId )
	if MissionId == x300884_g_MissionId1 then
		if sceneId == x300884_g_CountryDunhuang then
			return 1
		end
	elseif MissionId == x300884_g_MissionId2 then
		if sceneId == x300884_g_CountryLoulan then
			return 1
		end
	elseif MissionId == x300884_g_MissionId3 then
		if sceneId == x300884_g_CountryKunlun then
			return 1
		end
	elseif MissionId == x300884_g_MissionId4 then
		if sceneId == x300884_g_CountryLaiyin then
			return 1
		end
	end
...
function x300884_OnLogInOrDie( sceneId, selfId )
	
   x300884_OnMissionFailed( sceneId, selfId )
end



function x300884_OnMissionFailed( sceneId, selfId )
	
	x300884_DiGuoCiTanOnDie(sceneId, selfId)


end


function x300884_DiGuoCiTanOnDie(sceneId, selfId)
	
	if 0==0 then
		return
	end

	local MissionId = x300884_g_MissionId1
	...
	MissionId = x300884_g_MissionId2
	if IsHaveQuestNM( sceneId, selfId,MissionId  ) > 0 then	-- 如果有这个任务
		local misIndex = GetQuestIndexByID( sceneId, selfId, MissionId )
		SetQuestByIndex( sceneId, selfId, misIndex, 0, 0 )
		SetQuestByIndex( sceneId, selfId, misIndex, 7, 2 )
	...
	MissionId = x300884_g_MissionId3
	if IsHaveQuestNM( sceneId, selfId,MissionId  ) > 0 then	-- 如果有这个任务
		local misIndex = GetQuestIndexByID( sceneId, selfId, MissionId )
	...
	MissionId = x300884_g_MissionId4
	if IsHaveQuestNM( sceneId, selfId,MissionId  ) > 0 then	-- 如果有这个任务
		local misIndex = GetQuestIndexByID( sceneId, selfId, MissionId )
		SetQuestByIndex( sceneId, selfId, misIndex, 0, 0 )
		SetQuestByIndex( sceneId, selfId, misIndex, 7, 2 )
...
--取得当前次数
---------------------------------------------------------------------------------------------------
function x300884_GetDayCount(sceneId, selfId, nDayIndex, nDayCountIndex)
...
--设置当天次数
---------------------------------------------------------------------------------------------------
function x300884_SetDayCount(sceneId, selfId, nDayIndex, nDayCountIndex)
...
--检查任务接受结果
---------------------------------------------------------------------------------------------------
function x300884_ProcAcceptCheck(sceneId, selfId, NPCId)
	return 1
end
...
--向客户端发送任务信息
---------------------------------------------------------------------------------------------------
function x300884_DispatchMissionInfo( sceneId, selfId, NPCId, MissionId)
	...
	elseif GetLevel(sceneId, selfId) >=70 then
		...
		if MissionId == x300884_g_MissionId1 then
			...
			AddQuestText( sceneId,"  在本国边塞找到@npc_131511，传送到敦煌边塞，杀死敦煌边塞的大掌柜张五常。")
			AddQuestText( sceneId," ")
		...
		elseif MissionId == x300884_g_MissionId2 then
			...
			--任务目标
			AddQuestText( sceneId,"#Y任务目标：")
			AddQuestText( sceneId,"  在本国边塞找到@npc_131511，传送到楼兰边塞，杀死楼兰边塞的大掌柜张五常。")
			AddQuestText( sceneId," ")
		...
		elseif MissionId == x300884_g_MissionId3 then
			...
			--任务目标
			AddQuestText( sceneId,"#Y任务目标：")
			AddQuestText( sceneId,"  在本国边塞找到@npc_131511，传送到昆仑边塞，杀死昆仑边塞的大掌柜张五常。")
			AddQuestText( sceneId," ")
		...
		elseif MissionId == x300884_g_MissionId4 then
			...
			--任务目标
			AddQuestText( sceneId,"#Y任务目标：")
			AddQuestText( sceneId,"  在本国边塞找到@npc_131511，传送到天山边塞，杀死天山边塞的大掌柜张五常。")
			AddQuestText( sceneId," ")
	...
	EndQuestEvent()
	DispatchQuestInfoNM(sceneId, selfId, NPCId, x300884_g_ScriptId, MissionId,0);
end

---------------------------------------------------------------------------------------------------
--向客户端发送任务未完成
---------------------------------------------------------------------------------------------------
function x300884_DispatchContinueInfo( sceneId, selfId, NPCId,MissionId )
	...
	EndQuestEvent()
	DispatchQuestDemandInfo(sceneId, selfId, NPCId, x300884_g_ScriptId, MissionId,0);
...
--向客户端发送任务完成信息
---------------------------------------------------------------------------------------------------
function x300884_DispatchCompletedInfo( sceneId, selfId, NPCId,MissionId )
	...
	EndQuestEvent()
	DispatchQuestContinueInfoNM(sceneId, selfId, NPCId, x300884_g_ScriptId, MissionId);
...
--任务日志刷新
---------------------------------------------------------------------------------------------------
function x300884_QuestLogRefresh( sceneId, selfId, MissionId)

	local nRet = x300884_IsSuccessed( sceneId, selfId, MissionId )
	local str = "";

	if MissionId == x300884_g_MissionId1 then
		if nRet>0 then
			str = "  从@npc_30011到敦煌边塞杀死@npc_30019（1/1）"
		else
			str = "  从@npc_30011到敦煌边塞杀死@npc_30019（0/1）"
		end
	elseif MissionId == x300884_g_MissionId2 then
		if nRet>0 then
			str = "  从@npc_30008到楼兰边塞杀死@npc_30016（1/1）"
		else
			str = "  从@npc_30008到楼兰边塞杀死@npc_30016（0/1）"
		end
	elseif MissionId == x300884_g_MissionId3 then
		if nRet>0 then
			str = "  从@npc_30010到昆仑边塞杀死@npc_30018（1/1）"
		else
			str = "  从@npc_30010到昆仑边塞杀死@npc_30018（0/1）"
		end
	elseif MissionId == x300884_g_MissionId4 then
		if nRet>0 then
			str = "  从@npc_30009到天山边塞杀死@npc_30017（1/1）"
		else
			str = "  从@npc_30009到天山边塞杀死@npc_30017（0/1）"
		end
	else
		return -1;
	end
...
function x300884_ProcQuestLogRefresh( sceneId, selfId, MissionId)
