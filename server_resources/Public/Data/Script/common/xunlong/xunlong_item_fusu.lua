x310052_g_scriptId = 310052 


x310052_g_DestSceneName1 = "秦岭"
x310052_g_DestSceneName2 = "红沙漠"
x310052_g_DestSceneName3 = "波斯高原"
x310052_g_DestSceneName4 = "波兰王国"
x310052_g_DestSceneName5 = "拜占庭帝国"
x310052_g_DestSceneName6 = "西奈半岛"
x310052_g_DestSceneName7 = "九州岛"
x310052_g_DestSceneName8 = "印度河"
x310052_g_DestSceneName9 = "苏门答腊"
x310052_g_DestSceneId = {7,10,16,19,22,25,29,32,35}


function x310052_ProcEventEntry( sceneId, selfId, BagIndex )
	...
	local ret = CreateMonster( sceneId, 3059, PlayerPosX, PlayerPosZ, 0, -1, x310052_g_scriptId, 160038, 21)
	...
	--	x310052_UpdateTopList(sceneId, selfId)
	end
	...
	--	x310052_UpdateTopList(sceneId, selfId)
		...
		local strText = format("这里毫无灵气，无法召唤出天龙之魂！" )
		BeginQuestEvent(sceneId)
		AddQuestText(sceneId, strText)
		EndQuestEvent(sceneId)
		DispatchQuestTips(sceneId, selfId)
...
function x310052_IsSkillLikeScript( sceneId, selfId)
	return 0; --这个脚本需要动作支持    
	--返回0，走上面的ProcEventEntry逻辑
	--返回1，走下面的逻辑
end

function x310052_CancelImpacts( sceneId, selfId )
	return 0; --不需要这个接口，但要保留空函数,并且始终返回0。
end

function x310052_OnConditionCheck( sceneId, selfId )
	return 0
end

function x310052_OnDeplete( sceneId, selfId )

	return 0
end

function x310052_OnActivateOnce( sceneId, selfId )
end

function x310052_OnActivateEachTick( sceneId, selfId)
	return 1; --不是引导性脚本, 只保留空函数.
end

function x310052_OnDie(sceneId, selfId, killerId)--怪死亡添加日志
	
	x310052_UpdateTopList(sceneId, killerId)
	GamePlayScriptLog(sceneId, killerId, 551)
				
end

function x310052_UpdateTopList(sceneId,selfId)
