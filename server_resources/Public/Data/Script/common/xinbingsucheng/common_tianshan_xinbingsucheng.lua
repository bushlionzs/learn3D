...
x310221_g_LevelMax					= 40

x310221_g_GameId					= 1023
x310221_g_NeedMoney					= 2000

--MisDescBegin

x310221_g_ScriptId 					= 	310221                  --脚本ID

x310221_g_LevelLess					= 	25                      --场景开放等级  <=0 表示不限制


x310221_g_MissionName				= 	"新兵速成"

x310221_g_MaxKillNum				= 20

x310221_g_MaxCount					= 5


--MisDescEnd


function x310221_ProcEnumEvent( sceneId, selfId, targetId, MissionId )
...
--选中任务
function x310221_ProcEventEntry( sceneId, selfId, targetId, MissionId )

	if GetGameOpenById(x310221_g_GameId) ~= 1 then
	...
	if IsHaveQuestNM( sceneId, selfId, MissionId ) == 0 then	-- 如果没有这个任务
		...
		x310221_DispatchMissionInfo( sceneId, selfId, targetId, MissionId )
		...
		if killcount == x310221_g_MaxKillNum then
			x310221_DispatchCompletedInfo( sceneId, selfId, targetId,MissionId )
			--QuestComplateInfoNM( sceneId, selfId, targetId, MissionId ) -- 显示完成信息
		else
			x310221_DispatchContinueInfo( sceneId, selfId, targetId,MissionId )
...
--接受任务
function x310221_ProcQuestAccept( sceneId, selfId, targetId, MissionId )
	...
	if GetLevel(sceneId, selfId) < x310221_g_LevelLess then
		BeginQuestEvent(sceneId);
	...
	local daycount = x310221_GetDayCount(sceneId, selfId)
	if daycount >= x310221_g_MaxCount then
		BeginQuestEvent(sceneId);
	...
	--local daycount = x310221_GetDayCount(sceneId, selfId)	
	if daycount > 0 then
		if IsEnoughGold(sceneId, selfId,x310221_g_NeedMoney) < 1 then
			BeginQuestEvent(sceneId);
	...
	if ret > 0 then -- 如果可以接这个任务
		...
		if ret > 0 then
			x310221_ProcQuestLogRefresh( sceneId, selfId, MissionId)
			if daycount > 0 then
				...
				if goldMode == 1 then
					...
					if cash >= x310221_g_NeedMoney then
						CostMoney(sceneId, selfId, 2, x310221_g_NeedMoney,320)
					end
				else
					local cash = GetMoney(sceneId, selfId,2)
					local bind = GetMoney(sceneId, selfId,3)
					if bind >= x310221_g_NeedMoney then
						CostMoney(sceneId, selfId, 3, x310221_g_NeedMoney)
					elseif cash >= x310221_g_NeedMoney then
						CostMoney(sceneId, selfId, 2, x310221_g_NeedMoney,320)
					end
				end
			end
...
--任务完成
function x310221_ProcQuestSubmit( sceneId, selfId, targetId, selectId, MissionId )
	...
	if ret == 1 then -- 如果成功完成
		x310221_SetDayCount(sceneId, selfId)
		...
		x310221_GiveReward(sceneId,selfId)
		
		x310221_SetFirst(sceneId, selfId)
...
function x310221_GiveReward(sceneId,selfId)

	local nExpBonus = x310221_GetExp( sceneId, selfId )
...
function x310221_ProcQuestAttach( sceneId, selfId, npcId, npcGuid,misIndex, MissionId )
	if IsHaveQuestNM( sceneId, selfId, MissionId) == 0 then	-- 如果没有这个任务
		return
	end
...
--放弃
--********************************************************************
function x310221_ProcQuestAbandon( sceneId, selfId,MissionId )
	if IsHaveQuestNM( sceneId, selfId, MissionId ) <= 0 then
		return -1;
	end
...
--杀死怪物
function x310221_ProcQuestObjectKilled( sceneId, selfId, objdataId, objId, MissionId )
		...
		for i = 0, HumenNum-1 do
				...
				if humanObjId ~= -1 then
				    ...
				    if IsHaveQuestNM( sceneId, humanObjId, MissionId ) > 0 then	--如果这个人拥有任务
					    ...
					    if KilledNum == x310221_g_MaxKillNum-1 then
						    SetQuestByIndex( sceneId, humanObjId, misIndex, 0, KilledNum+1 )
											SetQuestByIndex( sceneId, humanObjId, misIndex, 7, 1 )
											BeginQuestEvent(sceneId)
											if ObjName == "" then
												strText = format( "#cffcf00【个人】新兵速成：您击毙了怪物(%d/%d)", KilledNum+1, x310221_g_MaxKillNum )
											else
												strText = format( "#cffcf00【个人】新兵速成：您击毙了%s(%d/%d)", ObjName, KilledNum+1, x310221_g_MaxKillNum )
											end
											AddQuestText( sceneId, strText )
											EndQuestEvent( sceneId )
											DispatchQuestTips( sceneId, humanObjId )
											x310221_ProcQuestLogRefresh( sceneId, humanObjId, MissionId)
									elseif KilledNum < x310221_g_MaxKillNum-1 then
											SetQuestByIndex( sceneId, humanObjId, misIndex, 0, KilledNum+1 )
											BeginQuestEvent(sceneId)
											if ObjName == "" then
												strText = format( "#cffcf00【个人】新兵速成：您击毙了怪物(%d/%d)", KilledNum+1, x310221_g_MaxKillNum )
...
--物品改变
function x310221_ProcQuestItemChanged( sceneId, selfId, itemdataId, MissionId )
	return 0
end
---------------------------------------------------------------------------------------------------
--检查任务接受结果
---------------------------------------------------------------------------------------------------
function x310221_ProcAcceptCheck(sceneId, selfId, NPCId)
	return 1
end

function x310221_GetCost(sceneId, selfId)
	local daycount =x310221_GetDayCount(sceneId,selfId)
	if daycount > 0 then
		return 2000
	end
...
--向客户端发送任务信息
---------------------------------------------------------------------------------------------------
function x310221_DispatchMissionInfo( sceneId, selfId, NPCId, MissionId)
	local awardexp = x310221_GetExp( sceneId, selfId )
	...
	AddQuestText( sceneId,"   消灭等级不比你低5级以上的20只怪物，你就可以完成新兵的训练。")
	 AddQuestText( sceneId, "\n#Y任务花费：" )
        local cost = x310221_GetCost(sceneId, selfId)
	...
	EndQuestEvent()
	DispatchQuestInfoNM(sceneId, selfId, NPCId, x310221_g_ScriptId, MissionId,0);
end

---------------------------------------------------------------------------------------------------
--向客户端发送任务未完成
---------------------------------------------------------------------------------------------------
function x310221_DispatchContinueInfo( sceneId, selfId, NPCId,MissionId )
	...
	EndQuestEvent()
	DispatchQuestDemandInfo(sceneId, selfId, NPCId, x310221_g_ScriptId, MissionId,0);
end

---------------------------------------------------------------------------------------------------
--向客户端发送任务完成信息
---------------------------------------------------------------------------------------------------
function x310221_DispatchCompletedInfo( sceneId, selfId, NPCId,MissionId )
	local awardexp = x310221_GetExp( sceneId, selfId )
	...
	EndQuestEvent()
	DispatchQuestContinueInfoNM(sceneId, selfId, NPCId, x310221_g_ScriptId, MissionId);

end

function x310221_ProcQuestLogRefresh( sceneId, selfId, MissionId)
	...
	local str = "  消灭20只等级适当的怪物("..killcount.."/"..tostring(x310221_g_MaxKillNum)..")";	
	BeginQuestEvent(sceneId)	
		local awardexp = x310221_GetExp( sceneId, selfId )
...
--取得此任务当天当前已完成次数
---------------------------------------------------------------------------------------------------
function x310221_GetDayCount(sceneId, selfId)
	if x310221_g_MaxCount > 0 then
...
--更新当天接任务次数
---------------------------------------------------------------------------------------------------
function x310221_SetDayCount(sceneId, selfId)
	local today = GetDayOfYear()
	local lastday = GetQuestData(sceneId, selfId, MD_XINBINGSUCHENG_DATE[1], MD_XINBINGSUCHENG_DATE[2], MD_XINBINGSUCHENG_DATE[3])
	if lastday ~= today then
