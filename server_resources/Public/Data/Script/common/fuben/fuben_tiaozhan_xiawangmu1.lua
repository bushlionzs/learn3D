...
x701104_g_ScriptId 					= 	701104                  --脚本ID

x701104_g_MissionIdList					=   {5203, 5224,5245,5266}

x701104_g_LevelLess					= 	65                      --场景开放等级  <=0 表示不限制


x701104_g_MissionName				= 	"【个人】挑战夏王墓（一）"
...
x701104_g_MissionHelp				=	"help1"          --任务提示信息
...
function x701104_ProcQuestAccept( sceneId, selfId, targetId, MissionId )
	...
	if x701104_GetMyHaveMissionId( sceneId, selfId)>0 then
		x701104_ShowTips(sceneId, selfId, "你有其他挑战副本任务，不能领取")
		return 0
	end
	...
	if ret > 0 then
		...
		BeginQuestEvent(sceneId);
		AddQuestText(sceneId, "您接受了任务:"..x701104_g_MissionName);
		EndQuestEvent();
		DispatchQuestTips(sceneId, selfId);
		Msg2Player(sceneId,selfId,"您接受了任务:"..x701104_g_MissionName,8,2)
...
function x701104_ProcQuestSubmit( sceneId, selfId, targetId, selectId, MissionId )
	...
	if x701104_CheckSubmit( sceneId, selfId, MissionId )>0 then
		...
		if ret>0 then
			...
			local str = "您完成了任务:"..x701104_g_MissionName;
			...
			x701104_ShowTips(sceneId, selfId, "交任务失败")
		end
...
function x701104_ProcQuestAbandon( sceneId, selfId,MissionId )
	...
	if x701104_GetMyHaveMissionId( sceneId, selfId ) <= 0 then	-- 如果有这个任务直接退出
		return 0
	end
	...
	BeginQuestEvent(sceneId);
		AddQuestText(sceneId, "您放弃了任务:"..x701104_g_MissionName);
	EndQuestEvent();
	DispatchQuestTips(sceneId, selfId);
	Msg2Player(sceneId,selfId,"您放弃了任务:"..x701104_g_MissionName,8,2)
...
function x701104_ProcQuestLogRefresh( sceneId, selfId, MissionId)
	...
	for i=0, x701104_g_KillCount-1 do
		...
		local strTemp = "击杀@npc_123744中的#G"..x701104_g_KillType[i+1].name;
	...
	AddQuestLogCustomText( sceneId,
							"",						-- 标题
							x701104_g_MissionName,        -- 任务名字
							str,		--任务目标
							"@npc_"..x701104_g_ReplyNpc,			--任务NPC
							x701104_g_MissionTarget,
							x701104_g_MissionInfo,	--任务描述
							x701104_g_MissionHelp					--任务小提示
							)
	EndQuestEvent()
