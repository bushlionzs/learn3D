...
x701030_g_ScriptName	= "提交三番令"

x701030_g_SubmitItem	= 11990011

x701030_g_TitleInfo		= "\t你是否觉得三番副本和这天上人间的玩法难以取舍？不要烦恼，找我“顽皮”就好了，只要你交给我一个#G三番令#W我就能给予你完成三番副本对应的经验。当然这么做会#Y占用一次三番副本次数#W。\n\t怎么样，你愿意交给我一个#G三番令#W，获得%d点经验吗？\n \n#G小提示：\n\t一天只能提交一次三番令。\n\t有江湖请柬、朝廷请柬效果时提交，经验奖励会翻倍。"

						
function x701030_ProcEventEntry(sceneId, selfId, NPCId, MissionId,idExt)	--点击该任务后执行此脚本
...
function x701030_ProcAccept( sceneId, selfId )
	...
	if week == 1 or week == 2 or week == 3 or week == 4 or week == 5 then
				Msg2Player(sceneId, selfId,format("提交#{_ITEM%d}失败，请于周六或周日再来", x701030_g_SubmitItem), 0, 2)
				Msg2Player(sceneId, selfId,format("提交#{_ITEM%d}失败，请于周六或周日再来", x701030_g_SubmitItem), 0, 3)
				return
	end			
	...
	if level >84 then
		Msg2Player( sceneId, selfId, "很抱歉，你等级过高，提交失败", 8, 3)
		Msg2Player( sceneId, selfId, "很抱歉，你等级过高，提交失败", 8, 2)
		return
	end

	if HaveItemInBag(sceneId, selfId, x701030_g_SubmitItem) <= 0 then
		local msg = format("很抱歉，你没有#{_ITEM%d}，提交失败", x701030_g_SubmitItem)
