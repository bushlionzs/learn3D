...
x700009_g_StartNPC 					=	{ type =42392,guid= 150593,x = 63, z=100, facedir =0, title="" } --寻路的启示坐标
x700009_g_StartNpcTalk				=  { "在下朱重八，代天下百姓在此谢过各位勇士前来相帮！","各位勇士请跟我前来大厅正殿，守护和氏璧！","和氏璧乃镇国之宝，决不可让这些贼匪所夺！如若敌方势力高于50，和氏璧危已……"}

x700009_g_EndNPC 					=	{ type =42392,guid= 150594,x = 63.153572, z=16.656811, facedir =0, title="" }
...
function x700009_OnFubenSceneCreated( sceneId )
	...
	SetFubenData_Param(sceneId, x700009_CSP_GUTNPC_ID,-1)
	x700009_CreateStartNpc( sceneId)
	--x700009_ShowTipsToAll(sceneId, "副本说明")
...
function x700009_EnterTick(sceneId, nowTickCount)
	if nowTickCount >= 2 then
		local gutNpc = GetFubenData_Param(sceneId, x700009_CSP_GUTNPC_ID)
		if gutNpc >= 0 and nowTickCount <= getn(x700009_g_StartNpcTalk)+1 then
			NpcTalk(sceneId, gutNpc, x700009_g_StartNpcTalk[nowTickCount-1],  -1)
		end
	...
	local gutNpc = GetFubenData_Param(sceneId, x700009_CSP_GUTNPC_ID)
	if gutNpc >= 0 then
			local x,z = GetWorldPos(sceneId, gutNpc)
			if x >= 63 and x <64 and z >= 16 and z < 17 then
				DeleteMonster(sceneId,gutNpc) --
				SetFubenData_Param(sceneId, x700009_CSP_GUTNPC_ID, -2)
				x700009_CreateEndNpc( sceneId)
			end
		end
...
function x700009_OnFubenFailed(sceneId, nParam)
	...
	for ii = 0, humancount - 1 do
		...
		CallScriptFunction(UTILITY_SCRIPT,"PlayEffect",sceneId,humanId,57 )
	end	
	if nParam==0 then
...
function x700009_OnFubenSuccess(sceneId)
	...
	x700009_ShowTipsToAll(sceneId, "成功击杀陈理，副本完成。")
...
function x700009_ProcAreaEntered( sceneId, selfId, zoneId, MissionId )
	--@param sceneId: 场景Id
	--@param selfId: 玩家ObjId (宠物进入EnterArea也会触发这个时间)
	--@param zoneId: 区域Id
	--@param MissionId: 任务id\
	SetFubenData_Param(sceneId, x700009_CSP_AREA_ID, zoneId)
	if zoneId == 0 then  --出口
		local status = GetFubenData_Param(sceneId, x700009_CSP_B_FUBEN_STATUS) 
		--print(status)
		
		if status == 1 then
			SetFubenData_Param(sceneId, x700009_CSP_AREA_FLAG,1)
			CallScriptFunction(700105,"PopupFubenMenu", sceneId, selfId, zoneId, x700009_g_MissionId, x700009_g_ScriptId)
...
--离开区域
---------------------------------------------------------------------------------------------------
function x700009_ProcAreaLeaved( sceneId, selfId, zoneId, MissionId )
	EnterAreaEventListHideNM(sceneId, selfId)
	SetFubenData_Param(sceneId, x700009_CSP_AREA_ID, -1)
end
--区域定时器
---------------------------------------------------------------------------------------------------
function x700009_ProcTiming(sceneId, selfId, ScriptId, MissionId)
	local zoneId = GetFubenData_Param(sceneId, x700009_CSP_AREA_ID )	
	if zoneId == 0 then  --出口
		local status = GetFubenData_Param(sceneId, x700009_CSP_B_FUBEN_STATUS) 
		--print(status)
		
		local flag = GetFubenData_Param(sceneId, x700009_CSP_AREA_FLAG)
		if flag == 1 then
			return
		end
		SetFubenData_Param(sceneId, x700009_CSP_AREA_FLAG,1)
		...
		if status == 1 then
			CallScriptFunction(700105,"PopupFubenMenu", sceneId, selfId, zoneId, x700009_g_MissionId, x700009_g_ScriptId)
		elseif status == 2 then
			CallScriptFunction(700105,"PopupFubenMenu", sceneId, selfId, zoneId, x700009_g_MissionId, x700009_g_ScriptId,1)
		end
...
function x700009_CreateStartNpc( sceneId)
	local nRet = 0
	if x700009_g_StartNPC.title~="" then
		nRet = CreateMonster(sceneId, x700009_g_StartNPC.type, x700009_g_StartNPC.x, x700009_g_StartNPC.z, 3, 0, -1, x700009_g_StartNPC.guid, -1, -1,x700009_g_StartNPC.facedir, "", x700009_g_StartNPC.title )
	else
		nRet = CreateMonster(sceneId, x700009_g_StartNPC.type, x700009_g_StartNPC.x, x700009_g_StartNPC.z, 3, 0, -1, x700009_g_StartNPC.guid, -1, -1,x700009_g_StartNPC.facedir )
	
	end
	SetFubenData_Param(sceneId, x700009_CSP_GUTNPC_ID,nRet)
	SetPatrolId(sceneId, nRet, 8)
end

function x700009_CreateEndNpc( sceneId)

	if x700009_g_EndNPC.title~="" then
		CreateMonster(sceneId, x700009_g_EndNPC.type, x700009_g_EndNPC.x, x700009_g_EndNPC.z, 3, 0, -1, x700009_g_EndNPC.guid, -1, -1,x700009_g_EndNPC.facedir, "", x700009_g_EndNPC.title )
	else
		CreateMonster(sceneId, x700009_g_EndNPC.type, x700009_g_EndNPC.x, x700009_g_EndNPC.z, 3, 0, -1, x700009_g_EndNPC.guid, -1, -1,x700009_g_EndNPC.facedir )
