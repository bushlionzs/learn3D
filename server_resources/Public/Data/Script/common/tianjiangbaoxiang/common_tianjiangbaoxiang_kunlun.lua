...
x310008_g_ScriptId = 310008

x310008_g_MissionList = { 8034,8035,8036,8037,8038,8039 }
x310008_g_MissionItem = { {mission=8034,item1=12030051, item2=12030052},
						  {mission=8035,item1=12030051, item2=12030053},
						  {mission=8036,item1=12030052, item2=12030053},
						  {mission=8037,item1=12030051, item2=12030052},
						  {mission=8038,item1=12030051, item2=12030053},
						  {mission=8039,item1=12030052, item2=12030053}
						}

x310008_g_AcceptMessage		= "您接受了任务：%s"
x310008_g_AbandonMessage	= "您放弃了任务：%s"
x310008_g_CompleteMessage	= "您完成了任务：%s"

x310008_g_MissionCommentMessage	=	"不在接任务时间，请关注活动公告"
x310008_g_MissionAcceptFailedMessage_Accepted	=	"你在本日已经做过本任务了，请明日再来"
x310008_g_MissionSubmitFailedMessage_OverTime	=	"你已经错过了交任务的时间，请放弃任务"
x310008_g_MissionAcceptTime1 = { 450, 480 }	--7*60+30, 8*60
x310008_g_MissionAcceptTime2 = { 750, 780 }	--12*60+30,13*60
x310008_g_MissionAcceptTime3 = { 1170, 1200 }	--18*60+30,19*60
x310008_g_AwardItem = 12030028
x310008_g_VirtualMissionId = 1367

x310008_g_RandItem = {11010100, 11010101, 11010102}

function x310008_GetMission()
	local index = random(1, 6);
	return x310008_g_MissionList[index]
end

function x310008_GetMDAccepted(sceneId, selfId, index)
	if index == 1 then
...
function x310008_SetMDAccepted(sceneId, selfId, index, value)
	if index == 1 then
...
function x310008_GetMDDate(sceneId, selfId)
...
function x310008_SetMDDate(sceneId, selfId, day)
	SetQuestData(sceneId, selfId, MD_TIANJIANG_BAOXIANG_DATE[1], MD_TIANJIANG_BAOXIANG_DATE[2], MD_TIANJIANG_BAOXIANG_DATE[3], day)
end


function x310008_NextDayReset(sceneId, selfId)
	local today = GetDayOfYear()
	local lastDay = x310008_GetMDDate(sceneId, selfId)

	if today ~= lastDay then
		x310008_SetMDDate(sceneId, selfId, 0)
		--x310008_SetMDAccepted(sceneId, selfId, 1, 0)
		--x310008_SetMDAccepted(sceneId, selfId, 2, 0)
		--x310008_SetMDAccepted(sceneId, selfId, 3, 0)
	end
end

function x310008_IsNextDay(sceneId, selfId)
...
function x310008_HaveAccepted( sceneId, selfId )
	--local minspan = GetMinOfDay()

	--if minspan >= x310008_g_MissionAcceptTime1[1] and minspan <= x310008_g_MissionAcceptTime1[2] then
		--if x310008_GetMDAccepted(sceneId, selfId, 1) > 0 then
			--return 1
		--end
	--elseif  minspan >= x310008_g_MissionAcceptTime2[1] and minspan <= x310008_g_MissionAcceptTime2[2] then
		--if x310008_GetMDAccepted(sceneId, selfId, 2) > 0 then
			--return 2
		--end
	--elseif  minspan >= x310008_g_MissionAcceptTime3[1] and minspan <= x310008_g_MissionAcceptTime3[2] then
		--if x310008_GetMDAccepted(sceneId, selfId, 3) > 0 then
			--return 3
		--end
	--end

	if x310008_GetMDDate(sceneId, selfId) > 0 then
		return 1
	end
...
function x310008_ProcAcceptCheckTime(  )
	...
	--if minspan >= x310008_g_MissionAcceptTime1[1] and minspan <= x310008_g_MissionAcceptTime1[2] then
		--return 1
	if  minspan >= x310008_g_MissionAcceptTime2[1] and minspan <= x310008_g_MissionAcceptTime2[2] then
		return 2
	elseif  minspan >= x310008_g_MissionAcceptTime3[1] and minspan <= x310008_g_MissionAcceptTime3[2] then
		return 3
	end
...
function x310008_ProcEnumEvent( sceneId, selfId, targetId, MissionId )
	MissionId = x310008_GetMission();

	for i, item in x310008_g_MissionList do
...
function x310008_ProcEventEntry( sceneId, selfId, targetId, MissionId )
	if IsHaveQuestNM( sceneId, selfId, MissionId ) == 0 then	-- 如果没有这个任务
		if x310008_IsNextDay(sceneId, selfId) > 0 then
			x310008_NextDayReset(sceneId, selfId) 
		end
		...
		local ret = QuestCheckAcceptNM( sceneId, selfId, targetId, MissionId )
			...
			if ret > 0 then	-- 如果可以接这个任务
				...
				AddQuestItemBonus(sceneId, x310008_g_AwardItem, 1)
				
				EndQuestEvent(sceneId);
				DispatchQuestInfoNM(sceneId, selfId, targetId, x310008_g_ScriptId, MissionId);
			else
...
function x310008_ProcQuestAccept( sceneId, selfId, targetId, MissionId )
	if x310008_ProcAcceptCheckTime() < 0 then
		local MissionName = GetQuestNameNM(sceneId, selfId, MissionId);
			BeginQuestEvent(sceneId);
			AddQuestText(sceneId, x310008_g_MissionCommentMessage);
			EndQuestEvent(sceneId);
			DispatchQuestTips(sceneId,selfId);
			Msg2Player(sceneId,selfId,x310008_g_MissionCommentMessage,8,2)
		return 
	end

	if x310008_HaveAccepted( sceneId, selfId ) > 0 then
		BeginQuestEvent(sceneId);
		AddQuestText(sceneId, x310008_g_MissionAcceptFailedMessage_Accepted);
		EndQuestEvent(sceneId);
		DispatchQuestTips(sceneId,selfId);
		Msg2Player(sceneId,selfId,x310008_g_MissionAcceptFailedMessage_Accepted,8,2)
		return
	end
	...
	if ret > 0 then -- 如果可以接这个任务
		...
		if ret > 0 then
			QuestCom( sceneId, selfId, x310008_g_VirtualMissionId)
			...
			x310008_SetMDDate(sceneId, selfId, day)
			
			--if x310008_ProcAcceptCheckTime(  ) == 1 then
				--x310008_SetMDDate(sceneId, selfId, day)
				--x310008_SetMDAccepted(sceneId, selfId, 1, 1)
			--elseif x310008_ProcAcceptCheckTime(  ) == 2 then
				--x310008_SetMDDate(sceneId, selfId, day)
				--x310008_SetMDAccepted(sceneId, selfId, 2, 1)
			--elseif x310008_ProcAcceptCheckTime(  ) == 3 then
				--x310008_SetMDDate(sceneId, selfId, day)
				--x310008_SetMDAccepted(sceneId, selfId, 3, 1)
			--else
				--BeginQuestEvent(sceneId);
				--AddQuestText(sceneId, "#Y"..GetQuestNameNM(sceneId, selfId, MissionId).."#W\n\t"..x310008_g_MissionCommentMessage);
				--EndQuestEvent(sceneId);
			...
			local MissionName = GetQuestNameNM( sceneId, selfId, MissionId )
			local message = format(x310008_g_AcceptMessage, MissionName);
			BeginQuestEvent(sceneId)
			...
			for i, item in x310008_g_MissionItem do
...
--任务放弃
function x310008_ProcQuestAbandon( sceneId, selfId, MissionId )
	local ret = 0
	...
	if ret == 1 then
		...
		local MissionName = GetQuestNameNM( sceneId, selfId, MissionId )
		local message = format(x310008_g_AbandonMessage, MissionName);
		BeginQuestEvent(sceneId);
		AddQuestText(sceneId, message);
		EndQuestEvent();
		DispatchQuestTips(sceneId, selfId);
		...
		QuestUnCom( sceneId, selfId, x310008_g_VirtualMissionId)

	end
end


function x310008_ProcQuestAttach( sceneId, selfId, targetId, npcGuid, misIndex, MissionId )
...
--返回1代表成功，0代表交任务失败
function x310008_ProcQuestSubmit( sceneId, selfId, targetId, selectId, MissionId )
	if IsHaveQuestNM( sceneId, selfId, MissionId ) == 0 then	-- 如果没有这个任务
		return
	end
	...
	CallScriptFunction( 203784, "Finishbaoxiang", sceneId, selfId)
	QuestUnCom( sceneId, selfId, x310008_g_VirtualMissionId)
	...
	x310008_SetMDDate(sceneId, selfId, day)
		...
		local MissionName = GetQuestNameNM( sceneId, selfId, MissionId )
		local message = format(x310008_g_CompleteMessage, MissionName);
		BeginQuestEvent(sceneId);
			AddQuestText(sceneId, message);
		EndQuestEvent();
		DispatchQuestTips(sceneId, selfId);
		...
		AddItem( sceneId, x310008_g_AwardItem, 1 )
		...
		if GetLevel(sceneId, selfId) >= 80 and random(1, 100) <= 22 then
			...
			rndItemid = x310008_g_RandItem[index]
		...
		if ret > 0 then
			...
			message = format("#Y获得物品#R#{_ITEM%d}#Y的奖励", x310008_g_AwardItem )
			BeginQuestEvent(sceneId);
			AddQuestText(sceneId, message);
			EndQuestEvent();
			DispatchQuestTips(sceneId, selfId);
...
function x310008_ProcQuestItemChanged( sceneId, selfId, itemdataId, MissionId )
...
function x310008_TianJiangBaoXiangDisconnect( sceneId, selfId )

end

function x310008_OnOpenItemBox( sceneId, selfId, targetId, gpType, needItemID )
	return CallScriptFunction( MISSION_SCRIPT, "OnOpenItemBox", sceneId, selfId, targetId, gpType, needItemID )
end

function x310008_OnRecycle( sceneId, selfId, targetId, gpType, needItemID )
	return CallScriptFunction( MISSION_SCRIPT, "OnRecycle", sceneId, selfId, targetId, gpType, needItemID )
end

function x310008_ProcQuestLogRefresh( sceneId, selfId, MissionId)
...
--打开
function x310008_OnProcOver( sceneId, selfId, targetId )
end

--打开后检查
function x310008_OpenCheck( sceneId, selfId, targetId )
end