...
x550107_g_ScriptId = 550107
x550107_g_TransPosLou="xianrenguloulanin"
x550107_g_TransPosLai="xianrengulaiyinin"
x550107_g_TransPosKun="xianrengukunlunin"
x550107_g_TransPosDun="xianrengudunhuangin"
x550107_g_Notice = "您的等级还不足#R20级#Y，无法前往仙人谷！"


--玩家进入一个 area 时触发
function x550107_ProcAreaEntered( sceneId, selfId, zoneId, MissionId )
	--print("x550107_ProcAreaEntered sceneId = " .. sceneId .. " selfId = " .. selfId)
	...
	if selfType == 1 then 
			...
			if nLevel < 20 then 
					Msg2Player(sceneId,selfId,"#Y"..x550107_g_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	--Bus进入区域 把bus和bus里的人跨场景
		...
		if busType == 1 then --BUS_TYPE_NORMAL
			...
			for	i = 0, memberCount - 1 do
				...
				x550107_TransToPosition(sceneId, ObjID)
			end
			--删除BUS,但乘员仍在车上
...
--		end
	...
	x550107_TransToPosition(sceneId, selfId)
end

function x550107_TransToPosition(sceneId, selfId)

	local nCountry = GetCurCountry(sceneId,selfId)
	
	if sceneId == 51 and nCountry == 0 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x550107_g_TransPosLou)
	elseif sceneId == 151 and nCountry == 1 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x550107_g_TransPosLai)
	elseif sceneId == 251 and nCountry == 2 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x550107_g_TransPosKun)
	elseif sceneId == 351 and nCountry == 3 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x550107_g_TransPosDun)
	else
		BeginQuestEvent(sceneId)
...
function x550107_ProcTiming( sceneId, monsterobjid )

end

--玩家离开一个 area 时触发
function x550107_ProcAreaLeaved( sceneId, monsterobjid, ScriptId, MissionId )

end


function x550107_ProcEventEntry( sceneId, monsterobjid )
