...
x300214_g_ScriptId = 300214
x300214_g_TransPos0	=	"caochang2bianjingdunhuang"
x300214_g_TransPos1	=	"caochang2bianjingdunhuang"
x300214_g_TransPos2	=	"caochang2bianjingdunhuang"
x300214_g_TransPos3	=	"caochang2bianjingdunhuang"
x300214_g_Notice = "您的等级还不足#R20级#Y，无法前往敦煌边塞！"

--玩家进入一个 area 时触发
function x300214_ProcAreaEntered( sceneId, selfId, zoneId, MissionId )
	------------------------------------------------------
	...
	--以上用于铁人三项禁止传送的处理
  	local nLevel = GetLevel(sceneId, selfId)
			if nLevel < 20 then 
					Msg2Player(sceneId,selfId,"#Y"..x300214_g_Notice.."#W",4,3)
					return
			end
	...
	if selfType == 9 then	--Bus进入区域 把bus和bus里的人跨场景
		...
		if busType == 1 then --BUS_TYPE_NORMAL
			...
			for	i = 0, memberCount - 1 do
				...
				x300214_TransToPosition(sceneId, ObjID)
			end
			--删除BUS,但乘员仍在车上
...
--		end
	...
	x300214_TransToPosition(sceneId, selfId)

end

function x300214_TransToPosition(sceneId, selfId)

	local guo = GetCurCountry(sceneId,selfId)
	
	if guo == 0 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300214_g_TransPos0)
		
		elseif guo == 1 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300214_g_TransPos1)
		
		elseif guo == 2 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300214_g_TransPos2)
		
		elseif guo == 3 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300214_g_TransPos3)
	
	end
end

--玩家在一个 area 呆了一段时间没走则定时触发
function x300214_ProcTiming( sceneId, monsterobjid )

end

--玩家离开一个 area 时触发
function x300214_ProcAreaLeaved( sceneId, monsterobjid, ScriptId, MissionId )

end


function x300214_ProcEventEntry( sceneId, monsterobjid )
