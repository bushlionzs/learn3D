...
x300479_g_ScriptId = 300479
x300479_g_TransPosLou="loulanbianjingenter2xiaoshuidao"
x300479_g_TransPosLai="laiyinbianjingenter2xiaoshuidao"
x300479_g_TransPosKun="kunlunbianjingenter2xiaoshuidao"
x300479_g_TransPosDun="dunhuangbianjingenter2xiaoshuidao"
x300479_g_Notice = "您的等级还不足#R20级#Y，无法前往秘道！"

--玩家进入一个 area 时触发
function x300479_ProcAreaEntered( sceneId, selfId, zoneId, MissionId )
	--print("x300479_ProcAreaEntered sceneId = " .. sceneId .. " selfId = " .. selfId)
	...
	if selfType == 1 then 
			...
			if nLevel < 20 then 
					Msg2Player(sceneId,selfId,"#Y"..x300479_g_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	--Bus进入区域 把bus和bus里的人跨场景
		...
		if busType == 1 then --BUS_TYPE_NORMAL
			...
			for	i = 0, memberCount - 1 do
				...
				x300479_TransToPosition(sceneId, ObjID)
			end
			--删除BUS,但乘员仍在车上
...
--		end
	...
	x300479_TransToPosition(sceneId, selfId)
end

function x300479_TransToPosition(sceneId, selfId)
	if sceneId == 51 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300479_g_TransPosLou)
	elseif sceneId == 151 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300479_g_TransPosLai)
	elseif sceneId == 251 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300479_g_TransPosKun)
	elseif sceneId == 351 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300479_g_TransPosDun)
	end
end

--玩家在一个 area 呆了一段时间没走则定时触发
function x300479_ProcTiming( sceneId, monsterobjid )

end

--玩家离开一个 area 时触发
function x300479_ProcAreaLeaved( sceneId, monsterobjid, ScriptId, MissionId )

end


function x300479_ProcEventEntry( sceneId, monsterobjid )
