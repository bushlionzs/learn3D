...
x300456_g_TransPos0="loulanbianjing2wangcheng"
x300456_g_TransPos1="laiyinbianjing2wangcheng"
x300456_g_TransPos2="kunlunbianjing2wangcheng"
x300456_g_TransPos3="dunhuangbianjing2wangcheng"


--玩家进入一个 area 时触发
function x300456_ProcAreaEntered( sceneId, selfId, zoneId, MissionId )
	------------------------------------------------------
	...
	if selfType == 9 then	--Bus进入区域 把bus和bus里的人跨场景
		...
		if busType == 1 then --BUS_TYPE_NORMAL
			...
			for	i = 0, memberCount - 1 do
				...
				x300456_TransToPosition(sceneId, ObjID)
			end
			--删除BUS,但乘员仍在车上
...
--		end
	...
	x300456_TransToPosition(sceneId, selfId)

end

function x300456_TransToPosition(sceneId, selfId)

	if sceneId == 74 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300456_g_TransPos0)
	elseif sceneId == 151 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300456_g_TransPos1)
	elseif sceneId == 251 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300456_g_TransPos2)
	elseif sceneId == 351 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300456_g_TransPos3)
	end
end

--玩家在一个 area 呆了一段时间没走则定时触发
function x300456_ProcTiming( sceneId, monsterobjid )

end

--玩家离开一个 area 时触发
function x300456_ProcAreaLeaved( sceneId, monsterobjid, ScriptId, MissionId )

end


function x300456_ProcEventEntry( sceneId, monsterobjid )
