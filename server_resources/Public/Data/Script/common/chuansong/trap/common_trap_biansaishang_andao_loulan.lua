...
x300233_g_ScriptId = 300233
x300233_g_TransPosloulan  ="bianjingshang2andao0"
x300233_g_TransPoslaiyin  ="bianjingshang2andao0"
x300233_g_TransPoskunlun  ="bianjingshang2andao0"
x300233_g_TransPosdunhuang="bianjingshang2andao0"
x300233_g_Notice = "您的等级还不足#R20级#Y，无法前往边塞暗道！"

--玩家进入一个 area 时触发
function x300233_ProcAreaEntered( sceneId, selfId, zoneId, MissionId )
	--print("x300233_ProcAreaEntered sceneId = " .. sceneId .. " selfId = " .. selfId)
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(sceneId, selfId)
			if nLevel < 20 then 
					Msg2Player(sceneId,selfId,"#Y"..x300233_g_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	--Bus进入区域 把bus和bus里的人跨场景
		...
		if busType == 1 then --BUS_TYPE_NORMAL
			...
			for	i = 0, memberCount - 1 do
				...
				x300233_TransToPosition(sceneId, ObjID)
			end
			--删除BUS,但乘员仍在车上
...
--		end
	...
	x300233_TransToPosition(sceneId, selfId)
end

function x300233_TransToPosition(sceneId, selfId)

	local guo = GetCurCountry(sceneId,selfId)
	
	if guo == 0 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300233_g_TransPosloulan)
		
		elseif guo == 1 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300233_g_TransPoslaiyin)
		
		elseif guo == 2 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300233_g_TransPoskunlun)
		
		elseif guo == 3 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300233_g_TransPosdunhuang)
	
	end
end

--玩家在一个 area 呆了一段时间没走则定时触发
function x300233_ProcTiming( sceneId, monsterobjid )

end

--玩家离开一个 area 时触发
function x300233_ProcAreaLeaved( sceneId, monsterobjid, ScriptId, MissionId )

end


function x300233_ProcEventEntry( sceneId, monsterobjid )
