...
x300283_g_ScriptId = 300283
x300283_g_TransPos0  ="wangcheng2migongone1"
x300283_g_TransPos1  ="wangcheng2migongone2"
x300283_g_TransPos2  ="wangcheng2migongone3"
x300283_g_TransPos3  ="wangcheng2migongone4"
x300283_g_Notice = "您的等级还不足#R25级#Y，无法前往王国迷宫！"

--玩家进入一个 area 时触发
function x300283_ProcAreaEntered( sceneId, selfId, zoneId, MissionId )
	--print("x300283_ProcAreaEntered sceneId = " .. sceneId .. " selfId = " .. selfId)
	...
	if selfType == 1 then 
		...
		local nLevel = GetLevel(sceneId, selfId)
			if nLevel < 25 then 
					Msg2Player(sceneId,selfId,"#Y"..x300283_g_Notice.."#W",4,3)
					return
			end
	end
	...
	if selfType == 9 then	--Bus进入区域 把bus和bus里的人跨场景
		...
		if busType == 1 then --BUS_TYPE_NORMAL
			...
			for	i = 0, memberCount - 1 do
				...
				x300283_TransToPosition(sceneId, ObjID)
			end
			--删除BUS,但乘员仍在车上
...
--		end
	...
	x300283_TransToPosition(sceneId, selfId)
end


function x300283_TransToPosition(sceneId, selfId)

	local nCountry = GetCurCountry(sceneId,selfId)

	if sceneId == 50 and nCountry == 0 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300283_g_TransPos0)
	elseif sceneId == 150 and nCountry == 1 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300283_g_TransPos1)
	elseif sceneId == 250 and nCountry == 2 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300283_g_TransPos2)
	elseif sceneId == 350 and nCountry == 3 then
		CallScriptFunction(COMMON_CHUANSONG_NORMAL, "TransferToWorld",sceneId,selfId,x300283_g_TransPos3)
	else
		BeginQuestEvent(sceneId)
...
function x300283_ProcTiming( sceneId, monsterobjid )

end

--玩家离开一个 area 时触发
function x300283_ProcAreaLeaved( sceneId, monsterobjid, ScriptId, MissionId )

end


function x300283_ProcEventEntry( sceneId, monsterobjid )
