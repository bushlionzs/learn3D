--95级地宫
--脚本号
x310041_g_ScriptId = 310041

--任务文本描述
x310041_g_MissionName="圣山地宫传入1"

--x310041_g_MD_AcceptTime = MD_YAOXIE_DIGONG_TIME

x310041_g_MaxHuman = 100
x310041_g_MaxHumanOther = 200 

x310041_g_ItemId = 12030027
...
function x310041_HaveAccepted(sceneId, selfId)
...
function x310041_ProcEnumEvent( sceneId, selfId, targetId, MissionId )
	if GetLevel(sceneId, selfId) < 95 then
		return
	end
    AddQuestNumText(sceneId,x310041_g_ScriptId,"圣山地宫(95级)",14,-1)
     
end


function x310041_ProcEventEntry( sceneId, selfId, targetId,idScript,idExt )
		...
		DispatchQuestInfo(sceneId,selfId,targetId,x310041_g_ScriptId,-1)
	return 
...
--检测接受条件
--**********************************
function x310041_ProcAcceptCheck( sceneId, selfId, NPCId )
		return 1
end
...
--接受
--**********************************
function x310041_ProcAccept( sceneId, selfId )
	...
	if x310041_HaveAccepted(sceneId, selfId) > 0 then
	...
	if bHave == 0 then
		if GetItemCount(sceneId, selfId, x310041_g_ItemId)  == 0 then
	...
	--if curNum < x310041_g_MaxHuman then
		if x310041_IsOpenedTime() > 0 then
			if GetItemCount(sceneId, selfId, x310041_g_ItemId)  > 0 then
				sign = 1
			else
...
--	elseif curNum >= x310041_g_MaxHuman and curNum < x310041_g_MaxHumanOther then
--		
--		if bHave  > 0 then
--			sign =2;	
--		else	
...
--	end
	...
	if sign == 0 then
		return
	elseif sign == 1 then
		DelItem(sceneId, selfId, x310041_g_ItemId, 1)
	elseif sign == 2 then
	...
	NewWorld(sceneId, selfId, NewSceneId,PosX ,PosZ, 310041 )
