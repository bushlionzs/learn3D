...
x300852_g_ScriptId = 300852
x300852_g_MissionName = "创建镖车"

--无敌impact在表中的索引,玩家进入BUS后获得一个无敌impact,
x300852_g_GodIndex = 1210
...
x300852_g_MissionId = {
						{ ID = 7608,iCostMoney = 1000, GameId = 51 },
						{ ID = 7609,iCostMoney = 2000, GameId = 61 },
						{ ID = 7610,iCostMoney = 3000,GameId = 71 },
						{ ID = 7611,iCostMoney = 5000,GameId = 81 },
						{ ID = 7612,iCostMoney = 6000,GameId = 741 },
						{ ID = 7613,iCostMoney = 7500,GameId = 751 },
					  }
...
function x300852_ProcEventEntry(sceneId, selfId, targetId)	--点击该任务后执行此脚本
...
--**********************************
function x300852_ProcEnumEvent(sceneId, selfId, targetId)
	--AddQuestNumText(sceneId, x300852_g_ScriptId, x300852_g_MissionName);
end


function x300852_OnCreateBus( sceneId, selfId,MissionId ,BusID)

    local PosX = 180.6
    local PosZ = 81.4
	local ObjID = CreateBus(sceneId, x300852_g_Carinfo[BusID].CarType, PosX, PosZ, x300852_g_Carinfo[BusID].BaseAI, x300852_g_Carinfo[BusID].AIScript, x300852_g_Carinfo[BusID].Script, selfId, x300852_g_GodIndex,MissionId,-1)
end

function x300852_OnCreateBusOK(sceneId, selfId, busId,targetId,MissionId)	
...
function x300852_DeleteBus(sceneId, ObjId)
...
--selfId：玩家ID
--targetId：BUSID
function x300852_OnIntoBusEvent(sceneId, selfId,targetId)
...
function x300852_OnIntoBusAfter(sceneId, selfId,BusID)
	...
	for i,itm in x300852_g_MissionId do
...
--玩家离开BUS
--selfId：玩家ID
function x300852_OnLeaveBusEvent(sceneId, selfId,BusId)
		BeginQuestEvent(sceneId)
...
function x300852_OnDie(sceneId,BusId,idKiller)
	...
	if idKiller == -1 then
		...
		if OwnerID ~= -1 then
			x300852_Abandon( sceneId, OwnerID  )
...
function x300852_OnTime(sceneId,BusId)

end

function x300852_OnLifeTimeOut(sceneId,BusId)
    local selfId = GetBusMemberObjId(sceneId,BusId,0)
    if(IsPlayerStateNormal(sceneId, selfId) ~= 0) then
		x300852_Abandon( sceneId, selfId  )
	end
	DeleteBus(sceneId, BusId,1)
end

function x300852_OnWaitTimeOut(sceneId,BusId)
	local selfId = GetBusMemberObjId(sceneId,BusId,0)
	if(IsPlayerStateNormal(sceneId, selfId) ~= 0) then
		x300852_Abandon( sceneId, selfId  )
...
function x300852_OnBusStopWhenOwnerFarAway(sceneId, ownerId)
end
...
function x300852_CleaupMission( sceneId, selfId )
...
--更新当天接任务次数
---------------------------------------------------------------------------------------------------
function x300852_SetDayCount(sceneId, selfId)
