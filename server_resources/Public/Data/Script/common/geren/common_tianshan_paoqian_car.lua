...
x300866_g_ScriptId = 300866
x300866_g_MissionName = "创建飞行器"

--无敌impact在表中的索引,玩家进入BUS后获得一个无敌impact,
x300866_g_GodIndex = 1210
x300866_g_Carinfo = {
...
x300866_g_MissionId = {
						{ ID = 7638,iCostMoney =5000 ,AcceptId = 281 },
						{ ID = 7639,iCostMoney = 10000, AcceptId = 291 },
						{ ID = 7640, iCostMoney = 15000, AcceptId = 301 },
						{ ID = 7641, iCostMoney = 30000, AcceptId = 761 },
						{ ID = 7642, iCostMoney = 40000, AcceptId = 771 },
						{ ID = 7643, iCostMoney = 50000, AcceptId = 781 },
					  }

x300866_g_MainScriptId = 300863
...
function x300866_ProcEventEntry(sceneId, selfId, targetId)	--点击该任务后执行此脚本
...
--**********************************
function x300866_ProcEnumEvent(sceneId, selfId, targetId, MissionId)
	--AddQuestNumText(sceneId, x300866_g_ScriptId, x300866_g_MissionName);
end


function x300866_OnCreateBus( sceneId, selfId,MissionId,BusID )

  local PosX,PosZ = GetWorldPos(sceneId, selfId)

	local ObjID = CreateBus(sceneId, x300866_g_Carinfo[BusID].CarType, PosX, PosZ, x300866_g_Carinfo[BusID].BaseAI, x300866_g_Carinfo[BusID].AIScript, x300866_g_Carinfo[BusID].Script, selfId, x300866_g_GodIndex,MissionId,-1)
end

--创建busd成功处理
function x300866_OnCreateBusOK(sceneId, selfId, busId,targetId,MissionId)
...
function x300866_DeleteBus(sceneId, ObjId)
...
--selfId：玩家ID
--targetId：BUSID
function x300866_OnIntoBusEvent(sceneId, selfId,targetId)
...
function x300866_OnIntoBusAfter(sceneId, selfId,BusID)
	...
	for i,itm in x300866_g_MissionId do

		if MissionId == itm.ID then
			local bRet = CallScriptFunction( x300866_g_MainScriptId, "CostMoney", sceneId, selfId, MissionId)
			if bRet == 1 then

				if AddQuest(sceneId, selfId, MissionId,x300866_g_MainScriptId,0,0,0,1) == 1 then
					--接任务写日志
					GamePlayScriptLog(sceneId,selfId,itm.AcceptId)

					--local misIndex = CallScriptFunction( x300866_g_MainScriptId, "GetQuestIndexByID", sceneId, selfId, MissionId)
					local misIndex = GetQuestIndexByID(sceneId, selfId, MissionId);
					SetQuestByIndex(sceneId, selfId, misIndex, 7, 0);
					CallScriptFunction( x300866_g_MainScriptId, "QuestLogRefresh", sceneId, selfId, MissionId)	
					local CarName = GetName( sceneId, selfId ).."的飞行器"
					SetBusTitle(sceneId , BusID, CarName)
...
--玩家离开BUS
--selfId：玩家ID
function x300866_OnLeaveBusEvent(sceneId, selfId,BusId)
		BeginQuestEvent(sceneId)
...
function x300866_OnDie(sceneId,BusId,idKiller)
	...
	if idKiller == -1 then
		...
		if OwnerID ~= -1 then
			x300866_Abandon( sceneId, OwnerID  )
...
function x300866_OnTime(sceneId,BusId)

end

function x300866_OnLifeTimeOut(sceneId,BusId)
    local selfId = GetBusMemberObjId(sceneId,BusId,0)
    if(IsPlayerStateNormal(sceneId, selfId) ~= 0) then
		x300866_Abandon( sceneId, selfId  )
	end
	DeleteBus(sceneId, BusId,1)
end

function x300866_OnWaitTimeOut(sceneId,BusId)
...
function x300866_OnBusStopWhenOwnerFarAway(sceneId, ownerId)
...
--放弃
--********************************************************************
function x300866_Abandon( sceneId, selfId )
	...
	for i,itm in x300866_g_MissionId do
		...
		if IsHaveQuestNM( sceneId, selfId, itm.ID) ~= 0 then	-- 如果有这个任务
			...
			x300866_SetDayCount(sceneId, selfId)
			return
		end
	end
...
--放弃
--********************************************************************
function x300866_CleaupMission( sceneId, selfId )
...
--更新当天接任务次数
---------------------------------------------------------------------------------------------------
function x300866_SetDayCount(sceneId, selfId)
