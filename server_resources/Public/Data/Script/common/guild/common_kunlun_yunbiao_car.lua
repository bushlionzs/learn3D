...
x300870_g_ScriptId = 300870
x300870_g_MissionName = "创建镖车"

--无敌impact在表中的索引,玩家进入BUS后获得一个无敌impact,
x300870_g_GodIndex = 1210
x300870_g_Carinfo = {CarType = 4,BaseAI = 3,AIScript = 3,Script = 300870 }

x300870_g_CostMoney = 10000
--**********************************
...
function x300870_ProcEventEntry(sceneId, selfId, targetId)	--点击该任务后执行此脚本
...
--**********************************
function x300870_ProcEnumEvent(sceneId, selfId, targetId, MissionId)
	--AddQuestNumText(sceneId, x300870_g_ScriptId, x300870_g_MissionName);
end


function x300870_OnCreateBus( sceneId, selfId,MissionId )

  local PosX,PosZ = GetWorldPos(sceneId, selfId)
	local ObjID = CreateBus(sceneId, x300870_g_Carinfo.CarType, PosX, PosZ, x300870_g_Carinfo.BaseAI, x300870_g_Carinfo.AIScript, x300870_g_Carinfo.Script, selfId, x300870_g_GodIndex,MissionId,-1)
end

--创建busd成功处理
function x300870_OnCreateBusOK(sceneId, selfId, busId,targetId,MissionId)	
...
function x300870_DeleteBus(sceneId, ObjId)
...
--selfId：玩家ID
--targetId：BUSID
function x300870_OnIntoBusEvent(sceneId, selfId,targetId)
...
function x300870_OnIntoBusAfter(sceneId, selfId,BusID)
...
--玩家离开BUS
--selfId：玩家ID
function x300870_OnLeaveBusEvent(sceneId, selfId,BusId)
		BeginQuestEvent(sceneId)
...
function x300870_OnTime(sceneId,BusId)

end

function x300870_OnLifeTimeOut(sceneId,BusId)
...
function x300870_OnBusStopWhenOwnerFarAway(sceneId, ownerId)
end

function x300870_OnWaitTimeOut(sceneId,BusId)
...
function x300870_OnDie(sceneId,BusId,idKiller)
	...
	if ObjType == 1 then     --被玩家杀死 
		...
		if OwnerCountry == KillerCountry then
	        ...
	        AddMoney(sceneId,idKiller,0,x300870_g_CostMoney)
		  	BeginQuestEvent(sceneId);AddQuestText(sceneId, "您击杀了敌国的神兽，并获得了奖励！");EndQuestEvent(sceneId);DispatchQuestTips(sceneId,idKiller)
			Msg2Player(sceneId,idKiller,format("您击杀了敌国的神兽，并获得了现银#{_MONEY%d}。", x300870_g_CostMoney),8,2)	
		end 
		...
		if OwnerID ~= -1 then		
			...
			msg1 = "昆仑王国"..GuildTitle1.."的神兽被"..Country..GuildTitle.."的"..killerName.."击杀"
