...
--MisDescBegin
x300908_g_ScriptId		=	300908
x300908_g_MissionName	=	"【帮会】丹木"
x300908_g_MissionTarget	=	"　收集#G丹木#W"
x300908_g_MissionInfo	=	"\t随着帮会的蓬勃发展，我们需要更高级的木材来满足日益增长的需求。生长在#R中立地图#W的丹木是一种神奇的木头，终年不见阳光却能生长茂盛，传说仙人们都拿它来制作酒器。在仙人谷的二层，三层，四层和五层，生长着这种神奇的丹木。那里充斥着重重危险，你愿意进入谷内带回五个丹木么？当帮主发布收集令时2小时内回复任务可获得翻倍奖励（经验，帮贡）和2点使命点！"
x300908_g_MissionHelp	=	"\t#G丹木#W只生长在地下#R中立地图#W@npc_37000，@npc_38000，@npc_39000。并且数量稀少。随着你所在帮会等级的提高，你获得的奖励也会相应增加。"
x300908_g_MissionRuse	=	"\t当你到达#R中立地图#W，进行收集#G丹木#W时，如果受到攻击，将无法收集成功。你可以进入角落，将周围的怪物杀死后，再进行收集。#R中立地图#W的怪物比较密集，您最好好组队进行收集！"
x300908_g_MissionCompleted = "我需要的丹木，你已经找到了？太感谢了，辛苦了，这是给你的酬谢。"
x300908_g_SubmitNPCGUID	=	126020
x300908_g_GroupPoint	=	{ {type=10,id =124,target = "丹木"}}

x300908_g_NPCName		= "@npc_126020"

x300908_g_ExtTarget		=	{ {type=20,n=10,target="马车"} }
x300908_g_LevelLess		= 	40 
--x300908_g_BonusMoney8	=   100
--MisDescEnd
x300908_g_BonusMoney8	=   50
x300908_g_GuildExpBonus =	1
--x300908_g_BangExp 		=   1
x300908_g_Mis_Count		= 1
x300908_g_Leader_Index	= 5
x300908_g_PreMissionId	= 7006


function x300908_IsHavePreMission(sceneId, selfId)
	local MissionId1  = 7700 
...
function x300908_ProcEnumEvent( sceneId, selfId, targetId, MissionId )
	if x300908_IsHavePreMission(sceneId, selfId) > 0 then
		return 0
	end
	...
	if level < 40  then
  	...
  	if IsHaveQuestNM( sceneId, selfId, MissionId ) == 0 then	-- 如果没有这个任务
		...
		AddQuestNumText( sceneId, MissionId, x300908_g_MissionName ,8,state)	
		--GetGuildQuestData(sceneId, selfId, GD_GUILD_INDEX_GATHER_ISSUE_TIME, x300908_g_ScriptId,MissionId,"OnCallBackEnumerate",targetId)
		return 
	end
...
--选中任务
function x300908_ProcEventEntry( sceneId, selfId, targetId, MissionId )
--local GuildLevel = GetGuildSimpleData(GetGuildID( sceneId, selfId ))--得到帮会简单数据(等级) 
	--if 	GuildLevel == 2 then		     	
		if IsHaveQuestNM( sceneId, selfId, MissionId ) > 0 then	-- 如果有这个任务
			local bDone = x300908_CheckSubmit(sceneId, selfId,targetId, MissionId);
			if(bDone > 0) then
				x300908_DispatchCompletedInfo( sceneId, selfId, targetId,MissionId) --完成任务显示
			else
				x300908_DispatchContinueInfo( sceneId, selfId, targetId,MissionId ) --未完成任务显示
			end
		else
			x300908_DispatchMissionInfo( sceneId, selfId, targetId,MissionId )		--任务信息显示
		end
	--end
end
...
--检查任务是否可提交
---------------------------------------------------------------------------------------------------
function x300908_CheckSubmit(sceneId, selfId,targetId,MissionId)

	local misIndex = GetQuestIndexByID(sceneId, selfId, MissionId);
	local state = GetQuestStateNM(sceneId,selfId,targetId,MissionId)
	AddQuestNumText( sceneId, MissionId,x300908_g_MissionName, 7,state ) 
...
--向客户端发送任务完成信息
---------------------------------------------------------------------------------------------------
function x300908_DispatchCompletedInfo( sceneId, selfId, targetId,MissionId )
	
	BeginQuestEvent(sceneId)

		--任务完成信息
		AddQuestText(sceneId,"#Y"..x300908_g_MissionName)
		AddQuestText(sceneId,x300908_g_MissionCompleted)
		...
		--2 帮供
		if x300908_g_BonusMoney8>0 then
			AddQuestMoneyBonus8(sceneId,x300908_g_BonusMoney8)
		end
		--3 帮会经验
		-- if x300908_g_GuildExpBonus>0 then
			-- AddQuestGuildExpBonus(sceneId,x300908_g_GuildExpBonus)
		-- end
	EndQuestEvent(sceneId)
	DispatchQuestContinueInfoNM(sceneId, selfId, targetId, x300908_g_ScriptId, MissionId);
...
--向客户端发送任务未完成
---------------------------------------------------------------------------------------------------
function x300908_DispatchContinueInfo( sceneId, selfId, targetId,MissionId )
	
	BeginQuestEvent(sceneId)

		--任务继续信息
		AddQuestText(sceneId,"#Y"..x300908_g_MissionName)
		AddQuestText(sceneId,format("%s", x300908_g_MissionInfo))
		AddQuestText(sceneId," ")
	...
	--任务提示信息
	EndQuestEvent(sceneId)
	DispatchQuestDemandInfo(sceneId, selfId, targetId, x300908_g_ScriptId, MissionId,0);
...
--向客户端发送任务信息
---------------------------------------------------------------------------------------------------
function x300908_DispatchMissionInfo( sceneId, selfId, NPCId,MissionId )
	...
	BeginQuestEvent(sceneId)
		...
		AddQuestText( sceneId,format("%s", x300908_g_MissionTarget))
		AddQuestText( sceneId," ")

		--提示信息
		if x300908_g_MissionHelp ~= "" then
			AddQuestText(sceneId,"#Y任务提示：")
			AddQuestText(sceneId,format("%s", x300908_g_MissionHelp))
			AddQuestText(sceneId," ")
		end
		...
		-- end
	EndQuestEvent(sceneId)
	DispatchQuestInfoNM(sceneId, selfId, NPCId, x300908_g_ScriptId, MissionId);
end
...
--获得奖励
---------------------------------------------------------------------------------------------------
function x300908_OnReturn1(sceneId, selfId,MissionData,MissionId,targetId)
	...
	x300908_GetBonus( sceneId,selfId, isMultiple )
end

function x300908_GetBonus( sceneId, selfId, isMultiple )
...
--奖励帮贡
    if x300908_g_BonusMoney8 > 0 then
		    AddGuildUserPoint(sceneId,selfId,x300908_g_BonusMoney8)	--增加帮贡
			local BonusMoney8 = x300908_g_BonusMoney8
			BeginQuestEvent(sceneId)
			AddQuestText(sceneId, "获得#R帮贡"..BonusMoney8.."点#o的奖励");
			EndQuestEvent(sceneId)
			DispatchQuestTips(sceneId,selfId)	
			Msg2Player(sceneId,selfId,format("获得#R帮贡"..BonusMoney8.."点#o的奖励", x300908_g_BonusMoney8),8,2)
			if isMultiple == 1 then
			local pos = GetGuildOfficial(sceneId, selfId)
				if pos == 5 then
					local guildid =GetGuildID(sceneId, selfId)
				...
				elseif pos == 4 then
					...
					EndQuestEvent(sceneId)
					DispatchQuestTips(sceneId,selfId)	
					Msg2Player(sceneId,selfId,format("帮会多倍任务额外获得#R帮贡"..BonusMoney8.."点#o的奖励", x300908_g_BonusMoney8),8,2)
				end
			end
	end
	-- 奖励帮会经验
	-- local bangExp = x300908_g_BangExp 
	-- if bangExp > 0 then
...
--接受
--**********************************
function x300908_ProcQuestAccept( sceneId, selfId, targetId, MissionId )
	...
	return 
 	...
 	local level = GetLevel(sceneId,selfId)
	if level < x300908_g_LevelLess then
		BeginQuestEvent(sceneId)
	...
	if x300908_GetDayCount(sceneId, selfId) >= x300908_g_Mis_Count then
		BeginQuestEvent(sceneId)
   ...
   --GetGuildQuestData(sceneId, selfId, GD_GUILD_INDEX_GATHER_ISSUE_TIME, x300908_g_ScriptId,MissionId,"OnReturn")
...
--完成任务
--返回1代表成功，0代表交任务失败
function x300908_ProcQuestSubmit( sceneId, selfId, targetId, selectId, MissionId )
...
--	DispatchQuestTips(sceneId,selfId)
	...
	x300908_SetDayCount(sceneId, selfId)
	GetGuildQuestData(sceneId, selfId, GD_GUILD_INDEX_MULTI_MISSION_HEIMU, x300908_g_ScriptId,MissionId,"OnReturn1",targetId)
...
function x300908_ProcQuestAttach( sceneId, selfId, npcId, npcGuid,misIndex, MissionId )
	
	if IsHaveQuestNM( sceneId, selfId, MissionId) == 0 then	-- 如果没有这个任务
		return
	end

	if npcGuid == x300908_g_SubmitNPCGUID then
...
--放弃
--********************************************************************
function x300908_ProcQuestAbandon( sceneId, selfId,MissionId )
...
function x300908_OnReturn(sceneId, selfId,MissionData,MissionId,targetId)
...
--接受
--********************************************************************
function x300908_ProcAccept( sceneId, selfId )

end

--物品改变
function x300908_ProcQuestItemChanged( sceneId, selfId, itemdataId, MissionId, nChangeType )
	...
	if nChangeType ==1 then
		...
		if itemNum < NeedNum then
			...
			x300908_QuestLogRefresh( sceneId, selfId, MissionId );
		end
...
--任务日志刷新
---------------------------------------------------------------------------------------------------
function x300908_QuestLogRefresh( sceneId, selfId, MissionId)
	...
	if bHaveMission > 0 then
		...
		if x300908_g_BonusMoney8 > 0 then
			AddQuestMoneyBonus8(sceneId, x300908_g_BonusMoney8 )	--增加帮贡
		end
		--3 帮会经验
		-- if x300908_g_GuildExpBonus>0 then
			-- AddQuestGuildExpBonus(sceneId,x300908_g_GuildExpBonus)
		...
		AddQuestLogCustomText( sceneId,
								"",								-- 标题
								x300908_g_MissionName,			-- 任务名字
								x300908_g_MissionTarget..text,	--任务目标
								x300908_g_NPCName,				--任务NPC
								x300908_g_MissionRuse,			--任务攻略
								x300908_g_MissionInfo,			--任务描述
								""								--任务小提示
								)
		EndQuestEvent(sceneId)
...
function x300908_ProcQuestLogRefresh( sceneId, selfId, MissionId)
...
--取得此任务当天当前已完成次数
---------------------------------------------------------------------------------------------------
function x300908_GetDayCount(sceneId, selfId)

	if x300908_g_Mis_Count > 0 then
...
--更新当天接任务次数
---------------------------------------------------------------------------------------------------
function x300908_SetDayCount(sceneId, selfId)
	local today = GetDayOfYear()
