...
--事件交互入口
--**********************************
function x310367_ProcEventEntry( sceneId, selfId, bagIndex )
end
...
--**********************************
function x310367_IsSkillLikeScript( sceneId, selfId)
...
--**********************************
function x310367_CancelImpacts( sceneId, selfId )
...
--**********************************
function x310367_OnConditionCheck( sceneId, selfId )
	--校验使用的物品
...
--**********************************
function x310367_OnDeplete( sceneId, selfId )
    -- 暂时取消消耗
...
--注：这里是技能生效一次的入口
--**********************************
function x310367_OnActivateOnce( sceneId, selfId)
    ...
    if CallScriptFunction( 310364, "ProcUseShovel", sceneId, selfId, 1) == 1 then
        x310367_LogicDeplete( sceneId, selfId)
    end
...
--注：这里是技能生效一次的入口
--**********************************
function x310367_OnActivateEachTick( sceneId, selfId)
	return 1; --不是引导性脚本, 只保留空函数.
end

function x310367_LogicDeplete( sceneId, selfId)
    -- print( "*-*-*-*-*-*-*-*-*-*-*-*-*-*" )
    -- 获得铁铲的背包索引值
