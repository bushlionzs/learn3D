...
x570018_g_MissionName				= "第六轮兑换[国传钻石碎片]"
x570018_g_EnterInfo					= "\t1个#G溶火石#W兑换1个#G国传钻石碎片#W，\n\t#R每轮仅能兑换一次，请慎重选择兑换物品！"
-------------------------
--***********************
--ProcEnumEvent
--***********************
...
function x570018_CheckRequest(sceneId, selfId)
	...
	if( x570018_g_DayCountUntil > 0 ) then
		...
		if x570018_GetDayCount(sceneId, selfId) >= x570018_g_DayCountUntil then
			...
			local str = format("本轮仅能兑换%d次",x570018_g_DayCountUntil)
	...
	if nCurrentGameStep~=x570018_G_STEP then
		local str = format("当前波数不是第%d波",x570018_G_STEP)
	...
	if ItemCnt<x570018_g_NeedItemCount then
		local str = format("所需任务道具不足%d个",x570018_g_NeedItemCount)
