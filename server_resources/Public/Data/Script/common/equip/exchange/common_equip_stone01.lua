...
x570010_g_ExtraType					=	1 --1: 荣誉， 2： 声望
x570010_g_ExtraCount				=	1000 -- 所需荣誉/声望值
x570010_g_BonusItemId				= 11000214



x570010_g_MissionName				= "【兑换】国传海蓝钻石"
x570010_g_EnterInfo					= "\t2个#G国传钻石碎片#W和1000#G荣誉#W\n\t兑换1个#G国传海蓝钻石#W。"
-------------------------
--***********************
--ProcEnumEvent
--***********************
...
function x570010_CheckRequest(sceneId, selfId)
	...
	if ItemCnt<x570010_g_NeedItemCount then
		local str =  format("碎片不足%d个！",x570010_g_NeedItemCount)
	...
	if x570010_g_ExtraType==1 then		--荣誉
		nExtraCount = GetHonor( sceneId, selfId )
	elseif x570010_g_ExtraType==2 then	--声望
	...
	if nExtraCount<x570010_g_ExtraCount then
		...
		if x570010_g_ExtraType==1 then		--荣誉
			str = format("您荣誉不足%d！",x570010_g_ExtraCount)
		elseif x570010_g_ExtraType==2 then	--声望
			str = format("您声望不足%d！",x570010_g_ExtraCount)
		else
			return 0;
		end
