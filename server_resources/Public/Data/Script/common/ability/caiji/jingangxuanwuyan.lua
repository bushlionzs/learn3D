...
x540111_g_GrowpointId = 611 --对应生长点ID

x540111_g_AbilitySkillName = "采集矿石" --需要的生活技能名称
x540111_g_AbilitySkillType = 12 --需要的生活技能ID
x540111_g_AbilitySkillLevel = 7 --需要的生活技能ID的等级
--x540111_g_AbilitySkillLimit = 0 --需要的生活技能ID的熟练度
x540111_g_AddSkillLimit = 1 --采集成功获得的熟练度点数
x540111_g_SkillLMax_1 =150 --技能1级时熟练度上限
x540111_g_SkillLMax_2 =200 --技能2级时熟练度上限
x540111_g_SkillLMax_3 =250 --技能3级时熟练度上限
x540111_g_SkillLMax_4 =300	--技能4级时熟练度上限
x540111_g_SkillLMax_5 =350 --技能5级时熟练度上限
x540111_g_SkillLMax_6 =400 --技能6级时熟练度上限
x540111_g_SkillLMax_7 =450 --技能7级时熟练度上限
x540111_g_SkillLMax_8 =500 --技能8级时熟练度上限
x540111_g_SkillLMax = 0 --实际当前熟练度上限

x540111_g_CostVigorLow = 6		--采集技能为需要的最低等级时采集成功一次时扣除的活力点数
x540111_g_CostVigorHigh = 7   --采集技能大于需要的最低等级时采集成功一次时扣除的活力点数
x540111_g_CostVigor = 0       --实际扣除的活力点数

x540111_g_ItemNumLow = 1 --刚够级时获得个数，后面有可能性多获得一个物品
x540111_g_ItemNumHigh = 3 --高一级时获得个数，后面有可能性多获得一个物品
x540111_g_ItemNumOver = 4 --高两级以上时获得个数，后面没有可能多获得物品
x540111_g_ItemNum = 0  --实际获得的物品个数

x540111_g_ItemIndex = 11030007 --对应收集物品的ID
x540111_g_ItemName = "圣徒级矿石" 	--对应收集物品的名称
x540111_g_DoubleItemLow = 900 --有<当前熟练度>/300的可能性多获得一个物品
x540111_g_DoubleItemHigh = 1000 --有<当前熟练度>/400的可能性多获得一个物品

x540111_g_SpecialItemSkillLevel = 7   --可以采集到特殊物品的最低生活技能等级，-1表示始终采集不到特殊物品
x540111_g_SpecialItemLowPencent = 10 --每次采集到特殊物品的概率，-1表示无,最大100
x540111_g_SpecialItemHighPencent = 10  --最大100
x540111_g_SpecialItemIndex = 11030501 --每次采集的特殊物品的ID，-1表示无,>0为有效数值
x540111_g_SpecialItemName = "溶火石"  --每次采集的特殊物品的名称，空表示无
x540111_g_SpecialItemNum = 0 --实际获得的特殊物品个数
...
function	 x540111_OnOpen(sceneId,selfId,targetId)
	...
	if SkillLevel == nil then
		TipsToPlayer(sceneId,selfId,"#Y你还没有学习#R"..x540111_g_AbilitySkillName)
	else
		if SkillLevel <= 0 then
			...
			if SkillLevel >= x540111_g_AbilitySkillLevel then
				...
				TipsToPlayer(sceneId,selfId,"#Y"..x540111_g_AbilitySkillName.." 等级不足#R"..x540111_g_AbilitySkillLevel.."级")
			end
		end
	end
	return 1
				...
				if NowVigor >= x540111_g_CostVigor then
					...
					if ret > 0 then
						...
						if x540111_g_SpecialItemIndex > 0 and x540111_g_SpecialItemNum > 0 then
							TipsToPlayer (sceneId,selfId,"#Y获得#G"..x540111_g_ItemNum.."个#Y"..x540111_g_ItemName.."和#G"..x540111_g_SpecialItemNum.."个#Y"..x540111_g_SpecialItemName)
						else
							TipsToPlayer (sceneId,selfId,"#Y获得#G"..x540111_g_ItemNum.."个#Y"..x540111_g_ItemName)
						end
						TipsToPlayer (sceneId,selfId,"#Y消耗#G"..x540111_g_CostVigor.."点#Y活力")
						if SkillLevel < x540111_g_AbilitySkillLevel + 2 then
							if SkillLevel > 8 then 
								SetAbilityExp(sceneId,selfId,x540111_g_AbilitySkillType,SkillBaseLimit + x540111_g_AddSkillLimit)
								TipsToPlayer (sceneId,selfId,"#Y"..x540111_g_AbilitySkillName.."获得#G"..x540111_g_AddSkillLimit.."点#Y熟练度")
							elseif SkillLevel > 0 and SkillLevel <= 8 then
								...
								elseif SkillBaseLimit + x540111_g_AddSkillLimit > x540111_g_SkillLMax and SkillBaseLimit < x540111_g_SkillLMax then
									...
									TipsToPlayer (sceneId,selfId,"#Y"..x540111_g_AbilitySkillName.."获得#G"..x540111_g_SkillLMax-SkillBaseLimit.."点#Y熟练度")
								else
