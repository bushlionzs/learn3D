...
x585001_g_CostFee								= 55000	--55两金子
x585001_g_MissionName       		= "购买星紫大礼包#R(55两现金)"
x585001_g_NoMoney								= "对不起，您的现金不足，无法购买！"
...
function x585001_GiveBonus( sceneId,selfId )
	...
	if GetGameOpenById( 1028 ) == 0 then
		...
		BeginAddItem(sceneId)
			AddBindItem(sceneId,x585001_g_BonusItem,1)	--12030425 星紫大礼包
		local ret = EndAddItem(sceneId,selfId)
		if ret > 0 then
		
			if x585001_PayMoney(sceneId,selfId,x585001_g_CostFee,308) == 1 then		--308为supersale现金促销ID
		
				BeginQuestEvent(sceneId)
				AddQuestText(sceneId,format("恭喜您获得了礼包道具！扣除了%d两现金！",x585001_g_CostFee/1000))
				...
				BeginQuestEvent(sceneId,selfId)
					AddQuestText(sceneId,x585001_g_NoMoney)	--钱不够
				EndQuestEvent()
				DispatchQuestTips(sceneId,selfId)
			end		
