...
x300665_g_GrowpointId = 109 		--对应生长点ID
x300665_g_ItemIndex = 12010001
x300665_g_BufIndex = 1210

--生成函数开始************************************************************************
function 	x300665_OnCreate(sceneId,growPointType,x,y)
	local ItemBoxId = ItemBoxEnterScene(x, y, x300665_g_GrowpointId, sceneId, 0, x300665_g_ItemIndex)
end
--生成函数结束**********************************************************************


--打开前函数开始&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
function	 x300665_OnOpen(sceneId,selfId,targetId)
    return x300665_OnOpenItemBox(sceneId, selfId, targetId, x300665_g_GrowpointId, x300665_g_ItemIndex)
end
--打开前函数结束&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&


--回收函数开始########################################################################
function	 x300665_OnRecycle(sceneId,selfId,targetId)
	return   x300665_OnGuildRecycle( sceneId, selfId, targetId, x300665_g_GrowpointId, x300665_g_ItemIndex )
end
--回收函数结束########################################################################
...
--打开后函数开始@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
function	x300665_OnProcOver(sceneId,selfId,targetId)
...
function	x300665_OpenCheck(sceneId,selfId,AbilityId,AblityLevel)
...
--采集任务
--打开ItemBox
function x300665_OnOpenItemBox( sceneId, selfId, targetId, gpType, needItemID )
	...
	--得到粮仓当前帮会
	local guild = GetGuildBattleSceneData_Param(sceneId,LD_GUILD_BATTLE_BARN4_GUILD)
...
--回收
function x300665_OnGuildRecycle( sceneId, selfId, targetId, gpType, needItemID )
	...
	SetGuildBattleSceneData_Param( sceneId,LD_GUILD_BATTLE_BARN4_GUILD,GetGuildID(sceneId, selfId) )

	--重设被占领时间
	SetGuildBattleSceneData_Param( sceneId,LD_GUILD_BATTLE_BARN4_TIME,-1 )

    CallScriptFunction( GUILDBATTLE_SCRIPTID,"UpdateSceneDataFlag",sceneId,4 )
