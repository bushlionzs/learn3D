...
x300668_g_GrowpointId = 113 		--对应生长点ID
x300668_g_ItemIndex = 12010001
x300668_g_Flag = 1             		-- 1:B方仓库
x300668_g_BufIndex = 1210

--生成函数开始************************************************************************
function 	x300668_OnCreate(sceneId,growPointType,x,y)
	local ItemBoxId = ItemBoxEnterScene(x, y, x300668_g_GrowpointId, sceneId, 0, x300668_g_ItemIndex)
end
--生成函数结束**********************************************************************


--打开前函数开始&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
function	 x300668_OnOpen(sceneId,selfId,targetId)
    return x300668_OnOpenItemBox(sceneId, selfId, targetId, x300668_g_GrowpointId, x300668_g_ItemIndex)
end
--打开前函数结束&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&


--回收函数开始########################################################################
function	 x300668_OnRecycle(sceneId,selfId,targetId)
	return   x300668_OnGuildRecycle( sceneId, selfId, targetId, x300668_g_GrowpointId, x300668_g_ItemIndex )
end
--回收函数结束########################################################################
...
--打开后函数开始@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
function	x300668_OnProcOver(sceneId,selfId,targetId)
...
function	x300668_OpenCheck(sceneId,selfId,AbilityId,AblityLevel)
	--CallScriptFunction( MISSION_SCRIPT, "OpenCheck", sceneId, selfId, targetId )
end


--**********************************************************************
function x300668_OnOpenItemBox( sceneId, selfId, targetId, gpType, needItemID )
	...
	if nFlag ~= x300668_g_Flag then      --敌方仓库：不可以存储
    	...
    	if IsHaveFlag == 0 then
			...
			--else
			--	SetGuildBattleSceneData_Param( sceneId,LD_GUILD_BATTLE_A_FLAG_NUM+nFlag,LD_GUILD_BATTLE_FLAG_COUNT )
			--end
...
--回收
function x300668_OnGuildRecycle( sceneId, selfId, targetId, gpType, needItemID )
