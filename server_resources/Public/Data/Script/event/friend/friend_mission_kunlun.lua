...
x300723_g_ScriptId = 300723                 --脚本ID
...
function x300723_ProcEventEntry( sceneId, selfId, targetId, MissionId, index )
	...
	if haveDoneMissionId > 0 then
		...
		AddQuestText(sceneId, "\t好友度:"..x300723_g_addFriendPoint.."点")
...
function x300723_DoAccept( sceneId, selfId )
	...
	for i = 0, teamSize - 1 do
		...
		local str = format("您领取了任务:%s", x300723_g_MissionName)
		...
		for j = i + 1, teamSize - 1 do
			...
			--str = format("由于领取任务：%s，你与%s增加了%d好友度", x300723_g_MissionName, bName, x300723_g_addFriendPoint)
			--BeginQuestEvent(sceneId);
			--AddQuestText(sceneId, str);
			--EndQuestEvent();
			--DispatchQuestTips(sceneId, aId);
			--str = format("由于领取任务：%s，你与%s增加了%d好友度", x300723_g_MissionName, aName, x300723_g_addFriendPoint)
			--BeginQuestEvent(sceneId);
			--AddQuestText(sceneId, str);
			--EndQuestEvent();
...
function x300723_ProcQuestSubmit( sceneId, selfId, targetId, selectId, MissionId )
  ...
  if IsHaveQuestNM( sceneId,selfId,haveDoneMissionId )<= 0 then
	...
	for i=x300723_g_ParamIndexBegin, x300723_g_ParamIndexEnd do
			...
			guid = tonumber(format("%u",guid))
			if guid > 0 and GetFriendName(sceneId, selfId, guid) ~= "" then
				local str = format("你与%s增加了%d点好友度", GetFriendName(sceneId, selfId, guid), x300723_g_addFriendPoint)
				strTable[i] = str
				guidTable[i] = guid
...
function x300723_GetMissionHelp(sceneId, selfId)
	...
	if tonumber(6548) == tonumber(subMission) then
		szMissionHelp = "到王国迷宫一层消灭200只@npc_90251后向@npc_129057复命。"
	elseif tonumber(6530) == tonumber(subMission) then
		szMissionHelp = "到王国迷宫二层消灭200只@npc_90253后向@npc_129057复命。"
	elseif tonumber(6531) == tonumber(subMission) then
		szMissionHelp = "到王国迷宫三层消灭200只@npc_90255后向@npc_129057复命。"
	elseif tonumber(6532) == tonumber(subMission) then
		szMissionHelp = "到王国迷宫四层消灭200只@npc_90257后向@npc_129057复命。"
	elseif tonumber(6533) == tonumber(subMission) then
		szMissionHelp = "到王国迷宫五层消灭200只@npc_90259后向@npc_129057复命。"
	elseif tonumber(6534) == tonumber(subMission) then
		szMissionHelp = "到王国迷宫六层消灭200只@npc_90261后向@npc_129057复命。"
	elseif tonumber(6535) == tonumber(subMission) then
		szMissionHelp = "到王国迷宫七层消灭200只@npc_90263后向@npc_129057复命。"
	elseif tonumber(6536) == tonumber(subMission) then
		szMissionHelp = "到王国迷宫八层消灭200只@npc_14004后向@npc_129057复命。"
	elseif tonumber(6537) == tonumber(subMission) then
		szMissionHelp = "到王国迷宫九层消灭200只@npc_14004后向@npc_129057复命。"
	end
...
function x300723_GetMissionTarget(sceneId, selfId)
	...
	if tonumber(6548) == tonumber(subMission) then
		szMissionTarget = "  除掉200只@npc_90251" .. szMonsterNum
	elseif tonumber(6530) == tonumber(subMission) then
		szMissionTarget = "  除掉200只@npc_90253" .. szMonsterNum
	elseif tonumber(6531) == tonumber(subMission) then
		szMissionTarget = "  除掉200只@npc_90255" .. szMonsterNum
	elseif tonumber(6532) == tonumber(subMission) then
		szMissionTarget = "  除掉200只@npc_90257" .. szMonsterNum
	elseif tonumber(6533) == tonumber(subMission) then
		szMissionTarget = "  除掉200只@npc_90259" .. szMonsterNum
	elseif tonumber(6534) == tonumber(subMission) then
		szMissionTarget = "  除掉200只@npc_90261" .. szMonsterNum
	elseif tonumber(6535) == tonumber(subMission) then
		szMissionTarget = "  除掉200只@npc_90263" .. szMonsterNum
	elseif tonumber(6536) == tonumber(subMission) then
...
function x300723_QuestLogRefresh( sceneId, selfId, MissionId)
	...
	local exp = x300723_CalcMissionEXP(sceneId, selfId)
	BeginQuestEvent(sceneId)
		local szMissionBonus = "\n#Y\n奖励内容：\n#W好友度:"..x300723_g_addFriendPoint.."点";
		local szMissionExp = "\n#W经验值:"..exp.."点";
	AddQuestLogCustomText( sceneId,
							"标题",						-- 标题
							x300723_g_MissionName,        -- 任务名字
