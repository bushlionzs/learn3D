...
x203401_g_Name	={}
x203401_g_ExtTarget={{type=20,n=1,target="找到@npc_125016"}}
x203401_g_MissionName="【新手】白鹿长老"
x203401_g_MissionInfo="\t获得新兵器之后，记得按下“B”键打开背包，右键点击你刚刚获得的武器，就可以装备上新武器了。\n\t白鹿长老正在西北方等待着你的到来。"  --任务描述
x203401_g_MissionTarget="找到@npc_125016"		
x203401_g_MissionCompleted="\t真没想到你这么快就来了！\n\t看来你在前面两位长老那里得到了不少收获啊。 "					--完成任务npc说话的话
x203401_g_ContinueInfo=""
x203401_g_MissionHelp =	"#G按下“B”键打开背包，右键点击就可以装备你刚刚获得的武器。#W"
...
x203401_g_NpcGUID ={{ guid = 125016, name = "白鹿长老"} }
--MisDescEnd     
...
function x203401_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	local zhiye = GetZhiye( sceneId, selfId)+1
	if IsQuestHaveDone(sceneId, selfId, x203401_g_MissionId[zhiye]) > 0 then
		return 
	end
		...
		if x203401_ProcAcceptCheck(sceneId, selfId, NPCId) > 0 then
					...
					if retmiss ==0 then                                          ------添加失败 ==0 ，1 成功                                                                  
		  			...
		  			x203401_Msg2toplayer( sceneId, selfId,0)
						--if x203401_CheckSubmit(sceneId, selfId, NPCId) > 0 then
								local misIndex = GetQuestIndexByID(sceneId,selfId,x203401_g_MissionId[zhiye])       ---- 取得任务状态                                               
...
function x203401_Msg2toplayer( sceneId, selfId,type)
		local zhiye = GetZhiye( sceneId, selfId)+1
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x203401_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x203401_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203401_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203401_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x203401_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x203401_g_MissionName.."！", 0, 3)
				if x203401_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G经验"..x203401_g_ExpBonus.."#cffcf00的奖励", 0, 2)
  			end
				if x203401_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#cffcf00获得#G银卡"..x203401_g_BonusMoney1.."文#cffcf00的奖励", 0, 2)
	  		end
				if x203401_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#cffcf00获得#G现银"..x203401_g_BonusMoney2.."文#cffcf00的奖励", 0, 2)
	  		end
				if x203401_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#cffcf00获得#G金卡"..x203401_g_BonusMoney3.."文#cffcf00的奖励", 0, 2)
				end
				if x203401_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G朝廷声望值"..x203401_g_BonusMoney4.."#cffcf00的奖励", 0, 2)
				end
				if x203401_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G声望"..x203401_g_BonusMoney5.."#cffcf00的奖励", 0, 2)
				end
				if x203401_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G荣誉"..x203401_g_BonusMoney6.."#cffcf00的奖励", 0, 2)
				end
		end
end
...
function x203401_ProcQuestAttach( sceneId, selfId, npcId, npcGuid, misIndex, MissionId )
    ...
    if bFind == 0 then
		...
		if IsHaveQuest(sceneId,selfId, x203401_g_MissionId[zhiye]) > 0 then
				local state = GetQuestStateNM(sceneId,selfId,npcId,x203401_g_MissionId[zhiye])        ---取得任务状态 ，根据状态添加按钮 AddQuestTextNM
				AddQuestTextNM( sceneId, selfId, npcId, x203401_g_MissionId[zhiye], state, -1 )
		end
end
