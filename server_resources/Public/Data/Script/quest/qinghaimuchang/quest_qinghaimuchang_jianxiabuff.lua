...
x203725_g_MissionIdPre =3498
x203725_g_MissionId = 3463
x203725_g_LevelLess	= 	1
x203725_g_MissionIdNext = 3475
x203725_g_Name	={}
x203725_g_ExtTarget={{type=20,n=1,target="找到@npc_129045学习并使用2级七星剑阵技能"}}
x203725_g_MissionName="【新手】职业的进阶" 
x203725_g_MissionInfo="\t你已经熟悉了生存的必要条件和王城周围的环境，现在你需要加强的就是你的自身实力了，王城的技能大师可以帮你快速地提升自身的能力。\n\t现在，你可以去找他了。"  --任务描述
x203725_g_MissionTarget=""		
x203725_g_MissionCompleted="\t我就是这里的技能大师，如果你愿意，我将是你终生的导师。\n\t活到老学到老，等你觉得自己杀敌变得缓慢的时候记得来找我学习新的技能。"					--完成任务npc说话的话
x203725_g_ContinueInfo="\t不清楚如何提升技能等级？在我这里就可以学习、提升剑侠的技能。\n\t点击#G剑侠技能学习#W按钮就能进入技能学习界面，里面#G明亮的技能#W表示你现在能掌握的技能，点击相应图标即可学习技能，当然你要有一定的经验和银子！\n\t最后记得学习完技能后，要将自己#G技能栏内技能的图标#W拖到下方#G快捷栏内#W，才能在需要的时候，方便快捷的使用。"
x203725_g_MissionHelp =	"#G您可以按下键盘上的#R2#G键，或者左键点击您下方技能快捷栏中的#R七星剑阵#G技能。#W"
x203725_g_DemandItem ={}
x203725_g_ExpBonus = 855
x203725_g_BonusMoney1 =132
x203725_g_BonusMoney2 =0
x203725_g_BonusMoney3 =0
x203725_g_BonusMoney4 =0
x203725_g_BonusMoney5 =20
x203725_g_BonusMoney6 =0
x203725_g_BonusItem	=	{}
x203725_g_BonusChoiceItem ={}
x203725_g_NpcGUID ={{ guid = 129045, name = "剑侠技能大师"} } --人物回复人
--MisDescEnd
--**********************************
...
function x203725_ProcEnumEvent(sceneId, selfId, NPCId, MissionId)
	...
	if IsQuestHaveDone(sceneId, selfId, x203725_g_MissionId) > 0 then
		return 
	end
	if IsQuestHaveDone(sceneId, selfId, x203725_g_MissionIdPre)> 0 then
		if GetZhiye( sceneId, selfId)==1 then
			local state = GetQuestStateNM(sceneId,selfId,NPCId,x203725_g_MissionId)
			AddQuestTextNM( sceneId, selfId, NPCId, x203725_g_MissionId, state, -1 )
		end
	end
end
...
function x203725_DispatchMissionInfo( sceneId, selfId, NPCId )
		...
		for i, item in x203725_g_BonusItem do
...
function x203725_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	...
	if IsHaveQuest(sceneId,selfId, x203725_g_MissionId) > 0 then
			if x203725_CheckSubmit(sceneId, selfId, NPCId) <= 0 then
						...
						EndQuestEvent()
						DispatchQuestEventList(sceneId, selfId, NPCId, x203725_g_ScriptId, x203725_g_MissionId);
			else
						BeginQuestEvent(sceneId)
						AddQuestText(sceneId,"#Y"..x203725_g_MissionName)
						...
						EndQuestEvent()
						DispatchQuestContinueInfoNM(sceneId, selfId, NPCId, x203725_g_ScriptId, x203725_g_MissionId);
			end
  elseif x203725_ProcAcceptCheck(sceneId, selfId, NPCId) > 0 then
	  	...
	  	AddQuestText(sceneId,"#Y"..x203725_g_MissionName)
			...
			for i, item in x203725_g_ExtTarget do
			...
			x203725_DispatchMissionInfo( sceneId, selfId, NPCId )
	  	EndQuestEvent()
			DispatchQuestInfoNM(sceneId, selfId, NPCId, x203725_g_ScriptId, x203725_g_MissionId);
  end
...
function x203725_CheckSubmit( sceneId, selfId, NPCId)
	local misIndex = GetQuestIndexByID( sceneId, selfId, x203725_g_MissionId )
...
function x203725_ProcQuestAccept( sceneId, selfId, NPCId, MissionId )
		...
		--检查前置任务
		local FrontMissiontId1, FrontMissiontId2, FrontMissiontId3 = GetFrontQuestIdNM( sceneId, selfId,x203725_g_MissionId  )
		...
		if x203725_ProcAcceptCheck(sceneId, selfId, NPCId) > 0 then
					local retmiss = AddQuest( sceneId, selfId, x203725_g_MissionId, x203725_g_ScriptId, 0, 0, 0, 1)
					if retmiss ==0 then
...
function x203725_ProcQuestAbandon(sceneId, selfId, MissionId)
		...
		DelQuest(sceneId, selfId, x203725_g_MissionId)
...
function x203725_CheckPlayerBagFull( sceneId ,selfId,selectRadioId )
	...
	for j, item in x203725_g_BonusItem do
...
function x203725_ProcQuestSubmit(sceneId, selfId, NPCId, selectRadioId, MissionId)
	...
	elseif x203725_CheckPlayerBagFull( sceneId ,selfId,selectRadioId ) <= 0 then
				...
				DispatchQuestTips(sceneId,selfId)
				return 0
	elseif DelQuest(sceneId, selfId, x203725_g_MissionId) > 0 then
				x203725_Msg2toplayer( sceneId, selfId,2)
				QuestCom(sceneId, selfId, x203725_g_MissionId)
...
local bFind  = 0
    for i ,item in x203725_g_NpcGUID do
