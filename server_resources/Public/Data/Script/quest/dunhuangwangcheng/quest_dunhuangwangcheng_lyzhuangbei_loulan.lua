...
x256260_g_Name	={}
x256260_g_ExtTarget={{type=20,n=1,target="兑换一件世传帽子"}}
x256260_g_MissionName="【个人】龙眼石兑换"
x256260_g_MissionInfo="\t亲爱的朋友，在我的手中有很多更高级的世传君主套装，换上这些套装你的能力会有更多的提升。你手中如果有足够的龙眼石，不妨考虑兑换一件。\n\t点击龙眼石兑换后，会打开龙眼石兑换商店。左键点击选择兑换物品（#G建议兑换世传帽子#W），在龙眼石数量足够的情况下点击兑换，兑换就会成功。"  --任务描述
x256260_g_MissionTarget=""		
x256260_g_MissionCompleted="\t这件装备必然对你日后成长和能力提高起到不可低估的作用。\n\t一定要记住，日后再获得龙眼石还可以来我这里兑换装备。"					--完成任务npc说话的话
x256260_g_ContinueInfo="\t你不想兑换一件世传帽子吗？"
...
function x256260_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x256260_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x256260_g_MissionId) > 0 then
...
function x256260_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务："..x256260_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务："..x256260_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务："..x256260_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务："..x256260_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x256260_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x256260_g_MissionName.."！", 0, 3)
				if x256260_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x256260_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x256260_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x256260_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x256260_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x256260_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x256260_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x256260_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x256260_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x256260_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x256260_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x256260_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x256260_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x256260_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
