...
x256268_g_MissionName="【个人】国家使命"
x256268_g_MissionInfo="\t我等你很久了，到了你这样的修为的人，应该开始为国家尽一份力了。\n\t找到@npc_125500，@npc_125501，@npc_125502，@npc_125503完成他们所交代的任务，你会获得丰厚的奖励。"  --任务描述
x256268_g_MissionTarget=""		
x256268_g_MissionCompleted="\t这等为国效力的事，乃是我等臣民应尽之责。\n\t记得每天都去他们那里完成任务，臣民百姓会记得你，你的奖励也不会少。"					--完成任务npc说话的话
x256268_g_ContinueInfo="\t失败不可怕，可怕的是你自此丧失斗志。"
...
x256268_g_NpcGUID ={{ guid = 125543, name = "哥舒沙鸣"} }
--MisDescEnd
--**********************************
-- 异国刺杀完成标记
...
function x256268_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x256268_g_MissionId) > 0 then
		return
	end
	if IsHaveQuest(sceneId,selfId, x256268_g_MissionId) > 0 then
...
function x256268_Msg2toplayer( sceneId, selfId,type)
    if type == 0 then
        Msg2Player(sceneId, selfId, "您接受了任务："..x256268_g_MissionName.."！", 0, 2)
        Msg2Player(sceneId, selfId, "您接受了任务："..x256268_g_MissionName.."！", 0, 3)
    elseif type == 1 then
        Msg2Player(sceneId, selfId, "您放弃了任务："..x256268_g_MissionName.."！", 0, 2)
        Msg2Player(sceneId, selfId, "您放弃了任务："..x256268_g_MissionName.."！", 0, 3)
    elseif type == 2 then
        Msg2Player(sceneId, selfId, "您完成了任务"..x256268_g_MissionName.."！", 0, 2)
        Msg2Player(sceneId, selfId, "您完成了任务"..x256268_g_MissionName.."！", 0, 3)      
        if x256268_g_ExpBonus > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R经验"..x256268_g_ExpBonus.."#Y的奖励", 0, 2)
        end
        if x256268_g_BonusMoney1 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x256268_g_BonusMoney1.."文#Y的奖励", 0, 2)
        end
        if x256268_g_BonusMoney2 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R现银"..x256268_g_BonusMoney2.."文#Y的奖励", 0, 2)
        end
        if x256268_g_BonusMoney3 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x256268_g_BonusMoney3.."文#Y的奖励", 0, 2)
        end
        if x256268_g_BonusMoney4 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x256268_g_BonusMoney4.."#Y的奖励", 0, 2)
        end
        if x256268_g_BonusMoney5 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R声望"..x256268_g_BonusMoney5.."#Y的奖励", 0, 2)
        end
        if x256268_g_BonusMoney6 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x256268_g_BonusMoney6.."#Y的奖励", 0, 2)
        end
    end
end
...
function x256268_ProcQuestLogRefresh( sceneId, selfId, MissionId)
    ...
    local strTarget = format( "回报@npc_%d", x256268_g_NpcGUID[1].guid)
