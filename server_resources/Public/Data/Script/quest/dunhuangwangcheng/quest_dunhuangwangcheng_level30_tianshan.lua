...
x256285_g_Name	={}
x256285_g_ExtTarget={{type=20,n=1,target="按任务攻略的指引升级到30级"}}
x256285_g_MissionName="【个人】三十则立"
x256285_g_MissionInfo="\t你好，亲爱的朋友，我这里有些你应该知道的事情：\n\t@npc_126041,你#G每天#W都可以通过帮助他，获得高额的经验回报。\n\t组队参加@npcsp_三环副本_126040也是快速升级的途径。当然，之前别忘记去@npc_126028领取双倍经验。\n\t公车发放人，会在每天的#G13：00-14：00，18：30-19：15#W放出神兽玄武供人乘坐。\n\t@npc_126058,每天的#G12：30-13：00，19：30-20：00#W会发布天降宝箱任务。"  --任务描述
x256285_g_MissionTarget=""		
x256285_g_MissionCompleted="\t果然是自古英雄出少年，三十级是一个不错的成就，但也是新的开始。\n\t我这里为你准备了一些小礼物，希望能对你的历程有所帮助。"					--完成任务npc说话的话
x256285_g_ContinueInfo="别太心急，我为你的三十级准备了不少礼物，但是......你应该先到三十级再来。"
...
x256285_g_NpcGUID ={{ guid = 126002, name = "万事通"} }
--MisDescEnd
--**********************************
-- 最大等级
...
function x256285_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x256285_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x256285_g_MissionId) > 0 then
...
function x256285_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x256285_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x256285_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x256285_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x256285_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x256285_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x256285_g_MissionName.."！", 0, 3)
				if x256285_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x256285_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x256285_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x256285_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x256285_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x256285_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x256285_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x256285_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x256285_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x256285_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x256285_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x256285_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x256285_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x256285_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
