...
x256297_g_Name	={}
x256297_g_ExtTarget={{type=20,n=1,target="在金币商店贵宾区一星麒麟商店购买一个VIP募集券交给帮会管理员"}}
x256297_g_MissionName="【个人】第一次帮会募集"
x256297_g_MissionInfo="\t经验和帮贡有一个很重要也很简单的获得方式：帮会募集。我想，你首先应当熟悉一下帮会募集的流程。在#G金币商店贵宾区一星麒麟商店#W购买一个#GVIP募集券#W交给我吧"  --任务描述
x256297_g_MissionTarget=""		
x256297_g_MissionCompleted="\t很好，虽然这#GVIP募集券#W只能上交一次，但以后你每天都可以在普通的金币商店购买普通的#G募集券#W交给我。"					--完成任务npc说话的话
x256297_g_ContinueInfo="\t怎么？还没有拿来#GVIP募集券#W吗？"
x256297_g_MissionHelp =	"VIP募集券在金币商店贵宾区一星麒麟商店可以购买，但需要您的充值达到一定的金额，才能获得其中购买相应物品的权限。每个帮会每天最多接受50个VIP募集券。"
...
x256297_g_NpcGUID ={{ guid = 129022, name = "帮会管理员"} }
--MisDescEnd
-- 最大等级
...
function x256297_ProcEnumEvent(sceneId, selfId, NPCId, MissionId)
	...
	if IsQuestHaveDone(sceneId, selfId, x256297_g_MissionId) > 0 then --已完成过任务
		return 
	end
...
function x256297_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
...
function x256297_GetBonus( sceneId, selfId,NpcID)
	  if x256297_g_ExpBonus > 0 then
		...
		if x256297_g_BonusMoney8 > 0 then
		AddGuildUserPoint(sceneId,selfId,x256297_g_BonusMoney8)	--增加帮贡
...
function x256297_Msg2toplayer( sceneId, selfId,type)
			if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x256297_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x256297_g_MissionName.."！", 0, 3)
			elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x256297_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x256297_g_MissionName.."！", 0, 3)
			elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x256297_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x256297_g_MissionName.."！", 0, 3)
			if x256297_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x256297_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
			if x256297_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x256297_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
			if x256297_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x256297_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
			if x256297_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x256297_g_BonusMoney3.."文#Y的奖励", 0, 2)
			end
			if x256297_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x256297_g_BonusMoney4.."#Y的奖励", 0, 2)
			end
			if x256297_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x256297_g_BonusMoney5.."#Y的奖励", 0, 2)
			end
			if x256297_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x256297_g_BonusMoney6.."#Y的奖励", 0, 2)
			end
		end
end
