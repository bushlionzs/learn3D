...
x270002_g_Name	={}
x270002_g_ExtTarget={{type=20,n=1,target="到@npc_139827处参加点燃圣火仪式"}}
x270002_g_MissionName="【主线】熊熊圣火"
x270002_g_MissionInfo="\t#Y@myname#W大侠，在你的帮助下，圣火令失而复得，我教上下对你感激不尽。阳教主马上就重燃圣火，让圣火照亮世间，永远指引我们明教！"  --任务描述
x270002_g_MissionTarget=""		
x270002_g_MissionCompleted="\t嗯，明教行事一向怪异，我以后还是少掺和他们的事情好了！"					--完成任务npc说话的话
x270002_g_ContinueInfo="\t听说明教点燃圣火的仪式很特别，你见识过了么？"
...
x270002_g_NpcGUID ={{ guid = 139827, name = "黛猗丝"}, { guid = 140417, name = "包打听"}}
--MisDescEnd
--**********************************
...
function x270002_ProcEventEntry(sceneId, selfId, NPCId, MissionId,which)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x270002_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x270002_g_MissionId) > 0 then
	    ...
	    BeginQuestEvent(sceneId)
			AddQuestText(sceneId,format("参观点燃圣火仪式   %d/1", GetQuestParam(sceneId,selfId,misIndex,0) ))
			EndQuestEvent(sceneId)
			DispatchQuestTips(sceneId,selfId)
			BeginQuestEvent(sceneId)
			AddQuestText(sceneId,"我听说了你所做的事情，我们明教会永远记得你的恩惠的！")
			EndQuestEvent()
			DispatchQuestEventList(sceneId, selfId, NPCId, x270002_g_ScriptId, x270002_g_MissionId);
	    if x270002_g_doing == 0 then
	    		x270002_g_PROTECTINFO.StartTime = GetCurrentTime()
					...
					if x270002_g_PROTECTINFO.ScneneTimerIndex >=0 then
							...
							x270002_g_xiexun=CreateMonster(sceneId, 18,97,252,3,1,-1,-1,20,55000,180,"谢逊","金毛狮王")
							x270002_g_yintianzheng=CreateMonster(sceneId, 419,98,251,3,1,-1,-1,20,55000,0,"殷天正","白眉鹰王")
							x270002_g_weiyixiao=CreateMonster(sceneId, 18,99,252,3,1,-1,-1,20,55000,0,"韦一笑","青翼蝠王")
							...
							if IsObjValid (sceneId,x270002_g_daiyisi) == 1 then
									NpcTalk(sceneId, x270002_g_daiyisi, "圣火仪式现在开始！", -1)
							end
          end
	    end
	  end
...
function x270002_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x270002_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x270002_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x270002_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x270002_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x270002_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x270002_g_MissionName.."！", 0, 3)
				if x270002_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x270002_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x270002_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x270002_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x270002_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x270002_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x270002_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x270002_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x270002_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x270002_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x270002_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x270002_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x270002_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x270002_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
		...
		if IsHaveQuest(sceneId,selfId, x270002_g_MissionId) > 0 then
				if npcGuid == x270002_g_NpcGUID[1].guid then
					...
					if GetQuestParam(sceneId,selfId,misIndex,0) ==0 then 
						AddQuestNumText(sceneId, x270002_g_MissionId, "教主马上来举行点燃圣火仪式",0,1)
					end
...
function x270002_OnScneneTimer(sceneId)
	if x270002_g_doing==1   then
		if GetCurrentTime() - x270002_g_PROTECTINFO.StartTime >= 7   then
						x270002_g_doing = 2
						if IsObjValid (sceneId,x270002_g_yintianzheng) == 1 then
								NpcTalk(sceneId, x270002_g_yintianzheng, "我就知道我教会平安渡过这次难关的。", -1)
						end
		end
	elseif x270002_g_doing==2   then
		if GetCurrentTime() - x270002_g_PROTECTINFO.StartTime >= 10   then
						x270002_g_doing = 3
						if IsObjValid (sceneId,x270002_g_weiyixiao) == 1 then
								NpcTalk(sceneId, x270002_g_weiyixiao, "这次多亏了有这位少侠出手相助。", -1)
						end
		end
	elseif x270002_g_doing==3   then
		if GetCurrentTime() - x270002_g_PROTECTINFO.StartTime >= 13   then
						x270002_g_doing = 4
						if IsObjValid (sceneId,x270002_g_yintianzheng) == 1 then
								NpcTalk(sceneId, x270002_g_yintianzheng, "恩，能遇到少侠真乃我教之幸事！", -1)
						end
            x270002_g_mingjiaodizi=CreateMonster(sceneId, 1610,96,252.7,3,1,-1,-1,20,42000,180,"明教弟子")
            x270002_g_mingjiaodizi2=CreateMonster(sceneId, 1610,97,253,3,1,-1,-1,20,42000,180,"明教弟子")
            x270002_g_mingjiaodizi3=CreateMonster(sceneId, 1610,97,254,3,1,-1,-1,20,42000,180,"明教弟子")
            x270002_g_mingjiaodizi4=CreateMonster(sceneId, 1610,103,252,3,1,-1,-1,20,42000,180,"明教弟子")
            x270002_g_mingjiaodizi5=CreateMonster(sceneId, 1610,102,252,3,1,-1,-1,20,42000,180,"明教弟子")
            x270002_g_mingjiaodizi6=CreateMonster(sceneId, 1610,103,252,3,1,-1,-1,20,42000,180,"明教弟子")
	...
	elseif x270002_g_doing==4   then
		if GetCurrentTime() - x270002_g_PROTECTINFO.StartTime >= 19   then
						x270002_g_doing = 5
						if IsObjValid (sceneId,x270002_g_daiyisi) == 1 then
								NpcTalk(sceneId, x270002_g_daiyisi, "请阳教主入场点燃圣火！", -1)
						end
		end
	elseif x270002_g_doing==5   then
		if GetCurrentTime() - x270002_g_PROTECTINFO.StartTime >= 21   then
            ...
            x270002_g_yangdingtian=CreateMonster(sceneId, 1616,101,255,3,1,-1,-1,20,34000,180,"阳顶天")
	...
	elseif x270002_g_doing==10   then
		if GetCurrentTime() - x270002_g_PROTECTINFO.StartTime >= 43   then
						x270002_g_doing = 11
						if IsObjValid (sceneId,x270002_g_yangdingtian) == 1 then
								NpcTalk(sceneId, x270002_g_yangdingtian, "熊熊圣火，焚我残躯！怜我世人，忧患实多！", -1)
						end
		end
	elseif x270002_g_doing==11   then
		if GetCurrentTime() - x270002_g_PROTECTINFO.StartTime >= 47   then
						x270002_g_doing = 12
						if IsObjValid (sceneId,x270002_g_yangdingtian) == 1 then
								NpcTalk(sceneId, x270002_g_yangdingtian, "生亦何欢，死亦何苦？为善除恶，惟我明教。", -1)
						end
						if IsObjValid (sceneId,x270002_g_yintianzheng) == 1 then
								NpcTalk(sceneId, x270002_g_yintianzheng, "生亦何欢，死亦何苦？为善除恶，惟我明教。", -1)
						end
						if IsObjValid (sceneId,x270002_g_weiyixiao) == 1 then
								NpcTalk(sceneId, x270002_g_weiyixiao, "生亦何欢，死亦何苦？为善除恶，惟我明教。", -1)
						end
						if IsObjValid (sceneId,x270002_g_xiexun) == 1 then
								NpcTalk(sceneId, x270002_g_xiexun, "生亦何欢，死亦何苦？为善除恶，惟我明教。", -1)
						end
						if IsObjValid (sceneId,x270002_g_mingjiaodizi) == 1 then
								NpcTalk(sceneId, x270002_g_mingjiaodizi, "生亦何欢，死亦何苦？为善除恶，惟我明教。", -1)
						end
						if IsObjValid (sceneId,x270002_g_mingjiaodizi2) == 1 then
								NpcTalk(sceneId, x270002_g_mingjiaodizi2, "生亦何欢，死亦何苦？为善除恶，惟我明教。", -1)
						end
						if IsObjValid (sceneId,x270002_g_mingjiaodizi3) == 1 then
								NpcTalk(sceneId, x270002_g_mingjiaodizi3, "生亦何欢，死亦何苦？为善除恶，惟我明教。", -1)
						end
						if IsObjValid (sceneId,x270002_g_mingjiaodizi4) == 1 then
								NpcTalk(sceneId, x270002_g_mingjiaodizi4, "生亦何欢，死亦何苦？为善除恶，惟我明教。", -1)
						end
						if IsObjValid (sceneId,x270002_g_mingjiaodizi5) == 1 then
								NpcTalk(sceneId, x270002_g_mingjiaodizi5, "生亦何欢，死亦何苦？为善除恶，惟我明教。", -1)
						end
						if IsObjValid (sceneId,x270002_g_mingjiaodizi6) == 1 then
								NpcTalk(sceneId, x270002_g_mingjiaodizi6, "生亦何欢，死亦何苦？为善除恶，惟我明教。", -1)
						end
		end
