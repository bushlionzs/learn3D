...
x270009_g_Name	={}
x270009_g_ExtTarget={{type=20,n=1,target="探听清楚明教的状况"}}
x270009_g_MissionName="【主线】明教惊魂<二>"
x270009_g_MissionInfo="\t罢罢罢，世人笑我太痴颠，我笑世人看不穿！拿好，这便是布袋和尚给你备下的厚礼！"  --任务描述
x270009_g_MissionTarget=""		
x270009_g_MissionCompleted="\t#Y@myname#W大侠，阳某人误会了你，真是惭愧，惭愧！请大侠安神定绪，稍事休息，杨某还有要事相求。"					--完成任务npc说话的话
x270009_g_ContinueInfo="\t这些波斯人，莫不是吃了熊心豹子胆，居然敢闯入我们这里撒野？"
...
x270009_g_NpcGUID ={{ guid = 139806, name = "阳顶天"} }
--MisDescEnd
--**********************************
...
function x270009_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x270009_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x270009_g_MissionId) > 0 then
				...
				if x270009_g_doing == 0 then
					...
					if retmiss ==0 then
		  			...
		  			x270009_g_PROTECTINFO.StartTime = GetCurrentTime()
						...
						x270009_g_kanshou=CreateMonster(sceneId, 1610,24,313,3,1,-1,-1,20,-1,180,"明教弟子")
						x270009_g_kanshou2=CreateMonster(sceneId, 1610,25,309,3,1,-1,-1,20,-1,0,"明教弟子")
...
function x270009_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x270009_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x270009_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x270009_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x270009_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x270009_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x270009_g_MissionName.."！", 0, 3)
				if x270009_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x270009_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x270009_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x270009_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x270009_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x270009_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x270009_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x270009_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x270009_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x270009_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x270009_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x270009_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x270009_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x270009_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
...
function x270009_OnScneneTimer(sceneId)
	if x270009_g_doing==1   then
		if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 2   then
						x270009_g_doing = 2
						NpcTalk(sceneId, x270009_g_budaiheshang, "波斯人的走狗，拿个破盒子来试探我们你以为我们看不出来吗？", -1)
            LuaScenceM2Near(sceneId, x270009_g_acceptid,"布袋和尚：波斯人的走狗，拿个破盒子来试探我们你以为我们看不出来吗？",0,-1)
		end
	elseif x270009_g_doing==2   then
		if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 5   then
						x270009_g_doing = 3
						NpcTalk(sceneId, x270009_g_kanshou, "小子，你一进门就发现你不对劲！", -1)
            LuaScenceM2Near(sceneId, x270009_g_acceptid,"明教弟子：小子，你一进门就发现你不对劲！",0,-1)
		end
	elseif x270009_g_doing == 3   then
			if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 7   then
						NpcTalk(sceneId, x270009_g_kanshou2, "是啊，还敢过来领赏，老子倒要赏你个死法，让你选！", -1)
            LuaScenceM2Near(sceneId, x270009_g_acceptid,"明教弟子：是啊，还敢过来领赏，老子倒要赏你个死法，让你选！",0,-1)
        		x270009_g_doing =4
			end
	elseif x270009_g_doing == 4   then
			if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 10   then
						LuaScenceM2Near(sceneId, x270009_g_acceptid,"明教弟子：波斯明教进攻了，大家快来！",0,-1)
	...
	elseif x270009_g_doing == 5   then
			if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 13   then
						NpcTalk(sceneId, x270009_g_budaiheshang, "什么，这么快就来了？你们快去！", -1)
            LuaScenceM2Near(sceneId,  x270009_g_acceptid,"布袋和尚：什么，这么快就来了？你们快去！",0,-1)
						x270009_g_doing =6
			end
	elseif x270009_g_doing == 6   then
			if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 16   then
						x270009_g_doing =7
						NpcTalk(sceneId, x270009_g_kanshou2, "是！", -1)
						NpcTalk(sceneId, x270009_g_kanshou, "是！", -1)
            LuaScenceM2Near(sceneId, x270009_g_acceptid,"明教弟子：是",0,-1)
	...
	elseif x270009_g_doing == 7   then
			if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 18   then
						x270009_g_doing =8
						NpcTalk(sceneId, x270009_g_budaiheshang, "哼！以为我们中毒就对付不了你们了吗？", -1)
            LuaScenceM2Near(sceneId, x270009_g_acceptid,"布袋和尚：哼！以为我们中毒就对付不了你们了吗？",0,-1)
			end
	elseif x270009_g_doing == 8   then
			if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 21   then
						x270009_g_doing =9
						NpcTalk(sceneId, x270009_g_budaiheshang, "走，我带你去见你的主子去！", -1)
            LuaScenceM2Near(sceneId, x270009_g_acceptid,"布袋和尚：走，我带你去见你的主子去！",0,-1)
            SendSpecificImpactToUnit(sceneId, x270009_g_budaiheshang, x270009_g_budaiheshang, x270009_g_budaiheshang, 1281, 0);
            x270009_g_suduona=CreateMonster(sceneId, 1608,67.4494,258.27,3,1,-1,-1,20,-1,0,"苏多娜","波斯明教")
	...
	elseif x270009_g_doing == 10   then
			if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 25   then
						x270009_g_doing =11
						NpcTalk(sceneId, x270009_g_suduona, "小女子苏多娜给阳教主请安！", -1)
            LuaScenceM2Near(sceneId, x270009_g_acceptid,"苏多娜：小女子苏多娜给阳教主请安！",0,-1)
			end
	elseif x270009_g_doing == 11   then
			if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 28   then
						x270009_g_doing =12
						NpcTalk(sceneId, x270009_g_yangdingtian, "废话少说，你们擅闯明教圣地，所为何事？", -1)
            LuaScenceM2Near(sceneId, x270009_g_acceptid,"阳顶天：废话少说，你们擅闯明教圣地，所为何事？",0,-1)
			end
	elseif x270009_g_doing == 12   then
			if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 31   then
						x270009_g_doing =13
						NpcTalk(sceneId, x270009_g_suduona, "好，教主如此爽快，我就不绕弯子了。我们特地来收回我教圣物“圣火令”的！", -1)
            LuaScenceM2Near(sceneId, x270009_g_acceptid,"苏多娜：好，教主如此爽快，我就不绕弯子了。我们特地来收回我教圣物“圣火令”的！",0,-1)
			end
	elseif x270009_g_doing == 13   then
			if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 34   then
						x270009_g_doing =14
						NpcTalk(sceneId, x270009_g_suduona, "我们拿到圣火令打开它却发现缺少了一样东西，我想这东西一定在阳教主身上。", -1)
            LuaScenceM2Near(sceneId, x270009_g_acceptid,"苏多娜：我们拿到圣火令打开它却发现缺少了一样东西,我想这东西一定在阳教主身上。",0,-1)
			end
	elseif x270009_g_doing == 14   then
			if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 37   then
						x270009_g_doing =15
						NpcTalk(sceneId, x270009_g_yangdingtian, "什么？这么说本教失踪的圣火令是被你等拿走了，那休怪阳某人不客气了。", -1)
            LuaScenceM2Near(sceneId, x270009_g_acceptid,"阳顶天：什么？这么说本教失踪的圣火令是被你等拿走了，那休怪阳某人不客气了。",0,-1)
			end
	elseif x270009_g_doing == 15   then
			if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 40   then
						x270009_g_doing =16
						NpcTalk(sceneId, x270009_g_budaiheshang, "臭娘们，来了你就别想走了。留在这里让我破破戒！", -1)
            LuaScenceM2Near(sceneId, x270009_g_acceptid,"布袋和尚：臭娘们，来了你就别想走了。留在这里让我破破戒！",0,-1)
			end
	elseif x270009_g_doing == 16   then
			if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 43   then
						x270009_g_doing =17
						NpcTalk(sceneId, x270009_g_suduona, "哼，还敢嘴硬！你们中毒已深，现在恐怕连站都站不稳了吧。", -1)
            LuaScenceM2Near(sceneId, x270009_g_acceptid,"苏多娜：哼，还敢嘴硬！你们中毒已深，现在恐怕连站都站不稳了吧。",0,-1)
			end
	elseif x270009_g_doing == 17   then
			if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 46   then
						x270009_g_doing =18
						NpcTalk(sceneId, x270009_g_yangdingtian, "是吗？你这么确认？", -1)
            LuaScenceM2Near(sceneId, x270009_g_acceptid,"阳顶天：是吗？你这么确认？",0,-1)
			end
	...
	elseif x270009_g_doing == 19   then
			if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 50   then
						x270009_g_doing =20
						NpcTalk(sceneId, x270009_g_suduona, "什么？难道毒还没有发作？", -1)
            LuaScenceM2Near(sceneId, x270009_g_acceptid,"苏多娜：什么？难道毒还没有发作？",0,-1)
			end
	elseif x270009_g_doing == 20   then
			if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 53   then
						x270009_g_doing =21
						NpcTalk(sceneId, x270009_g_suduona, "小子，原来你是明教的人！居然被你给骗了，我们走！", -1)
            LuaScenceM2Near(sceneId, x270009_g_acceptid,"苏多娜：小子，原来你是明教的人！居然被你给骗了，我们走！",0,-1)
			end
	elseif x270009_g_doing == 21   then
			if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 56   then
						x270009_g_doing =22
						NpcTalk(sceneId, x270009_g_yangdingtian, "看来我们误会这位少侠了，他并不是波斯的人。", -1)
            LuaScenceM2Near(sceneId, x270009_g_acceptid,"阳顶天：看来我们误会这位少侠了，他并不是波斯的人。",0,-1)
	...
	elseif x270009_g_doing == 22   then
			if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 60   then
						x270009_g_doing =23
						NpcTalk(sceneId, x270009_g_yangdingtian, "少侠！我们误以为你是波斯派来的。现在事情已经澄清了，恰好我明教正逢大难，希望你能助我教渡过难关。", -1)
            LuaScenceM2Near(sceneId, x270009_g_acceptid,"阳顶天：少侠！我们误以为你是波斯派来的。现在事情已经澄清了，恰好我明教正逢大难，希望你能助我教渡过难关。",0,-1)
			end
	elseif x270009_g_doing==23 then
			if GetCurrentTime() - x270009_g_PROTECTINFO.StartTime >= 65   then
					if IsHaveQuest(sceneId,x270009_g_acceptid, x270009_g_MissionId) > 0 then
									...
									BeginQuestEvent(sceneId)
									AddQuestText(sceneId,format("探听清楚明教的状况   %d/1", GetQuestParam(sceneId,x270009_g_acceptid,misIndex,0) ))
									EndQuestEvent(sceneId)
