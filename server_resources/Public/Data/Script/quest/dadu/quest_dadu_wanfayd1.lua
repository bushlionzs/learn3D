...
x256224_g_Name	={}
x256224_g_ExtTarget={{type=20,n=1,target="完成@npc_28828"},{type=20,n=1,target="答对@npc_28829"},{type=20,n=1,target="完成@npc_28830"},{type=20,n=1,target="进入@npc_28831"},}
x256224_g_MissionName="【个人】大都嘉年华"
x256224_g_MissionInfo="\t京城的繁华岂是王城可比，机会随处都在。你可以去@npc_139129，@npc_139133，@npc_139100，@npc_139104他们那里看看，大都嘉年华一系列有趣的任务都在等待着你。"  --任务描述
x256224_g_MissionTarget=""		
x256224_g_MissionCompleted="\t从你兴高采烈的样子上看大嘉年华任务让你过的很愉快。\n\t记得每天都去他们那里看看，在乐趣之余你会有很大的收获。"					--完成任务npc说话的话
x256224_g_ContinueInfo="\t怎么愁眉苦脸的，大都的嘉年华不好玩吗？"
...
x256224_g_NpcGUID ={{ guid = 140417, name = "包打听"} }
--MisDescEnd
--**********************************
-- 益者三友完成标记
...
function x256224_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x256224_g_MissionId) > 0 then
		return
	end
	if IsHaveQuest(sceneId,selfId, x256224_g_MissionId) > 0 then
...
function x256224_Msg2toplayer( sceneId, selfId,type)
    if type == 0 then
        Msg2Player(sceneId, selfId, "您接受了任务："..x256224_g_MissionName.."！", 0, 2)
        Msg2Player(sceneId, selfId, "您接受了任务："..x256224_g_MissionName.."！", 0, 3)
    elseif type == 1 then
        Msg2Player(sceneId, selfId, "您放弃了任务："..x256224_g_MissionName.."！", 0, 2)
        Msg2Player(sceneId, selfId, "您放弃了任务："..x256224_g_MissionName.."！", 0, 3)
    elseif type == 2 then
        Msg2Player(sceneId, selfId, "您完成了任务"..x256224_g_MissionName.."！", 0, 2)
        Msg2Player(sceneId, selfId, "您完成了任务"..x256224_g_MissionName.."！", 0, 3)      
        if x256224_g_ExpBonus > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R经验"..x256224_g_ExpBonus.."#Y的奖励", 0, 2)
        end
        if x256224_g_BonusMoney1 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x256224_g_BonusMoney1.."文#Y的奖励", 0, 2)
        end
        if x256224_g_BonusMoney2 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R现银"..x256224_g_BonusMoney2.."文#Y的奖励", 0, 2)
        end
        if x256224_g_BonusMoney3 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x256224_g_BonusMoney3.."文#Y的奖励", 0, 2)
        end
        if x256224_g_BonusMoney4 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x256224_g_BonusMoney4.."#Y的奖励", 0, 2)
        end
        if x256224_g_BonusMoney5 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R声望"..x256224_g_BonusMoney5.."#Y的奖励", 0, 2)
        end
        if x256224_g_BonusMoney6 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x256224_g_BonusMoney6.."#Y的奖励", 0, 2)
        end
    end
end
...
function x256224_ProcQuestLogRefresh( sceneId, selfId, MissionId)
    ...
    local strTarget = format( "任务已经完成，请回报@npc_%d", x256224_g_NpcGUID[1].guid)
