...
x260015_g_MissionName="【国家】为国效力<二>"
...
x260015_g_MissionTarget ="   获得12点功勋"
x260015_g_MissionMethod = "功勋大于12"              --任务攻略
x260015_g_MissionText = "\t这一次，我对你的期望是#G12点#W功勋值。\n\t每天在发布出国令期间成功#G完成出国各项任务#W，会获得一定的功勋值。\n\t当你攒够#G12点#W功勋值的时候，再来找我吧。"					--任务描述
x260015_g_MissionCompleted = "\t你的功勋值不足！只有每天在发布#G出国令期间#W成功完成出国任务，才会获得功勋值。"
x260015_g_MissionEnd = "\t#G12点#W功勋，有一定的进步！但是还远远不足。 \n\t坚持每天完成出国任务吧。"
x260015_g_MissionHelp = "\t只有每天在发布#G出国令期间#W成功完成出国任务，才会获得功勋值。"					--任务小提示
...
function x260015_ProcEventEntry( sceneId, selfId, targetId )	--点击该任务后执行此脚本
	if IsHaveQuestNM( sceneId, selfId, x260015_g_MissionId ) > 0 then	-- 如果有这个任务
			local bDone = x260015_CheckSubmit(sceneId, selfId,targetId, x260015_g_MissionId);
	        if (bDone > 0) then
				x260015_DispatchCompletedInfo( sceneId, selfId, targetId,x260015_g_MissionId) --完成任务显示
			else
			x260015_DispatchContinueInfo( sceneId, selfId, targetId,x260015_g_MissionId ) --未完成任务显示
			end
	else
			x260015_DispatchMissionInfo( sceneId, selfId, targetId,x260015_g_MissionId )		--任务信息显示
	end
end
...
function x260015_ProcAcceptCheck( sceneId, selfId, NPCId )
	...
	if IsHaveQuestNM( sceneId, selfId, MissionId ) > 0 then
		x260015_ShowTips(sceneId, selfId, "您已经接取了该任务，请先完成当前任务")
		return 0;
	end 
	return 1
end
...
function x260015_ProcQuestAccept( sceneId, selfId, NPCId, MissionId)
    ...
    if IsQuestFullNM(sceneId,selfId)==1 then
        x260015_ShowTips(sceneId, selfId, "可接任务数量已满")
        return 
    end
...
function x260015_ProcQuestSubmit( sceneId, selfId, targetId,selectRadioId, MissionId )
    ...
    if GetExploit(sceneId, selfId) < x260015_g_NeedExploit then
        --x260015_ShowTips(sceneId,selfId,"你的功勋不够，需要"..x260015_g_NeedExploit)
		BeginQuestEvent(sceneId)
		AddQuestText(sceneId, "你的功勋不够，需要"..x260015_g_NeedExploit.."点才能领取奖励！ ");
		EndQuestEvent(sceneId)
		DispatchQuestTips(sceneId,selfId)	
	  	Msg2Player(sceneId,selfId,"你的功勋不够，需要"..x260015_g_NeedExploit.."点才能领取奖励！ ",8,2)
    else
...
function x260015_ProcQuestAttach( sceneId, selfId, npcId, npcGuid,misIndex, MissionId )
	if IsHaveQuestNM( sceneId, selfId, x260015_g_MissionId) == 0 then	-- 如果没有这个任务
		return
	end
...
function x260015_ProcQuestLogRefresh( sceneId, selfId, MissionId)
	...
	AddQuestLogCustomText( sceneId,
							...
							MissionName,        -- 任务名字
							target,		--任务目标
							"@npc_"..x260015_g_ReplyNpcId,			--任务NPC
							x260015_g_MissionMethod,             --任务攻略
							x260015_g_MissionText,					--任务描述
							x260015_g_MissionHelp					--任务小提示
							)
