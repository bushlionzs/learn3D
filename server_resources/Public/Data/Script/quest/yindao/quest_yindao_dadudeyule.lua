...
x203800_g_Name	={}
x203800_g_ExtTarget={{type=20,n=1,target="找到@npc_139104"}}
x203800_g_MissionName="【个人】大都的娱乐<一>"
x203800_g_MissionInfo="\t我听说在大都有个地方可以跳舞，里面的妞儿们都个比个美艳，不知道你去过没？如果想要去舞厅的话，就找@npc_139104吧。"  --任务描述
x203800_g_MissionTarget=""		
x203800_g_MissionCompleted="\t帅哥美女们注意了，这里就是全大都最有名的舞厅，希望休闲一下的就来找我吧！"					--完成任务npc说话的话
...
x203800_g_NpcGUID ={{ guid = 139104, name = "杜十娘"} }
--MisDescEnd
--**********************************
...
function x203800_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x203800_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x203800_g_MissionId) > 0 then
...
function x203800_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x203800_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x203800_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203800_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203800_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x203800_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x203800_g_MissionName.."！", 0, 3)
				if x203800_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x203800_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x203800_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x203800_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x203800_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x203800_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x203800_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x203800_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x203800_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x203800_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x203800_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x203800_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x203800_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x203800_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
