...
x203791_g_Name	={}
x203791_g_ExtTarget={{type=20,n=1,target="找到@npc_126023"}}
x203791_g_MissionName="【个人】寻求名师"
x203791_g_MissionInfo="\t你是否觉得现在升级速度变慢了？对这个世界有很多不解的地方？现在的你需要一位师傅帮助你更加快速的升级。\n\t去找@npc_126023吧，他会帮你找到理想的师傅。"  --任务描述
x203791_g_MissionTarget=""		
x203791_g_MissionCompleted="\t你需要一位贤明的师傅？没问题，在我的帮助下，一定如你所愿。 "					--完成任务npc说话的话
...
x203791_g_NpcGUID ={{ guid = 126023, name = "师徒管理员"} }
--MisDescEnd     
...
function x203791_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x203791_g_MissionId) > 0 then
		return 
	end
		...
		if x203791_ProcAcceptCheck(sceneId, selfId, NPCId) > 0 then
					...
					if retmiss ==0 then                                          ------添加失败 ==0 ，1 成功                                                                  
		  			...
		  			x203791_Msg2toplayer( sceneId, selfId,0)
						--if x203791_CheckSubmit(sceneId, selfId, NPCId) > 0 then
								local misIndex = GetQuestIndexByID(sceneId,selfId,x203791_g_MissionId)       ---- 取得任务状态                                               
...
function x203791_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x203791_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x203791_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203791_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203791_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x203791_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x203791_g_MissionName.."！", 0, 3)
				if x203791_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x203791_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x203791_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x203791_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x203791_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x203791_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x203791_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x203791_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x203791_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x203791_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x203791_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x203791_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x203791_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x203791_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
...
function x203791_ProcQuestAttach( sceneId, selfId, npcId, npcGuid, misIndex, MissionId )
    ...
    if bFind == 0 then
		...
		if IsHaveQuest(sceneId,selfId, x203791_g_MissionId) > 0 then
				local state = GetQuestStateNM(sceneId,selfId,npcId,x203791_g_MissionId)        ---取得任务状态 ，根据状态添加按钮 AddQuestTextNM
				AddQuestTextNM( sceneId, selfId, npcId, x203791_g_MissionId, state, -1 )
		end
end
