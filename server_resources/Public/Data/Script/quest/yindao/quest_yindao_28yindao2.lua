...
x203788_g_Name	={}
x203788_g_ExtTarget={{type=20,n=1,target="完成一整轮@npc_28802"}}
x203788_g_MissionName="【个人】王国内务"
x203788_g_MissionInfo="\t你好，亲爱的朋友，王国中有不少官员正在寻求人手帮忙，你可以去@npc_129042那里看看，当你将他的一整轮任务都完成后，我会好好的奖励你。"  --任务描述
x203788_g_MissionTarget=""		
x203788_g_MissionCompleted="\t看来你已经对王国内务了如指掌了。\n\t记得每天都去他那里看看，一定会对你有不少帮助。"					--完成任务npc说话的话
x203788_g_ContinueInfo="别太心急，我为你准备了不少礼物，但是......你应该先完成一轮内政任务。"
...
x203788_g_NpcGUID ={{ guid = 129056, name = "万事通"} }
--MisDescEnd
--**********************************
-- 征税完成标记
--x203788_g_MP_Tax        = 1
...
function x203788_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x203788_g_MissionId) > 0 then
		return
	end
	if IsHaveQuest(sceneId,selfId, x203788_g_MissionId) > 0 then
...
function x203788_Msg2toplayer( sceneId, selfId,type)
    if type == 0 then
        Msg2Player(sceneId, selfId, "您接受了任务"..x203788_g_MissionName.."！", 0, 2)
        Msg2Player(sceneId, selfId, "您接受了任务"..x203788_g_MissionName.."！", 0, 3)
    elseif type == 1 then
        Msg2Player(sceneId, selfId, "您放弃了任务"..x203788_g_MissionName.."！", 0, 2)
        Msg2Player(sceneId, selfId, "您放弃了任务"..x203788_g_MissionName.."！", 0, 3)
    elseif type == 2 then
        Msg2Player(sceneId, selfId, "您完成了任务"..x203788_g_MissionName.."！", 0, 2)
        Msg2Player(sceneId, selfId, "您完成了任务"..x203788_g_MissionName.."！", 0, 3)
        if x203788_g_ExpBonus > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R经验"..x203788_g_ExpBonus.."#Y的奖励", 0, 2)
        end
        if x203788_g_BonusMoney1 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x203788_g_BonusMoney1.."文#Y的奖励", 0, 2)
        end
        if x203788_g_BonusMoney2 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R现银"..x203788_g_BonusMoney2.."文#Y的奖励", 0, 2)
        end
        if x203788_g_BonusMoney3 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x203788_g_BonusMoney3.."文#Y的奖励", 0, 2)
        end
        if x203788_g_BonusMoney4 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x203788_g_BonusMoney4.."#Y的奖励", 0, 2)
        end
        if x203788_g_BonusMoney5 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R声望"..x203788_g_BonusMoney5.."#Y的奖励", 0, 2)
        end
        if x203788_g_BonusMoney6 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x203788_g_BonusMoney6.."#Y的奖励", 0, 2)
        end
    end
end
...
function x203788_ProcQuestLogRefresh( sceneId, selfId, MissionId)
    ...
    local strTarget = format( "任务已经完成，请回报@npc_%d", x203788_g_NpcGUID[1].guid)
