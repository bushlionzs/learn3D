...
x203821_g_Name	={}
x203821_g_ExtTarget={{type=20,n=1,target="添加一个好友，将好友度提升到10"}}
x203821_g_MissionName="【个人】君子之交"
x203821_g_MissionInfo="\t去寻找和你志趣相投，又能肝胆相照的一位好友吧。\n\t在组队完成任务过程中你可以邀请那些你喜欢的人为好友。在王国中可以成为好友的人数不胜数，一起组队完成#G“好友任务或打怪”#W将好友度提升得10。"	
x203821_g_MissionTarget=""		
x203821_g_MissionCompleted="\t恭喜你交到密友，看到你们在完成任务时的默契配合我很欣慰。"					--完成任务npc说话的话
x203821_g_ContinueInfo="\t邀请那些你喜欢的人为好友，一起组队完成#G“好友任务或打怪”#W将好友度提升得10。"	
x203821_g_MissionHelp ="一起组队完成#G“好友任务或打怪”#W可以将好友度提升得10。"
...
x203821_g_NpcGUID ={{ guid = 123759, name = "朋尔多"} }
--MisDescEnd
--**********************************
-- 最大等级
...
function x203821_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x203821_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x203821_g_MissionId) > 0 then
...
function x203821_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x203821_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x203821_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203821_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203821_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x203821_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x203821_g_MissionName.."！", 0, 3)
				if x203821_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x203821_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x203821_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x203821_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x203821_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x203821_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x203821_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x203821_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x203821_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x203821_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x203821_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x203821_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x203821_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x203821_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
