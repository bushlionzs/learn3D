...
x203813_g_Name	={}
x203813_g_ExtTarget={{type=20,n=1,target="找到@npc_123837"}}
x203813_g_MissionName="【个人】全新的职业"
x203813_g_MissionInfo="\t在成吉思汗2的新世界里，有着新的六个职业。如果你对新职业有兴趣，可以去找王城内的转职管理员，他会将你转变成全新的职业！\n\t别担心费用的问题，他会送给你进行两次转职所需的道具。"  --任务描述
x203813_g_MissionTarget=""		
x203813_g_MissionCompleted="\t没错，我为所有人提供转换职业和装备的服务。并且，我会给你两次转职所需的道具，如果你对新职业不满意，还有一次转换回原本职业的机会。 "					--完成任务npc说话的话
...
x203813_g_NpcGUID ={{ guid = 123837, name = "转职大师"} }
--MisDescEnd     
...
function x203813_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x203813_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x203813_g_MissionId) > 0 then
		...
		if x203813_ProcAcceptCheck(sceneId, selfId, NPCId) > 0 then
					...
					if retmiss ==0 then                                          ------添加失败 ==0 ，1 成功                                                                  
		  			...
		  			x203813_Msg2toplayer( sceneId, selfId,0)
						--if x203813_CheckSubmit(sceneId, selfId, NPCId) > 0 then
								local misIndex = GetQuestIndexByID(sceneId,selfId,x203813_g_MissionId)       ---- 取得任务状态                                               
...
function x203813_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x203813_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x203813_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203813_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203813_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x203813_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x203813_g_MissionName.."！", 0, 3)
				if x203813_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x203813_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x203813_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x203813_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x203813_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x203813_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x203813_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x203813_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x203813_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x203813_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x203813_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x203813_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x203813_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x203813_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
...
function x203813_ProcQuestAttach( sceneId, selfId, npcId, npcGuid, misIndex, MissionId )
    ...
    if bFind == 0 then
		...
		if IsHaveQuest(sceneId,selfId, x203813_g_MissionId) > 0 then
				local state = GetQuestStateNM(sceneId,selfId,npcId,x203813_g_MissionId)        ---取得任务状态 ，根据状态添加按钮 AddQuestTextNM
				AddQuestTextNM( sceneId, selfId, npcId, x203813_g_MissionId, state, -1 )
		end
end
