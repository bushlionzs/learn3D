...
x203784_g_Name	={}
x203784_g_ExtTarget={{type=20,n=1,target="完成一次@npc_28838"},{type=20,n=1,target="乘坐一次@npc_28842"},}
x203784_g_MissionName="【个人】王城的精彩"
x203784_g_MissionInfo="\t谢谢你的问候，我这些时日一切安好。对了，在@npc_129060，@npc_129179那里有些小任务需要完成，你也好趁此机会历练一下自己，不必担心任务都很简单。"  --任务描述
x203784_g_MissionTarget=""		
x203784_g_MissionCompleted="\t看你满面红光，一定是大有收获。\n\t记得每天都去他们那里看看，收获定然是不会少的。"					--完成任务npc说话的话
x203784_g_ContinueInfo="\t做事就要做到底，不可半途而废。"
...
x203784_g_NpcGUID ={{ guid = 129056, name = "万事通"} }
--MisDescEnd
--**********************************
-- 天降宝箱完成标记
...
function x203784_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x203784_g_MissionId) > 0 then
		return
	end
	if IsHaveQuest(sceneId,selfId, x203784_g_MissionId) > 0 then
...
function x203784_Msg2toplayer( sceneId, selfId,type)
    if type == 0 then
        Msg2Player(sceneId, selfId, "您接受了任务："..x203784_g_MissionName.."！", 0, 2)
        Msg2Player(sceneId, selfId, "您接受了任务："..x203784_g_MissionName.."！", 0, 3)
    elseif type == 1 then
        Msg2Player(sceneId, selfId, "您放弃了任务："..x203784_g_MissionName.."！", 0, 2)
        Msg2Player(sceneId, selfId, "您放弃了任务："..x203784_g_MissionName.."！", 0, 3)
    elseif type == 2 then
        Msg2Player(sceneId, selfId, "您完成了任务"..x203784_g_MissionName.."！", 0, 2)
        Msg2Player(sceneId, selfId, "您完成了任务"..x203784_g_MissionName.."！", 0, 3)
        if x203784_g_ExpBonus > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R经验"..x203784_g_ExpBonus.."#Y的奖励", 0, 2)
        end
        if x203784_g_BonusMoney1 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x203784_g_BonusMoney1.."文#Y的奖励", 0, 2)
        end
        if x203784_g_BonusMoney2 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R现银"..x203784_g_BonusMoney2.."文#Y的奖励", 0, 2)
        end
        if x203784_g_BonusMoney3 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x203784_g_BonusMoney3.."文#Y的奖励", 0, 2)
        end
        if x203784_g_BonusMoney4 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x203784_g_BonusMoney4.."#Y的奖励", 0, 2)
        end
        if x203784_g_BonusMoney5 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R声望"..x203784_g_BonusMoney5.."#Y的奖励", 0, 2)
        end
        if x203784_g_BonusMoney6 > 0 then
            Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x203784_g_BonusMoney6.."#Y的奖励", 0, 2)
        end
    end
end
...
function x203784_ProcQuestLogRefresh( sceneId, selfId, MissionId)
    ...
    local strTarget = format( "任务已经完成，请回报@npc_%d", x203784_g_NpcGUID[1].guid)
    ...
    if baoxiang== 1 and Gongche == 1 and Lunhuitai == 1 then
        ...
        --if Lunhuitai == 0 then
        --    strTarget = strTarget.."  #W完成一次@npc_28846(0/1)"
        --else
        --    strTarget = strTarget.."  #G完成一次@npc_28846(1/1)"
        --end
    end
