...
x203812_g_Name	={}
x203812_g_ExtTarget={{type=20,n=1,target="找到@npc_132038"}}
x203812_g_MissionName="【个人】骑乘技能"
x203812_g_MissionInfo="\t看你的样子似乎还不知道啊，你的骑乘也是可以学习技能的，一匹拥有绝佳技能的骑乘，可以让你如虎添翼。王城的@npc_132038可以让你的骑乘学会新的技能，价格也不贵，你可以去他那里看看。"  --任务描述
x203812_g_MissionTarget=""		
x203812_g_MissionCompleted="\t骑乘的技能也有高低之分，要学习更高级的技能，需要有低级技能作为基础。 "					--完成任务npc说话的话
...
x203812_g_NpcGUID ={{ guid = 132038, name = "骑乘技能"} }
--MisDescEnd     
...
function x203812_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x203812_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x203812_g_MissionId) > 0 then
		...
		if x203812_ProcAcceptCheck(sceneId, selfId, NPCId) > 0 then
					...
					if retmiss ==0 then                                          ------添加失败 ==0 ，1 成功                                                                  
		  			...
		  			x203812_Msg2toplayer( sceneId, selfId,0)
						--if x203812_CheckSubmit(sceneId, selfId, NPCId) > 0 then
								local misIndex = GetQuestIndexByID(sceneId,selfId,x203812_g_MissionId)       ---- 取得任务状态                                               
...
function x203812_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x203812_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x203812_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203812_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203812_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x203812_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x203812_g_MissionName.."！", 0, 3)
				if x203812_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R经验"..x203812_g_ExpBonus.."#Y的奖励", 0, 2)
  			end
				if x203812_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R银卡"..x203812_g_BonusMoney1.."文#Y的奖励", 0, 2)
	  		end
				if x203812_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R现银"..x203812_g_BonusMoney2.."文#Y的奖励", 0, 2)
	  		end
				if x203812_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#Y获得#R金卡"..x203812_g_BonusMoney3.."文#Y的奖励", 0, 2)
				end
				if x203812_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R朝廷声望值"..x203812_g_BonusMoney4.."#Y的奖励", 0, 2)
				end
				if x203812_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R声望"..x203812_g_BonusMoney5.."#Y的奖励", 0, 2)
				end
				if x203812_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#Y获得#R荣誉"..x203812_g_BonusMoney6.."#Y的奖励", 0, 2)
				end
		end
end
...
function x203812_ProcQuestAttach( sceneId, selfId, npcId, npcGuid, misIndex, MissionId )
    ...
    if bFind == 0 then
		...
		if IsHaveQuest(sceneId,selfId, x203812_g_MissionId) > 0 then
				local state = GetQuestStateNM(sceneId,selfId,npcId,x203812_g_MissionId)        ---取得任务状态 ，根据状态添加按钮 AddQuestTextNM
				AddQuestTextNM( sceneId, selfId, npcId, x203812_g_MissionId, state, -1 )
		end
end
