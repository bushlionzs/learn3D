...
x203662_g_Name	={}
x203662_g_ExtTarget={{type=20,n=1,target="找到@npc_134013"}}
x203662_g_MissionName="【新手】武器的进阶"
x203662_g_MissionInfo="\t你已经对自己职业有了充分的认识，那么，去我周围的武器之灵那里找到更强大的兵器吧。"  --任务描述
x203662_g_MissionTarget="找到@npc_134013"		
x203662_g_MissionCompleted="\t你似乎被武器之灵的呼唤所指引，轻轻的伸出双手接住了你的新武器。"					--完成任务npc说话的话
x203662_g_ContinueInfo="难道你还不知道最适合你的武器是圣十字吗？"
x203662_g_MissionHelp =	"#G你只需要耐心观察，哪一把武器之灵有着黄色的问号，那么它就会给你合适的兵器。#W"
...
x203662_g_NpcGUID ={{ guid = 134013, name = "圣十字"} }
--MisDescEnd
--**********************************
...
function x203662_ProcEventEntry(sceneId, selfId, NPCId, MissionId)	--点击该任务后执行此脚本
	if IsQuestHaveDone(sceneId, selfId, x203662_g_MissionId) > 0 then
		return 
	end
	if IsHaveQuest(sceneId,selfId, x203662_g_MissionId) > 0 then
...
function x203662_Msg2toplayer( sceneId, selfId,type)
		if type == 0 then
				Msg2Player(sceneId, selfId, "您接受了任务"..x203662_g_MissionName.."！", 0, 2)
		  	Msg2Player(sceneId, selfId, "您接受了任务"..x203662_g_MissionName.."！", 0, 3)
		elseif type == 1 then
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203662_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您放弃了任务"..x203662_g_MissionName.."！", 0, 3)
		elseif type == 2 then
				Msg2Player(sceneId, selfId, "您完成了任务"..x203662_g_MissionName.."！", 0, 2)
				Msg2Player(sceneId, selfId, "您完成了任务"..x203662_g_MissionName.."！", 0, 3)
				if x203662_g_ExpBonus > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G经验"..x203662_g_ExpBonus.."#cffcf00的奖励", 0, 2)
  			end
				if x203662_g_BonusMoney1 > 0 then
	  		  Msg2Player(sceneId, selfId, "#cffcf00获得#G银卡"..x203662_g_BonusMoney1.."文#cffcf00的奖励", 0, 2)
	  		end
				if x203662_g_BonusMoney2 > 0 then
	  		  Msg2Player(sceneId, selfId, "#cffcf00获得#G现银"..x203662_g_BonusMoney2.."文#cffcf00的奖励", 0, 2)
	  		end
				if x203662_g_BonusMoney3 > 0 then
	  		  Msg2Player(sceneId, selfId, "#cffcf00获得#G金卡"..x203662_g_BonusMoney3.."文#cffcf00的奖励", 0, 2)
				end
				if x203662_g_BonusMoney4 > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G朝廷声望值"..x203662_g_BonusMoney4.."#cffcf00的奖励", 0, 2)
				end
				if x203662_g_BonusMoney5 > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G声望"..x203662_g_BonusMoney5.."#cffcf00的奖励", 0, 2)
				end
				if x203662_g_BonusMoney6 > 0 then
					Msg2Player(sceneId, selfId, "#cffcf00获得#G荣誉"..x203662_g_BonusMoney6.."#cffcf00的奖励", 0, 2)
				end
		end
end
