--脚本号
x414011_g_scriptId = 414011 --临时写这个,真正用的时候一定要改.
x414011_g_keyID = 12101000 --临时写这个,真正用的时候一定要改.
x414011_g_keyname = "符文钥匙" --临时写这个,真正用的时候一定要改.
...
function x414011_ProcEventEntry( sceneId, selfId, bagIndex )
	...
	if HaveKey > 0 then
		...
		BeginQuestEvent(sceneId)
			local strText = "#Y你使用一把"..x414011_g_keyname.."，开启了一个宝箱"
			AddQuestText(sceneId,strText);
		EndQuestEvent(sceneId)
		DispatchQuestTips(sceneId,selfId)
	else
		BeginQuestEvent(sceneId)
			local strText = "#Y开启宝箱，需要"..x414011_g_keyname.."一把"
			AddQuestText(sceneId,strText);
		EndQuestEvent(sceneId)
		DispatchQuestTips(sceneId,selfId)
	end
