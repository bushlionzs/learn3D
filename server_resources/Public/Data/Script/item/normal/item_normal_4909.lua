...
x404909_g_DestSceneName = "练功人偶"
x404909_g_Get_Exp_Max_Cnt = 10 --击败练功人偶或者玩家获得技能经验的人数上限为10人
x404909_g_BossType  = 9526
x404909_g_MissionID = 9091 

--怪物创建个数最大数量
x404909_g_Max_Monster_Per_Scene		=  10		--Monster创建个数
...
function x404909_ProcEventEntry( sceneId, selfId, BagIndex )
	...
	---------------------------------------------------		local ret = DelItemByIDInBag( sceneId, selfId, 12030217, 1) --删除物品
	---------------------------------------------------else
	---------------------------------------------------		print("------------------------------------")
	---------------------------------------------------		return 0
	---------------------------------------------------end
	---------------------------------------------------
	---------------------------------------------------local ret = CreateMonster( sceneId, x404909_g_BossType, PlayerPosX, PlayerPosZ, 1, -1, x404909_g_scriptId, nGuid, 21, 3*60000) --存活时间3分钟
    ---------------------------------------------------
	---------------------------------------------------if ret > 0 then
	---------------------------------------------------				local message = "练功人偶召唤的怪物出现了"
	---------------------------------------------------				Msg2Player(sceneId,selfId,message, 8, 2)
	---------------------------------------------------				Msg2Player(sceneId,selfId,message, 8, 3)
	---------------------------------------------------else
	---------------------------------------------------				local strText = format("这里练功人偶无法召出怪物" )
	---------------------------------------------------				BeginQuestEvent(sceneId)
	---------------------------------------------------				AddQuestText(sceneId, strText)
	---------------------------------------------------				EndQuestEvent(sceneId)
	---------------------------------------------------				DispatchQuestTips(sceneId, selfId)
...
function x404909_OnDie(sceneId, selfId, killerId)--怪死亡添加日志
...
function x404909_ProcMyTask( sceneId, selfId)
	...
	local r0 = random(-1,1)
	local r1 = random(-1,1)
	local ret = CreateMonster( sceneId, x404909_g_BossType, PlayerPosX+r0, PlayerPosZ+r1, 7, -1, x404909_g_scriptId, nGuid, 21, 3*60000) --存活时间3分钟
	...
	if ret > 0 then
					...
					local strText = format("这里无法召出练功人偶" )
					BeginQuestEvent(sceneId)
					AddQuestText(sceneId, strText)
					EndQuestEvent(sceneId)
					DispatchQuestTips(sceneId, selfId)
