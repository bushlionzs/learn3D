--脚本号
x414013_g_scriptId = 414013 --临时写这个,真正用的时候一定要改.
x414013_g_keyID = 12101002 --临时写这个,真正用的时候一定要改.
x414013_g_keyname = "符文钥匙" --临时写这个,真正用的时候一定要改.
...
function x414013_ProcEventEntry( sceneId, selfId, bagIndex )
	...
	if HaveKey > 0 then
		...
		BeginQuestEvent(sceneId)
			local strText = "#Y你使用一把"..x414013_g_keyname.."，开启了一个宝箱"
			AddQuestText(sceneId,strText);
		EndQuestEvent(sceneId)
		DispatchQuestTips(sceneId,selfId)
	else
		BeginQuestEvent(sceneId)
			local strText = "#Y开启宝箱，需要"..x414013_g_keyname.."一把"
			AddQuestText(sceneId,strText);
		EndQuestEvent(sceneId)
		DispatchQuestTips(sceneId,selfId)
	end
