...
--事件交互入口
--**********************************
function x418042_ProcEventEntry( sceneId, selfId, bagIndex )
-- 不需要这个接口，但要保留空函数
end
...
--**********************************
function x418042_IsSkillLikeScript( sceneId, selfId)
	return 1; --这个脚本需要动作支持
end
...
--**********************************
function x418042_CancelImpacts( sceneId, selfId )
...
--**********************************
function x418042_OnConditionCheck( sceneId, selfId )
...
--**********************************
function x418042_OnDeplete( sceneId, selfId )
...
--注：这里是技能生效一次的入口
--**********************************
function x418042_OnActivateOnce( sceneId, selfId, impactId )

	local a = GetLevel(sceneId,selfId)
	local b = x418042_g_LevelLim
	local xExp = 0
	local flag = 0

	for i, item in x418042_g_LevelExp do
		if i == a then
			flag = 1
		end
		if i == b then
...
--注：这里是技能生效一次的入口
--**********************************
function x418042_OnActivateEachTick( sceneId, selfId)
