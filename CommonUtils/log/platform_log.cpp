...
void print_log(const char* fmt, ...)
{
    ...
    int n = snprintf((char*)logbuffer, sizeof(logbuffer),
    ...
    vsnprintf(logbuffer + n, sizeof(logbuffer) - n, fmt, arglist);
    va_end(arglist);
...
public:
    ...
    virtual void write_log(uint32_t level, const char* filename, uint32_t line, const char* fmt, ...)
    {
		...
		int n = snprintf(logbuffer, sizeof(logbuffer) - 1, "[%s] [%s:%u]", g_log_level_str[level], filename, line);
    ...
    void write_log(const char* msg, uint32_t msg_size, uint32_t msg_id)
    {
        ...
        {
            if (_stat_log.last_day != pmt.tm_mday)
            {
                create_log_fd(&_stat_log, &pmt, _stat_log_name.c_str());
            }
            if (_stat_log.log_fd > 0)
            {
                write(_stat_log.log_fd, logbuffer, size);
                write(_stat_log.log_fd, msg, msg_size);
            }
    ...
    bool init(const char* log_directory)
    {
        ...
        _log_dir = utf8_to_acsi(_log_dir);
#else
            if (_log_dir[_log_dir.size() - 1] != '/')
            {
                _log_dir += "/";
        ...
        {
            std::string root_path = PlatformFileSystem::GetInstance()->getProcessDirectory();
#ifdef _WIN32
	...
	bool create_init_directory()
	{
		...
		_stat_log_name = _log_dir + get_stat_log_file_name();
		...
		_stat_log.last_hour = -1;
        _stat_log.last_day = -1;
        _stat_log.log_fd = -1;
#if (!defined(__ANDROID__)) && (!defined(__APPLE__))      
		...
		if (0 != create_log_fd(&_stat_log, &pmt, _stat_log_name.c_str()))
        {
            return false;
        }
#endif        
    ...
    void clean_expire_log(uint32_t day)
    {
        ...
        traverse_dir(_log_dir.c_str(), filelist, day);
...
static std::mutex _g_log_mutex;

bool platform_log_init(IPlatformLog* log, const char* log_directory, bool flow_log_split)
{
    if (false == _g_log_init)
    {
        std::lock_guard<std::mutex> guard(_g_log_mutex);
        if (false == _g_log_init)
        {
            PlatformFileSystem::GetInstance()->InitFileSystem();
            _g_log_init = true;
            if (NULL == log)
            {
