 ...
 * of bytes.
 */
void MD5Update(MD5Context* context, const string_view& data) 
{
    struct Context* ctx = reinterpret_cast<struct Context*>(context);
...
void MD5Sum(const void* data, size_t length, MD5Digest* digest)
{
    MD5Context ctx;
    MD5Init(&ctx);
    MD5Update(&ctx, string_view(reinterpret_cast<const char*>(data), length));
    MD5Final(digest, &ctx);
}

std::string MD5String(const string_view& str)
{
    MD5Digest digest;
