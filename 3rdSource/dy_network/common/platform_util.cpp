...
void CALLBACK CallBackTimer(UINT uTimerID, UINT uMsg, DWORD_PTR dwUser, DWORD_PTR dw1, DWORD_PTR dw2)
{
    ...
    /*if (current_time - last_timer_triger_time <= TIMER_DURATION / 2) {
        return;
    }
    last_timer_triger_time = current_time;*/
...
uint64_t platform_get_tickcount()
{
    return current_time;
}
...
platform_timer_t platform_create_timer(
    ...
    void* timer_param)
{
    uint64_t idx = (uint64_t)timer_param % g_timer_logic_thread_count;
    uint32_t tid =  g_timer_logic[idx].add_timer( duration , platfom_timer_callback , timer_self, timer_param);
    if (tid == 0)
    {
