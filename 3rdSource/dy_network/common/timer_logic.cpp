...
uint32_t CTimerLogic::add_timer(
	uint32_t duration, 
	...
	void* timer_param)
{
	...
	node->_timer_self = timer_self;
	node->_timer_param	= timer_param;
...
void CTimerLogic::cascade_timers( struct timer_vec_root *tv )
{
	...
	while( curr != head )
	{
        ...
        node->running = true;
        int32_t ret = node->m_platfom_timer_callback(node->m_timer_handler, node->_timer_self, node->_timer_param);
        node->running = false;
